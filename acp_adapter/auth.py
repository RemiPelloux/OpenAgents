"""ACP auth helpers — detect and advertise OpenAgents authentication methods."""
...
def detect_provider() -> Optional[str]:
    """Resolve the active OpenAgents runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if OpenAgents can resolve any runtime provider credentials."""
...
def build_auth_methods() -> list[Any]:
    ...
    not have OpenAgents provider credentials configured yet, so OpenAgents always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate OpenAgents using the currently configured "
