"""ACP agent server — exposes OpenAgents via the Agent Client Protocol."""
...
try:
    from openagents_cli import __version__ as HERMES_VERSION
except Exception:
...
def _path_from_file_uri(uri: str) -> Path | None:
    ...
    /mnt/<drive>/... so OpenAgents running in WSL can read it.
    """
    raw = (uri or "").strip()
    if not raw:
        return None
    ...
    if path is None:
        return [{
            ...
            "text": _format_resource_text(
                ...
                name=name,
                title=title,
                body="[Resource link only; OpenAgents cannot read non-file ACP resource URIs directly.]",
            ),
        }]
...
class HermesACPAgent(acp.Agent):
    """ACP Agent implementation wrapping OpenAgents AIAgent."""
    ...
    _SLASH_COMMANDS = {
        ...
        "version": "Show OpenAgents version",
    }
    ...
    _ADVERTISED_COMMANDS = (
        ...
        {
            "name": "version",
            "description": "Show OpenAgents version",
        },
    )
    ...
    def _session_modes(self, state: SessionState) -> SessionModeState:
        ...
        modes, which coexist with the model picker, so OpenAgents maps edit approval
    ...
    def _build_model_state(self, state: SessionState) -> SessionModelState | None:
        ...
        try:
            from openagents_cli.models import curated_models_for_provider, normalize_provider, provider_label
    ...
    def _resolve_model_selection(raw_model: str, current_provider: str) -> tuple[str, str]:
        ...
        try:
            from openagents_cli.models import detect_provider_for_model, parse_model_input
    ...
    def _build_usage_update(state: SessionState) -> UsageUpdate | None:
        ...
        the current request pressure.  OpenAgents estimates ``used`` from the same
    ...
    async def _send_session_info_update(
        ...
        *,
        ...
        """Send ACP native session metadata after OpenAgents changes it.

        When the internal OpenAgents head rotated (e.g. compression-driven session
        ...
        # openagents_state.py schema — only started_at/ended_at). Use "now" as
    ...
    async def _register_session_mcp_servers(
        ...
        try:
            ...
            enabled_toolsets = _expand_acp_enabled_toolsets(
                getattr(state.agent, "enabled_toolsets", None) or ["openagents-acp"],
    ...
    async def initialize(
        ...
        **kwargs: Any,
        ...
        return InitializeResponse(
            ...
            agent_info=Implementation(name="openagents", version=HERMES_VERSION),
    ...
    async def authenticate(self, method_id: str, **kwargs: Any) -> AuthenticateResponse | None:
        ...
        # OpenAgents' threat model (ACP is stdio-only, local-trust), but poor
        ...
        if normalized_method == TERMINAL_SETUP_AUTH_METHOD_ID:
            # Terminal auth launches OpenAgents setup/model selection out-of-band.
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        """Run OpenAgents on the user's prompt and stream events back to the editor."""
        state = self.session_manager.get_session(session_id)
        if state is None:
        ...
        try:
            # Snapshot the internal OpenAgents DB session id before the turn so we
        ...
        # OpenAgents' local "waiting for model response" interrupt status is metadata,
    ...
    def _cmd_version(self, args: str, state: SessionState) -> str:
        return f"OpenAgents v{HERMES_VERSION}"
    ...
    async def set_config_option(
        ...
        """Accept ACP config option updates even when OpenAgents has no typed ACP config surface yet."""
        state = self.session_manager.get_session(session_id)
        if state is None:
