"""ACP session manager — maps ACP sessions to OpenAgents AIAgent instances.

Sessions are persisted to the shared SessionDB (``~/.openagents/state.db``) so they
...
def _translate_acp_cwd(cwd: str) -> str:
    """Translate Windows ACP cwd values when OpenAgents itself is running in WSL.
    ...
    """
    from openagents_constants import is_wsl
...
def _register_task_cwd(task_id: str, cwd: str) -> None:
    ...
    Zed can launch OpenAgents from a Windows workspace while the ACP process runs
...
def _expand_acp_enabled_toolsets(
    ...
    expanded: List[str] = []
    for name in list(toolsets or ["openagents-acp"]):
...
@dataclass
class SessionState:
    """Tracks per-session state for an ACP-managed OpenAgents."""
...
class SessionManager:
    """Thread-safe manager for ACP sessions backed by OpenAgents AIAgent instances.
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        Args:
            agent_factory: Optional callable that creates an AIAgent-like object.
                           ...
                           using the current OpenAgents runtime provider configuration.
            db:            Optional SessionDB instance. When omitted, the default
                           SessionDB (``~/.openagents/state.db``) is lazily created.
        """
    ...
    def _get_db(self):
        ...
        Note: we resolve ``OPENAGENTS_HOME`` dynamically rather than relying on
    ...
    def _make_agent(
        ...
        *,
        ...
        # editor/session cwd instead of the OpenAgents daemon's process cwd.
