"""CLI entry point for the openagents ACP adapter.

Loads environment variables from ``~/.openagents/.env``, configures logging
...
def _load_env() -> None:
    """Load .env from OPENAGENTS_HOME (default ``~/.openagents``)."""
    from openagents_cli.env_loader import load_openagents_dotenv

    hermes_home = get_openagents_home()
    loaded = load_openagents_dotenv(hermes_home=hermes_home)
    if loaded:
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="openagents-acp",
        description="Run OpenAgents as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print OpenAgents version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive OpenAgents provider/model setup for ACP terminal auth",
    )
    parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.openagents/node/ "
...
def _run_setup() -> None:
    from openagents_cli.main import main as hermes_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "openagents", "model"]
        hermes_main()
    finally:
        sys.argv = old_argv
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting openagents ACP adapter")
