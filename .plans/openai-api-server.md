# OpenAI-Compatible API Server for OpenAgents
...
SSE streaming. By exposing this endpoint, openagents becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  openagents   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.openagents/.env`
...
4. Chat with openagents through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns openagents as a model) |
...
But openagents sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "openagents"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured openagents model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of openagents's core prompt
...
Once implemented, openagents works as a drop-in backend for:
