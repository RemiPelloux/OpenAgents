...
# $OPENAGENTS_HOME — auth.json, .env, config.yaml — ends up root-owned and
...
# returns "Provider authentication failed: OpenAgents is not logged into Nous
...
# $OPENAGENTS_HOME is uid-aligned with the supervised processes. When invoked
