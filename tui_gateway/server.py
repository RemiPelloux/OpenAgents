...
# appends every unhandled exception to ~/.openagents/logs/tui_gateway_crash.log
...
class _SlashWorker:
    ...
    def __init__(self, session_key: str, model: str):
        ...
        self.proc = subprocess.Popen(
            ...
            bufsize=1,
            cwd=os.getcwd(),
            # slash_worker runs the OpenAgents → needs provider credentials.
...
def _transfer_active_session_slot(
    ...
    *,
    ...
    if lease is None:
        return True
    try:
        from openagents_cli.active_sessions import transfer_active_session
...
def _finalize_session(session: dict | None, end_reason: str = "tui_close") -> None:
    ...
    if agent is not None:
        try:
            from openagents_cli.plugins import invoke_hook
...
def _max_live_sessions() -> int:
    try:
        from openagents_cli.active_sessions import coerce_max_concurrent_sessions
...
# ``profile`` on those calls; we open that profile's db and bind its OPENAGENTS_HOME
...
def _profile_scoped(handler):
    """Bind ``params['profile']``'s OPENAGENTS_HOME around a pet RPC handler.

    Pets are per-profile: ``display.pet.*`` lives in the profile's config.yaml and
    sprites install under its ``pets/`` dir (both resolve via ``get_openagents_home``).
    ...
    def wrapper(rid, params):
        ...
        token = set_openagents_home_override(home)
        try:
            return handler(rid, params)
        finally:
...
def _start_agent_build(sid: str, session: dict) -> None:
    ...
    def _build() -> None:
        ...
        try:
            ...
            # OPENAGENTS_HOME so config/skills/model resolve to it, and hand the
            # agent that profile's db so turns persist to the right state.db.
            session_db = None
            if profile_home:
                home_token = set_openagents_home_override(profile_home)
                try:
                    from openagents_state import SessionDB
...
def _ensure_session_db_row(session: dict) -> None:
    ...
    if str(model_config.get("provider") or "").strip().lower() == "custom":
        try:
            from openagents_cli.runtime_provider import canonical_custom_identity
...
def _apply_managed(cfg: dict) -> dict:
    ...
    openagents_cli.config.load_config, so without this a managed skin / reasoning_effort
...
def resolve_skin() -> dict:
    try:
        from openagents_cli.skin_engine import init_skin_from_config, get_active_skin
...
def _resolve_startup_runtime() -> tuple[str, str | None]:
    ...
    try:
        from openagents_cli.models import detect_static_provider_for_model
...
def _load_enabled_toolsets() -> list[str] | None:
    ...
    if explicit and validate_toolset is not None:
        ...
        try:
            ...
            from openagents_cli.tools_config import _parse_enabled_flag
...
def _apply_model_switch(
    ...
    *,
    ...
    custom_provs = None
    try:
        from openagents_cli.config import get_compatible_custom_providers, load_config
...
def _session_info(agent, session: dict | None = None) -> dict:
    ...
    try:
        ...
        pass
    try:
        from openagents_cli.banner import get_available_skills
    ...
    try:
        ...
        pass
    try:
        from openagents_cli.banner import get_update_result
...
def _wire_callbacks(sid: str):
    ...
    def secret_cb(env_var, prompt, metadata=None):
        ...
        from openagents_cli.config import save_env_value_secure
...
def _available_personalities(cfg: dict | None = None) -> dict:
    try:
        ...
        try:
            from openagents_cli.config import load_config as _load_full_cfg
...
def _resolve_runtime_with_fallback(
    ...
    ``openagents_cli/cli_agent_setup_mixin.py``: when the primary provider
    raises ``AuthError``, walk the configured ``fallback_providers`` /
    ``fallback_model`` chain before giving up.
    """
    from openagents_cli.auth import AuthError
    from openagents_cli.runtime_provider import resolve_runtime_provider
...
def _make_agent(
    ...
    # block. Dashboard /api/ws uses openagents_cli.mcp_startup; TUI stdio keeps
...
def _(rid, params: dict) -> dict:
    ...
    # and each turn re-bind OPENAGENTS_HOME. None/own profile → launch (unchanged).
...
def _(rid, params: dict) -> dict:
    ...
    _enable_gateway_prompts()
    home_token = (
        set_openagents_home_override(str(profile_home)) if profile_home is not None else None
    )
    try:
        db.reopen_session(target)
        raw_history = db.get_messages_as_conversation(target)
    ...
    with _session_resume_lock:
        ...
        try:
            init_home_token = (
                set_openagents_home_override(str(profile_home))
                if profile_home is not None
                else None
            )
            try:
                ...
                if init_home_token is not None:
                    reset_openagents_home_override(init_home_token)
            if sid in _sessions:
                ...
                # Remember the profile home so each turn re-binds OPENAGENTS_HOME (the
...
def _pet_state_rows(spritesheet) -> list[str]:
    ...
    OpenAgents has to support both the legacy 8-row petdex atlas and the current
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from openagents_cli.pets import _set_active
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from agent.pet import store
        from openagents_cli.pets import _clear_active_if
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if new_slug != slug:
            try:
                from openagents_cli.pets import _rename_active_if
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from openagents_cli.pets import _set_enabled
...
def _(rid, params: dict) -> dict:
    ...
    """
    try:
        from openagents_cli.pets import set_pet_scale
...
def _pet_gen_root():
    ...
    root = get_openagents_home() / "cache" / "pet-gen"
...
# (agent/billing_view.py + openagents_cli/nous_billing.py) — same as /credits.


def _serialize_billing_error(exc) -> dict:
    """Map a BillingError into the result.error envelope the TUI branches on."""
    from openagents_cli.nous_billing import (
        BillingRateLimited,
...
def _(rid, params: dict) -> dict:
    ...
    """
    from openagents_cli.nous_billing import BillingError, post_charge
...
def _(rid, params: dict) -> dict:
    ...
    """
    from openagents_cli.nous_billing import BillingError, get_charge_status
...
def _(rid, params: dict) -> dict:
    ...
    """
    from openagents_cli.nous_billing import BillingError, patch_auto_top_up
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from openagents_cli.auth import step_up_nous_billing_scope
...
def _(rid, params: dict) -> dict:
    ...
    lines = [
        ...
        "",
        f"Session ID: {key}",
        f"Path: {display_openagents_home()}",
    ]
...
def _(rid, params: dict) -> dict:
    ...
    agent = session["agent"]
    # Mirror the classic CLI /save: snapshot under the OpenAgents profile home
    # (~/.openagents/sessions/saved/) rather than the project/workspace CWD, and
...
#
# Layout:  $OPENAGENTS_HOME/spawn-trees/<session_id>/<timestamp>.json
...
def _spawn_trees_root():
    ...
    root = get_openagents_home() / "spawn-trees"
    root.mkdir(parents=True, exist_ok=True)
    return root
...
def _run_prompt_submit(rid, sid: str, session: dict, text: Any) -> None:
    ...
    def run():
        approval_token = None
        session_tokens = []
        home_token = None  # per-turn OPENAGENTS_HOME override for a resumed remote profile
        ...
        try:
            ...
            if _profile_home_str:
                home_token = set_openagents_home_override(_profile_home_str)
            ...
            if images:
                try:
                    ...
                    from openagents_cli.config import load_config as _tui_load_config
            ...
            if (
                ...
                try:
                    from openagents_cli.voice import speak_text
                    ...
                    logger.warning("voice TTS skipped: openagents_cli.voice unavailable")
                except Exception as e:
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from openagents_cli.clipboard import has_clipboard_image, save_clipboard_image
    except Exception as e:
...
def _(rid, params: dict) -> dict:
    ...
    prompt = "\n".join(
        ...
        for line in [
            ...
            "Restart exactly the app intended for the Preview URL, not OpenAgents Desktop itself.",
            ...
            "Before declaring success, verify the Preview URL responds with the intended app, not OpenAgents Desktop. If it serves Hermes/Desktop UI or another unrelated app, stop that process and report failure.",
    ...
    if key == "model":
        try:
            ...
            if session:
                ...
                from openagents_cli.model_switch import parse_model_flags
...
def _is_repo_junk(root: str) -> bool:
    ...
    anything under OPENAGENTS_HOME (~/.openagents by default) — config/sessions/skills,
    ...
    hermes_home = os.path.realpath(str(get_openagents_home()))
...
def _(rid, params: dict) -> dict:
    ...
    if key == "provider":
        try:
            from openagents_cli.models import list_available_providers, normalize_provider
    ...
    if key == "profile":
        ...
        return _ok(rid, {"home": str(_hermes_home), "display": display_openagents_home()})
    if key == "project":
...
def _(rid, params: dict) -> dict:
    try:
        from openagents_cli.main import _has_any_provider_configured
        ...
        if not provider_configured and provider == "bedrock" and source in {
            ...
            return _ok(
                rid,
                {
                    ...
                    "source": source,
                    "error": "No OpenAgents provider is configured.",
                },
            )
...
@method("reload.env")
def _(rid, params: dict) -> dict:
    """Re-read ``~/.openagents/.env`` into the gateway process via
    ``openagents_cli.config.reload_env``, matching classic CLI's ``/reload``
...
@method("cli.exec")
def _(rid, params: dict) -> dict:
    """Run `python -m openagents_cli.main` with argv; capture stdout/stderr (non-interactive only)."""
    ...
    try:
        r = subprocess.run(
            [sys.executable, "-m", "openagents_cli.main", *argv],
            capture_output=True,
            text=True,
            timeout=min(int(params.get("timeout", 240)), 600),
            cwd=os.getcwd(),
            # cli.exec runs `python -m openagents_cli.main` (can drive the agent) →
...
def _(rid, params: dict) -> dict:
    ...
    if name == "moa":
        ...
        try:
            from openagents_cli.moa_config import moa_usage, normalize_moa_config
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from openagents_cli.commands import SlashCommandCompleter
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from openagents_cli.config import is_managed, save_env_value
        from openagents_cli.inventory import build_models_payload, load_picker_context
        ...
        # Save the key to ~/.openagents/.env
...
def _(rid, params: dict) -> dict:
    ...
    """
    try:
        from openagents_cli.auth import PROVIDER_REGISTRY, clear_provider_auth
        from openagents_cli.config import remove_env_value
    ...
    * ``tts`` — toggle speech-output of agent replies. Requires mode on
    ...
    if action in {"on", "off"}:
        ...
        if not enabled:
            ...
            try:
                from openagents_cli.voice import stop_continuous
...
def _(rid, params: dict) -> dict:
    ...
    try:
        if action == "start":
            ...
            from openagents_cli.voice import start_continuous
...
def _failure_messages(url: str, port: int, system: str) -> list[str]:
    from openagents_cli.browser_connect import manual_chrome_debug_command
...
def _browser_connect(rid, params: dict) -> dict:
    ...
    from openagents_cli.browser_connect import DEFAULT_BROWSER_CDP_URL
    ...
    try:
        ...
        if parsed.scheme in {"ws", "wss"} and parsed.path.startswith(
            ...
            if not ok and _is_default_local_cdp(parsed):
                from openagents_cli.browser_connect import try_launch_chrome_debug
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if action == "install":
            from openagents_cli.skills_hub import do_install
        ...
        if action == "browse":
            from openagents_cli.skills_hub import browse_skills
        ...
        if action == "inspect":
            from openagents_cli.skills_hub import inspect_skill
