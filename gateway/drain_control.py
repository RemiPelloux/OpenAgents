...
Contract (presence-based, mirroring ``.restart_notify.json``):
  ...
  * begin-drain  → write ``{OPENAGENTS_HOME}/.drain_request.json`` with
...
Why the epoch (NS-570). ``OPENAGENTS_HOME`` is a **durable** store — on Hermes
...
def drain_request_path(home: Optional[Path] = None) -> Path:
    """Absolute path to the drain-request marker, respecting OPENAGENTS_HOME."""
    base = home if home is not None else get_openagents_home()
...
def write_drain_request(
    *, principal: str = "drain-control", home: Optional[Path] = None
    ...
    later survives a machine restart on the durable OPENAGENTS_HOME volume can be
...
def drain_requested(*, home: Optional[Path] = None) -> bool:
    ...
    treated as absent: it survived a container/VM restart (OPENAGENTS_HOME is a
    durable Fly volume on OpenAgents Cloud) and the lifecycle action that triggered
