"""Relay/connector support package for the OpenAgents gateway.
...
def relay_connection_auth() -> tuple[Optional[str], Optional[str]]:
    ...
    ``~/.openagents/.env``): ``GATEWAY_RELAY_ID`` identifies the enrolled instance,
...
def self_provision_relay() -> bool:
    ...
    up. The creds live ONLY in process memory — never written to ``~/.openagents/.env``.
    ...
    try:
        from openagents_cli.auth import resolve_nous_access_token
    ...
    try:
        ...
        host = ""
    gateway_id = os.environ.get("GATEWAY_RELAY_ID", "").strip() or f"gw-{host or 'openagents'}"
