...
The PID file lives at ``{OPENAGENTS_HOME}/gateway.pid``.  OPENAGENTS_HOME defaults to
``~/.openagents`` but can be overridden via the environment variable.  This means
separate OPENAGENTS_HOME directories naturally get separate PID files — a property
...
_GATEWAY_KIND = "openagents-gateway"
...
def _get_pid_path() -> Path:
    """Return the path to the gateway PID file, respecting OPENAGENTS_HOME."""
...
def _get_lock_dir() -> Path:
    ...
    return state_home / "openagents" / _LOCKS_DIRNAME
...
def _gateway_command_subcommand(command: str | None) -> str | None:
    """Return the OpenAgents gateway lifecycle subcommand from a command line.
    ...
    test also matched ``openagents_cli.main gateway status`` and even unrelated
    ...
    (``"C:\\Program Files\\...\\openagents-gateway.exe"``) survive, and strips
    ...
    for token in tokens:
        ...
        if basename in ("openagents-gateway", "openagents-gateway.exe"):
            return "run"

    joined = " ".join(tokens)
    has_gateway_entry = (
        "openagents_cli.main" in joined
        or "openagents_cli/main.py" in joined
        or any(t.rsplit("/", 1)[-1] in ("openagents", "hermes.exe") for t in tokens)
    )
...
def _looks_like_gateway_process(pid: int) -> bool:
    """Return True when the live PID still looks like the OpenAgents gateway."""
...
def _profile_name_for_home(profile_home: Path) -> Optional[str]:
    """Return the profile id a OPENAGENTS_HOME directory represents, or None.

    A named profile's home is ``<root>/profiles/<name>`` (immediate parent is
    ``profiles``).  The root/default home (``~/.openagents`` or ``$OPENAGENTS_HOME``)
...
def _command_line_belongs_to_profile(command: str, profile_home: Path) -> bool:
    ...
    Mirrors ``openagents_cli.gateway._matches_current_profile`` so the dashboard's
    ...
    explicit ``OPENAGENTS_HOME=<path>``) on its argv; the default/root gateway runs
    ...
    # a non-matching explicit OPENAGENTS_HOME= on the argv. OPENAGENTS_HOME is usually
...
def read_runtime_status(path: Optional[Path] = None) -> Optional[dict[str, Any]]:
    ...
    can do so without mutating ``OPENAGENTS_HOME`` in-process.  Defaults to
...
def get_runtime_status_running_pid(
    ...
    *,
    ...
    OPENAGENTS_HOME.  Pass it when validating *another* profile's state file (the
...
def acquire_scoped_lock(
    ...
    at once (e.g. the same Telegram bot token across different OPENAGENTS_HOME dirs).
    """
...
def _consume_pid_marker_for_self(
    ...
    *,
    ...
    # running under a different OPENAGENTS_HOME. When two profile gateway
    # services share the same default ~/.openagents (OPENAGENTS_HOME not set
    ...
    # field is absent in markers written by older OpenAgents versions; treat
    ...
    if replacer_home is not None and replacer_home != str(get_openagents_home()):
        return False
...
def write_takeover_marker(target_pid: int) -> bool:
    ...
    try:
        ...
        record = {
            ...
            "replacer_hermes_home": str(get_openagents_home()),
