...
class GatewayConfig:
    ...
    sessions_dir: Path = field(
        default_factory=lambda: get_openagents_home() / "sessions"
    )
    ...
    # gateway behaves exactly as before — single OPENAGENTS_HOME, no profile stamping.
...
def load_gateway_config() -> GatewayConfig:
    ...
    2. ~/.openagents/config.yaml (primary user-facing config)
    3. ~/.openagents/gateway.json (legacy — provides defaults under config.yaml)
    4. Built-in defaults
    """
