...
There are two ways to add a platform to the OpenAgents gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.openagents/plugins/` (or under `plugins/platforms/`
...
**zero changes to core OpenAgents code**.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `openagents_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the OpenAgents core.
...
And add it to the `openagents-gateway` composite:
...
## 12. Status Display (`openagents_cli/status.py`)
...
## 13. Gateway Setup Wizard (`openagents_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ openagents_cli/ toolsets.py \
  --include="*.py" -l | sort -u
