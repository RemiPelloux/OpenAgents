...
- GET  /v1/models                  — lists openagents as an available model
- GET  /v1/capabilities            — machine-readable API capabilities for external UIs
- GET  /api/sessions               — list client-visible OpenAgents sessions
- POST /api/sessions               — create an empty OpenAgents session
...
AnythingLLM, NextChat, ChatBox, etc.) can connect to openagents
...
def _hermes_version() -> str:
    """Return the openagents version string, or "dev" if it can't be resolved.

    Tries the installed package metadata first (authoritative for a pip/uv
    install), then the in-tree ``openagents_cli.__version__`` (covers editable /
    ...
    def __init__(self, max_size: int = MAX_STORED_RESPONSES, db_path: str = None):
        ...
        if db_path is None:
            try:
                ...
                db_path = str(get_openagents_home() / "response_store.db")
            except Exception:
                db_path = ":memory:"
        ...
        # gracefully on NFS/SMB/FUSE-mounted OPENAGENTS_HOME (same filesystem
        # issue addressed for state.db/kanban.db — see
        # openagents_state._WAL_INCOMPAT_MARKERS).
...
def _derive_chat_session_id(
    ...
    the same OpenAgents session (and therefore the same Docker container sandbox
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    and routes them through openagents's AIAgent.
    """
    ...
    def _resolve_max_concurrent_runs() -> int:
        ...
        default = 10
        try:
            from openagents_cli.config import cfg_get, load_config
    ...
    async def _handle_models(self, request: "web.Request") -> "web.Response":
        """GET /v1/models — return openagents as an available model."""
    ...
    async def _handle_capabilities(self, request: "web.Request") -> "web.Response":
        ...
        try:
            ...
            profile_home = Path(
                os.environ.get("OPENAGENTS_HOME", Path.home() / ".openagents")
            )
            profiles = {
        ...
        return web.json_response({
            ...
            "runtime": {
                ...
                "split_runtime": False,
                "description": (
                    "The API server creates a server-side OpenAgents AIAgent; "
    ...
    async def _handle_list_sessions(self, request: "web.Request") -> "web.Response":
        """GET /api/sessions — list persisted OpenAgents sessions."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
    ...
    async def _handle_create_session(self, request: "web.Request") -> "web.Response":
        """POST /api/sessions — create an empty OpenAgents session row."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
    ...
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        if provided_session_id:
            ...
            # conversation map to the same OpenAgents session.  The first user
    ...
    def _notify_orchestrator_outcome(
        ...
        *,
        ...
        if not task_id:
            return
        try:
            from plugins.openos_engineering.orchestrator_client import (
                notify_task_outcome,
            )
            ...
            logger.exception(
                "[api_server] orchestrator outcome notify failed for %s", run_id
            )
            ...
            def _record_and_publish() -> None:
                ...
                if event.get("opencode"):
                    ...
                    if isinstance(previous, dict):
                        ...
                        current["files_edited"] = list(
                            dict.fromkeys([*previous_files, *current_files])
                        )
                    session_ids = list(
                        self._run_statuses.get(run_id, {}).get("opencode_session_ids")
                        or []
                    )
            ...
            if event_type == "tool.started":
                ...
                if tool_name == "invoke_opencode":
                    ...
                    if isinstance(result, str) and "Evidence: " in result:
                        try:
                            evidence = json.loads(
                                result.rsplit("Evidence: ", 1)[1].strip()
                            )
    ...
    async def _handle_runs(self, request: "web.Request") -> "web.Response":
        ...
        task_ctx = (
            body.get("task_context")
            if isinstance(body.get("task_context"), dict)
            else {}
        )
        ...
        async def _run_and_close():
            try:
                ...
                agent = self._create_agent(
                    ...
                    agent_profile=body.get("agent_profile")
                    if isinstance(body.get("agent_profile"), str)
                    else None,
                )
    ...
    async def _handle_openagentui_run(self, request: "web.Request") -> "web.Response":
        ...
        workflow = _oaui_store.get_workflow(
            workflow_id
        ) or _oaui_store.find_workflow_by_name(workflow_id)
        if workflow is None:
            return web.json_response(
                _openai_error(
                    f"Unknown OpenAgentUI workflow: {workflow_id}",
        ...
        if inputs is not None and not isinstance(inputs, dict):
            return web.json_response(
                _openai_error("'inputs' must be an object"), status=400
            )
        ...
        execution = await loop.run_in_executor(
            None, lambda: _run_workflow(workflow, inputs or {})
        )
        return web.json_response(execution.to_dict(), status=202)

    async def _handle_openagentui_get_execution(
        self, request: "web.Request"
    ) -> "web.Response":
        ...
        if (
            execution is None
            or execution.workflow_id != request.match_info["workflow_id"]
        ):
            return web.json_response(
                _openai_error("Execution not found", code="execution_not_found"),
                status=404,
            )
        return web.json_response(execution.to_dict())

    async def _handle_openagentui_approval(
        self, request: "web.Request"
    ) -> "web.Response":
        ...
        raw_choice = (
            str(body.get("choice") or body.get("decision") or "").strip().lower()
        )
        decision = {"approve": "approved", "allow": "approved", "deny": "rejected"}.get(
            raw_choice, raw_choice
        )
        if decision not in {"approved", "rejected"}:
            return web.json_response(
                _openai_error(
                    "'choice' must be one of: approve, deny (or approved/rejected)"
                ),
                status=400,
            )
        ...
        try:
            ...
            return web.json_response(
                _openai_error(str(exc), code="approval_error"), status=409
            )
        return web.json_response(execution.to_dict())
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            self._app.router.add_post(
                "/v1/openagentui/workflows/{workflow_id}/run",
                self._handle_openagentui_run,
            )
            self._app.router.add_get(
            ...
            # ~/.openagents/config.yaml and plant persistence. Sandboxing (Docker /
