...
# Audio file extensions OpenAgents recognizes for native audio delivery.
...
from openagents_constants import get_default_openagents_root, get_openagents_dir, get_openagents_home


GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    "Secure secret entry is not supported over messaging. "
    "Load this skill in the local CLI to be prompted, or add the key to ~/.openagents/.env manually."
)
...
# Default location: {OPENAGENTS_HOME}/cache/images/ (legacy: image_cache/)
IMAGE_CACHE_DIR = get_openagents_dir("cache/images", "image_cache")
...
AUDIO_CACHE_DIR = get_openagents_dir("cache/audio", "audio_cache")
...
VIDEO_CACHE_DIR = get_openagents_dir("cache/videos", "video_cache")
...
DOCUMENT_CACHE_DIR = get_openagents_dir("cache/documents", "document_cache")
SCREENSHOT_CACHE_DIR = get_openagents_dir("cache/screenshots", "browser_screenshots")
_OPENAGENTS_HOME = get_openagents_home()
_HERMES_ROOT = get_default_openagents_root()
...
MEDIA_DELIVERY_SAFE_ROOTS = (
    ...
    _OPENAGENTS_HOME / "image_cache",
    _OPENAGENTS_HOME / "audio_cache",
    _OPENAGENTS_HOME / "video_cache",
    _OPENAGENTS_HOME / "document_cache",
    _OPENAGENTS_HOME / "browser_screenshots",
    ...
    _OPENAGENTS_HOME / "cache" / "images",
    _OPENAGENTS_HOME / "cache" / "audio",
    _OPENAGENTS_HOME / "cache" / "videos",
    _OPENAGENTS_HOME / "cache" / "documents",
    _OPENAGENTS_HOME / "cache" / "screenshots",
)
...
def _profile_cache_roots() -> List[Path]:
    """Return per-profile canonical cache roots under the shared OpenAgents root.

    Profile gateways write generated artifacts to
    ``<root>/profiles/<name>/cache/{images,audio,...}``. The static safe-roots
    list only covers the *active* OPENAGENTS_HOME's cache, so a gateway running at
    the root (e.g. ``OPENAGENTS_HOME=/opt/data``) while the model emits a
    ...
    is consulted (which otherwise wins when OPENAGENTS_HOME is symlinked under a
...
def _media_delivery_denied_paths() -> List[Path]:
    ...
    # The active OpenAgents profile and shared OpenAgents root both contain control
    ...
    # OPENAGENTS_HOME root. The set mirrors the canonical read guard in
    ...
    # tree, so skills/, logs/, and ad-hoc agent-written files under ~/.openagents
    ...
    for hermes_root in (_OPENAGENTS_HOME, _HERMES_ROOT):
...
def _path_under_denied_prefix(resolved: Path) -> bool:
    ...
    secrets (``~/.openagents/.env``, ``auth.json``) are *separate, more-specific*
...
def validate_media_delivery_path(path: str) -> Optional[str]:
    ...
    # credential/secret stores under the OpenAgents root (~/.openagents/.env,
    ...
    # ``MEDIA:~/.openagents/google_token.json``) remain rejected.
...
class BasePlatformAdapter(ABC):
    ...
    # OpenAgents commands.  Default "/" (most platforms deliver "/approve" etc.
    ...
    def prefers_fresh_final_streaming(
        ...
        implementation supports. Telegram is the motivating case: OpenAgents sends
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        ...
        if session_key in self._active_sessions:
            ...
            from openagents_cli.commands import should_bypass_active_session
