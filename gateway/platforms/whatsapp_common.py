...
class WhatsAppBehaviorMixin:
    ...
    DEFAULT_REPLY_PREFIX: str = "⚕ *OpenAgents*\n────────────\n"
...
def resolve_whatsapp_bridge_dir() -> Path:
    """Resolve the WhatsApp bridge directory, mirroring to OPENAGENTS_HOME if needed.

    When the install tree is read-only (e.g., Docker /opt/hermes), this function
    mirrors the bridge source to a writable OPENAGENTS_HOME location and returns that
    ...
    # Try OPENAGENTS_HOME location first
    ...
    # Install dir is read-only, mirror to OPENAGENTS_HOME if needed
    if hermes_home_bridge.exists():
        return hermes_home_bridge

    # Mirror the bridge source to OPENAGENTS_HOME
    try:
