...
from openagents_cli.config import cfg_get, clear_model_endpoint_credentials
from utils import (
    atomic_json_write,
    atomic_yaml_write,
...
class GatewaySlashCommandsMixin:
    ...
    def _typed_command_prefix_for(self, platform) -> str:
        """Return the prefix users can always type to reach OpenAgents commands.
    ...
    async def _handle_version_command(self, event: MessageEvent) -> str:
        """Handle /version — show the running OpenAgents version."""
    ...
    async def _handle_help_command(self, event: MessageEvent) -> str:
        ...
        from openagents_cli.commands import gateway_help_lines
        lines = [
    ...
    async def _handle_codex_runtime_command(self, event: MessageEvent) -> str:
        ...
        Same surface as the CLI handler in cli.py:
            ...
            /codex-runtime auto             — OpenAgents default runtime
    ...
    async def _handle_personality_command(self, event: MessageEvent) -> str:
        ...
        if not personalities:
            return t("gateway.personality.none_configured", path=display_openagents_home())
    ...
    async def _handle_skills_command(self, event: MessageEvent) -> str:
        ...
        the pending JSON file under ``~/.openagents/pending/skills/``. (Note this is
        ...
        if args and args[0].lower() == "diff" and len(out) > 3000:
            ...
            out = (out[:3000]
                   + "\n… (truncated — full diff in "
                     f"~/.openagents/pending/skills/{pending_id}.json)")
        return out
    ...
    async def _handle_sessions_command(self, event: MessageEvent) -> str:
        ...
        from openagents_cli.session_listing import (
    ...
    async def _handle_update_command(self, event: MessageEvent) -> str:
        """Handle /update command — update OpenAgents to the latest version.
        ...
        if is_managed():
            return f"✗ {format_managed_message('update OpenAgents')}"
