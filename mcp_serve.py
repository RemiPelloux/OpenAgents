"""
OpenAgents MCP Server — expose messaging conversations as MCP tools.
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using OPENAGENTS_HOME."""
    try:
        from openagents_constants import get_openagents_home
        return get_openagents_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("OPENAGENTS_HOME", Path.home() / ".hermes")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from openagents_constants import get_openagents_home
        directory_file = get_openagents_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("OPENAGENTS_HOME", Path.home() / ".hermes")
...
class EventBridge:
    ...
    This is the OpenAgents equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from openagents_constants import get_openagents_home
            db_file = get_openagents_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("OPENAGENTS_HOME", Path.home() / ".hermes")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the OpenAgents MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "openagents",
        instructions=(
            "OpenAgents messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the OpenAgents MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
