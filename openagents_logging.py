"""Centralized logging setup for OpenAgents.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.openagents/logs/`` (profile-aware via ``get_openagents_home()``).
...
Component separation:
    ...
    gui.log receives dashboard-side records from ``openagents_cli.web_server``,
    ``openagents_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
# essentially always in OpenAgents (TUI, gateway, ``hy_memory`` server, MCP
...
from openagents_constants import get_config_path, get_openagents_home
...
COMPONENT_PREFIXES = {
    ...
    "tools": ("tools",),
    "cli": ("openagents_cli", "cli"),
    "cron": ("cron",),
    "gui": (
        "openagents_cli.web_server",
        "openagents_cli.pty_bridge",
        "tui_gateway",
        "uvicorn",
    ),
}
...
def setup_logging(
    *,
    ...
    """Configure the OpenAgents logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    hermes_home
        Override for the OpenAgents home directory.  Falls back to
        ``get_openagents_home()`` (profile-aware).
    log_level
    ...
    home = hermes_home or get_openagents_home()
    log_dir = home / "logs"
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    def __init__(self, *args, **kwargs):
        from openagents_cli.config import is_managed
