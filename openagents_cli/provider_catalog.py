...
they were not: the CLI picker read :data:`openagents_cli.models.CANONICAL_PROVIDERS`
...
* ``auth_type`` / ``api_key_env_vars`` / ``base_url_env_var`` from
  :data:`openagents_cli.auth.PROVIDER_REGISTRY` (credential truth), and
...
# openagents_cli.auth.PROVIDER_REGISTRY and providers.base.ProviderProfile.
...
def provider_catalog() -> list[ProviderDescriptor]:
    ...
    # OpenAgents overlays carry auth_type for providers that have no registry/profile
    ...
    for order, entry in enumerate(CANONICAL_PROVIDERS):
        ...
        # OpenAgents overlay (e.g. moa → "virtual"), then api_key.
        auth_type = (
