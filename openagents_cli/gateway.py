...
# display_openagents_home is imported lazily at call sites to avoid ImportError
# when openagents_constants is cached from a pre-update version during `hermes update`.
...
def _scan_gateway_pids(
    ...
    current_home = str(get_openagents_home().resolve())
    ...
    def _matches_current_profile(command: str) -> bool:
        ...
        # the command doesn't advertise *some other* profile. OPENAGENTS_HOME
...
def find_gateway_pids(
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            ...
            OpenAgents profile are returned.
    """
...
def find_profile_gateway_processes(
    ...
    """Return running gateway PIDs mapped to OpenAgents profiles via PID files."""
    _exclude = set(exclude_pids or set())
    processes: list[ProfileGatewayProcess] = []
    try:
        from gateway.status import get_running_pid
        from openagents_cli.profiles import list_profiles
    except Exception:
        return processes
...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    args = [get_python_path(), "-m", "openagents_cli.main"]
    if profile != "default":
...
def _capture_gateway_argv(pid: int) -> list[str] | None:
    ...
    Windows Scheduled Task running ``pythonw.exe -m openagents_cli.main gateway
...
def launch_detached_gateway_restart_by_cmdline(
    ...
    ``gateway run`` whose OPENAGENTS_HOME or argv doesn't match a known profile).
...
def _spawn_gateway_restart_watcher(old_pid: int, run_argv: list[str]) -> bool:
    ...
    from openagents_cli._subprocess_compat import (
    ...
    if sys.platform == "win32":
        try:
            from openagents_cli.gateway_windows import (
    ...
    watcher = textwrap.dedent(
        ...
        # the env (VIRTUAL_ENV / PYTHONPATH / OPENAGENTS_HOME) the windowless
        # base interpreter needs to import openagents_cli.  Empty on POSIX, where
...
def _sync_hermes_home_from_systemd_unit(system: bool) -> None:
    """When acting on a system-scope unit, adopt its ``OPENAGENTS_HOME``.

    Under ``sudo``, ``OPENAGENTS_HOME`` is stripped and ``HOME=/root``, so
    :func:`get_openagents_home` falls back to ``/root/.hermes`` — the wrong
    profile. The unit file pins ``OPENAGENTS_HOME`` for the actual gateway
    ...
    unit_home = env.get("OPENAGENTS_HOME", "").strip()
    if not unit_home:
        return
    current = os.environ.get("OPENAGENTS_HOME", "").strip()
    if current == unit_home:
        return
    os.environ["OPENAGENTS_HOME"] = unit_home
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    if is_linux() and is_container():
        ...
        try:
            from openagents_cli.service_manager import detect_service_manager, get_service_manager
...
def _gateway_list() -> None:
    ...
    """
    try:
        from openagents_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
def _reap_unsupervised_gateway_orphans() -> bool:
    ...
    in-process under a ``gateway restart`` argv (openagents_cli/gateway.py restart
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (OPENAGENTS_HOME-scoped).
...
from openagents_constants import is_container, is_termux, is_wsl
...
def _container_systemd_operational() -> bool:
    ...
    This is NOT our OpenAgents Docker image — that one runs s6-overlay as
...
_SERVICE_BASE = "openagents-gateway"
SERVICE_DESCRIPTION = "OpenAgents Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current OPENAGENTS_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (OPENAGENTS_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    from openagents_constants import get_default_openagents_root

    home = get_openagents_home().resolve()
    default = get_default_openagents_root().resolve()
    if home == default:
        return ""
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary OPENAGENTS_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(hermes_home: str | None = None, default_root: str | Path | None = None) -> str:
    """Return ``--profile <name>`` only when OPENAGENTS_HOME is a named profile.

    For ``~/.openagents/profiles/<name>``, returns ``"--profile <name>"``.
    For the default profile or hash-based custom paths, returns the empty string.

    Args:
        hermes_home: Optional explicit OPENAGENTS_HOME path. Defaults to the current
            ``get_openagents_home()`` value. Should be passed when generating a
            service definition for a different user (e.g. system service).
        default_root: Optional OpenAgents root to compare against. Used when
            generating a system service for another user from a sudo/root
            process, where ``Path.home()`` and ``get_default_openagents_root()``
            refer to root but the target profile lives under the service user.
    """
    import re
    from openagents_constants import get_default_openagents_root

    home = Path(hermes_home or str(get_openagents_home())).resolve()
    default = Path(default_root).resolve() if default_root else get_default_openagents_root().resolve()
    if home == default:
        return ""
    profiles_root = (default / "profiles").resolve()
    try:
...
def get_service_name() -> str:
    """Derive a systemd service name scoped to this OPENAGENTS_HOME.

    Default ``~/.openagents`` returns ``openagents-gateway`` (backward compatible).
    Profile ``~/.openagents/profiles/coder`` returns ``openagents-gateway-coder``.
    Any other OPENAGENTS_HOME appends a short hash for uniqueness.
    """
...
# Legacy service names from older OpenAgents installs that predate the
# openagents-gateway rename. Kept as an explicit allowlist (NOT a glob) so
# profile units (openagents-gateway-*.service) and unrelated third-party
# "openagents" units are never matched.
_LEGACY_SERVICE_NAMES: tuple[str, ...] = ("hermes.service",)

# ExecStart content markers that identify a unit as running our gateway.
# A legacy unit is only flagged when its file contains one of these.
_LEGACY_UNIT_EXECSTART_MARKERS: tuple[str, ...] = (
    "openagents_cli.main gateway",
    "openagents_cli/main.py gateway",
    "gateway/run.py",
    " hermes gateway ",
    "/hermes gateway ",
)
...
def _find_legacy_hermes_units() -> list[tuple[str, Path, bool]]:
    """Return ``[(unit_name, unit_path, is_system)]`` for legacy OpenAgents gateway units.

    Detects unit files installed by older OpenAgents versions that used a
    different service name (e.g. ``hermes.service`` before the rename to
    ``openagents-gateway.service``). When both a legacy unit and the current
    ``openagents-gateway.service`` are active, they fight over the same bot
    ...
    * Explicit allowlist of legacy names (no globbing). Profile units such
      as ``openagents-gateway-coder.service`` and unrelated third-party
...
def has_legacy_hermes_units() -> bool:
    """Return True when any legacy OpenAgents gateway unit files exist."""
    return bool(_find_legacy_hermes_units())


def print_legacy_unit_warning() -> None:
    """Warn about legacy OpenAgents gateway unit files if any are installed.
    ...
    if not legacy:
        return
    print_warning("Legacy OpenAgents gateway unit(s) detected from an older install:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
        print_info(f"    {path}  ({scope} scope)")
    print_info("  These run alongside the current openagents-gateway service and")
...
def remove_legacy_hermes_units(
    ...
    """Stop, disable, and remove legacy OpenAgents gateway unit files.
    ...
    legacy = _find_legacy_hermes_units()
    if not legacy:
        print("No legacy OpenAgents gateway units found.")
        return 0, []
    ...
    print()
    print("Legacy OpenAgents gateway unit(s) found:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode OpenAgents often sets ``HOME`` to a profile-scoped directory, but
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.openagents`` → ``ai.openagents.gateway.plist`` (backward compatible).
    Profile ``~/.openagents/profiles/coder`` → ``ai.openagents.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.openagents.gateway-{suffix}" if suffix else "ai.openagents.gateway"
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.hermes/openagents  -> /home/alice/.hermes/openagents
...
def _hermes_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current OPENAGENTS_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_openagents_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.hermes                    → /home/alice/.hermes
      /root/.hermes/profiles/coder     → /home/alice/.hermes/profiles/coder
      /opt/custom-hermes               → /opt/custom-hermes  (kept as-is)
    """
    current_openagents = get_openagents_home().resolve()
    current_default = (Path.home() / ".hermes").resolve()
    target_default = Path(target_home_dir) / ".hermes"

    # Default ~/.openagents → remap to target user's default
    if current_hermes == current_default:
        return str(target_default)

    # Profile or subdir of ~/.openagents → preserve the relative structure
    try:
        relative = current_hermes.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.openagents) — keep as-is
...
def _build_service_path_dirs(project_root: Path | None = None) -> list[str]:
    ...
    hermes_home = get_openagents_home()
...
def _stable_service_working_dir() -> str:
    ...
    uses an absolute python interpreter and ``-m openagents_cli.main``, so module
    ...
    ``OPENAGENTS_HOME`` is the stable anchor: it is where config/state/logs live,
    it never moves, and it is guaranteed to exist whenever the gateway is
    meaningfully installed. Fall back to ``PROJECT_ROOT`` only if OPENAGENTS_HOME
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        # Anchor cwd to the target user's OPENAGENTS_HOME (stable, always exists)
...
User={username}
Group={group_name}
ExecStart={python_path} -m openagents_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
...
Environment="OPENAGENTS_HOME={hermes_home}"
Restart=always
RestartSec=5
...
"""
    ...
    hermes_home = str(get_openagents_home().resolve())
...
def _temp_home_in_service_definition(definition: str) -> str | None:
    """Return the temp-dir OPENAGENTS_HOME baked into a service definition, or None.

    A generated systemd unit / launchd plist carries the resolved OPENAGENTS_HOME
    ...
    exported a throwaway ``OPENAGENTS_HOME=/tmp/...`` — writing it to the real
    ...
    with ``OPENAGENTS_HOME=/tmp/hermes-e2e-<pr>`` exported; the restart path's
    ...
    Matches both systemd ``Environment="OPENAGENTS_HOME=..."`` lines and launchd
    ``<key>OPENAGENTS_HOME</key><string>...</string>`` pairs.
    """
    import re
    import tempfile

    candidates = re.findall(r'OPENAGENTS_HOME=([^"\n]+)', definition)
    candidates += re.findall(
        r"<key>OPENAGENTS_HOME</key>\s*<string>(.*?)</string>", definition, flags=re.S
    )
    temp_roots = {
...
def _refuse_temp_home_service_write(definition: str, kind: str) -> bool:
    """Refuse (with guidance) when a service definition carries a temp OPENAGENTS_HOME."""
    temp_home = _temp_home_in_service_definition(definition)
    if temp_home is None:
        return False
    print(
        f"✗ Refusing to write the gateway {kind}: OPENAGENTS_HOME resolves to a "
        f"temporary directory ({temp_home})."
    )
    print(
        "  This usually means a test/E2E environment exported OPENAGENTS_HOME. "
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # sandboxed by the test conftest (only OPENAGENTS_HOME is). If a test
    # exercises ``run_gateway()`` with a pytest-tmp OPENAGENTS_HOME, the freshly
    # generated unit bakes that ``/tmp/pytest-of-.../hermes_test`` path into
    # ``Environment="OPENAGENTS_HOME=..."``. Writing that to the developer's
    ...
    # OPENAGENTS_HOME into the unit (manual E2E homes like /tmp/hermes-e2e-NNN
    ...
    print(
        f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current OpenAgents install"
    )
    return True
...
def systemd_install(
    ...
    # before installing the new openagents-gateway.service. If both remain, they
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.openagents.gateway-{suffix}" if suffix else "ai.openagents.gateway"
...
def _launchd_unsupported_marker_path() -> Path:
    return get_openagents_home() / ".gateway-launchd-unsupported"
...
def _gateway_run_command() -> list[str]:
    """Build the `python -m openagents_cli.main [--profile X] gateway run --replace` argv.

    Profile-aware: honors the active OPENAGENTS_HOME via `_profile_arg()` so the
    detached fallback launches into the same profile as the CLI invocation.
    """
    cmd = [get_python_path(), "-m", "openagents_cli.main"]
...
def _spawn_detached_gateway() -> bool:
    ...
    """
    from openagents_cli._subprocess_compat import windows_detach_popen_kwargs

    log_dir = get_openagents_home() / "logs"
...
def generate_launchd_plist() -> str:
    ...
    prog_args = [
        ...
        "<string>-m</string>",
        "<string>openagents_cli.main</string>",
    ]
    if profile_arg:
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print(
        "↻ Updated gateway launchd service definition to match the current OpenAgents install"
    )
    return True
...
def launchd_status(deep: bool = False):
    ...
    # OpenAgents PID tracking — may be a detached fallback process spawned when
    ...
    # launchd PID and the OpenAgents PID file).
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current OpenAgents install")
    else:
        print("⚠ Service definition is stale relative to the current OpenAgents install")
    ...
    if deep:
        log_file = get_openagents_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def _guard_named_profile_under_multiplexer(force: bool = False) -> None:
    ...
    try:
        from openagents_constants import get_default_openagents_root
        default_root = get_default_openagents_root()
...
def _guard_existing_gateway_process_conflict(replace: bool = False) -> None:
    ...
    unrelated gateway processes from other OPENAGENTS_HOME roots.
    """
...
def _guard_official_docker_root_gateway() -> None:
    ...
    print_error(
        "Refusing to run the OpenAgents gateway as root inside the official Docker image."
    )
    print(
        "  The image entrypoint normally drops privileges to the 'openagents' user. "
        "If you override entrypoint in Docker Compose, include "
        "/opt/hermes/docker/entrypoint.sh before the OpenAgents command."
    )
    print(
        "  Running the gateway as root can leave root-owned files in "
        "$OPENAGENTS_HOME and break later non-root dashboard/gateway runs."
    )
    print(
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False, force: bool = False):
    ...
    print("┌─────────────────────────────────────────────────────────┐")
    print("│           ⚕ OpenAgents Gateway Starting...                 │")
    print("├─────────────────────────────────────────────────────────┤")
...
_PLATFORMS = [
    ...
    # plugins/platforms/matrix/adapter.py::register(). #41112.
    {
        ...
        "vars": [
            ...
            },
            {
                ...
                "password": False,
                "help": "Channel ID where OpenAgents delivers cron results and notifications.",
            },
            {
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. OpenAgents connects via the BlueBubbles REST API and receives",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Enter them below and OpenAgents will connect automatically over WebSocket",
        ],
        "vars": [
            {
                "name": "YUANBAO_APP_ID",
...
def _all_platforms() -> list[dict]:
    ...
    # User-installed platform plugins under ~/.openagents/plugins/ still require
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_openagents_home() / "whatsapp" / "session" / "creds.json"
...
def _setup_standard_platform(platform: dict):
    ...
    if platform.get("key") == "telegram":
        ...
        if choice.strip() == "1":
            try:
                from openagents_cli.telegram_managed_bot import (
...
def _setup_weixin():
    ...
    print()
    print_info("  1. OpenAgents will open Tencent iLink QR login in this terminal.")
    print_info("  2. Use WeChat to scan and confirm the QR code.")
    print_info(
        "  3. OpenAgents will store the returned account_id/token in ~/.openagents/.env."
    )
    print_info(
    ...
    try:
        credentials = asyncio.run(qr_login(str(get_openagents_home())))
    except KeyboardInterrupt:
        print()
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``openagents_cli.setup`` (which
    imports from this module for the remaining bespoke flows).
    """
    from openagents_cli import setup as _s
...
def _configure_platform(platform: dict) -> None:
    ...
    is needed here. User-installed platform plugins under ~/.openagents/plugins/
    ...
    if required:
        print_info(f"  Set these env vars in ~/.openagents/.env: {', '.join(required)}")
    else:
        print_info(
...
def _dispatch_via_service_manager_if_s6(
    ...
    :mod:`openagents_cli.service_manager` are caught and surfaced as
    actionable CLI messages (no raw ``CalledProcessError`` traceback).
    """
    from openagents_cli.service_manager import (
    ...
    if profile is None:
        ...
        # OPENAGENTS_HOME=<root>/profiles/<name>, "" for the default root,
...
def _maybe_redirect_run_to_s6_supervision(args) -> bool:
    ...
    # `docker logs` and ${OPENAGENTS_HOME}/logs/gateways/<profile>/current,
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if supports_systemd_services():
            ...
            print(
                "  nohup hermes gateway run > ~/.openagents/logs/gateway.log 2>&1 &  # background"
            )
            sys.exit(1)
        elif is_container():
            ...
            from openagents_cli.service_manager import detect_service_manager
            if detect_service_manager() == "s6":
        ...
        if supports_systemd_services() and (
            ...
            if pids:
                ...
                if is_termux():
                    print(
                        "  nohup hermes gateway run > ~/.openagents/logs/gateway.log 2>&1 &  # Best-effort background start"
                    )
                elif is_wsl():
                    print(
                        "  tmux new -s hermes 'hermes gateway run'         # persistent via tmux"
        ...
        # Stop, disable, and remove legacy OpenAgents gateway unit files from
