"""
OpenAgents Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``hermes desktop`` / ``hermes gui``)
     Built inside the agent checkout under ``$OPENAGENTS_HOME/openagents/``:
       ...
       - ``$OPENAGENTS_HOME/desktop-build-stamp.json`` (the build freshness stamp)
...
the app name ("Hermes"), separate from ``$OPENAGENTS_HOME``:
...
def source_built_gui_artifacts(hermes_home: Path) -> "list[Path]":
    ...
    Python agent runs from ``openagents/`` source + ``venv/`` and never
...
def agent_is_installed(hermes_home: Path) -> bool:
    """Return True when a usable Python agent install exists under OPENAGENTS_HOME.
    ...
    if (agent_root / "openagents_cli").is_dir():
        return True
...
def gui_install_summary(hermes_home: "Path | None" = None) -> dict:
    ...
    """
    home: Path = hermes_home if hermes_home is not None else get_openagents_home()
...
def uninstall_gui(
    ...
    Never touches ``openagents/openagents_cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$OPENAGENTS_HOME``.
