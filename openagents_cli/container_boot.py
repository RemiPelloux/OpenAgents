...
def _strip_container_argv_prefix(argv: Sequence[str]) -> list[str]:
    ...
    * **s6-overlay v3:** PID 1 is ``s6-svscan`` and the command lives on the
    ...
    if args and Path(args[0]).name == "openagents":
        args = args[1:]
    return args
...
def _is_dashboard_container(argv: Sequence[str]) -> bool:
    ...
    the gateway and dashboard containers share a bind-mounted OPENAGENTS_HOME,
...
def _write_reconcile_log(
    ...
    """Append one line per profile to $OPENAGENTS_HOME/logs/container-boot.log.
...
def main() -> int:
    ...
    # bind-mounted OPENAGENTS_HOME, both race to flock() the same s6-log lock
