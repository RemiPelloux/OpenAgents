...
async def auth_middleware(request: Request, call_next):
    ...
    if path.startswith("/api/openagentui/") and not getattr(
...
def _apply_model_assignment_sync(
    ...
    if scope == "main":
        ...
        if provider.strip().lower() in {"custom", "local"} and base_url:
            try:
                from openagents_cli.main import (
                    _auto_provider_name,
...
def _oauth_provider_disconnect_hint(
    ...
    if provider.get("flow") == "external":
        if _oauth_provider_disconnect_command(provider):
            ...
            return (
                "Managed outside OpenAgents — run the disconnect command to remove it."
            )
...
async def prune_sessions_endpoint(body: SessionPrune):
    ...
    profile_home = (
        _cron_profile_home(body.profile)[1] if body.profile else get_openagents_home()
    )
    db = _open_session_db_for_profile(body.profile)
    try:
...
if sys.platform.startswith("win"):
    try:
        from openagents_cli.win_pty_bridge import (
...
_BUILTIN_DASHBOARD_THEMES = [
    {
        "name": "default",
        "label": "Hermes Teal",
        "description": "Classic dark teal — the canonical OpenAgents look",
    },
    {
        "name": "default-large",
