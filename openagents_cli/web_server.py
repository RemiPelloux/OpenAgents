"""
OpenAgents — Web UI server.

Provides a FastAPI backend serving the Vite/React frontend and REST API
endpoints for managing configuration, environment variables, and sessions.

Usage:
    python -m openagents_cli.main web          # Start on http://127.0.0.1:9119
    python -m openagents_cli.main web --port 8080
"""
...
from openagents_cli._subprocess_compat import windows_detach_flags, windows_hide_flags
...
from openagents_cli.memory_providers import (
    MemoryProvider,
    ProviderField,
    get_memory_provider,
)
...
def _start_desktop_cron_ticker(stop_event: "threading.Event", interval: int = 60) -> None:
    ...
    real gateway on the same OPENAGENTS_HOME — whichever process grabs the lock
    first wins the tick.
    """
...
def _warm_gateway_module() -> None:
    try:
        import openagents_cli.gateway  # noqa: F401
    except Exception:
        pass
...
def _resolve_restart_drain_timeout() -> float:
    try:
        from openagents_cli.gateway import _get_restart_drain_timeout
...
async def _lifespan(app: "FastAPI"):
    ...
    # Fire openagents_cli.gateway import into a background thread so the event
...
app = FastAPI(title="OpenAgents", version=__version__, lifespan=_lifespan)

# Memory-provider OAuth connect routes live in the memory layer, not here.
from openagents_cli.memory_oauth import router as _memory_oauth_router  # noqa: E402
...
#
# This list is defined in ``openagents_cli.dashboard_auth.public_paths`` so the
...
# ---------------------------------------------------------------------------
from openagents_cli.dashboard_auth.public_paths import (
...
async def _dashboard_auth_gate(request: Request, call_next):
    from openagents_cli.dashboard_auth.middleware import gated_auth_middleware
...
async def _token_auth_seam(request: Request, call_next):
    ...
    through untouched.
    """
    from openagents_cli.dashboard_auth.token_auth import token_auth_middleware
...
_SCHEMA_OVERRIDES: Dict[str, Dict[str, Any]] = {
    ...
    "updates.non_interactive_local_changes": {
        "type": "select",
        "description": (
            "When the chat app / gateway updates OpenAgents (no terminal prompt), "
...
def _normalize_main_model_assignment(provider: str, model: str) -> tuple[str, str]:
    ...
    - The "Change" picker sends a real OpenAgents provider slug — fine.
    ...
    1. Vendor-name → Hermes-provider mapping: when the provider string is
       not a known OpenAgents provider/alias (e.g. ``moonshotai``, ``x-ai`` is
    ...
    """
    from openagents_cli.models import _KNOWN_PROVIDER_NAMES, normalize_provider
    from openagents_cli.model_normalize import normalize_model_for_provider
    ...
    if canonical not in _KNOWN_PROVIDER_NAMES and "/" in model_in:
        ...
        try:
            ...
            cur_provider = ""
        from openagents_cli.models import _AGGREGATOR_PROVIDERS
...
def _default_hermes_root_is_opt_data() -> bool:
    raw = os.environ.get("OPENAGENTS_HOME", "").strip()
    if not raw:
        return False
    try:
        from openagents_constants import get_default_openagents_root

        root = get_default_openagents_root().expanduser().resolve(strict=False)
    except (OSError, RuntimeError):
...
def _dashboard_local_update_managed_externally() -> bool:
    ...
    a container — e.g. the hermes-webui image sharing the OpenAgents source tree),
    ...
    if _default_hermes_root_is_opt_data():
        return True
    try:
        from openagents_constants import is_container
...
def _managed_files_policy(request: Request, *, create_root: bool = True) -> ManagedFilesPolicy:
    ...
    # to /opt/data only when the installation's OpenAgents root is actually /opt/data
...
# gate + path hardening as /api/fs). Logic lives in ``openagents_cli.web_git``;
# these are thin, executor-offloaded wrappers (git/gh can block).
# ---------------------------------------------------------------------------

from openagents_cli import web_git as _web_git  # noqa: E402
...
async def get_status(profile: Optional[str] = None):
    ...
    # across that await. Status only resolves get_openagents_home() at call time
    ...
    try:
        ...
        # Windows install the first import of openagents_cli.gateway blocks the
        ...
        try:
            from openagents_cli.dashboard_auth import list_providers as _list_providers
        ...
        if not auth_required:
            status.update({
                "hermes_home": str(get_openagents_home()),
...
async def get_system_stats():
    ...
    try:
        ...
        try:
            du = psutil.disk_usage(str(get_openagents_home()))
            info["disk"] = {
                "total": du.total,
                "used": du.used,
                "free": du.free,
...
async def get_portal_status():
    ...
    auth: Dict[str, Any] = {}
    try:
        from openagents_cli.auth import get_nous_auth_status
...
async def run_debug_share_endpoint(body: DebugShareRequest | None = None):
    ...
    *synchronous*: the whole point of ``debug share`` is the set of shareable
    ...
    """
    from openagents_cli.debug import build_debug_share
...
# streamed to a per-action log file under ``~/.openagents/logs/<action>.log`` so
# the dashboard can tail them back to the user.
# ---------------------------------------------------------------------------

_ACTION_LOG_DIR: Path = get_openagents_home() / "logs"
...
def _spawn_hermes_action(subcommand: List[str], name: str) -> subprocess.Popen:
    ...
    Uses the running interpreter's ``openagents_cli.main`` module so the action
    ...
    cmd = [_dashboard_spawn_executable(), "-m", "openagents_cli.main", *subcommand]
...
def _gateway_display_command(profile: Optional[str], verb: str) -> str:
    return " ".join(["openagents", *_gateway_subcommand(profile, verb)])
...
async def update_hermes():
    ...
    if _dashboard_local_update_managed_externally():
        message = (
            "OpenAgents updates are managed outside this dashboard in "
...
async def check_hermes_update(force: bool = False):
    """Report whether a OpenAgents update is available, without applying it.
    ...
    Returns:
        ...
        current_version: installed OpenAgents version string
    ...
    try:
        ...
        if force:
            try:
                (get_openagents_home() / ".update_check").unlink()
            except OSError:
                pass
...
async def speak_text(payload: TTSSpeakRequest):
    ...
    configured in ``~/.openagents/config.yaml`` under ``tts.``.
    """
...
async def search_sessions(q: str = "", limit: int = 20, profile: Optional[str] = None):
    ...
    try:
        ...
        try:
            ...
            # logs, or another OpenAgents surface. FTS can't find those unless the
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    OpenAgents supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
def _memory_provider_config_path(provider: MemoryProvider) -> Path:
    return get_openagents_home() / provider.name / "config.json"
...
# in openagents_cli/config.py — listed here for deterministic ordering in the UI.
...
def get_recommended_default_model(provider: str = ""):
    ...
    if slug == "nous":
        try:
            ...
            from openagents_cli.auth import get_provider_auth_state
...
async def set_model_assignment(body: ModelAssignment, profile: Optional[str] = None):
    ...
    Writes to ``~/.openagents/config.yaml`` — applies to **new** sessions only.
...
def _apply_model_assignment_sync(
    ...
    if scope == "main":
        ...
        # post-model-selection behaviour (openagents_cli/main.py
        ...
        if provider.strip().lower() == "nous":
            try:
                from openagents_cli.nous_subscription import apply_nous_managed_defaults
        ...
        if provider.strip().lower() in {"custom", "local"} and base_url:
            try:
                from openagents_cli.main import _auto_provider_name, _save_custom_provider
...
def _catalog_provider_env_metadata() -> dict:
    ...
    """
    try:
        from openagents_cli.provider_catalog import provider_catalog
    except Exception:
        return {}
    ...
    try:
        from openagents_cli.config import OPTIONAL_ENV_VARS as _OPT
    except Exception:
        _OPT = {}
    _non_provider_keys = {
...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {
    "telegram": {
        "name": "Telegram",
        "description": "Run OpenAgents from Telegram DMs, groups, and topics.",
    ...
    "discord": {
        "name": "Discord",
        "description": "Connect OpenAgents to Discord DMs, channels, and threads.",
    ...
    "slack": {
        "name": "Slack",
        "description": "Use OpenAgents from Slack via Socket Mode. Add allowed Slack member IDs so connected bots can respond.",
    ...
    "mattermost": {
        "name": "Mattermost",
        "description": "Connect OpenAgents to Mattermost channels and direct messages.",
    ...
    "matrix": {
        "name": "Matrix",
        "description": "Use OpenAgents in Matrix rooms and direct messages.",
    ...
    "whatsapp": {
        "name": "WhatsApp",
        "description": "Use OpenAgents through the bundled WhatsApp bridge with QR-based auth.",
    ...
    "homeassistant": {
        "name": "Home Assistant",
        "description": "Control your smart home from OpenAgents via Home Assistant.",
    ...
    "email": {
        "name": "Email",
        "description": "Talk to OpenAgents through an IMAP/SMTP mailbox.",
        "docs_url": "https://openagents.nousresearch.com/docs/user-guide/messaging/",
        "env_vars": (
            "EMAIL_ADDRESS",
            "EMAIL_PASSWORD",
            "EMAIL_IMAP_HOST",
    ...
    "dingtalk": {
        "name": "DingTalk",
        "description": "Connect OpenAgents to DingTalk groups (钉钉).",
    ...
    "feishu": {
        "name": "Feishu / Lark",
        "description": "Use OpenAgents inside Feishu / Lark.",
    ...
    "google_chat": {
        "name": "Google Chat",
        "description": "Connect OpenAgents to Google Chat via Cloud Pub/Sub.",
        "docs_url": "https://openagents.nousresearch.com/docs/user-guide/messaging/google_chat",
    },
    "wecom": {
    ...
    "weixin": {
        ...
        "docs_url": "https://openagents.nousresearch.com/docs/user-guide/messaging/weixin/",
        "env_vars": ("WEIXIN_ACCOUNT_ID", "WEIXIN_TOKEN", "WEIXIN_BASE_URL"),
        "required_env": ("WEIXIN_ACCOUNT_ID", "WEIXIN_TOKEN"),
    },
    "bluebubbles": {
        "name": "BlueBubbles (iMessage)",
        "description": "Use OpenAgents through iMessage via a BlueBubbles server.",
    ...
    "qqbot": {
        "name": "QQ Bot",
        "description": "Connect OpenAgents to a QQ Bot from the QQ Open Platform.",
    ...
    "teams": {
        "docs_url": "https://openagents.nousresearch.com/docs/user-guide/messaging/teams",
    },
    "yuanbao": {
        "name": "Yuanbao (元宝)",
        "description": "Connect OpenAgents to Tencent Yuanbao.",
        "docs_url": "",
        "required_env": (),
    },
    "api_server": {
        "name": "API server",
        "description": "Expose OpenAgents as an OpenAI-compatible HTTP API for tools like Open WebUI.",
        "docs_url": "https://openagents.nousresearch.com/docs/user-guide/messaging/",
        "env_vars": (
            "API_SERVER_ENABLED",
            "API_SERVER_KEY",
    ...
    "webhook": {
        ...
        "docs_url": "https://openagents.nousresearch.com/docs/user-guide/messaging/webhooks/",
...
_TELEGRAM_ONBOARDING_DEFAULT_URL = "https://setup.openagents.nousresearch.com"
...
async def start_telegram_onboarding(body: TelegramOnboardingStart):
    bot_name = (body.bot_name or "OpenAgents").strip() or "OpenAgents"
...
def _restart_gateway_after_telegram_onboarding(profile: Optional[str] = None) -> dict[str, Any]:
    ...
    saved token waits on a separate dashboard restart click, OpenAgents appears
...
async def get_messaging_platforms(profile: Optional[str] = None):
    ...
    # all resolve against the requested profile's OPENAGENTS_HOME.
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    1. ``~/.openagents/.anthropic_oauth.json`` — Hermes-managed PKCE flow (what
    ...
    2. ``ANTHROPIC_API_KEY`` → ``ANTHROPIC_TOKEN`` → ``CLAUDE_CODE_OAUTH_TOKEN``
       ...
       env during ``load_openagents_dotenv()``, so the same check covers them)
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into OpenAgents even
...
def _copilot_acp_status() -> Dict[str, Any]:
    ...
    OpenAgents never claims a login state it can't verify.
    """
    return {
        "logged_in": False,
        "source": "copilot_cli",
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    ...
    },
    {
        ...
        "docs_url": "https://openagents.nousresearch.com/docs/guides/xai-grok-oauth",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from openagents_cli import auth as hauth
        if provider_id == "nous":
...
def _oauth_provider_disconnect_command(provider: Dict[str, Any]) -> Optional[str]:
    ...
    user sees exactly what executes, and OpenAgents then stops resolving the token.
...
def _oauth_provider_disconnect_hint(provider: Dict[str, Any], status: Dict[str, Any]) -> Optional[str]:
    ...
    if provider.get("flow") == "external":
        if _oauth_provider_disconnect_command(provider):
            ...
            return "Managed outside OpenAgents — run the disconnect command to remove it."
...
async def disconnect_oauth_provider(
    ...
    with _profile_scope(profile):
        ...
        if provider_id == "anthropic":
            ...
            try:
                from openagents_cli.auth import clear_provider_auth
        ...
        try:
            from openagents_cli.auth import clear_provider_auth, invalidate_nous_auth_status_cache
...
#          → persists to ~/.openagents/.anthropic_oauth.json AND credential pool
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both OpenAgents file AND credential pool.
...
def _nous_poller(session_id: str) -> None:
    ...
    try:
        ...
        with _profile_scope(_oauth_session_profile(session_id)):
            ...
            from openagents_cli.auth import persist_nous_credentials
...
def _codex_full_login_worker(session_id: str) -> None:
    ...
    try:
        ...
        from openagents_cli.auth import _save_codex_tokens
...
async def prune_sessions_endpoint(body: SessionPrune):
    ...
    profile_home = _cron_profile_home(body.profile)[1] if body.profile else get_openagents_home()
    db = _open_session_db_for_profile(body.profile)
    try:
...
async def get_logs(
    ...
    from openagents_cli.logs import _read_tail, LOG_FILES

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_openagents_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from openagents_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
def _cron_profile_home(profile: Optional[str]) -> Tuple[str, Path]:
    """Resolve a profile query value to (profile_name, OPENAGENTS_HOME)."""
    from openagents_cli import profiles as profiles_mod
...
def _call_cron_for_profile(target_profile: Optional[str], func_name: str, *args, **kwargs):
    ...
    from the process OPENAGENTS_HOME at import time. The dashboard is a single
    ...
    with _CRON_PROFILE_LOCK:
        ...
        token = set_openagents_home_override(str(home))
        ...
        try:
            ...
            reset_openagents_home_override(token)
...
#
# Wraps the same config data layer the CLI uses (openagents_cli.mcp_config), so
...
async def list_mcp_servers(profile: Optional[str] = None):
    from openagents_cli.mcp_config import _get_mcp_servers
...
async def add_mcp_server(body: MCPServerCreate, profile: Optional[str] = None):
    from openagents_cli.mcp_config import _get_mcp_servers, _save_mcp_server
...
async def remove_mcp_server(name: str, profile: Optional[str] = None):
    from openagents_cli.mcp_config import _remove_mcp_server
...
async def test_mcp_server(name: str, profile: Optional[str] = None):
    ...
    from openagents_cli.mcp_config import _get_mcp_servers, _probe_single_server
    ...
    def _probe_scoped():
        ...
        # OPENAGENTS_HOME override (see mcp_tool._wrap_with_home_override), so
...
async def list_mcp_catalog(profile: Optional[str] = None):
    ...
    """
    try:
        from openagents_cli import mcp_catalog
    except Exception as exc:
...
async def install_mcp_catalog_entry(body: MCPCatalogInstall, profile: Optional[str] = None):
    ...
    # The -p subprocess rebinds OPENAGENTS_HOME-derived paths in the child.
...
#
# Wraps the same JSON store the CLI uses (openagents_cli.webhook); the webhook
...
async def list_webhooks():
    import openagents_cli.webhook as wh
...
async def list_credential_pool():
    ...
    from openagents_cli.auth import read_credential_pool
...
def _dashboard_backup_dir() -> Path:
    return get_openagents_home() / "backups"
...
async def list_hooks():
    ...
    form can offer them.
    """
    from openagents_cli.config import load_config as _load_config
    from agent import shell_hooks

    try:
        from openagents_cli.plugins import VALID_HOOKS
...
async def list_checkpoints():
    ...
    cp_dir = get_openagents_home() / "checkpoints"
    sessions = []
    total_bytes = 0
    if cp_dir.is_dir():
...
async def preview_skill_hub(identifier: str = ""):
    ...
    def _run():
        from openagents_cli.skills_hub import _resolve_source_meta_and_bundle
        from tools.skills_hub import create_source_router
...
class ProfileCreate(BaseModel):
    ...
    # because skills_hub.SKILLS_DIR is import-time-bound and the OPENAGENTS_HOME
...
def _write_profile_model(profile_dir: Path, provider: str, model: str) -> None:
    ...
    context-local OPENAGENTS_HOME override so the write lands in the target
    ...
    """
    from openagents_constants import set_openagents_home_override, reset_openagents_home_override

    token = set_openagents_home_override(str(profile_dir))
    try:
        provider, model = _normalize_main_model_assignment(provider, model)
        cfg = load_config()
...
def _write_profile_mcp_servers(profile_dir: Path, servers: List["MCPServerCreate"]) -> int:
    ...
    Scopes ``load_config``/``save_config`` to ``profile_dir`` via the
    context-local OPENAGENTS_HOME override (same mechanism as
    ...
    """
    from openagents_constants import set_openagents_home_override, reset_openagents_home_override
    from openagents_cli.mcp_security import validate_mcp_server_entry
...
def _disable_unselected_skills(profile_dir: Path, keep: List[str]) -> int:
    ...
    install.) Scoped to the profile via the OPENAGENTS_HOME override. Returns the
    number of skills newly disabled.
    """
    from openagents_constants import set_openagents_home_override, reset_openagents_home_override
    from openagents_cli.skills_config import get_disabled_skills, save_disabled_skills
...
async def create_profile_endpoint(body: ProfileCreate):
    ...
    # profile's OPENAGENTS_HOME at import). Returns PIDs for the UI to poll.
...
async def get_active_profile_endpoint():
    ...
    the running dashboard/gateway is scoped to (derived from OPENAGENTS_HOME).
    """
    from openagents_cli import profiles as profiles_mod
    try:
...
async def update_profile_model_endpoint(name: str, body: ProfileModelUpdate):
    ...
    to the named profile via the OPENAGENTS_HOME override.
    """
    profile_dir = _resolve_profile_dir(name)
    provider = (body.provider or "").strip()
    model = (body.model or "").strip()
...
async def describe_profile_auto_endpoint(name: str, body: ProfileDescribeAuto):
    ...
    _resolve_profile_dir(name)
    try:
        from openagents_cli import profile_describer
...
def _profile_scope(profile: Optional[str]):
    ...
    1. ``load_config``/``save_config`` resolve ``get_openagents_home()`` at call
       time — the context-local override from ``set_openagents_home_override``
    ...
    retargeted to the *current* ``get_openagents_home()`` so writes land in the
    live home even when the import-time binding is stale (e.g. the process
    imported the modules before a OPENAGENTS_HOME override, or under test
    isolation).
    """
    requested = (profile or "").strip()
...
def _config_profile_scope(profile: Optional[str]):
    ...
    ``set_openagents_home_override`` contextvar — it does NOT swap the
    ...
    which is all endpoints that resolve ``get_openagents_home()`` at call time
...
async def get_skills(profile: Optional[str] = None):
    ...
    from openagents_cli.skills_config import get_disabled_skills
    with _profile_scope(profile):
        config = load_config()
...
async def get_toolsets(profile: Optional[str] = None):
    from openagents_cli.tools_config import (
...
async def save_toolset_env(name: str, body: ToolsetEnvUpdate, profile: Optional[str] = None):
    ...
    Writes each ``key: value`` to ``~/.openagents/.env`` via ``save_env_value`` —
...
async def run_toolset_post_setup(
    ...
    write per-profile state must see the same OPENAGENTS_HOME the rest of the
...
if sys.platform.startswith("win"):
    try:
        from openagents_cli.win_pty_bridge import WinPtyBridge as PtyBridge, PtyUnavailableError
    ...
    try:
        from openagents_cli.pty_bridge import PtyBridge, PtyUnavailableError
        _PTY_BRIDGE_AVAILABLE = True
...
def _ws_auth_reason(ws: "WebSocket") -> tuple[Optional[str], str]:
    ...
    * ``?internal=<process-credential>`` — the process-lifetime internal
    ...
    if auth_required:
        ...
        from openagents_cli.dashboard_auth.audit import AuditEvent, audit_log
        from openagents_cli.dashboard_auth.ws_tickets import (
            TicketInvalid,
...
def _resolve_chat_argv(
    ...
    matching what ``openagents_cli.main._launch_tui`` does for the CLI path.
    ...
    ``OPENAGENTS_HOME`` at the profile dir in the child env. Every spawned
    process (the TUI and the ``tui_gateway.entry`` it launches) resolves
    ``get_openagents_home()`` from that env var at its own import, so the child
    ...
    """
    from openagents_cli.main import PROJECT_ROOT, _make_tui_argv
    ...
    if profile_dir is not None:
        env["OPENAGENTS_HOME"] = str(profile_dir)
    ...
    # inherits the profile OPENAGENTS_HOME set above.
...
def _build_gateway_ws_url() -> Optional[str]:
    ...
    if getattr(app.state, "auth_required", False):
        from openagents_cli.dashboard_auth.ws_tickets import internal_ws_credential
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if not _PTY_BRIDGE_AVAILABLE:
        await ws.send_text(
            ...
            "\x1b[33mInstall OpenAgents inside WSL2 to use the dashboard's /chat "
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Thin re-export of :func:`openagents_cli.dashboard_auth.prefix.normalise_prefix`
    ...
    """
    from openagents_cli.dashboard_auth.prefix import normalise_prefix
...
def mount_spa(application: FastAPI):
    ...
    # (the MC Pages app), not the OpenAgents backend. Intercept CSS asset
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",       "label": "Hermes Teal",         "description": "Classic dark teal — the canonical OpenAgents look"},
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.openagents/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.openagents/dashboard-themes/*.yaml for user-created themes.

    Returns a list of fully-normalised theme definitions ready to ship
    to the frontend, so the client can apply them without a secondary
    round-trip or a built-in stub.
    """
    themes_dir = get_openagents_home() / "dashboard-themes"
...
async def get_dashboard_themes():
    ...
    from `~/.openagents/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as openagents_cli.plugins):
    1. User plugins:    ~/.openagents/plugins/<name>/dashboard/manifest.json
    ...
    from openagents_cli.plugins import get_bundled_plugins_dir
    bundled_root = get_bundled_plugins_dir()
    search_dirs = [
        (get_openagents_home() / "plugins", "user"),
    ...
    # ``openagents_cli/plugins.py`` and the documented user contract.
...
def _merged_plugins_hub() -> Dict[str, Any]:
    ...
    plugins_root_resolved = (get_openagents_home() / "plugins").resolve()
    rows: List[Dict[str, Any]] = []
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from openagents_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from openagents_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from openagents_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from openagents_cli.plugins_cmd import dashboard_remove_user_plugin
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if plugin.get("source") == "project":
            _log.warning(
                ...
                "~/.openagents/plugins/ if you trust it)",
...
from openagents_cli.dashboard_auth.routes import router as _dashboard_auth_router  # noqa: E402
...
def start_server(
    ...
    try:
        from openagents_cli.nous_auth_keepalive import start_nous_auth_keepalive
    ...
    if app.state.auth_required:
        ...
        if not list_providers():
            ...
            # Each provider plugin that ships with OpenAgents exposes a
    ...
    async def _serve():
        ...
        with server.capture_signals():
            ...
            print(f"  OpenAgents Web UI → http://{host}:{actual_port}")
