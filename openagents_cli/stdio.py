...
Entry points (``cli.py`` ``main``, ``openagents_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. OpenAgents is running
...
def _default_windows_editor() -> str:
    ...
    Set this before launching OpenAgents (User env var in Windows Settings, or
    export in a PowerShell profile) and OpenAgents picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    ...
    *spawned* processes only — already-running shells (including the one the
    ...
    Any subprocess OpenAgents spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "openagents", "git", "cmd"),
        os.path.join(local_appdata, "openagents", "git", "bin"),
        os.path.join(local_appdata, "openagents", "git", "usr", "bin"),
        # OpenAgents venv Scripts directory — host of the hermes.exe shim itself,
        ...
        os.path.join(local_appdata, "openagents", "openagents", "venv", "Scripts"),
