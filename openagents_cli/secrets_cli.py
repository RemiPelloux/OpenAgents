...
def register_cli(parent_parser: argparse.ArgumentParser) -> None:
    ...
    Called from ``openagents_cli.main`` as part of building the top-level
...
def cmd_setup(args: argparse.Namespace) -> int:
    ...
    console.print(
        ...
        "Secrets will be pulled at the start of every OpenAgents process."
    )
    console.print(
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if not token_set:
        console.print(
            f"\n  [yellow]Enabled but {token_env} is not set — OpenAgents will skip BSM "
