...
# argparse runs (it sets ``OPENAGENTS_HOME`` and strips itself from ``sys.argv``),
...
def _inherited_flag(parser, *args, **kwargs):
    """Register a flag that ``openagents_cli.relaunch`` should carry over when
...
def build_top_level_parser():
    ...
    parser = argparse.ArgumentParser(
        prog="openagents",
        description="OpenAgents - AI assistant with tool-calling capabilities",
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=False,
        help="Ignore ~/.openagents/config.yaml and fall back to built-in defaults (credentials in .env are still loaded)",
    )
    _inherited_flag(
        parser,
        "--ignore-rules",
    ...
    chat_parser = subparsers.add_parser(
        ...
        description="Start an interactive chat session with OpenAgents",
    )
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=argparse.SUPPRESS,
        help="Ignore ~/.openagents/config.yaml and fall back to built-in defaults (credentials in .env are still loaded). Useful for isolated CI runs, reproduction, and third-party integrations.",
    )
    _inherited_flag(
        chat_parser,
        "--ignore-rules",
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=argparse.SUPPRESS,
        help="Troubleshooting mode: disable ALL customizations — user config, AGENTS.md/memory injection, plugins, and MCP servers (implies --ignore-user-config and --ignore-rules). Use to isolate whether a problem comes from your setup or from OpenAgents itself.",
    )
    chat_parser.add_argument(
        "--source",
        default=None,
