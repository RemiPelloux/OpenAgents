"""Helpers for loading OpenAgents .env files consistently across entrypoints."""
...
# load_openagents_dotenv() calls (user env + project env, gateway hot-reload,
# tests) don't spam the same warning multiple times.
_WARNED_KEYS: set[str] = set()

# Map of env-var name → source label ("bitwarden", etc.) for credentials
# that were injected by an external secret source during load_openagents_dotenv().
...
# OPENAGENTS_HOME paths we've already pulled external secrets for during this
# process.  ``load_openagents_dotenv()`` is called at module-import time from
# several hot modules (cli.py, openagents_cli/main.py, run_agent.py,
...
def get_secret_source(env_var: str) -> str | None:
    ...
    during the current process's ``load_openagents_dotenv()`` call.  Returns
...
def reset_secret_source_cache() -> None:
    """Forget which OPENAGENTS_HOME paths have already had external secrets applied.
...
def _sanitize_env_file_if_needed(path: Path) -> None:
    ...
    We delegate to ``openagents_cli.config._sanitize_env_lines`` which
    already knows all valid OpenAgents env-var names and can split
    concatenated lines correctly.
    """
    if not path.exists():
        return
    try:
        from openagents_cli.config import _sanitize_env_lines
    except ImportError:
...
def load_openagents_dotenv(
    *,
    hermes_home: str | os.PathLike | None = None,
    project_env: str | os.PathLike | None = None,
) -> list[Path]:
    """Load OpenAgents environment files with user config taking precedence.

    Behavior:
    - `~/.openagents/.env` overrides stale shell-exported values when present.
...
def _apply_managed_env() -> None:
    ...
    Managed scope is machine-global (independent of OPENAGENTS_HOME / profile). v1
...
def _apply_external_secret_sources(home_path: Path) -> None:
    ...
    locate the access token) but BEFORE the rest of OpenAgents reads
    ``os.environ`` for credentials.  Any failure here is logged and
    swallowed — external secret sources must never block startup.

    Idempotent within a process: subsequent calls for the same
    ``home_path`` are no-ops.  ``load_openagents_dotenv()`` runs at import
    time from several hot modules (cli.py, openagents_cli/main.py,
