...
from openagents_cli.config import get_openagents_home, get_config_path, load_config, save_config
...
def _warn_if_gateway_running(auto_yes: bool) -> None:
    """Check if a OpenAgents gateway is running with connected platforms.
    ...
    print()
    print_error(
        "OpenAgents gateway is running with active connections: "
        + ", ".join(connected)
    )
    print_info(
...
def _cmd_migrate(args):
    """Run the OpenClaw → OpenAgents migration."""
    ...
    if preview_conflicts > 0 and not overwrite:
        ...
        print_info(
            "Each conflict is an item whose target already exists in ~/.openagents/. "
    ...
    # ── Phase 2b: Pre-apply backup of the OpenAgents home ─────────
    # Delegates to openagents_cli.backup.create_pre_migration_backup(), which
    ...
    if not no_backup:
        try:
            from openagents_cli.backup import create_pre_migration_backup, _format_size
            ...
            print_info(
                "Re-run with --no-backup to skip, or free up disk space under the OpenAgents home."
            )
