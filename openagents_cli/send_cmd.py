...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    if not any(platforms.values()):
        ...
        print("channel discovery can populate ~/.openagents/channel_directory.json.")
        return _SUCCESS_EXIT
...
def _load_hermes_env() -> None:
    """Populate ``os.environ`` from ``~/.openagents/.env`` AND bridge top-level
    ...
    1. ``load_dotenv(~/.openagents/.env)`` — brings bot tokens into the env.
    2. Bridge top-level simple values from ``~/.openagents/config.yaml`` into
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    # Bridge ~/.openagents/.env and ~/.openagents/config.yaml into os.environ so the
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            "Pipe text from any shell script to any messaging platform OpenAgents "
            "is already configured for. Reuses the gateway's platform "
            "credentials (~/.openagents/.env + ~/.openagents/config.yaml) — no LLM, "
