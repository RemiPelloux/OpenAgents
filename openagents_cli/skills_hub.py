#!/usr/bin/env python3
"""
Skills Hub CLI — Unified interface for the OpenAgents Skills Hub.
...
def _existing_categories() -> List[str]:
    """Return sorted subdirectory names under ``~/.openagents/skills/`` that look
...
def _prompt_for_category(c: Console, existing: List[str]) -> str:
    ...
    if existing:
        ...
        c.print(
            "[bold]Category[/] [dim](optional — press Enter to install flat at ~/.openagents/skills/<name>/)[/]"
        )
    try:
        ...
        if bundle.source == "official":
            c.print(
                Panel(
                    ...
                    "It ships with openagents but is not activated by default.\n"
                    "Installing will copy it to your skills directory where the agent can use it.\n\n"
                    f"Files will be at: [cyan]{display_openagents_home()}/skills/{category + '/' if category else ''}{bundle.name}/[/]",
...
def do_list(
    ...
    ``skills.disabled`` list because ``-p`` swaps ``OPENAGENTS_HOME`` at process
    ...
    if target == "github":
        ...
        if not auth.is_authenticated():
            c.print(
                ...
                f"Set GITHUB_TOKEN in {display_openagents_home()}/.env or run 'gh auth login'.\n"
            )
            return
    ...
    try:
        resp = httpx.post(
            ...
            json={
                ...
                "body": f"Submitting the `{skill_name}` skill via OpenAgents Skills Hub.\n\n"
                f"This skill was scanned by the OpenAgents Skills Guard before submission.",
...
def skills_command(args) -> None:
    """Router for `hermes skills <subcommand>` — called from openagents_cli/main.py."""
