...
- ``openagents_cli.providers``        -- canonical provider identity + overlays
- ``openagents_cli.model_normalize``  -- per-provider name formatting
...
_HERMES_MODEL_WARNING = (
    "Nous Research OpenAgents 3 & 4 models are NOT agentic and are not designed "
    "for use with OpenAgents. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Nous Research OpenAgents 3 / OpenAgents 4 chat families.
# The previous substring check (`"openagents" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``hermes-brain:qwen3-14b-ctx16k`` that just
# happen to carry "openagents" in their tag but are fully tool-capable.
#
...
def is_nous_hermes_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real Nous OpenAgents 3/4 chat model.
...
def _check_hermes_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Nous OpenAgents 3/4 chat model."""
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from openagents_cli.model_switch import
...
def switch_model(
    ...
    if explicit_provider:
        ...
        from openagents_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
        from openagents_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    ...
    # =================================================================
        ...
        try:
            from openagents_cli.config import load_config
            from openagents_cli.moa_config import exact_moa_preset_name, normalize_moa_config
    ...
    if provider_changed or explicit_provider:
        ...
        if explicit_provider and user_providers:
            from openagents_cli.providers import resolve_user_provider as _ruser
    ...
    # Mirrors the same logic in openagents_cli.runtime_provider.resolve_runtime_provider;
...
def prewarm_picker_cache_async() -> Optional["_threading.Thread"]:
    ...
    def _warm() -> None:
        try:
            from openagents_cli.inventory import load_picker_context
...
def list_authenticated_providers(
    ...
    *,
    ...
    Uses the curated model lists from openagents_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from openagents_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # https://openagents.nousresearch.com/docs/api/model-catalog.json so
    # newly added Portal models surface in the /model picker without
    # requiring a OpenAgents release. Falls back to the in-repo
    ...
    for hermes_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        if not has_creds:
            try:
                from openagents_cli.auth import _load_auth_store
    ...
    from openagents_cli.providers import HERMES_OVERLAYS
    from openagents_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → OpenAgents provider ID.
    # HERMES_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use OpenAgents IDs ("copilot").
    _mdev_to_openagents = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in HERMES_OVERLAYS.items():
        if pid.lower() in seen_slugs:
            continue

        # Resolve OpenAgents slug — e.g. "github-copilot" → "copilot"
        ...
        # reflects the active region (eu.*, ap.*) not the static us.* list.
            ...
            try:
                ...
                from openagents_cli.auth import get_provider_auth_state as _nous_state
    ...
    # with `hermes model`).
    try:
        from openagents_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # OpenAgents writes ``models:`` as a dict keyed by model id
            # (see openagents_cli/main.py::_save_custom_provider); older
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # OpenAgents's own writer uses; a hyphen variant is accepted
            ...
            # run_agent.py, openagents_cli/config.py) already consume that dict.
...
def _prepend_moa_picker_provider(providers: List[dict], current_provider: str = "") -> List[dict]:
    ...
    """
    try:
        from openagents_cli.inventory import _moa_provider_row
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`openagents_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from openagents_cli.models import fetch_openrouter_models
