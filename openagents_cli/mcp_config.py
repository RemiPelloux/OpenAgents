...
configuration in ~/.openagents/config.yaml under the ``mcp_servers`` key.
"""
...
from openagents_cli.config import (
    ...
    get_env_value,
    save_env_value,
    get_openagents_home,  # noqa: F401 — used by test mocks
)
...
def _prompt(question: str, *, password: bool = False, default: str = "") -> str:
    from openagents_cli.cli_output import prompt as _shared_prompt
...
def _resolve_mcp_server_config(config: dict) -> dict:
    ...
    ``~/.openagents/.env`` into ``os.environ`` and recursively interpolate any
...
def cmd_mcp_add(args):
    ...
    # openagents_cli/main.py for why the dest is renamed.
            ...
            if auth_type == "header" or not auth_type:
                ...
                if existing_key:
                    ...
                    if api_key:
                        ...
                        _success(f"Saved to {display_openagents_home()}/.env as {env_key}")
    ...
    if _save_mcp_server(name, server_config):
        print()
        _success(f"Saved '{name}' to {display_openagents_home()}/config.yaml ({tool_count}/{total} tools enabled)")
