...
OpenAgents owns its own uv binary at ``$OPENAGENTS_HOME/bin/uv`` (or ``uv.exe`` on
Windows).  Every code path that needs uv resolves it from that single location.
If the binary is missing, ``ensure_uv()`` bootstraps it via the official
standalone installer with ``UV_UNMANAGED_INSTALL`` / ``UV_INSTALL_DIR`` pointed
at ``$OPENAGENTS_HOME/bin`` so the installer writes directly there — no PATH
...
def managed_uv_path() -> Path:
    """Return the path where OpenAgents keeps *its* uv binary.

    ``$OPENAGENTS_HOME/bin/uv`` on POSIX, ``$OPENAGENTS_HOME\\bin\\uv.exe`` on
...
class _UvResult(str):
    ...
    the call site from the *old*, already-imported ``openagents_cli.main`` against
...
def _install_uv(target: Path) -> None:
    ...
    ``$OPENAGENTS_HOME/bin/`` instead of ``~/.local/bin/``.
    """
