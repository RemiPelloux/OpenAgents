...
``<root>`` is the **shared OpenAgents root** (the parent of any active
...
the profile's OpenAgents home but are otherwise isolated: a worker spawned
...
In standard installs ``<root>`` is ``~/.openagents``. In Docker / custom
deployments where ``OPENAGENTS_HOME`` points outside ``~/.openagents`` (e.g.
``/opt/hermes``), ``<root>`` is ``OPENAGENTS_HOME``. Legacy env-var
...
from openagents_cli.sqlite_util import add_column_if_missing as _add_column_if_missing
...
def _fire_kanban_lifecycle_hook(event: str, task_id: str, **fields: Any) -> None:
    ...
    ``profile_name`` is resolved from the active OPENAGENTS_HOME so dispatcher- and
...
# enough that kebab-case names like ``atm10-server`` or ``openagents``
...
def kanban_home() -> Path:
    """Return the shared OpenAgents root that anchors the kanban board.

    Resolution order:

    1. ``HERMES_KANBAN_HOME`` env var when set and non-empty (explicit
       override for tests and unusual deployments).
    2. ``get_default_openagents_root()``, which already returns ``<root>``
       when ``OPENAGENTS_HOME`` is ``<root>/profiles/<name>``, and returns
       ``OPENAGENTS_HOME`` directly for Docker / custom deployments.

    The kanban board is shared across profiles **by design** (see the
    module docstring). Resolving the kanban paths through the active
    profile's ``OPENAGENTS_HOME`` would silently fork the board per profile,
...
CREATE TABLE IF NOT EXISTS tasks (
    ...
    workspace_path       TEXT,
    branch_name          TEXT,
    -- Optional link to a first-class Project (openagents_cli/projects_db). When set,
...
def _guard_existing_db_is_healthy(path: Path) -> None:
    ...
    all sources OpenAgents treats as user-controlled-but-trusted on the
    ...
    * Neither → :func:`kanban_db_path` resolves via
    ...
    if resolved in _INITIALIZED_PATHS:
        ...
        try:
            ...
            with _INIT_LOCK:
                from openagents_state import apply_wal_with_fallback
    ...
    with _cross_process_init_lock(path):
        ...
        try:
            ...
            with _INIT_LOCK:
                ...
                # See openagents_state._WAL_INCOMPAT_MARKERS for detection logic.
                from openagents_state import apply_wal_with_fallback
...
def _canonical_assignee(assignee: Optional[str]) -> Optional[str]:
    ...
    if assignee is None:
        return None
    from openagents_cli.profiles import normalize_profile_name
...
def create_task(
    ...
    *,
    ...
    if project_id:
        try:
            from openagents_cli import projects_db as _pdb
...
def _resolve_worktree_workspace(
    ...
    launched from, e.g. the OpenAgents checkout). If no anchor is configured
...
def resolve_workspace(task: Task, *, board: Optional[str] = None) -> Path:
    ...
    - ``worktree``: a real linked git worktree. If ``workspace_path`` names
      a repo root, OpenAgents treats it as an anchor and materializes a linked
      worktree at ``<repo>/.worktrees/<task-id>``. If ``workspace_path`` names
      a concrete target path, OpenAgents creates/reuses that linked worktree. With
      no ``workspace_path``, OpenAgents anchors on the board's ``default_workdir``
      ...
      dispatcher's CWD. When ``branch_name`` is empty, OpenAgents uses
      ``wt/<task-id>``.
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    whose assignee maps to a real OpenAgents profile.
    ...
    if not rows:
        return False
    try:
        from openagents_cli.profiles import profile_exists  # local import: avoids cycle
    except Exception:
...
def _dispatch_once_locked(
    ...
    *,
    ...
    if _default_assignee:
        try:
            from openagents_cli.profiles import profile_exists as _pe
    ...
    for row in ready_rows:
        ...
        if not row_assignee:
            ...
            if _default_assignee and _default_assignee_resolved:
                ...
                result.skipped_unassigned.append(row["id"])
                continue
        # Skip ready tasks whose assignee is not a real OpenAgents profile.
    ...
    for row in review_rows:
        ...
        if not row["assignee"]:
            ...
            continue
        try:
            from openagents_cli.profiles import profile_exists
        except Exception:
            profile_exists = None  # type: ignore[assignment]
...
def _module_hermes_argv() -> list[str]:
    """Return the interpreter-bound OpenAgents CLI invocation."""
    # ``openagents_cli.main`` is the console-script target declared in
...
def _absolute_hermes_path(path: str) -> str:
    """Return an absolute filesystem path for a resolved OpenAgents shim."""
...
def _hermes_path_argv(path: str) -> list[str]:
    """Return argv for a resolved OpenAgents executable path.
...
def _resolve_hermes_argv() -> list[str]:
    ...
    2. ``shutil.which("openagents")`` — the console-script shim, normalized to
    ...
    3. ``sys.executable -m openagents_cli.main`` — fallback for setups where
       OpenAgents is launched from a venv and the ``hermes`` shim is not on
       the dispatcher's ``$PATH`` (cron, systemd ``User=`` services,
       launchd jobs, detached processes, etc.). Goes through the running
       interpreter so the result is independent of ``$PATH``.

    Mirrors ``gateway.run._resolve_hermes_bin`` for the same reason. Kept
    local (not imported from gateway) because ``openagents_cli`` sits below
    ...
    hermes_bin = (
        _safe_which_no_cwd("openagents") if _IS_WINDOWS else shutil.which("openagents")
    )
    if hermes_bin:
...
def _resolve_worker_cli_toolsets(hermes_home: Optional[str]) -> Optional[list[str]]:
    ...
    try:
        ...
        token = set_openagents_home_override(hermes_home)
        try:
            cfg = load_config()
        ...
        _log.debug(
            "kanban worker: could not resolve CLI toolsets for OPENAGENTS_HOME=%r (%s)",
            hermes_home,
            exc,
        )
        return None
...
def _default_spawn(
    ...
    *,
    ...
    # Inject OPENAGENTS_HOME so the worker reads the profile-scoped config.yaml
    # (fallback_providers, toolsets, agent settings, etc.) instead of the root
    # config.  Without this, `env = dict(os.environ)` copies only the parent's
    # env, and when the child process starts `hermes -p <name>` the
    # _apply_profile_override() runs *before* openagents_constants is imported.
    # If OPENAGENTS_HOME is absent from the child's env, get_openagents_home() falls
    ...
    try:
        env["OPENAGENTS_HOME"] = resolve_profile_env(profile_arg)
    except FileNotFoundError:
        # Profile dir doesn't exist — defer resolution to the CLI's
        # _apply_profile_override() via HERMES_PROFILE (set below).
        # This only happens in test fixtures where the isolated
        # OPENAGENTS_HOME never had profiles created.
        pass
    if task.tenant:
    ...
    # rewrites OPENAGENTS_HOME), its kanban paths still match the
    # dispatcher's. Belt-and-braces with the `get_default_openagents_root()`
    ...
    cmd = [
        *_resolve_hermes_argv(),
        "-p",
        profile_arg,
        # Worker subprocesses switch to a profile-scoped OPENAGENTS_HOME above,
    ...
    worker_toolsets = _resolve_worker_cli_toolsets(env.get("OPENAGENTS_HOME"))
    if worker_toolsets:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Install OpenAgents or activate its venv before running the kanban dispatcher."
        )
...
def list_profiles_on_disk() -> list[str]:
    ...
    - the implicit ``default`` profile when the default OpenAgents root exists

    Reads profile paths directly so this module has no import dependency on
    ``openagents_cli.profiles`` (which pulls in a large chunk of the CLI startup
    path).
    """
    try:
        from openagents_constants import get_default_openagents_root
