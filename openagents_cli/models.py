...
from openagents_cli import __version__ as _HERMES_VERSION
...
def _codex_curated_models() -> list[str]:
    ...
    """
    from openagents_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
# $OPENAGENTS_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $OPENAGENTS_HOME/models_dev_cache.json directly (no network) so this
    runs at import time without blocking. Falls back to ``_XAI_STATIC_FALLBACK``
    when the cache is empty or unreadable. OpenAgents refreshes the cache from
...
def union_with_portal_free_recommendations(
    ...
    *,
    ...
    older OpenAgents that doesn't ship that model in its hardcoded curated
...
def union_with_portal_paid_recommendations(
    ...
    *,
    ...
    picker even if the user is running an older OpenAgents that doesn't ship
...
def check_nous_free_tier(*, force_fresh: bool = False) -> bool:
    ...
    try:
        from openagents_cli.nous_account import get_nous_portal_account_info
...
def _nous_recommended_disk_path() -> "Path":
    ...
    return get_openagents_home() / "cache" / "nous_recommended_cache.json"
...
def fetch_nous_recommended_models(
    ...
    *,
    ...
    (``$OPENAGENTS_HOME/cache/nous_recommended_cache.json``) as last-known-good.
...
#
# Some vendors expose several OpenAgents provider slugs (one per endpoint /
...
# in openagents_cli/web_server.py and ``partition_nous_models_by_tier`` — which can
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    openagents is tool-calling-first — every provider path assumes the model
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from openagents_cli.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — openagents
...
def get_curated_nous_model_ids() -> list[str]:
    ...
    """
    try:
        from openagents_cli.model_catalog import get_curated_nous_models
...
def _resolve_nous_pricing_credentials() -> tuple[str, str]:
    ...
    """
    try:
        from openagents_cli.auth import resolve_nous_runtime_credentials
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from openagents_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def _resolve_static_model_alias(
    ...
    try:
        from openagents_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def normalize_provider(provider: Optional[str]) -> str:
    ...
    ``openagents_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether OpenAgents should expose the /fast toggle for this model."""
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         when the env var is set in ``~/.openagents/.env``.
    ...
    """
    try:
        from openagents_cli.auth import resolve_api_key_provider_credentials
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    on the platform appear in ``/model`` without a OpenAgents release.
    """
    normalized = normalize_provider(provider)
    if normalized == "openrouter":
    ...
    if normalized == "openai-codex":
        ...
        # a OpenAgents release). Falls back to the hardcoded catalog if no token
    ...
    if normalized == "nous":
        ...
        try:
            from openagents_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
            creds = resolve_nous_runtime_credentials()
            if creds:
        ...
        # added Portal models still surface without a OpenAgents release.
...
#
# Cache strategy:
#   - One JSON file at $OPENAGENTS_HOME/provider_models_cache.json
...
def _provider_models_cache_path() -> Path:
    from openagents_constants import get_openagents_home
    return get_openagents_home() / "provider_models_cache.json"
...
def _credential_fingerprint(provider: str) -> str:
    ...
    relevant tokens live in ``$OPENAGENTS_HOME/auth.json`` and external
...
def copilot_default_headers() -> dict[str, str]:
    ...
    """
    try:
        from openagents_cli.copilot_auth import copilot_request_headers
...
def _ollama_cloud_cache_path() -> Path:
    ...
    from openagents_constants import get_openagents_home
    return get_openagents_home() / "ollama_cloud_models_cache.json"
    ...
    *,
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so OpenAgents cannot verify the model name."
    ...
    if api_models is not None:
        ...
        if requested_for_lookup in set(api_models):
            ...
            # fetch) so we only accept models OpenAgents actually ships.  (#46850)
