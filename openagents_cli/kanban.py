"""CLI for the OpenAgents Kanban board — ``hermes kanban …`` subcommand.
...
from openagents_cli import kanban_swarm as ks
...
def _check_dispatcher_presence() -> tuple[bool, str]:
    ...
    - ``running=True``: a gateway is alive for this OPENAGENTS_HOME and its
...
def build_parser(parent_subparsers: argparse._SubParsersAction) -> argparse.ArgumentParser:
    ...
    kanban_parser = parent_subparsers.add_parser(
        ...
        description=(
            "Durable SQLite-backed task board shared across OpenAgents profiles. "
            "Tasks are claimed atomically, can depend on other tasks, and "
            "are executed by a named profile in an isolated workspace. "
            "See https://openagents.nousresearch.com/docs/user-guide/features/kanban "
    ...
    p_asg = sub.add_parser(
        ...
        help="List known profiles + per-profile task counts "
             "(union of ~/.openagents/profiles/ and current assignees on the board)",
    )
...
def kanban_command(args: argparse.Namespace) -> int:
    ...
    # OPENAGENTS_HOME. Previously only `init` and `daemon` triggered
...
def _dispatch_boards(args: argparse.Namespace) -> int:
    ...
    DB, so a fresh OPENAGENTS_HOME that has never called ``kanban init``
...
def _cmd_init(args: argparse.Namespace) -> int:
    ...
    if profiles:
        ...
        print("No profiles found under ~/.openagents/profiles/.")
...
def _cmd_daemon(args: argparse.Namespace) -> int:
    ...
    def _ready_queue_nonempty() -> bool:
        ...
        task whose assignee maps to a real OpenAgents profile (i.e. one the
...
def _cmd_specify(args: argparse.Namespace) -> int:
    ...
    from openagents_cli import kanban_specify as spec
...
def _cmd_decompose(args: argparse.Namespace) -> int:
    ...
    from openagents_cli import kanban_decompose as decomp
