...
Catalog policy:
- Entries are added only by merging a PR into openagents. Presence in the
...
- Secrets prompted at install time go to ``~/.openagents/.env`` (the
...
from openagents_constants import get_openagents_home, get_optional_mcps_dir
...
from openagents_cli.cli_output import prompt as _prompt_input
...
def _catalog_root() -> Path:
    """Return the optional-mcps/ directory shipped with this OpenAgents install."""
...
def _parse_manifest(path: Path) -> CatalogEntry:
    ...
    if mv != _MANIFEST_VERSION:
        raise CatalogError(
            ...
            f"(this OpenAgents understands version {_MANIFEST_VERSION})"
        )
...
def list_catalog() -> List[CatalogEntry]:
    ...
    UIs can tell the user their OpenAgents is out of date.
    """
    root = _catalog_root()
    if not root.exists():
        return []
...
def catalog_diagnostics() -> List[tuple]:
    ...
    is one of:
      - ``future_manifest`` — manifest_version is newer than this Hermes
        understands. Update OpenAgents to install this entry.
...
def _install_root() -> Path:
    ...
    root = get_openagents_home() / "mcp-installs"
...
def _do_git_install(entry: CatalogEntry) -> Path:
    """Clone the entry's repo into ``~/.openagents/mcp-installs/<name>`` and run
...
def _prompt_env_vars(specs: List[EnvVarSpec]) -> Dict[str, str]:
    ...
    non-secrets alike to ~/.openagents/.env via save_env_value()."""
...
def _build_server_config(
    ...
    by openagents_cli/mcp_config.py."""
    cfg: dict = {}
    t = entry.transport
    if t.type == "stdio":
...
def _probe_tools(name: str) -> Optional[List[tuple]]:
    ...
    try:
        ...
        from openagents_cli.mcp_config import _probe_single_server
...
def install_entry(entry: CatalogEntry, *, enable: bool = True) -> None:
    ...
    from openagents_cli.mcp_config import _save_mcp_server
    ...
    print(color(
        ...
        f"Start a new OpenAgents session to load its tools.",
        Colors.GREEN,
    ))
