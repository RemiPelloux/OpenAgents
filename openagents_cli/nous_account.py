    ...
    *,
    ...
    if not account_info.logged_in:
        if account_info.inference_credential_present:
            return (
                f"Nous inference credentials are configured, but OpenAgents cannot verify "
...
def _fresh_account_info(
    *,
    ...
    try:
        from openagents_cli.auth import get_provider_auth_state, resolve_nous_access_token
...
def _info_from_valid_jwt(
    ...
    *,
    ...
    try:
        from openagents_cli.auth import _decode_jwt_claims
    except Exception:
        return None
