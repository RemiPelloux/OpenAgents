"""
Security advisory checker for OpenAgents.
...
run in environments where the rest of OpenAgents failed to import.
"""
...
ADVISORIES: tuple[Advisory, ...] = (
    Advisory(
        ...
        remediation=(
            ...
            "Rotate API keys in ~/.openagents/.env (OpenRouter, Anthropic, OpenAI, "
...
def _installed_version(pkg_name: str) -> Optional[str]:
    ...
    except ImportError:  # py<3.8 — OpenAgents requires 3.10+ but defensive.
        return None
    try:
...
# ``~/.openagents/cache/advisory_banner_seen`` (a single line per advisory ID:
...
def _banner_cache_path() -> Optional[Path]:
    try:
        ...
        cache_dir = Path(get_openagents_home()) / "cache"
