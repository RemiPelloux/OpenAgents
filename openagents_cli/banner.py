...
from openagents_cli import __version__ as VERSION, __release_date__ as RELEASE_DATE
...
def _fetch_pypi_latest(package: str = "openagents") -> Optional[str]:
...
def check_for_updates() -> Optional[int]:
    """Check whether a OpenAgents update is available.
    ...
    if embedded_rev:
        ...
        # $OPENAGENTS_HOME/openagents/ may be a stale copy from --clone-all;
        # Path(__file__) always resolves to the actual installed checkout.
        repo_dir = Path(__file__).parent.parent.resolve()
        if not (repo_dir / ".git").exists():
            repo_dir = hermes_home / "openagents"
        if not (repo_dir / ".git").exists():
...
def _resolve_repo_dir() -> Optional[Path]:
    """Return the active OpenAgents git checkout, or None if this isn't a git install.

    Prefers the running code's location over the profile-scoped path
    because ``$OPENAGENTS_HOME/openagents/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``openagents_cli/build_info.py``) and return it as a frozen
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    OpenAgents checkout. Cached per-process. Release URL always points at the
    canonical NousResearch/openagents repo (forks don't get a link).
    """
...
def format_banner_version_label() -> str:
    ...
    base = f"OpenAgents v{VERSION} ({RELEASE_DATE})"
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    # Pip-install warning — `pip install openagents` is not the supported
