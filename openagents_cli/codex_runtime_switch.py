...
The actual runtime resolution happens in openagents_cli.runtime_provider's
...
def parse_args(arg_string: str) -> tuple[Optional[str], list[str]]:
    ...
    if raw in {"off", "default", "disable", "openagents"}:
        return "auto", []
...
def apply(
    ...
    *,
    ...
    if new_value == "codex_app_server":
        ...
        # sees the same tool surface AND can call back into OpenAgents for
        # browser/web/delegate_task/vision/memory tools (#7 fix).
        # Failures are non-fatal — the runtime change still proceeds.
        try:
            from openagents_cli.codex_runtime_plugin_migration import migrate
            ...
            # Permissions + OpenAgents tool callback are always-on production
            ...
            if "hermes-tools" in mig_report.migrated:
                ...
                msg_lines.append(
                    ...
                    "only on the default OpenAgents runtime — they need the "
                    "agent loop context.)"
                )
        ...
        msg_lines.append(
            ...
            "OpenAgents tools available via MCP callback)."
        )
        msg_lines.append(
        ...
        msg_lines.append("OpenAI/Codex turns will use the default OpenAgents runtime.")
