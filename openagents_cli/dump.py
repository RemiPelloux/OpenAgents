...
Outputs a compact, plain-text summary of the user's OpenAgents setup
...
from openagents_cli.config import get_openagents_home, get_env_path, get_project_root, load_config
...
def _get_git_commit(project_root: Path) -> str:
    ...
    ``HERMES_GIT_SHA`` build-arg (see ``openagents_cli/build_info.py``).
...
def _gateway_status() -> str:
    ...
    try:
        from openagents_cli.gateway import get_gateway_runtime_snapshot
...
def run_dump(args):
    ...
    lines.append(f"hermes_home:      {display_openagents_home()}")
