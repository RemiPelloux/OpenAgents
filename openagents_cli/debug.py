"""``hermes debug`` debug tools for OpenAgents.
...
Currently supports:
    hermes debug share    Upload debug report (system info + logs) to a
                          ...
                          ``~/.openagents/logs/*.log`` are not leaked into
...
def _pending_file() -> Path:
    """Path to ``~/.openagents/pastes/pending.json``.
    ...
    """
    return get_openagents_home() / "pastes" / "pending.json"
...
⚠️  This will upload the following to a public paste service:
  • System info (OS, Python version, OpenAgents version, provider, which API keys
...
def delete_paste(url: str) -> bool:
    ...
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "openagents/debug-share"},
    )
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    The replacement is stateless: we append to ``~/.openagents/pastes/pending.json``
...
def _upload_paste_rs(content: str) -> str:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "openagents/debug-share",
        },
    )
    with urllib.request.urlopen(req, timeout=30) as resp:
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    from openagents_cli.logs import LOG_FILES

    filename = LOG_FILES.get(log_name)
    return (get_openagents_home() / "logs" / filename) if filename else None
...
def run_debug_share(args):
    ...
    print(f"\nShare these links with the OpenAgents team for support.")
