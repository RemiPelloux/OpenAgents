...
Consent records live under ``~/.openagents/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.openagents/config.yaml``
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.openagents/config.yaml.")
