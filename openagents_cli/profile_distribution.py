"""Profile distributions — shareable, packaged OpenAgents profiles via git.

A distribution is a OpenAgents profile published as a git repository (or
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "openagents_state.db", "response_store.db",
    ...
    "backups", "cache",
    # Infrastructure
    "openagents", ".worktrees", "profiles", "bin", "node_modules",
...
def check_hermes_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires OpenAgents {op}{target}, "
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a OpenAgents profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a OpenAgents distribution."
        )
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.openagents).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
