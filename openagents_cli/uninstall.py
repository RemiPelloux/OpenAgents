"""
OpenAgents Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.openagents/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove OpenAgents PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing openagents or hermes PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# OpenAgents" comment and following line
                if '# OpenAgents' in line or '# openagents' in line:
                    skip_next = True
                    continue
                if skip_next and ('openagents' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing hermes
                if 'openagents' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    ...
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "openagents",
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains openagents_cli reference)
                content = wrapper.read_text()
                if 'openagents_cli' in content or 'openagents' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(hermes_home: Path) -> list:
    ...
    the FHS fix).  Only symlinks that resolve into this OpenAgents home's ``node``
...
def uninstall_gateway_service():
    ...
    try:
        from openagents_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from openagents_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``OPENAGENTS_HOME`` and ``HERMES_GIT_BASH_PATH``
...
def _hermes_path_markers(hermes_home: Path) -> list[str]:
    ...
    # all get swept.  Also match the bare openagents install dir.
    markers = [root + "\\openagents", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if OPENAGENTS_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "openagents".  We don't want to catch
...
def remove_hermes_env_vars_windows() -> list[str]:
    """Delete OPENAGENTS_HOME and HERMES_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("OPENAGENTS_HOME", "HERMES_GIT_BASH_PATH"):
                try:
...
def _is_default_hermes_home(hermes_home: Path) -> bool:
    ...
    try:
        ...
        return hermes_home.resolve() == get_default_openagents_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its OPENAGENTS_HOME directory.
    ...
    current OPENAGENTS_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m openagents_cli.main` so we don't depend on a `hermes`
    #    wrapper that may be half-removed mid-uninstall.
    hermes_invocation = [_sys.executable, "-m", "openagents_cli.main", "--profile", name]
    ...
    # 3. Wipe the profile's OPENAGENTS_HOME directory.
    try:
...
def run_gui_uninstall(args):
    ...
    userData dir — nothing under ``$OPENAGENTS_HOME`` config/sessions/.env, and
    never the Python agent or its venv.
    """
    from openagents_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    print(color("│         ⚕ OpenAgents Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()

    if not summary["gui_installed"]:
        print("No OpenAgents Chat GUI installation was found.")
        print(f"  Checked: {hermes_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The OpenAgents stays installed.", Colors.CYAN))
    print()
    ...
    if agent_is_installed(hermes_home):
        ...
        print(f"  • The OpenAgents at {hermes_home / 'openagents'}")
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The OpenAgents is still installed. Run 'openagents' to use the CLI,")
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.openagents/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.openagents/ for future reinstall
    """
    project_root = get_project_root()
    hermes_home = get_openagents_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie OPENAGENTS_HOMEs
    ...
    # full wipe (code + ~/.openagents data); otherwise keep-data. Named profiles
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ OpenAgents Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    # their alias wrappers, and wiping their OPENAGENTS_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL OpenAgents data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the OpenAgents code but keep your configuration and data.")
...
def _perform_uninstall(
    *,
    ...
    wipe ``$OPENAGENTS_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    if _is_windows():
        ...
        log_info("Removing OPENAGENTS_HOME / HERMES_GIT_BASH_PATH User env vars...")
    ...
    #     (only when they still point into this OpenAgents home's node dir, so we
    ...
    #     but the packaged app + Electron userData live OUTSIDE OPENAGENTS_HOME and
    #     must be cleaned explicitly here.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from openagents_cli.gui_uninstall import uninstall_gui
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.openagents/, just remove the openagents subdir
    ...
    #     under OPENAGENTS_HOME but they're install tooling, not config — safe to
    ...
    # 5. Optionally remove ~/.openagents/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile OPENAGENTS_HOME dirs live under
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  iex (irm https://openagents.nousresearch.com/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://openagents.nousresearch.com/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using OpenAgents! ⚕")
    print()
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m openagents_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import openagents_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``openagents_constants`` + ``openagents_cli.colors``
    (and lazily ``openagents_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m openagents_cli.uninstall")
    parser.add_argument(
        "--mode",
