"""
Configuration management for OpenAgents.

Config files are stored in ~/.openagents/ for easy access:
- ~/.openagents/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.openagents/.env         - API keys and secrets
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.openagents/config.yaml`` causes ``load_config()``
...
#   the next ``subprocess.run([...])`` OpenAgents makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. OpenAgents itself starts
...
# * ``OPENAGENTS_HOME`` / ``HERMES_PROFILE`` / ``HERMES_CONFIG`` /
#   ``HERMES_ENV`` — OpenAgents runtime location flags. Writing these into
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # OpenAgents runtime location — never via dashboard env writer.
    # NOT a HERMES_* blanket: integration credentials (HERMES_GEMINI_*,
    # HERMES_LANGFUSE_*, HERMES_SPOTIFY_*, ...) ARE allowed.
    "OPENAGENTS_HOME", "HERMES_PROFILE", "HERMES_CONFIG", "HERMES_ENV",
})
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or OpenAgents runtime location "
            "(OPENAGENTS_HOME, HERMES_PROFILE, ...) cannot be persisted via "
            "the env writer. If you really need this, edit "
            "~/.openagents/.env directly."
        )
...
from openagents_cli.default_soul import DEFAULT_SOUL_MD, is_legacy_template_soul
...
def get_managed_system() -> Optional[str]:
    ...
    managed_marker = get_openagents_home() / ".managed"
...
def is_managed() -> bool:
    """Check if OpenAgents is running in package-manager-managed mode.

    Two signals: the HERMES_MANAGED env var (set by the systemd service),
    or a .managed marker file in OPENAGENTS_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade openagents"
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``openagents_cli/`` — i.e. the git checkout for source
    ...
    interpreter, NOT of ``$OPENAGENTS_HOME``, which is why a code-scoped stamp
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how OpenAgents was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.

    Resolution order:
    1. Code-scoped stamp ``<install tree>/.install_method`` (next to the
       running code) — the authoritative marker.
    2. Legacy home-scoped stamp ``$OPENAGENTS_HOME/.install_method`` — read for
    ...
    Why the stamp is code-scoped, not home-scoped (issue: shared ``~/.openagents``)
    --------------------------------------------------------------------------
    The install method describes *the binary that is running*, but
    ``$OPENAGENTS_HOME`` is a shared DATA directory — the Docker docs deliberately
    bind-mount it (``~/.openagents:/opt/data``) so config/sessions/memory persist
    and can be shared with a host-side Desktop/CLI install. When a
    containerised gateway and a host install share one ``$OPENAGENTS_HOME``, a
    ...
    The supported installs self-identify via the code-scoped stamp:
      ...
      - the published ``nousresearch/openagents`` image bakes a ``docker``
    ...
    # 1. Code-scoped stamp — authoritative, immune to shared $OPENAGENTS_HOME.
    try:
    ...
    #    host install whose shared $OPENAGENTS_HOME was stamped by a co-located
    #    container, and honouring it wrongly blocks ``hermes update``.
    try:
        method = (
            (get_openagents_home() / ".install_method")
...
def _running_in_container() -> bool:
    """Thin wrapper around ``openagents_constants.is_container`` (import-safe)."""
    try:
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$OPENAGENTS_HOME``, so that two installs sharing one data directory
...
def is_uv_tool_install() -> bool:
    """Return True when the *running* OpenAgents lives in a ``uv tool`` layout.

    ``uv tool install openagents`` places the install at
    ``.../uv/tools/openagents/...`` (default ``~/.local/share/uv/tools``,
    ...
    ``openagents`` happens to be uv-tool-installed on the machine while
    the *active* OpenAgents is a regular pip/venv install, causing
    ...
    def _has_uv_tool_marker(path: str) -> bool:
        ...
        return "/uv/tools/openagents/" in norm + "/"
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "homebrew":
        return "brew upgrade openagents"
    if method == "docker":
        return "docker pull nousresearch/openagents:latest"
    if method == "pip":
        if is_uv_tool_install():
            return "uv tool upgrade openagents"
        import shutil
        if shutil.which("uv"):
            return "uv pip install --upgrade openagents"
        return "pip install --upgrade openagents"
...
# openagents_cli/main.py; lives here so the wording stays consistent and we
...
OpenAgents runs as a published image (nousresearch/openagents), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull nousresearch/openagents:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate openagents
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm nousresearch/openagents:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/nousresearch/openagents/tags
  • Your config and session history live under ``$OPENAGENTS_HOME`` (``/opt/data``
...
def format_managed_message(action: str = "modify this OpenAgents installation") -> str:
    ...
    if managed_system == "NixOS":
        ...
        return (
            f"Cannot {action}: this OpenAgents installation is managed by NixOS "
            f"(HERMES_MANAGED={env_hint}).\n"
            "Edit services.openagents.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this OpenAgents installation is managed by Homebrew "
            f"(HERMES_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade openagents"
        )

    return (
        f"Cannot {action}: this OpenAgents installation is managed by {managed_system}.\n"
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from OPENAGENTS_HOME/.container-mode.
    ...
    container_mode_file = get_openagents_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "openagents")
    exec_user = info.get("exec_user", "openagents")
...
# Re-export from openagents_constants — canonical definition lives there.
from openagents_constants import get_openagents_home  # noqa: F811,E402
from utils import atomic_replace, fast_safe_load

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_openagents_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_openagents_home() / ".env"
...
def _resolve_hermes_uid_gid() -> tuple[Optional[int], Optional[int]]:
    ...
    Docker containers running OpenAgents commonly set these to map the in-container
    user to a host user so volume-mounted state files end up with the right
    ownership. The entrypoint chowns the top-level OPENAGENTS_HOME once, but
...
def _secure_dir(path):
    ...
    The mode can be overridden via the OPENAGENTS_HOME_MODE environment variable
    (e.g. OPENAGENTS_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse OPENAGENTS_HOME to reach a served subdirectory.
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("OPENAGENTS_HOME_MODE", "").strip()
...
def _is_container() -> bool:
    ...
    When OpenAgents runs in a container with volume-mounted config files, forcing
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into OPENAGENTS_HOME, upgrading legacy empty templates.
...
def ensure_hermes_home():
    """Ensure ~/.openagents directory structure exists with secure permissions.
...
def _ensure_hermes_home_managed(home: Path):
    ...
    if not home.is_dir():
        raise RuntimeError(
            f"OPENAGENTS_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # app, ACP) in a code workspace, OpenAgents adds a coding operating brief
    ...
    "terminal": {
        ...
        #             OPENAGENTS_HOME/home for persistent state (default)
        #   real    — force the real OS-user HOME
        #   profile — force OPENAGENTS_HOME/home when it exists (old strict
        ...
        # skipped. When empty, OpenAgents auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), OpenAgents sources the user's shell rc files
        ...
        # bundled OpenAgents image, which drops to the `hermes` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, OpenAgents sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and OpenAgents should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    "checkpoints": {
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.openagents/checkpoints/`` size (MB).  When
    ...
    # AGENTS.md, CLAUDE.md, .hermes.md, or .cursorrules before OpenAgents applies
    ...
    # single read_file page exceeds these limits, OpenAgents truncates the
    ...
    # See tools/kanban_tools.py and openagents_cli/kanban_db.py for the actual
    ...
    "display": {
        ...
        "pet": {
            ...
            # get_openagents_home()/pets/. Empty → first installed pet.
            "slug": "",
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; OpenAgents appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.openagents/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.openagents/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # response. If not, OpenAgents feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before OpenAgents auto-pauses the goal and
    ...
    # always goes to ~/.openagents/skills/.
    "skills": {
    ...
    "curator": {
        ...
        # skipped), snapshot ~/.openagents/skills/ into
        # ~/.openagents/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *OpenAgents*" header.
    ...
    # stored approval from ~/.openagents/shell-hooks-allowlist.json.
    ...
    "security": {
        ...
        # ``openagents_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow OpenAgents to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # $OPENAGENTS_HOME/plugins/<name>/) to relocate the trigger — e.g. "chronos",
    ...
    "code_execution": {
        ...
        #     openagents's own python (sys.executable). Maximum isolation
    ...
    #
    # Core OpenAgents tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.openagents/logs/.
    ...
    # update model picker lists without shipping a openagents release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://openagents.nousresearch.com/docs/api/model-catalog.json",
    ...
    "gateway": {
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.openagents/.env,
        ...
        # behavior: files must live under the OpenAgents cache, under
        ...
        # cache (~/.openagents/cache/{documents,images,audio,video,screenshots})
    ...
    # Session storage — controls automatic cleanup of ~/.openagents/state.db.
    ...
    "sessions": {
        ...
        # rewrites ``~/.openagents/sessions/session_{sid}.json`` on every turn
    ...
    "updates": {
        # Run a full ``hermes backup``-style zip of OPENAGENTS_HOME before every
        # ``hermes update``.  Backups land in ``<OPENAGENTS_HOME>/backups/`` and
        # can be restored with ``hermes import <path>``.  Off by default:
        # zipping a large OPENAGENTS_HOME (sessions DB, caches, skills) can add
    ...
    "lsp": {
        ...
        #              ``<OPENAGENTS_HOME>/lsp/bin/`` on first use.
    ...
    # rather than storing them in ~/.openagents/.env.
    "secrets": {
        "bitwarden": {
            ...
            # in ~/.openagents/.env (or your shell) and never in config.yaml.
            ...
            # ~/.openagents/bin/ on first use.  When False you must install
    ...
    "computer_use": {
        ...
        # by default upstream. OpenAgents disables it for our users unless they
        # explicitly opt in here. When false (default), OpenAgents sets
    ...
    # OpenAgents Desktop (Electron app) launch options. These only affect
...
OPTIONAL_ENV_VARS = {
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the OpenAgents auth store)",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'openagents' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote OpenAgents API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote OpenAgents API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote OpenAgents API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older OpenAgents versions) write ``models`` as
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``openagents_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — OpenAgents won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("OPENAGENTS_HOME", "~/.openagents")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$OPENAGENTS_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_openagents_home() / "plugins"
    ...
    #   3. Creates `~/.openagents/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_hermes_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_openagents_home() / "logs" / "curator"
    ...
    if isinstance(raw_mcp_servers, dict):
        try:
            from openagents_cli.mcp_security import validate_mcp_server_entry as _validate_mcp_server_entry
        except Exception:
    ...
    try:
        ...
        from openagents_cli.toolset_validation import validate_platform_toolsets
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.openagents/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.openagents/config.yaml.
    ...
    (which change ``OPENAGENTS_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
def save_config(
    ...
    *,
    ...
    """Save configuration to ~/.openagents/config.yaml.\n
...
def _parse_env_value(raw_value: str) -> str:
    """Parse the small .env value subset OpenAgents writes itself."""
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.openagents/.env.
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real OpenAgents env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.openagents/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.openagents/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.openagents/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.openagents/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to OpenAgents — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known OpenAgents vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.openagents/.env or environment."""
...
def show_config():
    ...
    print(color("│              ⚕ OpenAgents Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    from openagents_cli.auth import get_anthropic_key
