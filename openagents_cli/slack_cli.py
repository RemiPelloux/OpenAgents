...
def _build_full_manifest(
    ...
    for a OpenAgents deployment — users can tweak them in the Slack UI after
    pasting.
    ...
    ``/new``, etc. work inline.
    """
    from openagents_cli.commands import slack_app_manifest
    ...
    if include_assistant:
        features["assistant_view"] = {
            "assistant_description": "Chat with OpenAgents in threads and DMs.",
        }
    ...
    return {
        ...
        "display_information": {
            "name": bot_name[:35],
            "description": (bot_description or "Your OpenAgents on Slack")[:140],
...
def slack_manifest_command(args) -> int:
    ...
    Flags (all parsed in ``openagents_cli/main.py``):
      --write [PATH]  Write to file instead of stdout (default path:
                      ``$OPENAGENTS_HOME/slack-manifest.json``)
    ...
    description = getattr(args, "description", None) or "Your OpenAgents on Slack"
    ...
    if write_target is not None:
        if isinstance(write_target, bool) and write_target:
            ...
            try:
                ...
                target = Path(os.environ.get("OPENAGENTS_HOME") or str(Path.home() / ".hermes")) / "slack-manifest.json"
        else:
        ...
        print(
            "\nNext steps:\n"
            "  1. Open https://api.slack.com/apps and pick your OpenAgents app\n"
