"""
Unified self-relaunch for OpenAgents CLI.
...
from openagents_cli._parser import (
...
def resolve_hermes_bin() -> Optional[str]:
    ...
    Priority:
      ...
      2. ``shutil.which("openagents")`` on PATH.
      3. ``None`` → caller should fall back to ``python -m openagents_cli.main``.
    ...
    ``hermes.exe`` on PATH or the ``sys.executable -m openagents_cli.main``
    fallback.
    """
    argv0 = sys.argv[0]
    ...
    # PATH lookup
    path_bin = shutil.which("openagents")
    if path_bin:
        return path_bin
...
def build_relaunch_argv(
    ...
    *,
    ...
    if bin_path:
        ...
        argv = [sys.executable, "-m", "openagents_cli.main"]
...
def relaunch(
    ...
    *emulates* exec by spawning the child and exiting the parent, but
    ...
    ``python -m openagents_cli.main``) or a ``.cmd`` batch file, and both
