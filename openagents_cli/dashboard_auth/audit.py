...
Profile-aware location: ``$OPENAGENTS_HOME/logs/dashboard-auth.log``.
...
from ``openagents_constants`` or other openagents_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$OPENAGENTS_HOME/logs/dashboard-auth.log`` with the standard fallback.

    Mirrors ``openagents_constants.get_openagents_home`` semantics: env var wins,
    else ``~/.openagents``. A local copy avoids an import cycle with the
    middleware which lives below ``openagents_cli``.
    """
    home = os.environ.get("OPENAGENTS_HOME") or str(Path.home() / ".hermes")
