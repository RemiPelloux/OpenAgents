...
the existing module-level functions in openagents_cli.gateway and
openagents_cli.gateway_windows directly. This protocol is a thin facade
...
def _s6_running() -> bool:
    ...
    detection always fails. Since every OpenAgents runtime call inside the
...
# in ``openagents_cli.gateway`` (systemd/launchd) and ``openagents_cli.gateway_windows``
...
class SystemdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``systemd_*`` functions in openagents_cli.gateway.
    ...
    def start(self, name: str) -> None:
        from openagents_cli.gateway import systemd_start
        systemd_start()

    def stop(self, name: str) -> None:
        from openagents_cli.gateway import systemd_stop
        systemd_stop()

    def restart(self, name: str) -> None:
        from openagents_cli.gateway import systemd_restart
        systemd_restart()

    def is_running(self, name: str) -> bool:
        from openagents_cli.gateway import _probe_systemd_service_running
        _, running = _probe_systemd_service_running()
        return running


class LaunchdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``launchd_*`` functions in openagents_cli.gateway."""

    kind: ServiceManagerKind = "launchd"

    def start(self, name: str) -> None:
        from openagents_cli.gateway import launchd_start
        launchd_start()

    def stop(self, name: str) -> None:
        from openagents_cli.gateway import launchd_stop
        launchd_stop()

    def restart(self, name: str) -> None:
        from openagents_cli.gateway import launchd_restart
        launchd_restart()

    def is_running(self, name: str) -> bool:
        from openagents_cli.gateway import _probe_launchd_service_running
        return _probe_launchd_service_running()


class WindowsServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around ``openagents_cli.gateway_windows`` (Scheduled Task /
...
def _profile_dir_for_gateway_service(name: str) -> Path:
    ...
    ``gateway stop --all``. Do not write the caller's OPENAGENTS_HOME blindly;
    derive the shared profile root from the current OPENAGENTS_HOME and map the
...
def _seed_supervise_skeleton(svc_dir: Path) -> None:
    ...
    (the ``hermes`` user — UID 10000 — running every OpenAgents runtime
...
class S6ServiceManager:
    ...
    def _render_run_script(
        ...
        The script:
          1. Sources OPENAGENTS_HOME (and any extra env) via with-contenv —
             so e.g. ``-e OPENAGENTS_HOME=/data/hermes`` is honored at run
        ...
        root OPENAGENTS_HOME profile" (the implicit profile that exists at
        the top of $OPENAGENTS_HOME, not under profiles/). It must be
        spelled this way because ``_profile_suffix()`` returns the
        empty string for the root profile, and the dispatcher in
        ``openagents_cli.gateway`` maps that empty string to the
        ``gateway-default`` service slot. Passing ``-p default`` here
        would instead look up ``$OPENAGENTS_HOME/profiles/default/`` — a
        ...
        each supervised profile gateway loads its own ``OPENAGENTS_HOME``,
        ...
        allocator in ``openagents_cli.profiles._allocate_gateway_port``).
        ...
        # profile's OPENAGENTS_HOME. Without it, a gateway started OUTSIDE s6
        # (a stray ``hermes gateway run`` from a shell, an agent action, or
        # the Open WebUI helper) grabs the per-OPENAGENTS_HOME PID lock first;
        ...
        # Each profile is scoped to its own OPENAGENTS_HOME and s6 guarantees a
    ...
    def _render_log_run(profile: str) -> str:
        ...
        OQ8-C: persist to ``${OPENAGENTS_HOME}/logs/gateways/<profile>/``.
        CRITICAL: the OPENAGENTS_HOME path is sourced from the runtime env
        via with-contenv — NOT Python-substituted at registration time
        — so a container started with ``-e OPENAGENTS_HOME=/data/hermes``
        ...
        return (
            ...
            f': "${{OPENAGENTS_HOME:=/opt/data}}"\n'
            f'log_dir="$OPENAGENTS_HOME/logs/gateways/{prof}"\n'
            f'mkdir -p "$log_dir"\n'
            ...
            f'chown hermes:hermes "$OPENAGENTS_HOME/logs/gateways" 2>/dev/null || true\n'
