...
3. Running inside a container with no persistent volume mount over the
   OPENAGENTS_HOME data dir (state is ephemeral — lost on container restart).
...
def _running_as_root() -> Optional[str]:
    ...
    return (
        ...
        "full host compromise. Run OpenAgents as an unprivileged user (or in a "
...
def _container_no_volume_mount(hermes_home: Optional[Path]) -> Optional[str]:
    ...
    return (
        ...
        "OPENAGENTS_HOME data directory."
    )
