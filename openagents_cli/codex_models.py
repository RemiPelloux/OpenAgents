...
DEFAULT_CODEX_MODELS: List[str] = [
    ...
    # in openagents_cli/models.py — only the openai-codex (OAuth) provider
    ...
    # OpenAgents uses. Removed here. If OpenAI re-enables them on Codex backend,
...
_FORWARD_COMPAT_TEMPLATE_MODELS: List[tuple[str, tuple[str, ...]]] = [
    ...
    # entitlement; OpenAgents does not.
...
def _read_cache_models(codex_home: Path) -> List[str]:
    ...
    if isinstance(entries, list):
        for item in entries:
            ...
            # public OpenAI API, while OpenAgents openai-codex talks to the same
