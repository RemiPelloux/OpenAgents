...
`hermes backup` creates a zip archive of the entire ~/.openagents/ directory
(excluding the openagents repo and transient files).

`hermes import` restores from a backup zip, overlaying onto the current
OPENAGENTS_HOME root.
"""
...
from openagents_constants import get_default_openagents_root, get_openagents_home, display_openagents_home
...
# ``openagents`` is special-cased to root level only in ``_should_exclude``
# so that skill directories like ``skills/autonomous-ai-agents/openagents/``
...
# OPENAGENTS_HOME gets walked file-by-file, ballooning a backup to hundreds of
...
_EXCLUDED_DIRS = {
    "openagents",     # the codebase repo — re-clone instead
    ...
    # Python dependency trees (plugin / MCP-server venvs under OPENAGENTS_HOME) —
...
# Reserved archive subtree for provider state that lives OUTSIDE OPENAGENTS_HOME
...
def _collect_memory_provider_external_paths() -> List[Path]:
    ...
    outside OPENAGENTS_HOME, resolved from config only (no network, no init).
...
def _should_exclude(rel_path: Path) -> bool:
    ...
    for part in parts:
        if part not in _EXCLUDED_DIRS:
            continue
        # ``openagents`` only matches at the root level (first component).
        # Nested directories with the same name — e.g.
        # ``skills/autonomous-ai-agents/openagents/`` — must be preserved.
        if part == "openagents" and part != parts[0]:
            continue
        return True
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside OPENAGENTS_HOME.
...
def run_backup(args) -> None:
    """Create a zip backup of the OpenAgents home directory."""
    hermes_root = get_default_openagents_root()

    if not hermes_root.is_dir():
        print(f"Error: OpenAgents home directory not found at {hermes_root}")
        sys.exit(1)
    ...
    # Collect files
    print(f"Scanning {display_openagents_home()} ...")
    ...
    for dirpath, dirnames, filenames in os.walk(hermes_root, followlinks=False):
        ...
        # ``openagents`` is only pruned at the root level; nested dirs
        # with the same name (e.g. in skills/) must be preserved.
        is_root = rel_dir == Path(".")
        orig_dirnames = dirnames[:]
        dirnames[:] = [
            d for d in dirnames
            if d not in _EXCLUDED_DIRS or (d == "openagents" and not is_root)
        ]
    ...
    # outside OPENAGENTS_HOME, so the walk above never sees it. Ask the active
    ...
    if external_to_add:
        print(
            ...
            f"stored outside {display_openagents_home()}."
        )
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a OpenAgents backup.
    ...
    if not found:
        return False, (
            "zip does not appear to be a OpenAgents backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    if len(first_parts) == 1:
        ...
        if prefix in {".hermes", "openagents"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_openagents_home()}")
        ...
        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has OpenAgents configuration.")
        ...
        for member in members:
            ...
            # location (e.g. ~/.honcho/config.json), NOT under OPENAGENTS_HOME.
        ...
        if restored_external:
            print(
                ...
                f"their original location(s) outside {display_openagents_home()}."
            )
        ...
        if profiles_dir.is_dir():
            try:
                ...
                # openagents_cli.profiles might not be available (fresh install)
        ...
        # Guidance
        print()
        if not (hermes_root / "openagents").is_dir():
            print("Note: The openagents codebase was not included in the backup.")
        ...
        print("Done. Your OpenAgents configuration has been restored.")
...
# Critical state files to include in quick snapshots (relative to OPENAGENTS_HOME).
...
_QUICK_STATE_FILES = (
    ...
    # are at $OPENAGENTS_HOME/<name> for the default/root profile; on non-root
    # profiles the real path is outside OPENAGENTS_HOME and the entry is silently
...
# Relative path of the cron job database inside OPENAGENTS_HOME. Kept in sync with
...
def restore_cron_jobs_if_emptied(
    ...
    Args:
        ...
        hermes_home: Override for the OpenAgents home directory (tests).
...
def run_quick_backup(args) -> None:
    ...
    if snap_id:
        ...
        print(f"  {len(snaps)} snapshot(s) stored in {display_openagents_home()}/state-snapshots/")
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of OPENAGENTS_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<OPENAGENTS_HOME>/backups/pre-update-<timestamp>.zip`` and
    ...
    """
    hermes_root = hermes_home or get_default_openagents_root()
    if not hermes_root.is_dir():
        return None
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of OPENAGENTS_HOME under ``backups/`` before a
    ...
    ``<OPENAGENTS_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
