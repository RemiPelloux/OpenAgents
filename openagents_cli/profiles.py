"""
Profile management for multiple isolated OpenAgents instances.

Each profile is a fully independent OPENAGENTS_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.openagents/profiles/<name>/`` by default.

The "default" profile is ``~/.openagents`` itself — backward compatible,
...
_PROFILE_DIRS = [
    ...
    # See openagents_constants.get_subprocess_home().
    "home",
]
...
# default profile (``~/.openagents``).  Named profiles never contain these
# directories at root, so the exclusion is gated to avoid silently dropping
# user data from a named-profile source.
#
# Rationale per item:
#   openagents  — git repo checkout (~84 MB source + ~3 GB venv)
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Three categories:
      ...
      2. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         OpenAgents infrastructure directories that only the default profile
         (``~/.openagents``) ever contains.  Gated on ``source_dir`` actually
...
# Directories/files to exclude when exporting the default (~/.openagents) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "openagents",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
_RESERVED_NAMES = frozenset({
    "openagents", "default", "test", "tmp", "root", "sudo",
})

# OpenAgents subcommands that cannot be used as profile names/aliases
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the hermes root, NOT to the current OPENAGENTS_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where OPENAGENTS_HOME points outside
    ``~/.openagents``, profiles live under ``OPENAGENTS_HOME/profiles/`` so
...
def _get_default_hermes_home() -> Path:
    """Return the default (pre-profile) OPENAGENTS_HOME path.

    In standard deployments this is ``~/.openagents``.
    In Docker/custom deployments where OPENAGENTS_HOME is outside ``~/.openagents``
    (e.g. ``/opt/data``), returns OPENAGENTS_HOME directly.
    """
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the OpenAgents installation itself or a common system binary.  "
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its OPENAGENTS_HOME directory."""
...
def create_wrapper_script(name: str, target: Optional[str] = None) -> Optional[Path]:
    ...
    if is_windows:
        ...
        try:
            hermes_exe = shutil.which("openagents") or "openagents"
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    try:
        ...
        from openagents_cli.config import check_config_version, migrate_config
...
def _check_gateway_running(profile_dir: Path) -> bool:
    ...
    *this* reader — which is exactly the case for a dashboard process that is
    ...
    agree.  Parameterized by ``profile_dir`` so it never mutates ``OPENAGENTS_HOME``.
    """
    try:
...
# ``config.yaml``. ``config.yaml`` is the user-facing OpenAgents config
...
def profiles_to_serve(multiplex: bool) -> List[Tuple[str, Path]]:
    ...
    - ``multiplex=True``: returns the default profile plus every valid named
      profile under ``profiles/``, each paired with its own OPENAGENTS_HOME.
    ...
    ``set_openagents_home_override`` when scoping a turn to that profile.
    """
...
def create_profile(
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.openagents)."
        )
    ...
    if clone_from is not None or clone_all or clone_config:
        if clone_from is None:
            ...
            source_dir = get_openagents_home()
        else:
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.openagents/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
    ...
    if not env_path.exists():
        try:
            env_path.write_text(
                "# Per-profile secrets for this OpenAgents profile.\n"
    ...
    if not soul_path.exists():
        try:
            from openagents_cli.default_soul import DEFAULT_SOUL_MD
    ...
    # Cloned configs can be older than the running OpenAgents (or predate schema
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches OPENAGENTS_HOME at module level.
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "OPENAGENTS_HOME": str(profile_dir)},
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.openagents).\n"
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    ``OPENAGENTS_HOME`` and binds the port resolved by ``gateway/config.py``
    ...
    try:
        ...
        from openagents_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set OPENAGENTS_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("OPENAGENTS_HOME")
    try:
        os.environ["OPENAGENTS_HOME"] = str(profile_dir)
        from openagents_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["OPENAGENTS_HOME"] = old_home
        elif "OPENAGENTS_HOME" in os.environ:
            del os.environ["OPENAGENTS_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.openagents/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
...
def get_active_profile_name() -> str:
    """Infer the current profile name from OPENAGENTS_HOME.

    Returns ``"default"`` if OPENAGENTS_HOME is not set or points to ``~/.openagents``.
    Returns the profile name if OPENAGENTS_HOME points into ``~/.openagents/profiles/<name>``.
    Returns ``"custom"`` if OPENAGENTS_HOME is set to an unrecognized path.
    """
    from openagents_constants import get_openagents_home
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.openagents itself — its parent is ~/ and its
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    # dir.  Importing as "default" would target ~/.openagents itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.openagents). "
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a OPENAGENTS_HOME path string.

    Called early in the CLI entry point, before any hermes modules
    are imported, to set the OPENAGENTS_HOME environment variable.
    """
    canon = normalize_profile_name(profile_name)
    validate_profile_name(canon)
    profile_dir = get_profile_dir(canon)
