"""``hermes logs`` — view and filter OpenAgents log files.

Supports tailing, following, session filtering, level filtering,
component filtering, and relative time ranges.  All log files live
under ``~/.openagents/logs/``.
...
def tail_log(
    ...
    *,
    ...
    log_path = get_openagents_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when OpenAgents runs — try 'hermes chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_openagents_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_openagents_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_openagents_home()}/logs/")
        return

    print(f"Log files in {display_openagents_home()}/logs/:\n")
    found = False
