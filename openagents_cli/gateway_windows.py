...
_TASK_DESCRIPTION = "OpenAgents Gateway - Messaging Platform Integration"
...
def _preserve_hermes_home_path(path: str | Path) -> str:
    """Render Hermes-owned paths under the configured OPENAGENTS_HOME spelling.
    ...
    path lives under OPENAGENTS_HOME.
    """
    candidate = Path(path)
    try:
        from openagents_cli.config import get_openagents_home

        home = Path(get_openagents_home())
...
def _current_profile_cli_args() -> list[str]:
    """Return CLI args that preserve the current OpenAgents profile."""
    from openagents_cli.gateway import _profile_arg
...
def _launch_elevated_gateway_command(command: str, extra_args: list[str] | None = None) -> bool:
    ...
    """
    _assert_windows()
    args = ["-m", "openagents_cli.main", *_current_profile_cli_args(), "gateway", command]
    if extra_args:
...
def get_task_name() -> str:
    ...
    """
    _assert_windows()
    # Local import to avoid circular module initialization during openagents_cli boot.
    from openagents_cli.gateway import _profile_suffix
...
def get_task_script_path() -> Path:
    ...
    (or ``<OPENAGENTS_HOME>/gateway-service/<task_name>.cmd`` so per-profile
    OpenAgents installs stay self-contained).
    """
    _assert_windows()
    from openagents_cli.config import get_openagents_home

    script_dir = Path(get_openagents_home()) / "gateway-service"
...
def _stable_gateway_working_dir(project_root: Path) -> str:
    ...
    Mirror the POSIX service invariant: anchor at ``OPENAGENTS_HOME`` whenever it
    ...
    source checkout only if ``OPENAGENTS_HOME`` cannot be used yet. Preserve the
...
def _build_gateway_cmd_script(
    ...
    The script:
      ...
      - exports OPENAGENTS_HOME, PYTHONIOENCODING, VIRTUAL_ENV
      - invokes ``pythonw -m openagents_cli.main [--profile X] gateway run``
    ...
    lines.append(f'set "OPENAGENTS_HOME={hermes_home}"')
    lines.append('set "PYTHONIOENCODING=utf-8"')
    lines.append('set "HERMES_GATEWAY_DETACHED=1"')
    pythonw_path, venv_dir, extra_pythonpath = _resolve_detached_python(python_path)
    # VIRTUAL_ENV lets the gateway's own python detection find the venv
    # if someone imports openagents_constants-based logic during startup.
    ...
    prog_args = [pythonw_path, "-m", "openagents_cli.main"]
    if profile_arg:
...
def _build_gateway_vbs_script(
    ...
    lines = [
        ...
        f"env.Item({_quote_vbs_string('OPENAGENTS_HOME')}) = {_quote_vbs_string(hermes_home)}",
...
def _write_task_script() -> Path:
    ...
    hermes_home = str(Path(get_openagents_home()))
...
def _install_scheduled_task(task_name: str, script_path: Path) -> tuple[bool, str]:
    ...
    Always recreate instead of ``/Change``. Older OpenAgents builds and failed
...
def _build_gateway_argv() -> tuple[list[str], str, dict[str, str]]:
    ...
    argv = [python_exe, "-m", "openagents_cli.main"]
    if profile_arg:
        argv.extend(profile_arg.split())
    argv.extend(["gateway", "run"])

    env_overlay = {
        "OPENAGENTS_HOME": hermes_home,
...
def windowless_gateway_restart_spec(
    ...
    ``openagents_cli`` package without the venv launcher's site config.

    Returns ``(new_argv, working_dir, env_overlay)``.  ``new_argv``
    preserves every argument after the interpreter (``-m openagents_cli.main
    ...
    from openagents_cli.config import get_openagents_home
    from openagents_cli.gateway import PROJECT_ROOT
    ...
    try:
        hermes_home = str(Path(get_openagents_home()).resolve())
    except Exception:
        hermes_home = ""
    ...
    if hermes_home:
        env_overlay["OPENAGENTS_HOME"] = hermes_home
    _prepend_pythonpath(
        env_overlay,
...
def _spawn_detached(script_path: Path | None = None) -> int:
    ...
    We spawn ``pythonw.exe -m openagents_cli.main gateway run``
    ...
    log_dir = Path(get_openagents_home()) / "logs"
...
def _prompt_install_choices(
    ...
    from openagents_cli.setup import prompt_yes_no
...
def _install_startup_fallback(script_path: Path, start_now: bool, detail: str) -> None:
    ...
    from openagents_cli.gateway import find_gateway_pids, _profile_arg
    ...
    if not _is_running_as_admin() and not elevated_handoff:
        ...
        if prompt_yes_no("  Open the UAC prompt now?", False):
            if _launch_elevated_install(force=force, start_now=start_now, start_on_login=start_on_login):
                print("✓ Launched elevated OpenAgents gateway install prompt.")
                if start_now:
...
def _report_gateway_start(via: str) -> None:
    ...
    if pids:
        ...
        from openagents_cli.config import get_openagents_home
        print(f"    type {Path(get_openagents_home())}\\logs\\gateway.log")
        print(f"    type {Path(get_openagents_home())}\\logs\\gateway-stdio.log")


def _print_next_steps() -> None:
    from openagents_cli.config import get_openagents_home

    hermes_home = Path(get_openagents_home())
    print()
    print("Next steps:")
        ...
        if code == 0:
            ...
            if prompt_yes_no("  Open the UAC prompt now?", False):
                if _launch_elevated_uninstall():
                    print("✓ Launched elevated OpenAgents gateway uninstall prompt.")
                    print("  Approve the Windows UAC prompt, then run: hermes gateway status")
                    return
