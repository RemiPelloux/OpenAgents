"""
Multi-provider authentication system for OpenAgents.

Supports OAuth device code flows (Nous Portal, future: OpenAI Codex) and
traditional API key providers (OpenRouter, custom endpoints). Auth state
is persisted in ~/.openagents/auth.json with cross-process file locking.
...
from openagents_cli.config import get_openagents_home, get_config_path, read_raw_config
from openagents_constants import OPENROUTER_BASE_URL, secure_parent_dir
...
SPOTIFY_DOCS_URL = "https://openagents.nousresearch.com/docs/user-guide/features/spotify"
SPOTIFY_DASHBOARD_URL = "https://developer.spotify.com/dashboard"
SPOTIFY_ACCESS_TOKEN_REFRESH_SKEW_SECONDS = 120

XAI_OAUTH_DOCS_URL = "https://openagents.nousresearch.com/docs/guides/xai-grok-oauth"
OAUTH_OVER_SSH_DOCS_URL = "https://openagents.nousresearch.com/docs/guides/oauth-over-ssh"
...
def _resolve_api_key_provider_secret(
    ...
    if provider_id == "copilot":
        ...
        try:
            from openagents_cli.copilot_auth import resolve_copilot_token, get_copilot_api_token
    ...
    for env_var in pconfig.api_key_env_vars:
        # Check both os.environ and ~/.openagents/.env file
...
def _format_nous_entitlement_auth_error(error: AuthError) -> str:
    try:
        from openagents_cli.nous_account import (
...
# =============================================================================
# Auth Store — persistence layer for ~/.openagents/auth.json
# =============================================================================

def _auth_file_path() -> Path:
    path = get_openagents_home() / "auth.json"
    # Seat belt: if pytest is running and OPENAGENTS_HOME resolves to the real
    # user's auth store, refuse rather than silently corrupt it. This catches
    # tests that forgot to monkeypatch OPENAGENTS_HOME, tests invoked without the
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        ...
        if resolved == real_home_auth:
            raise RuntimeError(
                ...
                "Set OPENAGENTS_HOME to a tmp_path in your test fixture, or run "
...
def _global_auth_file_path() -> Optional[Path]:
    ...
    directory (classic mode, or custom OPENAGENTS_HOME that is not a profile).
    ...
    try:
        ...
        global_root = get_default_openagents_root()
    except Exception:
        return None
    profile_home = get_openagents_home()
    try:
...
def _load_global_auth_store() -> Dict[str, Any]:
    ...
    ``~/.openagents/auth.json`` even when OPENAGENTS_HOME is set to a profile
    path. The hermetic conftest does not redirect ``HOME``, so
    ``get_default_openagents_root()`` for a profile-shaped OPENAGENTS_HOME can
...
def _get_config_hint_for_unknown_provider(provider_name: str) -> str:
    ...
    """
    try:
        from openagents_cli.config import validate_config_structure
...
def resolve_provider(
    ...
    *,
    ...
    for pid, pconfig in PROVIDER_REGISTRY.items():
        ...
        for env_var in pconfig.api_key_env_vars:
            if has_usable_secret(os.getenv(env_var, "")):
                ...
                # OAuth but has a stale key in ~/.openagents/.env isn't silently
    ...
    raise AuthError(
        ...
        "OPENAI_API_KEY, etc.) in ~/.openagents/.env.",
...
# =============================================================================
# Spotify auth — PKCE tokens stored in ~/.openagents/auth.json
# =============================================================================
...
def _make_xai_callback_handler(expected_path: str) -> tuple[type[BaseHTTPRequestHandler], dict[str, Any]]:
    ...
    class _XAICallbackHandler(BaseHTTPRequestHandler):
        ...
        def do_GET(self) -> None:  # noqa: N802
            ...
            # (#27385 — "callback received but OpenAgents times out") can produce
            ...
            # ``~/.openagents/logs/agent.log`` which captures INFO+ unconditionally).
            try:
                logger.info(
...
def _spotify_interactive_setup(redirect_uri_hint: str) -> str:
    ...
    resulting client_id to ~/.openagents/.env, and return it.
    ...
    print("       App name:     anything (e.g. openagents)")
    ...
    print()
    print("Saved HERMES_SPOTIFY_CLIENT_ID to ~/.openagents/.env")
    print()
    return raw
...
# =============================================================================
# OpenAI Codex auth — tokens stored in ~/.openagents/auth.json (not ~/.codex/)
#
# OpenAgents maintains its own Codex OAuth session separate from the Codex CLI
# and VS Code extension. This prevents refresh token rotation conflicts
# where one app's refresh invalidates the other's session.
# =============================================================================

def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    """Read Codex OAuth tokens from OpenAgents auth store (~/.openagents/auth.json).
...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None, label: str = None) -> None:
    """Save Codex OAuth tokens to OpenAgents auth store (~/.openagents/auth.json)."""
...
def _recover_codex_tokens_from_cli(reason: str) -> Optional[Dict[str, str]]:
    """Adopt a valid Codex CLI token pair into OpenAgents auth, if available."""
...
def refresh_codex_oauth_pure(
    ...
    *,
    ...
    """Refresh Codex OAuth tokens without mutating OpenAgents auth state."""
...
def _refresh_codex_auth_tokens(
    ...
    Saves the new tokens to OpenAgents auth store automatically.
    """
    try:
        ...
        # Self-heal cross-store refresh_token rotation. OpenAgents keeps its OWN
        ...
        # the Codex CLI (or another OpenAgents process) rotates the shared token,
...
def resolve_codex_runtime_credentials(
    *,
    ...
    if should_refresh:
        # Re-read under lock to avoid racing with other OpenAgents processes
...
# =============================================================================
# xAI Grok OAuth — tokens stored in ~/.openagents/auth.json
# =============================================================================
...
def _write_through_xai_oauth_to_global_root(state: Dict[str, Any]) -> None:
    ...
    # ~/.openagents/auth.json even when OPENAGENTS_HOME points at a profile path
...
def _xai_validate_oauth_endpoint(url: str, *, field: str) -> str:
    ...
    output is cached in ``~/.openagents/auth.json``. A single MITM during initial
...
def refresh_xai_oauth_pure(
    ...
    *,
    ...
    # written by an older OpenAgents (or hand-edited) may carry a non-xAI
...
# ``get_default_openagents_root()`` returns — ``~/.openagents`` on Linux/macOS,
# ``%LOCALAPPDATA%\hermes`` on native Windows, or the Docker/custom root.
# It is OUTSIDE any named profile's OPENAGENTS_HOME so named profiles (which
...
def _nous_shared_auth_dir() -> Path:
    ...
    :func:`openagents_constants.get_default_openagents_root` returns — so
    Linux/macOS classic installs land at ``~/.openagents/shared/``, native
    Windows installs at ``%LOCALAPPDATA%\\hermes\\shared\\``, and
    Docker / custom ``OPENAGENTS_HOME`` deployments at
    ``<OPENAGENTS_HOME>/shared/``. Sits outside any named profile so all
    ...
    from openagents_constants import get_default_openagents_root
    return get_default_openagents_root() / "shared"


def _nous_shared_store_path() -> Path:
    path = _nous_shared_auth_dir() / NOUS_SHARED_STORE_FILENAME
    # Seat belt: if pytest is running and this resolves to a path under the
    # real user's OpenAgents root, refuse rather than silently corrupt cross-profile
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        from openagents_constants import get_default_openagents_root
        real_home_shared = (
            get_default_openagents_root() / "shared" / NOUS_SHARED_STORE_FILENAME
...
def _nous_shared_store_lock(timeout_seconds: float = AUTH_LOCK_TIMEOUT_SECONDS):
    ...
    try:
        ...
        # No OPENAGENTS_HOME yet (pre-setup): fall through without locking.
        yield
        return
    ...
    *,
    ...
    if code == "refresh_token_reused" or "reuse" in lowered or "reuse detected" in lowered:
        description = (
            ...
            "custom self-heal hook, or another OpenAgents install sharing "
            "~/.openagents/auth.json) called POST /api/oauth/token with Hermes's "
            "refresh token without persisting the rotated token back.\n"
            "Nous refresh tokens are single-use — only OpenAgents may call the "
    ...
    *,
    ...
    for item in data:
        ...
        if isinstance(model_id, str) and model_id.strip():
            mid = model_id.strip()
            # Skip OpenAgents models — they're not reliable for agentic tool-calling
            if "openagents" in mid.lower():
                continue
...
def _get_azure_foundry_auth_status() -> Dict[str, Any]:
    ...
    try:
        from openagents_cli.config import load_config, get_env_value
        cfg = load_config()
    except Exception:
        cfg = {}
...
def _update_config_for_provider(
    ...
    # model.api_key.
    from openagents_cli.config import clear_model_endpoint_credentials
...
def _prompt_model_selection(
    ...
    """
    from openagents_cli.models import _format_price_per_mtok
...
def _save_model_choice(model_id: str) -> None:
    ...
    """
    from openagents_cli.config import save_config, load_config
...
def _login_openai_codex(
    ...
    *,
    ...
    """OpenAI Codex login via device code flow. Tokens stored in ~/.openagents/auth.json."""
    ...
    if not force_new_login:
        try:
            ...
            if isinstance(_resolved_key, str) and _resolved_key and not _codex_access_token_is_expiring(_resolved_key, 60):
                print("Existing Codex credentials found in OpenAgents auth store.")
                try:
    ...
    # Run a fresh device code flow — OpenAgents gets its own OAuth session
    print()
    ...
    # Save tokens to OpenAgents auth store
    ...
    if not force_new_login:
        try:
            ...
            if isinstance(api_key, str) and api_key and not _xai_access_token_is_expiring(api_key, 60):
                print("Existing xAI OAuth credentials found in OpenAgents auth store.")
                try:
                    reuse = input("Use existing credentials? [Y/n]: ").strip().lower()
                except (EOFError, KeyboardInterrupt):
                    reuse = "y"
...
def _xai_oauth_build_authorize_url(
    *,
    ...
    # `referrer=openagents` lets xAI attribute Hermes-originated logins
    ...
    *,
    ...
    if not code_verifier:
        raise AuthError(
            ...
            "This is a bug in OpenAgents — please report at "
            "https://github.com/NousResearch/openagents/issues/26990.",
            provider="xai-oauth",
...
def _xai_oauth_loopback_login(
    *,
    ...
    if manual_paste:
        ...
        print("Open this URL to authorize OpenAgents with xAI:")
        print(authorize_url)
...
def _minimax_save_auth_state(auth_state: Dict[str, Any]) -> None:
    """Persist MiniMax OAuth state to OpenAgents auth store (~/.openagents/auth.json)."""
    with _auth_store_lock():
        auth_store = _load_auth_store()
...
def _minimax_oauth_login(
    *, region: str = "global", open_browser: bool = True,
    ...
    print(f"Starting OpenAgents login via MiniMax ({region}) OAuth...")
...
def _nous_device_code_login(
    *,
    ...
    print(f"Starting OpenAgents login via {pconfig.name}...")
    print(f"Portal: {portal_base_url}")
    if insecure:
        ...
        try:
            ...
            if model_ids:
                ...
                if free_tier:
                    ...
                    # as free so users on older OpenAgents builds still see
