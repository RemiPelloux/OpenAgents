"""On-demand supply-chain audit for OpenAgents installs.

Scans three surfaces a OpenAgents user actually controls and we can map to
upstream advisories without auth or extra binaries:

1. The OpenAgents venv (every PyPI dist via ``importlib.metadata``).
2. Python deps declared by user-installed plugins under ``~/.openagents/plugins``
...
def _discover_plugins(hermes_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.openagents/plugins``.
...
def run_audit(
    *,
    ...
    home = hermes_home or Path(get_openagents_home())
