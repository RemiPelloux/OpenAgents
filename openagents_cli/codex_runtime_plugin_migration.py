...
What translates (MCP servers):
  OpenAgents mcp_servers.<n>.command/args/env  → codex stdio transport
  OpenAgents mcp_servers.<n>.url/headers       → codex streamable_http transport
  OpenAgents mcp_servers.<n>.timeout           → codex tool_timeout_sec
  OpenAgents mcp_servers.<n>.connect_timeout   → codex startup_timeout_sec
...
MIGRATION_END_MARKER = "# end openagents managed section"
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in OpenAgents config.")
...
# OpenAgents keys that codex's MCP schema doesn't support — dropped during
...
_KEYS_DROPPED_WITH_WARNING = {
    # OpenAgents' sampling subsection — codex MCP has no equivalent
    "sampling",
}
...
def _translate_one_server(
    ...
    """Translate one OpenAgents MCP server config to the codex inline-table dict
    ...
    if has_command:
        ...
        # OpenAgents' transport: sse hint is informational; codex auto-negotiates
    ...
    for key in hermes_cfg:
        if key in _KEYS_DROPPED_WITH_WARNING:
            ...
            skipped.append(f"{key} (unknown OpenAgents key)")
...
def _format_toml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        # chars but env-var passthrough (OPENAGENTS_HOME, PYTHONPATH) could
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    what pytest's tempdir factory uses by default. If a OPENAGENTS_HOME pointing
    ...
    ``~/.openagents`` symlink that happens to live under ``/private/var/folders``
...
def _build_hermes_tools_mcp_entry() -> dict:
    ...
    OPENAGENTS_HOME and PYTHONPATH are passed through so the spawned process
    sees the same config + module layout the user is running."""
    import sys

    env: dict[str, str] = {}
    # OPENAGENTS_HOME passes through IF SET so the MCP subprocess sees the same
    # config / auth / sessions DB as the parent CLI. Read from os.environ
    # (not get_openagents_home()) on purpose: when the env var is unset we want
    # codex's subprocess to inherit whatever OPENAGENTS_HOME its launcher sets
    ...
    # — that would override the launcher's OPENAGENTS_HOME and pin the subprocess
    # to the wrong profile.
    #
    # The pytest-tempdir guard below catches the issue #26250 Bug C scenario:
    # a sibling test's monkeypatch.setenv("OPENAGENTS_HOME", tmp_path) would
    ...
    hermes_home = os.environ.get("OPENAGENTS_HOME") or ""
    if hermes_home and _looks_like_test_tempdir(hermes_home):
        hermes_home = ""
    if hermes_home:
        env["OPENAGENTS_HOME"] = hermes_home
...
def migrate(
    ...
    *,
    ...
    """Translate OpenAgents mcp_servers config + Codex curated plugins into
    ~/.codex/config.toml.

    Args:
        hermes_config: full ~/.openagents/config.yaml dict
        ...
        expose_hermes_tools: when True (default), register Hermes' own
            ...
            so the codex subprocess can call back into OpenAgents for tools
    ...
    if not isinstance(hermes_servers, dict):
        report.errors.append(
            "mcp_servers in OpenAgents config is not a dict; cannot migrate."
        )
        return report
    ...
    # codex subprocess can call back into OpenAgents for the tools codex
