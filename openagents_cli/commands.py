"""Slash command definitions and autocomplete for the OpenAgents CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of OpenAgents config/state", "Session",
    ...
    CommandDef("steer", "Inject a message after the next tool call without interrupting", "Session",
               args_hint="<prompt>"),
    CommandDef("goal", "Set a standing goal OpenAgents works on across turns until achieved", "Session",
    ...
    CommandDef("busy", "Control what Enter does while OpenAgents is working", "Configuration",
    ...
    CommandDef("openagentui", "Start/stop the local OpenAgentUI visual workflow builder",
               "Tools & Skills", aliases=("oaui",),
               args_hint="true|false|status",
               subcommands=("true", "false", "start", "stop", "status")),
    CommandDef("openagentconfig", "Run or manage saved OpenAgentUI workflows",
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.openagents/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update OpenAgents to the latest version", "Info"),
    CommandDef("version", "Show OpenAgents version", "Info", aliases=("v",)),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`openagents_cli.plugins.PluginContext.register_command`. They behave
...
# Telegram allows up to 100 BotCommands. OpenAgents ships ~50 built-in commands;
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    entries.append(("openagents", "Talk to OpenAgents or run a subcommand", "[subcommand] [args]"))
    seen.add("openagents")
...
def slack_app_manifest(request_url: str = "https://openagents.local/slack/commands") -> dict[str, Any]:
