"""
OpenAgents Plugin System
====================

Discovers, loads, and manages plugins from four sources:

1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with openagents;
   ``memory/`` and ``context_engine/`` subdirs are excluded — they have their
   own discovery paths)
2. **User plugins**   – ``~/.openagents/plugins/<name>/``
...
from openagents_cli.middleware import OBSERVER_SCHEMA_VERSION, VALID_MIDDLEWARE
...
# stderr in addition to ~/.openagents/logs/agent.log. Aimed at plugin authors
...
def _install_plugin_debug_handler(force: bool = False) -> None:
    ...
    is passed. Does not touch the root logger or other OpenAgents loggers.
    """
...
VALID_HOOKS: Set[str] = {
    ...
    # Kanban task lifecycle hooks. Fired by openagents_cli.kanban_db when a task
...
class PluginManifest:
    ...
    #              in ~/.openagents/plugins/ still gated by ``plugins.enabled``
    #              (untrusted code).
    kind: str = "standalone"
...
class PluginContext:
    ...
    def profile_name(self) -> str:
        """Return the active OpenAgents profile name (e.g. ``"default"``).

        Derived from ``OPENAGENTS_HOME`` via
        :func:`openagents_cli.profiles.get_active_profile_name`, so it works in
        ...
        running under ``~/.openagents/profiles/<name>``, or ``"custom"`` when
        ``OPENAGENTS_HOME`` points somewhere unrecognized.
        """
        try:
    ...
    def register_dashboard_auth_provider(self, provider) -> None:
        ...
        ``provider`` must be an instance of
        :class:`openagents_cli.dashboard_auth.DashboardAuthProvider`. Used by
        ...
        """
        from openagents_cli.dashboard_auth import (
    ...
    def register_auxiliary_task(
        ...
        *,
        ...
        # Lazy import to avoid circular: openagents_cli.main imports plugins indirectly
        from openagents_cli.main import _AUX_TASKS as _BUILTIN_AUX_TASKS
    ...
    def register_skill(
        ...
        ``~/.openagents/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def _discover_and_load_inner(self) -> None:
        ...
        #
        # Repo-shipped plugins live next to openagents_cli/. Two layouts are
        ...
        # 2. User plugins (~/.openagents/plugins/)
        user_dir = get_openagents_home() / "plugins"
        ...
        for manifest in winners.values():
            ...
            # path actually asks for that platform. Every platform OpenAgents ships
