...
TIPS = [
    ...
    "Ctrl+Z suspends OpenAgents to the background — run fg in your shell to resume.",
    ...
    "hermes mcp serve runs OpenAgents itself as an MCP server for other agents.",
    ...
    "hermes backup creates a zip backup of your entire OpenAgents home directory.",
    ...
    "hermes gateway install sets up OpenAgents as a system service (systemd/launchd).",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call OpenAgents tools programmatically — results stay out of context.",
    ...
    "When exiting, OpenAgents prints a resume command with session ID and stats.",
    ...
    "BlueBubbles brings iMessage to OpenAgents via a local macOS server.",
    ...
    "Checkpoints use shadow repos in ~/.openagents/checkpoints/ — your project's .git is never touched.",
    ...
    "hermes acp runs OpenAgents as an ACP server for VS Code, Zed, and JetBrains integration.",
    ...
    "Cron scripts live in ~/.openagents/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make OpenAgents your own.",
    ...
    "Before a gateway session reset, OpenAgents auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.openagents/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Profiles scope OpenAgents state via OPENAGENTS_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    "OPENAGENTS_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in OPENAGENTS_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.openagents/interrupt_debug.log with timestamps.",
    ...
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.openagents/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.openagents/skills/.hub/quarantine/ holds skills pending security review.",

    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — OpenAgents auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of OpenAgents config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while OpenAgents is working.',
    ...
    '/reload reloads ~/.openagents/.env into the running session — pick up new API keys without restarting.',
    ...
    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.openagents/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.openagents/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',
    ...
    'hermes dashboard embeds the full OpenAgents TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.openagents/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.openagents/dashboard-plugins/ — no npm build required.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.openagents/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.openagents/skills/ so drop-in skills appear without restarting the session.',
    ...
    'OPENAGENTS_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes OpenAgents block commands when the tirith scanner itself errors out.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.openagents/dashboard-plugins/.',
]
