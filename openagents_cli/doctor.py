...
Diagnoses issues with OpenAgents setup.
"""
...
from openagents_cli.config import get_project_root, get_openagents_home, get_env_path
from openagents_cli.env_loader import load_openagents_dotenv
from openagents_constants import display_openagents_home
from openagents_constants import agent_browser_runnable

PROJECT_ROOT = get_project_root()
OPENAGENTS_HOME = get_openagents_home()
_DHH = display_openagents_home()  # user-facing display path (e.g. ~/.openagents or ~/.openagents/profiles/coder)

# Load environment variables from ~/.openagents/.env so API key checks work
_env_path = get_env_path()
load_openagents_dotenv(hermes_home=_env_path.parent, project_env=PROJECT_ROOT / ".env")

from openagents_cli.colors import Colors, color
from openagents_cli.models import _HERMES_USER_AGENT
from openagents_constants import OPENROUTER_MODELS_URL
...
from openagents_constants import is_termux as _is_termux
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.openagents/.env contains provider auth/base URL settings."""
...
def _has_healthy_oauth_fallback_for_apikey_provider(provider_label: str) -> bool:
    ...
    if normalized == "minimax":
        try:
            from openagents_cli.auth import get_minimax_oauth_auth_status
...
def _check_version_consistency(issues: list[str]) -> None:
    """Verify pyproject.toml version matches openagents_cli.__version__.
    ...
    """
    try:
        from openagents_cli import __version__ as init_version
    except Exception:
        return
    ...
    if pyproject_version == init_version:
        ...
        _fail_and_issue(
            ...
            f"(pyproject.toml {pyproject_version} != openagents_cli/__init__.py {init_version})",
            "Re-sync version files (e.g. run 'hermes update', or set "
            "openagents_cli/__init__.py __version__ to match pyproject.toml)",
            issues,
        )
...
def _build_apikey_providers_list() -> list:
    ...
    try:
        ...
        try:
            from openagents_cli.providers import normalize_provider as _normalize_provider
...
def run_doctor(args):
    ...
    if ack_target:
        ...
        if ack_advisory(ack_target):
            ...
            print(color(
                ...
                f"Check ~/.openagents/config.yaml is writable.",
                Colors.RED,
            ))
            sys.exit(1)
        return
    ...
    print(color("│                 🩺 OpenAgents Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    # Detect drift between pyproject.toml and openagents_cli/__init__.py versions
    ...
    managed_scope_check()
    # Check ~/.openagents/.env (primary location for user config)
    env_path = OPENAGENTS_HOME / '.env'
    if env_path.exists():
    ...
    # Check ~/.openagents/config.yaml (primary) or project cli-config.yaml (fallback)
    config_path = OPENAGENTS_HOME / 'config.yaml'
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from openagents_cli.config import get_compatible_custom_providers as _compatible_custom_providers
                from openagents_cli.providers import (
            ...
            if runtime_provider and runtime_provider not in ("auto", "custom"):
                try:
                    if runtime_provider == "openrouter":
                        ...
                        from openagents_cli.auth import PROVIDER_REGISTRY, get_auth_status
                    ...
                    if not configured:
                        _fail_and_issue(
                            ...
                            "(check ~/.openagents/.env or run 'hermes setup')",
                            (
        ...
        if fallback_config.exists():
            ...
            if should_fix:
                ...
                if example_config.exists():
                    ...
                    from openagents_cli.config import DEFAULT_CONFIG, save_config
    ...
    if config_path.exists():
        ...
        try:
            import yaml
            from openagents_cli.config import load_env, remove_env_value
            with open(config_path, encoding="utf-8") as f:
                raw_config = yaml.safe_load(f) or {}
    ...
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give OpenAgents a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# OpenAgents Persona\n\n"
                "<!-- Edit this file to customize how OpenAgents communicates. -->\n\n"
    ...
    if state_db_path.exists():
        try:
            ...
            from openagents_state import _db_opens_cleanly, repair_state_db_schema
            ...
            from openagents_state import is_malformed_db_error, repair_state_db_schema
    ...
    if sys.platform != "win32":
        ...
        for _venv_name in ("venv", ".venv"):
            _candidate = PROJECT_ROOT / _venv_name / "bin" / "openagents"
        ...
        _cmd_link = _cmd_link_dir / "openagents"
    ...
    try:
        from openagents_constants import is_container as _is_container
    ...
    if _npm_bin:
        ...
        # The WhatsApp bridge may live under a writable OPENAGENTS_HOME mirror
            ...
            try:
                ...
                if total == 0:
                    ...
                    if audit_extra and audit_extra[0] == "--workspace":
                        ...
                        # tree — in that case it is an npm bug, not a OpenAgents one.
                        check_info(
    ...
    _section("Skills Hub")
    hub_dir = OPENAGENTS_HOME / "skills" / ".hub"
    if hub_dir.exists():
    ...
    try:
        import yaml as _yaml
        _mem_cfg_path = OPENAGENTS_HOME / "config.yaml"
    ...
    try:
        from openagents_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
