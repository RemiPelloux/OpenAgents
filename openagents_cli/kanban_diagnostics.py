...
def _rule_stranded_in_ready(task, events, runs, now, cfg) -> list[Diagnostic]:
    ...
    * Dispatcher is misconfigured (wrong board, wrong OPENAGENTS_HOME).
    ...
    works for OpenAgents profiles, registered lanes, external workers, and
...
def config_from_runtime_config(raw_config: Optional[dict]) -> dict:
    """Build diagnostics config from the full OpenAgents runtime config.
