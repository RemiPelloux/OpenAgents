...
def _setup_webhooks():
    ...
    print()
    print_info(
        "   Full guide: https://openagents.nousresearch.com/docs/user-guide/messaging/webhooks/"
    )
    print()
    ...
    print_info(
        "   https://openagents.nousresearch.com/docs/user-guide/messaging/webhooks/#configuring-routes"
    )
    print()
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(
            color(
                "  Would overwrite (conflicts with existing OpenAgents config):",
                Colors.YELLOW,
            )
        )
...
def _offer_openclaw_migration(hermes_home: Path) -> bool:
    ...
    if conflicts:
        print_info(
            f"Skipped {conflicts} item(s) that already exist in OpenAgents (use hermes claw migrate --overwrite to force)."
        )
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    print(
        color(
            "│     ⚕ OpenAgents Setup — Nous Portal (one-shot)             │",
            Colors.MAGENTA,
        )
    )
    print(
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(
                    color(f"│     ⚕ OpenAgents Setup — {label:<34s} │", Colors.MAGENTA)
                )
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
