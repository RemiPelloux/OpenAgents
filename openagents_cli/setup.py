"""
Interactive setup wizard for OpenAgents.
...
_DOCS_BASE = "https://openagents.nousresearch.com/docs"
...
# display_openagents_home imported lazily at call sites (stale-module safety during hermes update)
...
from openagents_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from openagents_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ OpenAgents Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure OpenAgents using environment variables or config commands:")
...
def is_noninteractive() -> bool:
    ...
    ``HERMES_NONINTERACTIVE=1`` (see ``openagents_cli/web_server.py``). In that
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from openagents_cli.curses_ui import curses_checklist
...
def _print_setup_summary(config: dict, hermes_home):
    ...
    if subscription_features.image_gen.managed_by_nous:
        ...
        try:
            ...
            from openagents_cli.plugins import _ensure_plugins_discovered
    ...
    if subscription_features.video_gen.managed_by_nous:
        ...
        try:
            ...
            from openagents_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
    ...
    print()
    print(f"   {color('openagents', Colors.GREEN)}              Start chatting")
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    from openagents_cli.main import select_provider_and_model
    try:
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    """
    try:
        from openagents_cli.auth import get_xai_oauth_auth_status
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.openagents/.env."
            )
    ...
    if selected == "neutts":
        ...
        # via the OpenAgents auth store) > existing XAI_API_KEY > prompt the user.
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where OpenAgents runs shell commands and code.")
...
def _setup_telegram_auto_result():
    ...
    try:
        from openagents_cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None

    profile_name: str | None = None
    try:
        profile_name = _profile_name_from_hermes_home(Path(get_openagents_home()))
    except Exception:
        pass
...
def _profile_name_from_hermes_home(hermes_home) -> str | None:
    """Return the active profile name when OPENAGENTS_HOME is a profile dir."""
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where OpenAgents delivers cron job results,")
...
def _setup_bluebubbles():
    ...
    print_info("Connects OpenAgents to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from openagents_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://openagents.nousresearch.com/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://openagents.nousresearch.com/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
...
def setup_gateway(config: dict):
    ...
    from openagents_cli.gateway import _all_platforms, _platform_status, _configure_platform

    print_header("Messaging Platforms")
    print_info("Connect to messaging platforms to chat with OpenAgents from anywhere.")
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``openagents_cli.auth`` — lists every supported
    ...
    """
    try:
        from openagents_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from openagents_cli.config
    ...
    if section_key == "model":
        ...
        from openagents_cli.gateway import _all_platforms, _platform_status
...
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure OpenAgents to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point OpenAgents at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point OpenAgents at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point OpenAgents at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point OpenAgents at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to OpenAgents equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing OpenAgents config):", Colors.YELLOW))
...
def _offer_openclaw_migration(hermes_home: Path) -> bool:
    ...
    # Execute the migration — overwrite=False so existing OpenAgents configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing OpenAgents config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in OpenAgents (use hermes claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    OpenAgents session — model selected, provider set, and web/image/tts/browser
    ...
    print(color("│     ⚕ OpenAgents Setup — Nous Portal (one-shot)             │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
    ...
    try:
        from openagents_cli.main import _model_flow_nous
...
def run_setup_wizard(args):
    ...
    """
    from openagents_cli.config import is_managed, managed_error
    if is_managed():
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ OpenAgents Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ OpenAgents Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your OpenAgents installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        print_success("You already have OpenAgents configured.")
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    try:
        ...
        from openagents_cli.tools_config import CONFIGURABLE_TOOLSETS, _get_plugin_toolset_keys
...
def _run_quick_setup(config: dict, hermes_home):
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect OpenAgents to messaging apps to chat from anywhere.")
