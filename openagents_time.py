...
Resolution order:
  ...
  2. ``timezone`` key in ``~/.openagents/config.yaml``
  3. Falls back to the server's local time (``datetime.now().astimezone()``)

Invalid timezone values log a warning and fall back safely — OpenAgents never
...
from openagents_constants import get_config_path
...
try:
    ...
    # Python 3.8 fallback (shouldn't be needed — OpenAgents requires 3.9+)
