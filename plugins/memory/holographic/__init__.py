...
Config in $OPENAGENTS_HOME/config.yaml (profile-scoped):
  plugins:
    hermes-memory-store:
      db_path: $OPENAGENTS_HOME/memory_store.db   # omit to use the default
      auto_extract: false
      default_trust: 0.5
...
class HolographicMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        ...
        _default_db = f"{display_openagents_home()}/memory_store.db"
        return [
            {
                "key": "db_path",
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        _hermes_home = str(get_openagents_home())
        _default_db = _hermes_home + "/memory_store.db"
        db_path = self._config.get("db_path", _default_db)
        # Expand $OPENAGENTS_HOME in user-supplied paths so config values like
        # "$OPENAGENTS_HOME/memory_store.db" or "~/.openagents/memory_store.db" both
        # resolve to the active profile's directory.
        if isinstance(db_path, str):
            db_path = db_path.replace("$OPENAGENTS_HOME", _hermes_home)
            db_path = db_path.replace("${OPENAGENTS_HOME}", _hermes_home)
