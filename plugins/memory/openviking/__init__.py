...
or a linked OpenViking CLI config:
  ...
  OPENVIKING_AGENT     — OpenAgents peer ID in OpenViking (default: hermes)
...
def _derive_openviking_user_text(content: Any) -> str:
    """Strip OpenAgents slash-skill scaffolding before sending content to OpenViking.
...
def _openviking_server_log_path() -> Path:
    try:
        ...
        home = Path(os.environ.get("OPENAGENTS_HOME", "")).expanduser() if os.environ.get("OPENAGENTS_HOME") else Path.home() / ".hermes"
...
def _print_openviking_ready(message: str, path: Optional[Path] = None) -> None:
    ...
    print("  Start a new OpenAgents session to activate.\n")
...
def _run_create_profile_setup(
    *,
    ...
    save_choice = select(
        ...
        [
            ("Keep in OpenAgents only", "write values only to OpenAgents .env"),
    ...
    _print_openviking_ready("Connection saved to OpenAgents .env.")
    return True
...
class OpenVikingMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        # against a concurrent increment. See openagents#28296 review.
    ...
    def post_setup(self, hermes_home: str, config: dict) -> None:
        ...
        from openagents_cli.memory_setup import _CANCELLED, _curses_select, _print_cancelled_setup, _prompt
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        try:
            ...
            if health_state == "unreachable":
                ...
                _emit_runtime_warning(
                    f"{health_message} OpenViking memory disabled for this OpenAgents run.",
                    warning_callback,
                )
                self._client = None
        except ImportError:
    ...
    def _messages_to_openviking_batch(
        ...
        *,
        ...
        """Convert OpenAgents canonical messages into OpenViking batch payloads."""
    ...
    def on_session_switch(
        ...
        **kwargs,
        ...
        and memory extraction never fires for it. See openagents#28296.
