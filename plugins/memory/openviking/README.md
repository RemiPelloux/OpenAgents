...
| `OPENVIKING_AGENT` | `hermes` | OpenAgents peer ID in OpenViking, used for peer-scoped memories |

When `OPENVIKING_API_KEY` is set, OpenAgents lets OpenViking derive account/user
identity from the key. In local or trusted deployments without an API key,
OpenAgents sends `OPENVIKING_ACCOUNT` and `OPENVIKING_USER` as identity headers.
...
OpenAgents built-in `memory` tool additions are mirrored to OpenViking after the
local memory operation succeeds:

| OpenAgents action | OpenViking operation |
|---------------|----------------------|
