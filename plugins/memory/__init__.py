...
1. Bundled providers: ``plugins/memory/<name>/`` (shipped with openagents)
2. User-installed providers: ``$OPENAGENTS_HOME/plugins/<name>/``
...
def _get_user_plugins_dir() -> Optional[Path]:
    """Return ``$OPENAGENTS_HOME/plugins/`` or None if unavailable."""
    try:
        from openagents_constants import get_openagents_home
        d = get_openagents_home() / "plugins"
...
def load_memory_provider(name: str) -> Optional["MemoryProvider"]:
    ...
    (``$OPENAGENTS_HOME/plugins/<name>/``) directories.  Bundled takes
