...
In single-bot mode (what OpenAgents currently supports), open_id works as a
...
class FeishuAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._dedup_state_path = get_openagents_home() / "feishu_seen_message_ids.json"
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            if not acquired:
                ...
                message = (
                    "Another local OpenAgents gateway is already using this Feishu app_id"
    ...
    def _write_update_prompt_response(answer: str) -> None:
        response_path = get_openagents_home() / ".update_response"
    ...
    def _on_message_read_event(self, data: P2ImMessageMessageReadV1) -> None:
        """Ignore read-receipt events that OpenAgents does not act on."""
        event = getattr(data, "event", None)
        message = getattr(event, "message", None)
    ...
    async def _handle_webhook_request(self, request: Any) -> Any:
        ...
        if payload.get("encrypt"):
            logger.error("[Feishu] Encrypted webhook payloads are not supported by OpenAgents webhook mode")
...
# Called by `hermes gateway setup` via _setup_feishu() in openagents_cli/gateway.py.
# =============================================================================
...
# dict in openagents_cli/gateway.py, and the _send_feishu dispatch in
...
async def _standalone_send(
    ...
    *,
    ...
    """
    if not FEISHU_AVAILABLE:
        return {"error": "Feishu dependencies not installed. Run: pip install 'openagents[feishu]'"}
...
def interactive_setup() -> None:
    ...
    Replaces the central _setup_feishu in openagents_cli/gateway.py and the static
    _PLATFORMS["feishu"] dict. CLI helpers are lazy-imported.
    """
    from openagents_cli.config import get_env_value, save_env_value
    from openagents_cli.setup import prompt_choice
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'openagents[feishu]'",
