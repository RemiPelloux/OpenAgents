...
class FeishuAdapter(BasePlatformAdapter):
    ...
    async def _handle_webhook_request(self, request: Any) -> Any:
        ...
        if payload.get("encrypt"):
            logger.error(
                "[Feishu] Encrypted webhook payloads are not supported by OpenAgents webhook mode"
            )
...
async def _standalone_send(
    ...
    *,
    ...
    if not FEISHU_AVAILABLE:
        return {
            "error": "Feishu dependencies not installed. Run: pip install 'openagents[feishu]'"
        }
