"""
LINE Messaging API platform adapter for OpenAgents.
...
to OpenAgents. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``OPENAGENTS_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            hermes_home = Path(get_openagents_home()).resolve()
        except Exception:
...
def interactive_setup() -> None:
    ...
    one optional public URL. Writes to ``~/.openagents/.env`` via ``openagents_cli.config``.
    """
    print()
    ...
    try:
        from openagents_cli.config import get_env_var, set_env_var
    except ImportError:
        print("openagents_cli.config not available; set LINE_* vars manually in ~/.openagents/.env")
        return
