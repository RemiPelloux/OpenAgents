...
Credential storage mirrors every other OpenAgents channel:

    * runtime SDK creds  -> ``~/.openagents/.env``  (``PHOTON_PROJECT_ID`` =
      project id, ``PHOTON_PROJECT_SECRET``) via ``save_env_value``
    * management metadata -> ``~/.openagents/auth.json`` under
...
# registers OpenAgents as its own client_id.
...
# Default name of the project OpenAgents provisions for the operator.
DEFAULT_PROJECT_NAME = "OpenAgents"
...
# ---------------------------------------------------------------------------
# auth.json helpers — share the file with the rest of openagents.

def _auth_json_path() -> Path:
    """Resolve ``~/.openagents/auth.json`` honouring the active OpenAgents profile."""
    try:
        from openagents_constants import get_openagents_home
        return Path(get_openagents_home()) / "auth.json"
    except Exception:
        return Path(os.path.expanduser("~/.openagents")) / "auth.json"
...
def load_project_credentials() -> Tuple[Optional[str], Optional[str]]:
    ...
    Precedence: process env (``~/.openagents/.env`` is loaded into the gateway's
...
def store_project_credentials(
    *,
    ...
    The runtime SDK creds land in ``~/.openagents/.env`` via the same
...
def _persist_runtime_env(spectrum_project_id: str, project_secret: str) -> None:
    """Write the SDK creds to ``~/.openagents/.env`` (canonical runtime store).
    ...
    try:
        ...
        logger.warning("photon: openagents_cli.config unavailable — skipping .env write")
        return
    try:
