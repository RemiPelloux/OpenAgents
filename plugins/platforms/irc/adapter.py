"""
IRC Platform Adapter for OpenAgents.

A plugin-based gateway adapter that connects to an IRC server and relays
messages to/from the OpenAgents.  Zero external dependencies — uses
...
class IRCAdapter(BasePlatformAdapter):
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        await self._send_raw(f"USER {self.nickname} 0 * :OpenAgents")
    ...
    async def disconnect(self) -> None:
        ...
        if self._writer and not self._writer.is_closing():
            try:
                await self._send_raw("QUIT :OpenAgents shutting down")
...
def interactive_setup() -> None:
    ...
    Lazy-imports ``openagents_cli.setup`` helpers so the plugin stays importable
    ...
    print_info("Connect OpenAgents to an IRC network. Uses Python stdlib — no extra packages needed.")
    ...
    print()
    print_success("IRC configuration saved to ~/.openagents/.env")
...
async def _standalone_send(
    ...
    *,
    ...
    try:
        ...
        await _raw(f"USER {standalone_nick} 0 * :OpenAgents (cron)")
