...
class SlackAdapter(BasePlatformAdapter):
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        tokens_file = get_openagents_home() / "slack_tokens.json"
        ...
        try:
            ...
            # OpenAgents does not currently need to route them into the agent loop.
            ...
            from openagents_cli.commands import slack_native_slashes
            import re as _re
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if client is None:
                return None
            seed_text = (
                f":thread: OpenAgents handoff — *{(name or 'session').strip()[:80]}*"
            )
    ...
    def _dm_top_level_threads_as_sessions(self) -> bool:
        ...
        own OpenAgents session — matching the per-thread behavior channels already
    ...
    async def _handle_slack_message(self, event: dict) -> None:
        ...
        if original_text.startswith("!"):
            try:
                from openagents_cli.commands import is_gateway_known_command
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        if slash_name in {"openagents", ""}:
            # Legacy /hermes <subcommand> [args] routing + free-form questions.
            # Empty slash_name falls into this branch for backward compat
            # with any caller that didn't populate command["command"].
            from openagents_cli.commands import slack_subcommand_map
...
# ``openagents_cli/{setup,gateway}.py``, and the ``_send_slack`` dispatch in
...
def interactive_setup() -> None:
    ...
    offers to set a home channel. Replaces ``openagents_cli/setup.py::_setup_slack``.
    """
    ...
    def _write_slack_manifest_and_instruct() -> None:
        """Generate the Slack manifest, write it under OPENAGENTS_HOME, and print
        paste-into-Slack instructions. Failures are non-fatal."""
        try:
            from openagents_cli.slack_cli import _build_full_manifest
            from openagents_constants import get_openagents_home
            import json as _json

            manifest = _build_full_manifest(
                bot_name="Hermes",
                bot_description="Your OpenAgents on Slack",
            )
            target = Path(get_openagents_home()) / "slack-manifest.json"
    ...
    print()
    print_info("   Full guide: https://openagents.nousresearch.com/docs/user-guide/messaging/slack/")
    print()
...
def _is_connected(config) -> bool:
    ...
    Looks up via ``openagents_cli.gateway.get_env_value`` at call time (not via the
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'openagents[slack]'",
        # Interactive setup wizard — replaces openagents_cli/setup.py::_setup_slack
        # and the static _PLATFORMS["slack"] dict in openagents_cli/gateway.py.
