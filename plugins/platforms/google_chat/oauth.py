...
- Per-user tokens (keyed by sender email):
    ``${OPENAGENTS_HOME}/google_chat_user_tokens/<sanitized_email>.json``
- Legacy single-user token (fallback, untouched for backward compat):
    ``${OPENAGENTS_HOME}/google_chat_user_token.json``
- Per-user pending OAuth state during /setup-files start → exchange:
    ``${OPENAGENTS_HOME}/google_chat_user_oauth_pending/<sanitized_email>.json``
- Legacy pending state:
    ``${OPENAGENTS_HOME}/google_chat_user_oauth_pending.json``
- OAuth client secret (profile-scoped — each profile registers its own):
    ``${OPENAGENTS_HOME}/google_chat_user_client_secret.json``
"""
...
# Use the project's OPENAGENTS_HOME helper so the token follows the user's
# profile (e.g. tests can override via OPENAGENTS_HOME=/tmp/...).
try:
    from openagents_constants import display_openagents_home, get_openagents_home
except (ModuleNotFoundError, ImportError):
    # Fallback for environments where openagents_constants isn't importable
...
def _hermes_home() -> Path:
    """Resolve OPENAGENTS_HOME at call time (NOT module import).

    Tests and ``OPENAGENTS_HOME=...`` env overrides need this to be late-
...
# ``ls ~/.openagents/google_chat_user_tokens/`` trivial.
...
def load_user_credentials(email: Optional[str] = None) -> Optional[Any]:
    ...
    try:
        ...
        logger.warning(
            ...
            "attachment delivery is disabled. Install openagents[google_chat]."
        )
        return None
...
def install_deps() -> bool:
    ...
    try:
        ...
        print("  pip install 'openagents[google_chat]'")
        return False
...
def store_client_secret(path: str) -> None:
    """Validate and copy the user's OAuth client_secret.json into OPENAGENTS_HOME."""
...
def exchange_auth_code(code: str, email: Optional[str] = None) -> None:
    ...
    rel_label = (
        f"{display_openagents_home()}/google_chat_user_tokens/{_sanitize_email(email)}.json"
        if email
        else f"{display_openagents_home()}/google_chat_user_token.json"
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Google Chat user-OAuth setup for OpenAgents (native attachment delivery)"
    )
