...
This adapter ships as a OpenAgents platform plugin under
``plugins/platforms/simplex/``. The OpenAgents plugin loader scans the
...
# Imported at module top because they're stdlib-only inside OpenAgents — no
...
def interactive_setup() -> None:
    ...
    auto-accept / home channel. Writes to ``~/.openagents/.env`` via
    ...
    try:
        ...
        print(
            "openagents_cli.config not available; set SIMPLEX_* vars manually in "
            "~/.openagents/.env"
        )
        return
