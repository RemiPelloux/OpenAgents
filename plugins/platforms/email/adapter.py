"""
Email platform adapter for the OpenAgents gateway.

Allows users to interact with OpenAgents by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "openagents" "version" "{_hermes_version}" '
            '"vendor" "NousResearch" '
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "OpenAgents")
...
# openagents_cli/gateway.py, and the _send_email dispatch in
