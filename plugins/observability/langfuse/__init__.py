"""langfuse — OpenAgents plugin for Langfuse observability.

Traces OpenAgents conversations, LLM calls, and tool usage to Langfuse.

Activation is handled by the OpenAgents plugin system — standalone plugins only
...
Required env vars (set via ``hermes tools`` or ~/.openagents/.env):
...
def _get_langfuse() -> Optional[Langfuse]:
    ...
    Activation of this plugin is controlled by the OpenAgents plugin system —
...
def _trace_key(
    ...
    *,
    ...
    Older OpenAgents paths only expose ``task_id``/``session_id``. Newer paths
...
def on_pre_llm_call(*, task_id: str = "", session_id: str = "", platform: str = "", model: str = "",
    ...
    # Older OpenAgents branches used pre_llm_call for request-scoped tracing and
    # passed the actual API messages. Current OpenAgents also has a turn-scoped
    ...
    # messages is a list only for legacy OpenAgents branches that fired
    # pre_llm_call with API messages directly. Current OpenAgents fires
...
def register(ctx) -> None:
    ...
    # OpenAgents versions.  pre_api_request / post_api_request fire per API
