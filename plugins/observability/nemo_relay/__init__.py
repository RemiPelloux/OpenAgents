"""nemo_relay — optional OpenAgents plugin for NeMo Relay observability."""
...
class _Settings:
    ...
    atif_agent_name: str = "OpenAgents"
...
class _Runtime:
    ...
    def ensure_session(self, kwargs: dict[str, Any]) -> _SessionState:
        ...
        if self.settings.atif_enabled and not self._plugins_toml_owns_exporter("atif"):
            state.atif_exporter = self.nemo_relay.AtifExporter(
                ...
                extra={"source": "openagents", "plugin": "observability/nemo_relay"},
            )
    ...
    def _run_managed_with_downstream_preservation(
        ...
        # execution so OpenAgents retry classification still sees it. The LLM and tool
...
def _load_settings() -> _Settings:
    ...
    return _Settings(
        ...
        atif_agent_name=_env("HERMES_NEMO_RELAY_ATIF_AGENT_NAME") or "OpenAgents",
...
def _original_downstream_error(exc: Exception) -> BaseException:
    # OpenAgents wraps downstream execution failures in a local/private exception
