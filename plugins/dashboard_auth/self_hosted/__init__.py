...
:class:`~openagents_cli.dashboard_auth.DashboardAuthProvider` methods and touches
...
``openagents_cli/dashboard_auth/routes.py``; this provider only:
  ...
  4. verifies the **ID token** (RS256/ES256) against the discovered
     ...
     onto a :class:`~openagents_cli.dashboard_auth.Session`.
...
class SelfHostedOIDCProvider(DashboardAuthProvider):
    ...
    def _session_from_tokens(
        ...
        *,
        ...
        OAuth access token is intentionally NOT stored — OpenAgents does not call
