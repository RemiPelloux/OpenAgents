"""disk-cleanup plugin — auto-cleanup of ephemeral OpenAgents session files.
...
1. ``post_tool_call`` hook — inspects ``write_file`` and ``terminal``
   ...
   under ``OPENAGENTS_HOME`` and tracks them silently.  Zero agent
   compliance required.
...
2. ``on_session_end`` hook — when any test files were auto-tracked
   ...
   logs a single line to ``$OPENAGENTS_HOME/disk-cleanup/cleanup.log``.
...
All operations are scoped to OPENAGENTS_HOME and /tmp/hermes-*.
...
def _handle_slash(raw_args: str) -> Optional[str]:
    ...
    if sub == "track":
        ...
        return (
            f"Not tracked (already present, missing, or outside OPENAGENTS_HOME): "
            f"{path_arg}"
        )
...
def register(ctx) -> None:
    ...
    ctx.register_command(
        ...
        description="Track and clean up ephemeral OpenAgents session files.",
    )
