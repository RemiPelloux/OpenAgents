"""disk_cleanup — ephemeral file cleanup for OpenAgents.
...
Rules:
  ...
  - empty dirs    → always delete (under OPENAGENTS_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly OPENAGENTS_HOME and /tmp/hermes-*
Never touches: ~/.openagents/logs/ or any system directory.
"""
...
try:
    ...
    def get_openagents_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("OPENAGENTS_HOME") or "").strip()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$OPENAGENTS_HOME/logs/``."""
    return get_openagents_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$OPENAGENTS_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under OPENAGENTS_HOME or ``/tmp/hermes-*``.
...
_EMPTY_DIR_PROTECTED_TOP_LEVEL = frozenset({
    ...
    "openagents", "backups", "profiles", ".worktrees",
})
...
# Paths under $OPENAGENTS_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    """
    # Lazily build the set once per process so OPENAGENTS_HOME is resolved
    # exactly once.
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside OPENAGENTS_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under OPENAGENTS_HOME, but never recurse into known
    # durable state trees.  Some installs place the OpenAgents checkout, venv,
    # and desktop build under OPENAGENTS_HOME; a full rglob over that tree can
...
def guess_category(path: Path) -> Optional[str]:
    ...
    try:
        ...
        # Path isn't under OPENAGENTS_HOME (e.g. /tmp/hermes-*) — fall through.
        pass
