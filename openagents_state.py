#!/usr/bin/env python3
"""
SQLite State Store for OpenAgents.
...
# openagents_cli/kanban_db.py for ~30 call sites) would re-log the same
...
def apply_wal_with_fallback(
    ...
    *,
    ...
    Shared by :class:`SessionDB` and ``openagents_cli.kanban_db.connect`` so
...
def _log_wal_fallback_once(db_label: str, exc: Exception) -> None:
    ...
    connection on every operation — see openagents_cli/kanban_db.py) would
...
class SessionDB:
    ...
    def __init__(self, db_path: Path = None, read_only: bool = False):
        ...
        try:
            ...
            # ``openagents_state._set_last_init_error(None)`` explicitly.
    ...
    def search_sessions_by_id(
        ...
        a session id from logs, CLI output, or another OpenAgents surface and jump
    ...
    def bind_telegram_topic(
        ...
        *,
        ...
        """Bind one Telegram DM topic thread to one OpenAgents session.

        A OpenAgents session may only be linked to one Telegram topic in MVP.
    ...
    def is_telegram_session_linked_to_topic(self, *, session_id: str) -> bool:
        """Return True if a OpenAgents session is already bound to any Telegram DM topic.
