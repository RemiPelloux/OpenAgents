...
* Python plugins and shell hooks compose naturally: both flow through
  :func:`openagents_cli.plugins.invoke_hook` and its aggregators.  Python
...
* First-use consent is gated by the allowlist under
  ``~/.openagents/shell-hooks-allowlist.json``.  Non-TTY callers must pass
  ``accept_hooks=True`` (resolved from ``--accept-hooks``,
  ``HERMES_ACCEPT_HOOKS``, or ``hooks_auto_accept: true`` in config)
  for registration to succeed without a prompt.
* Registration is idempotent — safe to invoke from both the CLI entry
  point (``openagents_cli/main.py``) and the gateway entry point
  (``gateway/run.py``).
...
**stdout** (JSON, optional — anything else is ignored)::
    ...
    {"action":   "block", "message": "Forbidden command"}   # OpenAgents-canonical
...
from openagents_cli._subprocess_compat import IS_WINDOWS, windows_hide_flags
...
def register_from_config(
    ...
    *,
    ...
    ``cfg`` is the full parsed config dict (``openagents_cli.config.load_config``
...
def _parse_response(event: str, stdout: str) -> Optional[Dict[str, Any]]:
    """Translate stdout JSON into a OpenAgents wire-shape dict.

    For ``pre_tool_call`` the Claude-Code-style ``{"decision": "block",
    "reason": "..."}`` payload is translated into the canonical Hermes
    ``{"action": "block", "message": "..."}`` shape expected by
    :func:`openagents_cli.plugins.get_pre_tool_call_block_message`.  This is
...
def allowlist_path() -> Path:
    ...
    return get_openagents_home() / ALLOWLIST_FILENAME
...
def _prompt_and_record(
    ...
    *,
    ...
    print(
        f"\n⚠ OpenAgents is about to register a shell hook that will run a\n"
...
def run_once(
    ...
    ``kwargs`` is the same dict that :func:`openagents_cli.plugins.invoke_hook`
