...
def _write_through_provider_state_to_global_root(
    ...
    ``openagents_cli.auth._write_through_xai_oauth_to_global_root`` (which covers
...
class CredentialPool:
    ...
    def _sync_xai_oauth_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        xAI OAuth refresh tokens are single-use.  When another OpenAgents process
    ...
    def _sync_device_code_entry_to_auth_store(self, entry: PooledCredential) -> None:
        ...
        try:
            with _auth_store_lock():
                ...
                # openagents_cli.auth._save_xai_oauth_tokens (#43589); the pool
        ...
        try:
            ...
            if self.provider == "xai-oauth":
                ...
                if auth_mod._is_terminal_xai_oauth_refresh_error(exc):
                    ...
                    return None
            # For openai-codex: same race as xAI/nous — another OpenAgents process
    ...
    def _available_entries(self, *, clear_expired: bool = False, refresh: bool = False) -> List[PooledCredential]:
        ...
        for entry in self._entries:
            ...
            # by other processes (Claude Code CLI, other OpenAgents profiles).
            if (self.provider == "anthropic" and entry.source == "claude_code"
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    try:
        from openagents_cli.auth import is_source_suppressed as _is_suppressed
    except ImportError:
        def _is_suppressed(_p, _s):  # type: ignore[misc]
            return False

    if provider == "anthropic":
        # Only auto-discover external credentials (Claude Code, OpenAgents PKCE)
        ...
        # that masquerade.  Prefer ~/.openagents/.env over os.environ for the
        ...
        # OpenAgents auth store or env vars, so resolve them here.
        # Use refresh_if_expiring=False to avoid network calls during
        # pool loading / provider discovery.
        try:
            from openagents_cli.auth import resolve_qwen_runtime_credentials
        ...
        # MiniMax OAuth tokens live in ~/.openagents/auth.json providers.minimax-oauth.
        ...
        # the OpenAgents auth store.  Without this gate the removal is instantly
        ...
        # OpenAgents owns its own Codex auth state — we do NOT auto-import from
        ...
        if isinstance(tokens, dict) and tokens.get("access_token"):
            ...
            from openagents_cli.auth import DEFAULT_XAI_OAUTH_BASE_URL
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # Prefer ~/.openagents/.env over os.environ — the user's config file is the
    # authoritative source for OpenAgents credentials. Stale env vars from parent
    ...
    # won't be re-seeded from the user's shell environment or ~/.openagents/.env.
    ...
    try:
        from openagents_cli.auth import is_source_suppressed as _is_source_suppressed
    except ImportError:
    ...
    def _secret_source_for_env(env_var: str) -> Optional[str]:
        try:
            from openagents_cli.env_loader import get_secret_source
    ...
    if provider == "openrouter":
        # Prefer ~/.openagents/.env over os.environ
