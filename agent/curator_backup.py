...
A pre-run snapshot of ``~/.openagents/skills/`` (excluding ``.curator_backups/``
itself) is taken before any mutating curator pass. Snapshots are tar.gz
files under ``~/.openagents/skills/.curator_backups/<utc-iso>/`` with a
...
``~/.openagents/cron/jobs.json`` as ``cron-jobs.json`` when it exists. Cron
...
def _backups_dir() -> Path:
    return get_openagents_home() / "skills" / ".curator_backups"
...
def _cron_jobs_file() -> Path:
    """Source path for the live cron jobs store (``~/.openagents/cron/jobs.json``)."""
    return get_openagents_home() / "cron" / "jobs.json"
...
def snapshot_skills(
    ...
    """Create a tar.gz snapshot of ``~/.openagents/skills/`` and prune old ones.
    ...
    if not skills.exists():
        logger.debug("No ~/.openagents/skills/ directory — nothing to back up")
        return None
...
def rollback(backup_id: Optional[str] = None) -> Tuple[bool, str, Optional[Path]]:
    """Restore ``~/.openagents/skills/`` from a snapshot.
    ...
    Strategy:
      ...
      4. Extract the chosen snapshot into ``~/.openagents/skills/``.
