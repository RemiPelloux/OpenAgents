...
as ``kind: backend``) or ``~/.openagents/plugins/image_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
def _images_cache_dir() -> Path:
    """Return ``$OPENAGENTS_HOME/cache/images/``, creating parents as needed."""
    from openagents_constants import get_openagents_home

    path = get_openagents_home() / "cache" / "images"
...
def save_b64_image(
    ...
    *,
    ...
    """Decode base64 image data and write it under ``$OPENAGENTS_HOME/cache/images/``.
...
def save_url_image(
    ...
    *,
    ...
    """Download an image URL and write it under ``$OPENAGENTS_HOME/cache/images/``.
