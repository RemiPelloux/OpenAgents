...
Resolution order for text tasks (auto mode):
  ...
  3. Nous Portal (~/.openagents/auth.json active provider)
...
def _compression_threshold_for_model(
    ...
    *,
    ...
    consumed before OpenAgents triggers summarization.  Higher values delay
...
_OR_HEADERS_BASE = {
    "HTTP-Referer": "https://openagents.nousresearch.com",
    "X-Title": "OpenAgents",
...
# in lockstep with openagents_cli.__version__ across every Portal call site
...
def _nous_extra_body() -> dict:
    ...
    Computed at call time so a hot-reloaded ``openagents_cli.__version__`` is
...
_AUTH_JSON_PATH = get_openagents_home() / "auth.json"
...
def _codex_cloudflare_headers(access_token: str) -> Dict[str, str]:
    ...
    """
    headers = {
        "User-Agent": "codex_cli_rs/0.0.0 (OpenAgents)",
...
def _endpoint_speaks_anthropic_messages(base_url: str) -> bool:
    ...
    Mirrors ``openagents_cli.runtime_provider._detect_api_mode_for_url`` so the
...
def _read_nous_auth() -> Optional[dict]:
    """Read and validate ~/.openagents/auth.json for an active Nous provider.
...
def _nous_api_key(provider: dict) -> str:
    ...
    from openagents_cli.auth import _nous_invoke_jwt_is_usable
...
def _resolve_nous_pool_runtime_api(*, force_refresh: bool = False) -> Optional[tuple[str, str]]:
    ...
    try:
        from openagents_cli.auth import _agent_key_is_usable
...
def _resolve_xai_oauth_for_aux() -> Optional[Tuple[str, str]]:
    ...
    Falls back to ``openagents_cli.auth``'s singleton runtime resolver for older
    ...
    try:
        from openagents_cli.auth import resolve_xai_oauth_runtime_credentials
...
def _read_codex_access_token() -> Optional[str]:
    """Read a valid, non-expired Codex OAuth access token from OpenAgents auth store.
    ...
    try:
        from openagents_cli.auth import _read_codex_tokens
...
def _resolve_api_key_provider() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    """
    try:
        from openagents_cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    except ImportError:
    ...
    for provider_id, pconfig in PROVIDER_REGISTRY.items():
        ...
        if provider_id == "anthropic":
            ...
            try:
                from openagents_cli.auth import is_provider_explicitly_configured
        ...
        if pool_present:
            ...
            if base_url_host_matches(base_url, "api.kimi.com"):
                ...
                from openagents_cli.models import copilot_default_headers
...
def _try_nous(vision: bool = False) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    model = _NOUS_MODEL
    try:
        from openagents_cli.models import get_nous_recommended_aux_model
...
def _try_azure_foundry(
    *,
    ...
    :func:`openagents_cli.runtime_provider._resolve_azure_foundry_runtime` —
    ...
    * Non-default ``model.base_url`` overrides are honored.
    ...
    """
    try:
        from openagents_cli.runtime_provider import _resolve_azure_foundry_runtime
...
def _resolve_auto(
    ...
    #    old OPENAI_BASE_URL lingers in ~/.openagents/.env. ──
    if not _stale_base_url_warned:
        ...
        if (_env_base and _cfg_provider
            ...
            logger.warning(
                ...
                "OPENAI_BASE_URL from ~/.openagents/.env",
    ...
    if main_provider == "moa":
        try:
            from openagents_cli.config import load_config
            from openagents_cli.moa_config import resolve_moa_preset
...
def resolve_provider_client(
    ...
    try:
        from openagents_cli.runtime_provider import _get_named_custom_provider
    ...
    try:
        ...
        logger.debug("openagents_cli.auth not available for provider %s", provider)
        return None, None
    ...
    if pconfig.auth_type == "api_key":
        ...
        if provider == "copilot" and final_model and not raw_codex:
            try:
                from openagents_cli.models import _should_use_copilot_responses_api
...
def _get_auxiliary_task_config(task: str) -> Dict[str, Any]:
    ...
    :meth:`openagents_cli.plugins.PluginContext.register_auxiliary_task`) the
