...
from openagents_cli.timeouts import get_provider_request_timeout
...
def init_agent(
    ...
    Args:
        ...
        skip_context_files (bool): If True, skip auto-injection of project context files
            (SOUL.md, .hermes.md, AGENTS.md, CLAUDE.md, .cursorrules) from the cwd / OPENAGENTS_HOME
            into the system prompt. Use this for batch processing and data generation to avoid
            polluting trajectories with user-specific persona or project instructions.
        load_soul_identity (bool): If True, still use ~/.openagents/SOUL.md as the primary
    ...
    agent._cache_ttl = "5m"
    try:
        from openagents_cli.config import load_config as _load_pc_cfg
    ...
    # both live under ~/.openagents/logs/.  Idempotent, so gateway mode
    # (which creates a new AIAgent per message) won't duplicate handlers.
    from openagents_logging import setup_logging, setup_verbose_logging
    ...
    if agent.verbose_logging:
        ...
        # on the fact that openagents_logging.setup_logging() does not
        ...
        # handler level inside openagents_logging.py, not here.
        pass
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        if _is_bedrock_anthropic:
            ...
            if (
                ...
                try:
                    from openagents_cli.auth import build_minimax_oauth_token_provider
        ...
        try:
            from openagents_cli.config import load_config as _load_br_cfg
    ...
    # Session logs go into ~/.openagents/sessions/ alongside gateway sessions
    hermes_home = get_openagents_home()
    agent.logs_dir = hermes_home / "sessions"
    agent.logs_dir.mkdir(parents=True, exist_ok=True)
    # Per-session JSON snapshot writer (~/.openagents/sessions/session_{sid}.json)
    ...
    try:
        from openagents_cli.config import load_config as _load_sess_cfg
    ...
    try:
        from openagents_cli.config import load_config as _load_agent_config
            ...
            if _mem_provider_name and _mem_provider_name.strip():
                ...
                if agent._memory_manager.providers:
                    ...
                    try:
                        ...
                        _init_kwargs["agent_workspace"] = "openagents"
                    except Exception:
                        pass
    ...
    if _config_context_length is None and _custom_providers:
        try:
            from openagents_cli.config import get_custom_provider_context_length
    ...
    if _engine_name != "compressor":
        ...
        if _selected_engine is None:
            _candidate = None
            try:
                from openagents_cli.plugins import get_plugin_context_engine
    ...
    if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH:
        raise ValueError(
            ...
            f"by OpenAgents.  Choose a model with at least "
