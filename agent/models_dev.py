...
Data resolution order (like TypeScript OpenCode):
  ...
  2. Disk cache (~/.openagents/models_dev_cache.json)
...
# ---------------------------------------------------------------------------
# Provider ID mapping: OpenAgents ↔ models.dev
# ---------------------------------------------------------------------------

# OpenAgents provider names → models.dev provider IDs
...
# Reverse mapping: models.dev → OpenAgents (built lazily)
...
def _get_cache_path() -> Path:
    ...
    return get_openagents_home() / "models_dev_cache.json"
...
def _get_provider_models(provider: str) -> Optional[Dict[str, Any]]:
    """Resolve a OpenAgents provider ID to its models dict from models.dev.
...
# Gemma models whose TPM quotas are too small for normal OpenAgents traffic.
...
def get_provider_info(provider_id: str) -> Optional[ProviderInfo]:
    ...
    Accepts either a OpenAgents provider ID (e.g. "kilocode") or a models.dev
    ID (e.g. "kilo").  Returns None if the provider is not in the catalog.
    """
    # Resolve OpenAgents ID → models.dev ID
...
def get_model_info(
    ...
    Accepts OpenAgents or models.dev provider ID.  Tries exact match then
