...
``<OPENAGENTS_HOME>/lsp/bin/``, so we don't pollute the user's global
toolchain.
...
# ``<OPENAGENTS_HOME>/lsp/bin/`` first, then on PATH.
#
# Optional fields:
...
def _install_npm(
    ...
    if npm is None:
        ...
        return None
    staging = hermes_lsp_bin_dir().parent  # <OPENAGENTS_HOME>/lsp/
