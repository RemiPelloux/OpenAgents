...
def _resolve_requests_verify() -> bool | str:
    ...
    by default. OpenAgents also honours HERMES_CA_BUNDLE (its own convention)
...
def _get_model_metadata_cache_path() -> Path:
    ...
    return get_openagents_home() / "cache" / "openrouter_model_metadata.json"
...
# Minimum context length required to run OpenAgents.  Models with fewer
...
DEFAULT_CONTEXT_LENGTHS = {
    ...
    # so these hardcoded fallbacks prevent OpenAgents from probing-down to
...
def _get_context_cache_path() -> Path:
    ...
    return get_openagents_home() / "context_length_cache.yaml"
        ...
        with httpx.Client(timeout=3.0, headers=headers) as client:
            ...
            if server_type == "ollama":
                ...
                if resp.status_code == 200:
                    ...
                    # OpenAgents grow conversations past the runtime limit and Ollama
...
def get_model_context_length(
    ...
    if effective_provider in {"copilot", "copilot-acp", "github-copilot"}:
        try:
            from openagents_cli.models import get_copilot_model_context
...
def estimate_request_tokens_rough(
    ...
    *,
    ...
    Includes the major payload buckets OpenAgents sends to providers:
