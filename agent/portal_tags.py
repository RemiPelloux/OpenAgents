...
Every OpenAgents request that hits the Nous Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Nous can attribute usage to OpenAgents and bucket it by client release.
...
The version is sourced live from ``openagents_cli.__version__`` so it auto-aligns
...
``openagents_cli.__version__`` is the canonical source of truth.
"""
...
def _hermes_version() -> str:
    """Return the current OpenAgents release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``openagents_cli`` cannot be imported (should
...
def nous_portal_tags() -> List[str]:
    ...
    """
    return ["product=openagents", openagents_client_tag()]
