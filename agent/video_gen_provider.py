...
as ``kind: backend``) or ``~/.openagents/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
def _videos_cache_dir() -> Path:
    """Return ``$OPENAGENTS_HOME/cache/videos/``, creating parents as needed."""
    from openagents_constants import get_openagents_home

    path = get_openagents_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$OPENAGENTS_HOME/cache/videos/``.
