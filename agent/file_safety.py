...
def get_read_block_error(path: str) -> Optional[str]:
    """Return an error message when a read targets a denied OpenAgents path.

    Three categories are blocked:

      * Internal OpenAgents cache files under ``OPENAGENTS_HOME/skills/.hub`` —
        readable metadata that an attacker could use as a prompt-injection
        carrier.
      * Credential / secret stores under OPENAGENTS_HOME and the global Hermes
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    or ``cat ~/.openagents/.env`` and exfiltrate the file. The read-deny exists
    ...
    # Resolve BOTH the active OPENAGENTS_HOME (profile-aware) AND the global
    # OpenAgents root so credential stores at <root>/auth.json etc. are also
    # blocked when running under a profile (OPENAGENTS_HOME points at
    ...
    for hd in hermes_dirs:
        ...
        for blocked in blocked_dirs:
            try:
                ...
                continue
            return (
                f"Access denied: {path} is an internal OpenAgents cache file "
                "and cannot be read directly to prevent prompt injection. "
                "Use the skills_list or skill_view tools instead."
            )

    # Credential / secret stores. Exact-file matches under either
    # OPENAGENTS_HOME or <root>.
    ...
    for hd in hermes_dirs:
        for name in credential_file_names:
            ...
            if resolved == blocked:
                return (
                    f"Access denied: {path} is a OpenAgents credential store "
    ...
    for hd in hermes_dirs:
        ...
        if resolved == mcp_tokens:
            return (
                f"Access denied: {path} is the OpenAgents MCP token directory "
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is a OpenAgents MCP token file "
            "and cannot be read directly. (Defense-in-depth — not a "
            "security boundary; the terminal tool can still bypass.)"
        )
...
#
# OpenAgents profiles are separate OPENAGENTS_HOME dirs under
...
# edited skills under both ``~/.openagents/profiles/hermes-security/skills/``
# AND ``~/.openagents/skills/`` (the default profile's skills) without realizing
# the second path belonged to a different profile.
# ---------------------------------------------------------------------------

# Profile-scoped directories under OPENAGENTS_HOME / <root> / <root>/profiles/<X>/
...
def _resolve_active_profile_name() -> str:
    """Return the active profile name derived from OPENAGENTS_HOME.

    ``~/.openagents``              -> ``"default"``
    ``~/.openagents/profiles/X``  -> ``"X"``
...
def classify_cross_profile_target(path: str) -> Optional[dict]:
    ...
    Returns ``None`` when the target is outside OpenAgents scope, or is inside
...
def get_cross_profile_warning(path: str) -> Optional[str]:
    ...
    OpenAgents entirely. Caller is expected to surface the warning to the
    ...
    return (
        ...
        f"belongs to OpenAgents profile {info['target_profile']!r}, but the "
...
#
#   <OPENAGENTS_HOME>/profiles/<name>/sandboxes/<backend>/<task>/home/.hermes/...
#
...
# regardless of which OpenAgents profile is active. It does NOT cover the
...
def classify_sandbox_mirror_target(path: str) -> Optional[dict]:
    """Classify a write target as a sandbox-mirror of authoritative OpenAgents state.
    ...
    Detection is path-shape-only — does not require any OpenAgents resolver to
    succeed, so it works correctly even when called from contexts where
    OPENAGENTS_HOME resolution would be ambiguous.
    """
    try:
        target = Path(os.path.expanduser(str(path))).resolve()
    except (OSError, RuntimeError):
...
def get_sandbox_mirror_warning(path: str) -> Optional[str]:
    ...
    return (
        ...
        f"Writes here land on a copy that the host OpenAgents process never "
        f"reads — the authoritative file is likely {info['inner_path']!r} "
        f"under the real OPENAGENTS_HOME. Use the host-side tool for "
...
def classify_container_mirror_target(
    ...
    under the mirror prefix. Otherwise returns:
      ...
      * ``inner_path``: portion under the mirror root (what the agent
        likely meant to address in the host OPENAGENTS_HOME)
    """
...
def get_container_mirror_warning(
    ...
    sandbox mirror of authoritative OpenAgents state.
    ...
    return (
        ...
        f"bind-mounted home — a per-task mirror that the host OpenAgents "
        f"process never reads. The authoritative file is "
        f"{info['inner_path']!r} under the real OPENAGENTS_HOME. Use the "
