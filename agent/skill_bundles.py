...
Storage
-------
Bundles live in ``~/.openagents/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``OPENAGENTS_HOME``). Each file looks like::
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under OPENAGENTS_HOME.

    Honors ``HERMES_BUNDLES_DIR`` for tests; falls back to
    ``<OPENAGENTS_HOME>/skill-bundles``.
    """
    override = os.environ.get("HERMES_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_openagents_home() / "skill-bundles"
