...
upstream prefix cache warm.  See ``openagents-dev``'s
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  OpenAgents never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want OPENAGENTS_HOME persona while keeping
    ...
    # Pointer to the openagents skill + docs for user questions about OpenAgents itself.
    ...
    # Active-profile hint — names the OpenAgents profile the agent is running
    # under so it doesn't conflate ~/.openagents/skills/ (default profile) with
    # ~/.openagents/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    if active_profile == "default":
        stable_parts.append(
            "Active OpenAgents profile: default. Other profiles (if any) live "
            "under ~/.openagents/profiles/<name>/. Each profile has its own "
        ...
        stable_parts.append(
            f"Active OpenAgents profile: {active_profile}. This session reads "
            f"and writes ~/.openagents/profiles/{active_profile}/. The default "
            f"profile's data lives at ~/.openagents/skills/, ~/.openagents/plugins/, "
            f"~/.openagents/cron/, ~/.openagents/memories/ — those belong to a "
...
def build_system_prompt(agent: Any, system_message: Optional[str] = None) -> str:
    ...
    one cached block — OpenAgents never rebuilds or reinjects parts of it
