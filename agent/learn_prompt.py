...
# (HARDLINE)" and the openagents-dev new-skill salvage reference. Embedded in
# the prompt so the agent authors skills the way a maintainer would by hand.
_AUTHORING_STANDARDS = """\
Follow the OpenAgents skill-authoring standards exactly. These are the same
...
- version: 0.1.0
- author: always the literal value `openagents`. NEVER fill it from the host
...
4. "## How to Run" — the canonical invocation, framed through OpenAgents tools.
...
- Reference OpenAgents tools by name in backticks: `terminal`, `read_file`,
