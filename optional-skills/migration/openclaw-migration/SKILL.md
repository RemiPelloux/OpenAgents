...
description: Migrate a user's OpenClaw customization footprint into OpenAgents. Imports Hermes-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.
version: 1.0.0
author: OpenAgents (Nous Research)
license: MIT
platforms: [linux, macos, windows]
metadata:
  hermes:
    tags: [Migration, OpenClaw, Hermes, Memory, Persona, Import]
    related_skills: [openagents]
---

# OpenClaw -> OpenAgents Migration

Use this skill when a user wants to move their OpenClaw setup into OpenAgents with minimal manual cleanup.
...
- import `SOUL.md` into the OpenAgents home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into OpenAgents memory entries
- merge OpenClaw command approval patterns into OpenAgents `command_allowlist`
- migrate Hermes-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS`, and map OpenClaw workspace settings to OpenAgents working-directory configuration
- copy OpenClaw skills into `~/.openagents/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen OpenAgents workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.openagents/tts/`
- archive non-secret docs that do not have a direct OpenAgents destination
...
- `~/.openagents/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py`

Do not guess a shorter path like `~/.openagents/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.openagents/skills/migration/openclaw-migration/`.
...
OpenAgents CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a OpenAgents workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing OpenAgents skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of OpenAgents unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty OpenAgents targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean OpenAgents destination. Unsupported auth blobs must still be reported as skipped.
...
- OpenAgents persona state imported
- OpenAgents memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.openagents/skills/openclaw-imports/`
