...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("OPENAGENTS_HOME", os.path.expanduser("~/.openagents")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("OPENAGENTS_HOME", Path.home() / ".hermes")) / "skills" / "red-teaming" / "godmode"
...
# ═══════════════════════════════════════════════════════════════════
# OpenAgents config paths
# ═══════════════════════════════════════════════════════════════════

OPENAGENTS_HOME = Path(os.getenv("OPENAGENTS_HOME", Path.home() / ".hermes"))
CONFIG_PATH = OPENAGENTS_HOME / "config.yaml"
PREFILL_PATH = OPENAGENTS_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/OpenAgents models — already uncensored, just needs clean prompt
    "openagents": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "openagents" in model_lower or "nous" in model_lower:
        return "openagents"
...
def _get_current_model() -> tuple:
    """Read current model and provider from OpenAgents config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.openagents/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart OpenAgents for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart OpenAgents for changes to take effect.")
