...
try:
    ...
    _SKILL_DIR = (
        Path(os.getenv("OPENAGENTS_HOME", Path.home() / ".hermes"))
        / "skills"
        / "red-teaming"
        / "godmode"
    )
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print(
                    "[DONE] Jailbreak locked in. Restart OpenAgents for changes to take effect."
                )
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print(
            "[UNDO] Jailbreak removed. Restart OpenAgents for changes to take effect."
        )
