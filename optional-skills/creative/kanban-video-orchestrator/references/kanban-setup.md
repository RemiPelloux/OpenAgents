...
3. **Configure profiles** — patch each profile's
   `~/.openagents/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
...
Each profile has a YAML config at `~/.openagents/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.openagents/profiles/{profile}/config.yaml")
with open(p) as f:
...
Each profile gets a `SOUL.md` at `~/.openagents/profiles/<name>/SOUL.md` that
...
the OpenAgents `.env` (`${OPENAGENTS_HOME:-$HOME/.hermes}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _hermes_env="${OPENAGENTS_HOME:-$HOME/.hermes}/.env"
