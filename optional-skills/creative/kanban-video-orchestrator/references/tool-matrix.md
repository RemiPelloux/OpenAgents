...
Maps each role archetype to the OpenAgents skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public openagents
...
## OpenAgents skills relevant to video production

### Visual / rendering skills (`openagents/skills/creative/`)
...
### Design / pre-production skills (`openagents/skills/creative/`)
...
### Audio / media skills (`openagents/skills/creative/` + `skills/media/`)
...
These are **not** OpenAgents skills but external CLIs / APIs that profiles invoke.
...
## Built-in OpenAgents tools for media review

These are native OpenAgents tools — not invoked via terminal but through their own
...
key is present in `${OPENAGENTS_HOME:-~/.openagents}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every OpenAgents profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${OPENAGENTS_HOME:-~/.openagents}/.env` → environment variable.
