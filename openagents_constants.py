"""Shared constants for OpenAgents.
...
_UNSET = object()
_OPENAGENTS_HOME_OVERRIDE: ContextVar[str | object] = ContextVar(
    "_OPENAGENTS_HOME_OVERRIDE", default=_UNSET
)


def set_openagents_home_override(path: str | Path | None) -> Token:
    """Set a context-local OpenAgents home override and return its reset token.

    This is for in-process, per-task scoping.  It deliberately does not mutate
    ``os.environ`` because that is shared by every thread in the process.
    """
    value: str | object = _UNSET if path is None else str(path)
    return _OPENAGENTS_HOME_OVERRIDE.set(value)


def reset_openagents_home_override(token: Token) -> None:
    """Restore the previous context-local OpenAgents home override."""
    _OPENAGENTS_HOME_OVERRIDE.reset(token)


def get_openagents_home_override() -> str | None:
    """Return the active context-local OpenAgents home override, if any."""
    override = _OPENAGENTS_HOME_OVERRIDE.get()
    if override is _UNSET or not override:
        return None
    return str(override)


def _get_platform_default_openagents_home() -> Path:
    """Return the platform-native default OpenAgents home path."""
    if sys.platform == "win32":
        local_appdata = os.environ.get("LOCALAPPDATA", "").strip()
        base = Path(local_appdata) if local_appdata else Path.home() / "AppData" / "Local"
        modern = base / "openagents"
        legacy = base / "hermes"
...
def get_openagents_home() -> Path:
    """Return the OpenAgents home directory (default: platform-native path).

    Reads OPENAGENTS_HOME env var, falls back to the platform-native default.
    This is the single source of truth — all other copies should import this.

    When ``OPENAGENTS_HOME`` is unset but an ``active_profile`` file indicates
    ...
    expected to propagate ``OPENAGENTS_HOME`` explicitly (see the systemd
    template in ``openagents_cli/gateway.py`` and the kanban dispatcher in
    ``openagents_cli/kanban_db.py``).  See https://github.com/NousResearch/openagents/issues/18594.
    """
    ...
    if not _profile_fallback_warned:
        try:
            fallback_home = _get_platform_default_openagents_home()
        ...
        if active and active != "default":
            ...
            msg = (
                f"[OPENAGENTS_HOME fallback] OPENAGENTS_HOME is unset but active "
                f"profile is {active!r}. Falling back to {fallback_home}, which "
                f"is the DEFAULT profile — not {active!r}. Any data this "
                f"process writes will land in the wrong profile. The "
                f"subprocess spawner should pass OPENAGENTS_HOME explicitly "
                f"(see issue #18594)."
            )
            try:
    ...
    return _get_platform_default_openagents_home()


def get_default_openagents_root() -> Path:
    """Return the root OpenAgents directory for profile-level operations.

    In standard deployments this is the platform-native OpenAgents home
    (``~/.openagents`` on POSIX, ``%LOCALAPPDATA%\\hermes`` on native Windows).

    In Docker or custom deployments where ``OPENAGENTS_HOME`` points outside
    ``~/.openagents`` (e.g. ``/opt/data``), returns ``OPENAGENTS_HOME`` directly
    — that IS the root.

    In profile mode where ``OPENAGENTS_HOME`` is ``<root>/profiles/<name>``,
    returns ``<root>`` so that ``profile list`` can see all profiles.
    Works both for standard (``~/.openagents/profiles/coder``) and Docker
    (``/opt/data/profiles/coder``) layouts.

    Import-safe — no dependencies beyond stdlib.
    """
    native_home = _get_platform_default_openagents_home()
    env_home = os.environ.get("OPENAGENTS_HOME", "")
    if not env_home:
        return native_home
    env_path = Path(env_home)
    try:
        env_path.resolve().relative_to(native_home.resolve())
        # OPENAGENTS_HOME is under ~/.openagents (normal or profile mode)
        return native_home
    except ValueError:
        pass
    ...
    # Not a profile path — OPENAGENTS_HOME itself is the root
    return env_path
...
def _get_packaged_data_dir(name: str) -> Path | None:
    ...
    Used to discover bundled skills/optional-skills when OpenAgents is installed
...
def get_optional_skills_dir(default: Path | None = None) -> Path:
    ...
    tree and expose it via ``OPENAGENTS_OPTIONAL_SKILLS``.
    """
    override = os.getenv("OPENAGENTS_OPTIONAL_SKILLS", "").strip()
    if override:
        return Path(override)
    ...
    if default is not None:
        return default
    return get_openagents_home() / "optional-skills"
...
def get_optional_mcps_dir(default: Path | None = None) -> Path:
    ...
    package tree and expose it via ``OPENAGENTS_OPTIONAL_MCPS``.
    """
    override = os.getenv("OPENAGENTS_OPTIONAL_MCPS", "").strip()
    if override:
        return Path(override)
    ...
    if default is not None:
        return default
    return get_openagents_home() / "optional-mcps"
...
def get_bundled_skills_dir(default: Path | None = None) -> Path:
    ...
    Resolution order:
        ...
        4. ``<OPENAGENTS_HOME>/skills`` last-resort
    """
    ...
    if default is not None:
        return default
    return get_openagents_home() / "skills"


def get_openagents_dir(new_subpath: str, old_name: str) -> Path:
    """Resolve a OpenAgents subdirectory with backward compatibility.
    ...
    Args:
        new_subpath: Preferred path relative to OPENAGENTS_HOME (e.g. ``"cache/images"``).
        old_name: Legacy path relative to OPENAGENTS_HOME (e.g. ``"image_cache"``).
...
def iter_hermes_node_dirs(home: Path | None = None) -> list[Path]:
    ...
    ``$OPENAGENTS_HOME/node/bin``. Include both shapes on every platform so mixed
    or migrated installs still work.
    """
    root = home or get_openagents_home()
    dirs = [root / "node"]
...
def node_tool_runnable(path: str | None) -> bool:
    ...
    Hermes-managed Node trees live under ``$OPENAGENTS_HOME/node`` (or a profile's
    ``OPENAGENTS_HOME``). A partial upgrade or interrupted install can leave
...
def _heal_managed_node_windows() -> bool:
    """Redownload the portable Node zip into ``%OPENAGENTS_HOME%\\node`` on Windows."""
    import re
    import tempfile
...
def heal_hermes_managed_node() -> bool:
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "OPENAGENTS_HOME": str(get_openagents_home())},
            capture_output=True,
            timeout=300,
            check=False,
        )
...
def display_openagents_home() -> str:
    """Return a user-friendly display string for the current OPENAGENTS_HOME.

    Uses ``~/`` shorthand for readability::

        default:  ``~/.openagents``
        profile:  ``~/.openagents/profiles/coder``
        custom:   ``/opt/hermes-custom``

    Use this in **user-facing** print/log messages instead of hardcoding
    ``~/.openagents``.  For code that needs a real ``Path``, use
    :func:`get_openagents_home` instead.
    """
    home = get_openagents_home()
    try:
...
def secure_parent_dir(path: Path) -> None:
    ...
    prevent catastrophic host bricking when ``OPENAGENTS_HOME`` or other path
    env vars resolve to an unexpected location.

    See https://github.com/NousResearch/openagents/issues/25821.
    """
...
def _profile_home_path(env: dict[str, str] | None = None) -> str | None:
    """Return ``{OPENAGENTS_HOME}/home`` when the profile-home directory exists."""
    hermes_home = get_openagents_home_override() or (env or {}).get("OPENAGENTS_HOME") or os.getenv("OPENAGENTS_HOME")
    if not hermes_home:
        return None
...
def get_real_home(env: dict[str, str] | None = None) -> str:
    """Return the OS user's real home directory, avoiding OpenAgents profile HOME.

    ``OPENAGENTS_HOME`` scopes OpenAgents state. ``HOME`` is reserved for the OS/user
    account and the many external CLIs that store credentials under ``~``.
    If a parent process is already running with ``HOME={OPENAGENTS_HOME}/home``,
...
def get_subprocess_home(env: dict[str, str] | None = None) -> str | None:
    ...
    * ``auto`` (default): host installs keep the real user HOME; containers use
      ``{OPENAGENTS_HOME}/home`` for persistent state. If a host parent already has
      HOME pointed at the profile home, repair subprocesses back to real HOME.
    * ``real``: always prefer the real OS-user HOME.
    * ``profile``: use ``{OPENAGENTS_HOME}/home`` when it exists, preserving the
...
def is_container() -> bool:
    ...
    See: NousResearch/openagents#47111
    """
...
def get_config_path() -> Path:
    """Return the path to ``config.yaml`` under OPENAGENTS_HOME.

    Replaces the ``get_openagents_home() / "config.yaml"`` pattern repeated
    in 7+ files (skill_utils.py, openagents_logging.py, openagents_time.py, etc.).
    """
    return get_openagents_home() / "config.yaml"


def get_skills_dir() -> Path:
    """Return the path to the skills directory under OPENAGENTS_HOME."""
    return get_openagents_home() / "skills"



def get_env_path() -> Path:
    """Return the path to the ``.env`` file under OPENAGENTS_HOME."""
