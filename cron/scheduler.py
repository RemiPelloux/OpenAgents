...
Uses a file-based lock (~/.openagents/cron/.tick.lock) so only one tick
...
# the module) fail with ModuleNotFoundError for openagents_time et al.
...
from openagents_cli.config import load_config, _expand_env_vars
from openagents_time import now as _hermes_now
...
def _merge_mcp_into_per_job_toolsets(per_job: list[str], cfg: dict) -> list[str]:
    ...
    if "no_mcp" in per_job:
        return result
    # lazy import: avoid heavy openagents_cli import at cron module load (matches
    # _resolve_cron_enabled_toolsets' fallback) and share one MCP-membership
    # computation with the gateway/CLI platform resolver.
    from openagents_cli.tools_config import enabled_mcp_server_names
...
def _resolve_cron_enabled_toolsets(job: dict, cfg: dict) -> list[str] | None:
    ...
    try:
        from openagents_cli.tools_config import _get_platform_tools  # lazy: avoid heavy import at cron module load
...
def _get_openagents_home() -> Path:
    """Resolve OpenAgents home dynamically while preserving test monkeypatch hooks.

    Cron is per-profile by design (#4707): the in-process ticker runs inside a
    profile-scoped gateway, so resolving the active OPENAGENTS_HOME at call time
    ...
    """
    return _hermes_home or get_openagents_home()


def _get_lock_paths() -> tuple[Path, Path]:
    """Resolve cron lock paths at call time so profile/env changes are honored."""
    hermes_home = _get_openagents_home()
...
def _run_job_script(script_path: str) -> tuple[bool, str]:
    ...
    Scripts must reside within OPENAGENTS_HOME/scripts/.  Both relative and
    ...
    * anything else — run with the current Python interpreter
    ...
    Args:
        script_path: Path to the script.  Relative paths are resolved
            against OPENAGENTS_HOME/scripts/.  Absolute and ~-prefixed paths
    ...
    """
    scripts_dir = _get_openagents_home() / "scripts"
    ...
    # escape — scripts MUST reside within OPENAGENTS_HOME/scripts/.
    try:
...
def run_job(job: dict) -> tuple[bool, str, str, Optional[str]]:
    ...
    try:
        ...
        try:
            load_dotenv(str(_get_openagents_home() / ".env"), override=True, encoding="utf-8")
        except UnicodeDecodeError:
            load_dotenv(str(_get_openagents_home() / ".env"), override=True, encoding="latin-1")
        ...
        try:
            import yaml
            _cfg_path = str(_get_openagents_home() / "config.yaml")
        ...
        if prefill_file:
            ...
            if not pfpath.is_absolute():
                pfpath = _get_openagents_home() / pfpath
            if pfpath.exists():
                try:
        ...
        agent = AIAgent(
            ...
            # OPENAGENTS_HOME. When a workdir is configured, also inject project
