...
A *suggestion* is a ready-to-run cron job spec that OpenAgents surfaces to the
...
Storage mirrors ``cron/jobs.py``: ``~/.openagents/cron/suggestions.json``, atomic
...
# profile's cron store. Anchor on get_openagents_home() (profile home), not the
# shared default root. See cron/jobs.py for the full rationale.
CRON_DIR = get_openagents_home().resolve() / "cron"
