...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      openagents = self'.packages.default;
      hermesVenv = openagents.hermesVenv;
    ...
    in {
      ...
      checks = {
        ...
        build-package = pkgs.runCommand "hermes-build-package" { } ''
          echo "PASS: package built at ${openagents}"
          mkdir -p $out
          echo "ok" > $out/result
        '';
        ...
        package-contents = pkgs.runCommand "hermes-package-contents" { } ''
          ...
          test -x ${openagents}/bin/hermes || (echo "FAIL: hermes binary missing"; exit 1)
          test -x ${openagents}/bin/openagents || (echo "FAIL: openagents binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${openagents}/bin/hermes version 2>&1 | grep -qi "openagents" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "hermes-entry-points-sync" { } ''
          ...
          for bin in hermes openagents openagents-acp; do
            test -x ${openagents}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "hermes-cli-commands" { } ''
          ...
          ${openagents}/bin/hermes --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${openagents}/bin/hermes --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "hermes-bundled-skills" { } ''
          ...
          test -d ${openagents}/share/openagents/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${openagents}/share/openagents/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "HERMES_BUNDLED_SKILLS" ${openagents}/bin/hermes || \
        ...
        bundled-plugins = pkgs.runCommand "hermes-bundled-plugins" { } ''
          ...
          test -d ${openagents}/share/openagents/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${openagents}/share/openagents/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "HERMES_BUNDLED_PLUGINS" ${openagents}/bin/hermes || \
        ...
        bundled-locales = pkgs.runCommand "hermes-bundled-locales" { } ''
          ...
          test -d ${openagents}/share/openagents/locales || (echo "FAIL: locales directory missing"; exit 1)
          echo "PASS: locales directory exists"

          LOC_COUNT=$(find ${openagents}/share/openagents/locales -name "*.yaml" | wc -l)
          test "$LOC_COUNT" -ge 16 || (echo "FAIL: expected >=16 catalogs, found $LOC_COUNT"; exit 1)
          echo "PASS: $LOC_COUNT locale catalogs found"

          test -f ${openagents}/share/openagents/locales/en.yaml || (echo "FAIL: en.yaml missing"; exit 1)
          echo "PASS: en.yaml present"

          grep -q "HERMES_BUNDLED_LOCALES" ${openagents}/bin/hermes || \
          ...
          export HOME=$(mktemp -d)
          RENDERED=$(cd "$HOME" && HERMES_BUNDLED_LOCALES=${openagents}/share/openagents/locales \
        ...
        bundled-tui = pkgs.runCommand "hermes-bundled-tui" { } ''
          ...
          test -d ${openagents}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${openagents}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "HERMES_TUI_DIR" ${openagents}/bin/hermes || \
        ...
        hermes-node = pkgs.runCommand "hermes-node-version" { } ''
          ...
          grep -q "HERMES_NODE" ${openagents}/bin/hermes || \
            (echo "FAIL: HERMES_NODE not set in wrapper"; exit 1)
          echo "PASS: HERMES_NODE present in wrapper"

          HERMES_NODE=$(sed -n "s/^export HERMES_NODE='\(.*\)'/\1/p" ${openagents}/bin/hermes)
        ...
        managed-guard = pkgs.runCommand "hermes-managed-guard" { } ''
          ...
          check_blocked "config set" ${openagents}/bin/hermes config set model foo
          check_blocked "config edit" ${openagents}/bin/hermes config edit
        ...
        extra-python-packages = let
          ...
          hermesWithExtra = openagents.override {
        ...
        in pkgs.runCommand "hermes-extra-python-packages" { } ''
          ...
          if grep -q "PYTHONPATH" ${openagents}/bin/hermes; then
        ...
        extra-dependency-groups = let
          hermesWithGroups = openagents.override {
        ...
        in pkgs.runCommand "hermes-config-roundtrip" {
          ...
          merge_and_load() {
            local hermes_home="$1"
            export OPENAGENTS_HOME="$hermes_home"
