# nix/nixosModules.nix — NixOS module for openagents
#
# Two modes:
...
# container recreation. Environment variables are written to $OPENAGENTS_HOME/.env
...
{ inputs, ... }: {
  ...
  let
    cfg = config.services.openagents;
    effectivePackage =
    ...
    openagents = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-openagents)
    ...
    # OPENAGENTS_HOME via addToSystemPackages, so they can save settings through the
    ...
    containerEntrypoint = pkgs.writeShellScript "hermes-container-entrypoint" ''
      ...
      # Ensure OPENAGENTS_HOME is owned by the target user.
      ...
      if [ -n "''${OPENAGENTS_HOME:-}" ] && [ -d "$OPENAGENTS_HOME" ]; then
        find "$OPENAGENTS_HOME" \! -user "$HERMES_UID" -exec chown "$HERMES_UID:$HERMES_GID" {} +
      fi
    ...
    # so they can update without recreation. Env vars go through $OPENAGENTS_HOME/.env.
  ...
  in {
    options.services.openagents = with lib; {
      enable = mkEnableOption "OpenAgents gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = openagents;
        description = "The openagents package to use.";
      };
      ...
      stateDir = mkOption {
        ...
        description = "State directory. Contains .hermes/ subdir (OPENAGENTS_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative OpenAgents config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $OPENAGENTS_HOME/.env at activation time.
          OpenAgents reads this file on every startup via load_openagents_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $OPENAGENTS_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $OPENAGENTS_HOME/mcp-tokens/.
              '';
            };
      ...
      extraPlugins = mkOption {
        ...
        description = ''
          ...
          at its root. OpenAgents discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in openagents's
      ...
      addToSystemPackages = mkOption {
        ...
        description = ''
          ...
          OPENAGENTS_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.openagents symlink to the service
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.openagents.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the hermes CLI to system PATH and export OPENAGENTS_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.openagents/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ effectivePackage ];
        environment.variables.OPENAGENTS_HOME = "${cfg.stateDir}/.hermes";
      })
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.openagents.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.openagents: container.enable is true and container.hostUsers
      ...
      {
        system.activationScripts."openagents-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
    ...
    HERMES_CONTAINER_MODE_EOF
            ...
            ${lib.concatStringsSep "\n" (map (user:
              ...
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.hermes" ]; then
                  rm -f "${symlinkPath}"
                  echo "openagents: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.openagents -> stateDir/.hermes for each hostUser so the
            ...
            (lib.concatStringsSep "\n" (map (user:
              ...
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  ...
                  echo "openagents: backing up existing ${symlinkPath} to $_backup"
          ...
          # OpenAgents reads $OPENAGENTS_HOME/.env at startup via load_openagents_dotenv(),
    ...
    HERMES_NIX_ENV_EOF
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.openagents = {
          ...
          environment = {
            HOME = cfg.stateDir;
            OPENAGENTS_HOME = "${cfg.stateDir}/.hermes";
            HERMES_MANAGED = "true";
          ...
          serviceConfig = {
            ...
            # $OPENAGENTS_HOME/.env by the activation script. load_openagents_dotenv()
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.openagents = {
          description = "OpenAgents Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              ${containerBin} create \
                ...
                --env OPENAGENTS_HOME=${containerDataDir}/.hermes \
