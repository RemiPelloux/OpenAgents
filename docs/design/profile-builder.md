...
needs, and a profile is just a OPENAGENTS_HOME directory — so the dashboard is the
right home for a full-featured builder, and it can reuse everything that
already exists.

A profile = a full `~/.openagents/profiles/<name>/` directory with its own:
...
| Model + Provider | ModelsPage | `_write_profile_model(profile_dir, …)` | yes — OPENAGENTS_HOME override, already wired into create endpoint |
| MCPs | McpPage | `mcp_config._save_mcp_server` + `/api/mcp/catalog` | yes — wrap with OPENAGENTS_HOME override |
...
### Seam #1 — hub-skill install cannot use the OPENAGENTS_HOME override

`tools/skills_hub.py` binds `SKILLS_DIR = OPENAGENTS_HOME / "skills"` at **module
import time**. The context-local `set_openagents_home_override()` swap (which makes
...
runs `python -m openagents_cli.main <subcommand>`, and `_apply_profile_override()`
...
A fresh subprocess re-imports `skills_hub` with the profile's OPENAGENTS_HOME bound
...
2. write model (synchronous, OPENAGENTS_HOME override)
3. write selected MCP servers (synchronous, OPENAGENTS_HOME override)
...
class ProfileCreate(BaseModel):
    ...
    mcp_servers: List[MCPServerCreate] = []      # synchronous, OPENAGENTS_HOME override
...
page afterward. Mirror `_write_profile_model`'s OPENAGENTS_HOME-override helper for
...
- Backend E2E with isolated OPENAGENTS_HOME: POST a full create body
