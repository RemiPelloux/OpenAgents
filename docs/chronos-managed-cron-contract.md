...
Chronos lets a hosted OpenAgents gateway **scale to zero** while idle and still
...
**dashboard app** (`openagents_cli/web_server.py`) — the agent's always-reachable
...
- **Multi-replica agents:** the store CAS makes the fire at-most-once across N
  gateway replicas sharing one `OPENAGENTS_HOME` — exactly one replica runs each
  fire.
