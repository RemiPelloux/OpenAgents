# OpenAgents Middleware

OpenAgents middleware is the behavior-changing companion to observer hooks.
...
- Rewrite LLM provider request kwargs before OpenAgents calls the provider.
- Rewrite tool arguments before guardrails, approval checks, hooks, and tool
  execution see them.
- Wrap the actual LLM execution callback while preserving OpenAgents retry,
  streaming, interrupt, and hook behavior.
- Wrap the actual tool execution callback while preserving OpenAgents guardrails,
...
OpenAgents stores those trace entries in later observer hook payloads as
`middleware_trace`.
...
If multiple plugins register the same execution middleware kind, OpenAgents runs
them as a nested chain in registration order. Middleware failures are fail-open:
OpenAgents logs a warning and continues with the next middleware or the base
runtime path.
...
For each provider request, OpenAgents applies middleware in this order:
...
For each tool call, OpenAgents applies middleware in this order:

1. Parse and coerce model-provided tool arguments.
2. Apply `tool_request` middleware.
3. Run the normal OpenAgents pre-execution path against the effective arguments:
...
For isolated local testing, use one `OPENAGENTS_HOME` for plugin enablement and the
agent run:

```bash
export OPENAGENTS_HOME=/tmp/hermes-middleware-test
...
Return the same response shape OpenAgents expects from the provider adapter. Do not
...
- If execution middleware raises before calling `next_call(...)`, OpenAgents treats
...
- If execution middleware calls `next_call(...)` successfully and then raises
  during post-processing, OpenAgents preserves the downstream result and does not
  run the provider or tool a second time.
- If downstream provider or tool execution fails, middleware may let that error
  propagate or translate it deliberately. OpenAgents does not convert downstream
