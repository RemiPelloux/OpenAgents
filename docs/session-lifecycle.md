...
The canonical transcript store is SQLite via `SessionDB` (from `openagents_state`). The
...
Counts consecutive restarts via a JSON file (`{OPENAGENTS_HOME}/restart_counts.json`). If a
