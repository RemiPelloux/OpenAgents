...
class TestRmtreeWritableScopeGuard:
    ...
    exception — the result is a silent ``shutil.rmtree(~/.openagents/)``
    ...
    def test_refuses_hermes_home_itself(self, tmp_path):
        """``~/.openagents/`` itself is what the #48200 wipe destroyed."""
        from tools.skills_sync import _rmtree_writable

        openagents = tmp_path / "home"
        hermes.mkdir()
        (hermes / "skills").mkdir()
...
class TestNoBundledSkillsOptOut:
    ...
    installer's `--no-skills` flag (default ~/.openagents) rely on so bundled
    ...
    def test_marker_skips_sync(self, tmp_path):
        ...
        with (
            ...
            patch("tools.skills_sync.OPENAGENTS_HOME", hermes_home),
        ):
            result = sync_skills(quiet=True)
...
class TestOptOutToggleAndRemove:
    ...
    def test_marker_toggle(self, tmp_path):
        ...
        home = tmp_path / "home"
        home.mkdir()
        with patch("tools.skills_sync.OPENAGENTS_HOME", home):
    ...
    def test_remove_keeps_user_modified(self, tmp_path):
        ...
        with (
            ...
            patch("tools.skills_sync.SKILLS_DIR", skills_dir),
            patch("tools.skills_sync.MANIFEST_FILE", manifest_file),
            patch("tools.skills_sync.OPENAGENTS_HOME", home),
        ):
            sync_skills(quiet=True)
            # User edits 'beta'
