...
class TestSaveClipboardImage:
    ...
    def test_dispatches_to_windows_on_win32(self, tmp_path):
        ...
        with patch("openagents_cli.clipboard.sys") as mock_sys:
            ...
            with patch(
                "openagents_cli.clipboard._windows_save", return_value=False
            ) as m:
                save_clipboard_image(dest)
                m.assert_called_once_with(dest)
...
class TestMacosPngpaste:
    ...
    def test_not_installed(self, tmp_path):
        with patch(
            "openagents_cli.clipboard.subprocess.run", side_effect=FileNotFoundError
        ):
    ...
    def test_timeout_returns_false(self, tmp_path):
        dest = tmp_path / "out.png"
        with patch(
            "openagents_cli.clipboard.subprocess.run",
...
class TestMacosOsascript:
    ...
    def test_clipboard_info_fails(self, tmp_path):
        with patch(
            "openagents_cli.clipboard.subprocess.run", side_effect=Exception("fail")
        ):
            assert _macos_osascript(tmp_path / "out.png") is False
...
class TestWaylandSave:
    ...
    def test_bmp_extraction_with_pillow_convert(self, tmp_path):
        ...
        with patch("openagents_cli.clipboard.subprocess.run", side_effect=fake_run):
            with patch(
                "openagents_cli.clipboard._convert_to_png", side_effect=fake_convert
            ):
                assert _wayland_save(dest) is True
...
class TestLinuxSave:
        ...
        with patch("openagents_cli.clipboard._is_wsl", return_value=True):
            with patch("openagents_cli.clipboard._wsl_save", return_value=False):
                with patch.dict(os.environ, {}, clear=True):
                    with patch(
                        "openagents_cli.clipboard._xclip_save", return_value=True
                    ) as m:
                        assert _linux_save(dest) is True
                        m.assert_called_once_with(dest)
    ...
    def test_wayland_tried_when_display_set(self, tmp_path):
        ...
        with patch("openagents_cli.clipboard._is_wsl", return_value=False):
            with patch.dict(os.environ, {"WAYLAND_DISPLAY": "wayland-0"}):
                with patch(
                    "openagents_cli.clipboard._wayland_save", return_value=True
                ) as m:
                    assert _linux_save(dest) is True
                    m.assert_called_once_with(dest)
    ...
    def test_wayland_fails_falls_through_to_xclip(self, tmp_path):
        ...
        with patch("openagents_cli.clipboard._is_wsl", return_value=False):
            with patch.dict(os.environ, {"WAYLAND_DISPLAY": "wayland-0"}):
                with patch(
                    "openagents_cli.clipboard._wayland_save", return_value=False
                ):
                    with patch(
                        "openagents_cli.clipboard._xclip_save", return_value=True
                    ) as m:
...
class TestHasClipboardImageWin32:
    ...
    def test_dispatches_on_win32(self):
        with patch("openagents_cli.clipboard.sys") as mock_sys:
            mock_sys.platform = "win32"
            with patch(
                "openagents_cli.clipboard._windows_has_image", return_value=True
            ) as m:
...
class TestConvertToPng:
    ...
    def test_imagemagick_failure_preserves_original(self, tmp_path):
        ...
        with patch.dict(sys.modules, {"PIL": None, "PIL.Image": None}):
            with patch(
                "openagents_cli.clipboard.subprocess.run", side_effect=fake_run_fail
            ):
...
class TestHasClipboardImage:
    ...
    def test_macos_dispatch(self):
        with patch("openagents_cli.clipboard.sys") as mock_sys:
            mock_sys.platform = "darwin"
            with patch(
                "openagents_cli.clipboard._macos_has_image", return_value=True
            ) as m:
                assert has_clipboard_image() is True
                m.assert_called_once()
    ...
    def test_linux_wsl_dispatch(self):
        with patch("openagents_cli.clipboard.sys") as mock_sys:
            ...
            with patch("openagents_cli.clipboard._is_wsl", return_value=True):
                with patch(
                    "openagents_cli.clipboard._wsl_has_image", return_value=True
                ) as m:
                    assert has_clipboard_image() is True
                    m.assert_called_once()
    ...
    def test_wsl_falls_through_to_wayland_when_windows_path_empty(self):
        ...
        with patch("openagents_cli.clipboard.sys") as mock_sys:
            ...
            with patch("openagents_cli.clipboard._is_wsl", return_value=True):
                with patch(
                    "openagents_cli.clipboard._wsl_has_image", return_value=False
                ) as wsl:
                    with patch.dict(os.environ, {"WAYLAND_DISPLAY": "wayland-0"}):
                        with patch(
                            "openagents_cli.clipboard._wayland_has_image",
                            return_value=True,
                        ) as wl:
                            assert has_clipboard_image() is True
    ...
    def test_linux_wayland_dispatch(self):
        with patch("openagents_cli.clipboard.sys") as mock_sys:
            ...
            with patch("openagents_cli.clipboard._is_wsl", return_value=False):
                with patch.dict(os.environ, {"WAYLAND_DISPLAY": "wayland-0"}):
                    with patch(
                        "openagents_cli.clipboard._wayland_has_image", return_value=True
                    ) as m:
                        assert has_clipboard_image() is True
                        m.assert_called_once()
    ...
    def test_linux_x11_dispatch(self):
        with patch("openagents_cli.clipboard.sys") as mock_sys:
            ...
            with patch("openagents_cli.clipboard._is_wsl", return_value=False):
                with patch.dict(os.environ, {}, clear=True):
                    with patch(
                        "openagents_cli.clipboard._xclip_has_image", return_value=True
                    ) as m:
                        assert has_clipboard_image() is True
                        m.assert_called_once()
...
class TestTryAttachClipboardImage:
    ...
    def test_mixed_success_and_failure(self, cli):
        ...
        with patch(
            "openagents_cli.clipboard.save_clipboard_image", side_effect=results
        ):
            cli._try_attach_clipboard_image()
            cli._try_attach_clipboard_image()
            cli._try_attach_clipboard_image()
