...
def _restore_tool_modules():
    original_hermes_home = os.environ.get("OPENAGENTS_HOME")
    original_modules = {
        name: module
        for name, module in sys.modules.items()
        if name == "tools"
        or name.startswith("tools.")
        or name == "openagents_cli"
        or name.startswith("openagents_cli.")
        or name == "modal"
    ...
    try:
        ...
        if original_hermes_home is None:
            ...
            os.environ["OPENAGENTS_HOME"] = original_hermes_home
        _reset_modules(("tools", "openagents_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    openagents_cli = types.ModuleType("openagents_cli")
    openagents_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["openagents_cli"] = openagents_cli
    hermes_home = tmp_path / "hermes-home"
    os.environ["OPENAGENTS_HOME"] = str(hermes_home)
    sys.modules["openagents_cli.config"] = types.SimpleNamespace(
        get_openagents_home=lambda: hermes_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="openagents", create_if_missing=create_if_missing)
