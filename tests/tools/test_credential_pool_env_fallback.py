...
- _seed_from_env reads API keys from ~/.openagents/.env when not in os.environ
...
def isolated_hermes_home(tmp_path, monkeypatch):
    """Point OPENAGENTS_HOME at a temp dir and clear known API key env vars.
...
def _write_env_file(home: Path, **kwargs) -> None:
    """Write key=value pairs to ~/.openagents/.env."""
    lines = [f"{k}={v}" for k, v in kwargs.items()]
    (home / ".env").write_text("\n".join(lines) + "\n")


class TestCredentialPoolSeedsFromDotEnv:
    """_seed_from_env must read keys from ~/.openagents/.env, not just os.environ.
    ...
    load_openagents_dotenv, the credential pool must still discover it.
    """
...
class TestAuthResolvesFromDotEnv:
    """_resolve_api_key_provider_secret must also read from ~/.openagents/.env."""
