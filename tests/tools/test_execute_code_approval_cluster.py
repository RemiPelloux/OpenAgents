...
def test_env_scrub_hermes_allowlist_and_secret_blocks():
    ...
    env = {
        ...
        "OPENAGENTS_HOME": "/h", "HERMES_PROFILE": "p",
    ...
    for kept in ("OPENAGENTS_HOME", "HERMES_PROFILE", "HERMES_CONFIG", "HERMES_ENV", "PATH"):
...
def test_env_scrub_logs_dropped_hermes_vars(caplog):
    ...
    env = {
        "OPENAGENTS_HOME": "/h",          # allowlisted → kept, not logged
    ...
    assert "OPENAGENTS_HOME" in out and "PATH" in out
...
def test_env_scrub_no_log_when_nothing_dropped(caplog):
    ...
    with caplog.at_level(logging.DEBUG, logger="tools.code_execution_tool"):
        _scrub_child_env(
            {"OPENAGENTS_HOME": "/h", "PATH": "/usr/bin"},
