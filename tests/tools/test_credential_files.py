...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape OPENAGENTS_HOME."""
    ...
    def test_legitimate_file_still_works(self, tmp_path, monkeypatch):
        """Normal files inside OPENAGENTS_HOME must still be registered."""
        hermes_home = tmp_path / ".hermes"
        hermes_home.mkdir()
        monkeypatch.setenv("OPENAGENTS_HOME", str(hermes_home))
    ...
    def test_nested_subdir_inside_hermes_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of OPENAGENTS_HOME must be allowed."""
        hermes_home = tmp_path / ".hermes"
        hermes_home.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside OPENAGENTS_HOME pointing outside must be rejected."""
        hermes_home = tmp_path / ".hermes"
        hermes_home.mkdir()
        monkeypatch.setenv("OPENAGENTS_HOME", str(hermes_home))
        ...
        # The resolved path escapes OPENAGENTS_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape OPENAGENTS_HOME."""
        hermes_home = tmp_path / ".hermes"
        hermes_home.mkdir()
        monkeypatch.setenv("OPENAGENTS_HOME", str(hermes_home))
    ...
    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside OPENAGENTS_HOME via config must still mount."""
        hermes_home = tmp_path / ".hermes"
        hermes_home.mkdir()
        monkeypatch.setenv("OPENAGENTS_HOME", str(hermes_home))
...
class TestCacheDirectoryMounts:
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        ``has content`` for ``get_openagents_dir``'s populated-legacy check
        (see #27602 — empty legacy stubs are no longer honoured).
        """
        hermes_home = tmp_path / ".hermes"
        hermes_home.mkdir()
        # Use legacy dir name with content — get_openagents_dir prefers
