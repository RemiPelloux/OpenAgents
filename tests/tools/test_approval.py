...
class TestApprovalModeParsing:
    def test_unquoted_yaml_off_boolean_false_maps_to_off(self):
        with mock_patch("openagents_cli.config.load_config", return_value={"approvals": {"mode": False}}):
            assert _get_approval_mode() == "off"

    def test_string_off_still_maps_to_off(self):
        with mock_patch("openagents_cli.config.load_config", return_value={"approvals": {"mode": "off"}}):
            assert _get_approval_mode() == "off"
...
class TestTeePattern:
    ...
    def test_tee_hermes_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee ~/.openagents/.env")
        assert dangerous is True
        assert key is not None
    ...
    def test_tee_custom_hermes_home_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee $OPENAGENTS_HOME/.env")
        assert dangerous is True
        assert key is not None
...
class TestHermesConfigWriteProtection:
    ...
    ~/.openagents/config.yaml (#14639). config.yaml IS the security policy
    ...
    def test_redirect_overwrite(self):
        dangerous, key, desc = detect_dangerous_command("echo 'approvals:' > ~/.openagents/config.yaml")
        assert dangerous is True
        assert key is not None

    def test_append(self):
        dangerous, key, desc = detect_dangerous_command("echo '  mode: off' >> ~/.openagents/config.yaml")
        assert dangerous is True

    def test_tee(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee ~/.openagents/config.yaml")
        assert dangerous is True

    def test_cp_over_config(self):
        dangerous, key, desc = detect_dangerous_command("cp /tmp/evil.yaml ~/.openagents/config.yaml")
        assert dangerous is True
    ...
    def test_sed_in_place(self):
        ...
        dangerous, key, desc = detect_dangerous_command("sed -i 's/manual/off/' ~/.openagents/config.yaml")
        assert dangerous is True
        assert "hermes config" in desc.lower() or "in-place" in desc.lower()

    def test_sed_in_place_long_flag(self):
        dangerous, key, desc = detect_dangerous_command("sed --in-place 's/manual/off/' ~/.openagents/config.yaml")
        assert dangerous is True
    ...
    def test_sed_in_place_absolute_hermes_home_env(self):
        env_path = get_openagents_home() / ".env"
        dangerous, key, desc = detect_dangerous_command(
    ...
    def test_custom_hermes_home(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee $OPENAGENTS_HOME/config.yaml")
        assert dangerous is True
    ...
    def test_perl_in_place_config(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -i -pe 's/approvals.mode: on/approvals.mode: off/' ~/.openagents/config.yaml"
        )
        assert dangerous is True
    ...
    def test_ruby_in_place_config(self):
        dangerous, key, desc = detect_dangerous_command(
            "ruby -i -pe 'gsub(/manual/, \"off\")' ~/.openagents/config.yaml"
        )
        assert dangerous is True
    ...
    def test_perl_in_place_env(self):
        dangerous, key, desc = detect_dangerous_command(
            "perl -i -pe 's/SECRET=old/SECRET=new/' ~/.openagents/.env"
        )
        assert dangerous is True
    ...
    def test_perl_in_place_separate_flag_token(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -p -i -e 's/approvals.mode: on/approvals.mode: off/' ~/.openagents/config.yaml"
        )
        assert dangerous is True
    ...
    def test_perl_in_place_backup_suffix(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -i.bak -pe 's/x/y/' ~/.openagents/config.yaml"
        )
        assert dangerous is True
    ...
    def test_perl_eval_no_inplace_safe(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -wne 'print' ~/.openagents/config.yaml"
        )
        assert dangerous is False

    def test_read_is_safe(self):
        # Reading config is not a write — must not trip.
        dangerous, key, desc = detect_dangerous_command("cat ~/.openagents/config.yaml")
        assert dangerous is False

    def test_normal_yaml_write_safe(self):
        # A non-OpenAgents config.yaml in a project dir is handled by the project
...
class TestSensitiveRedirectPattern:
    ...
    def test_redirect_to_custom_hermes_home_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x > $OPENAGENTS_HOME/.env")
        assert dangerous is True
        assert key is not None
...
class TestSensitiveCopyMovePattern:
    ...
    shell rc files, or ~/.openagents/config.yaml/.env must require approval — the
...
class TestWindowsAbsolutePathFolding:
    ...
    ~/.openagents/ in dangerous-command detection.
    ...
    SSH, and OpenAgents config/env files returned "safe" without an approval prompt.
    The OS-specific ``Path.home()`` / ``get_openagents_home()`` tests above only
    exercise this branch on a Windows host; these monkeypatch a Windows-style
    HOME/OPENAGENTS_HOME so the fold is verified on the POSIX CI runner too."""
    ...
    def test_windows_hermes_home_config_folds(self, monkeypatch):
        # OpenAgents home nests under the user home on Windows; it must fold before
        # the user-home rewrite eats its prefix.
        monkeypatch.setenv("HOME", r"C:\Users\tester")
        monkeypatch.setenv("OPENAGENTS_HOME", r"C:\Users\tester\.hermes")
        dangerous, key, _ = detect_dangerous_command(
...
class TestGatewayProtection:
    ...
    def test_gateway_run_with_disown_detected(self):
        cmd = "kill 1605 && cd ~/.openagents/openagents && source venv/bin/activate && python -m openagents_cli.main gateway run --replace &disown; echo done"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "systemctl" in desc

    def test_gateway_run_with_ampersand_detected(self):
        cmd = "python -m openagents_cli.main gateway run --replace &"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_gateway_run_with_nohup_detected(self):
        cmd = "nohup python -m openagents_cli.main gateway run --replace"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_gateway_run_with_setsid_detected(self):
        cmd = "openagents_cli.main gateway run --replace &disown"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
    ...
    def test_systemctl_restart_flagged(self):
        ...
        cmd = "systemctl --user restart openagents-gateway"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
...
class TestPgrepKillExpansion:
    ...
    def test_kill_dollar_pidof_detected(self):
        ...
        cmd = "kill -TERM $(pidof openagents_cli.main)"
        dangerous, _, desc = detect_dangerous_command(cmd)
        assert dangerous is True
...
class TestLaunchctlGatewayLifecycle:
    """launchctl stop/kickstart/bootout/unload against the OpenAgents service
    ...
    def test_launchctl_stop_hermes_detected(self):
        cmd = "launchctl stop ai.openagents.gateway"
        dangerous, _, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "launchd" in desc.lower() or "openagents" in desc.lower()

    def test_launchctl_kickstart_hermes_detected(self):
        cmd = "launchctl kickstart -k system/ai.openagents.gateway"
        dangerous, _, _ = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_launchctl_bootout_hermes_detected(self):
        cmd = "launchctl bootout system/ai.openagents.gateway"
        dangerous, _, _ = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_launchctl_unload_hermes_detected(self):
        cmd = "launchctl unload ~/Library/LaunchAgents/ai.openagents.gateway.plist"
        dangerous, _, _ = detect_dangerous_command(cmd)
        assert dangerous is True
    ...
    * ``tirith_fail_open: false`` → surface a Tirith-style warning through
    ...
    def test_fail_open_true_allows_silently_on_import_error(self):
        ...
        with _patch("builtins.__import__", side_effect=self._make_failing_import(real_import)):
            with _patch("openagents_cli.config.load_config", return_value=cfg):
