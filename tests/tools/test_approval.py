...
class TestTeePattern:
    ...
    def test_tee_custom_hermes_home_env(self):
        dangerous, key, desc = detect_dangerous_command(
            "echo x | tee $OPENAGENTS_HOME/.env"
        )
        assert dangerous is True
        assert key is not None
...
class TestHermesConfigWriteProtection:
    ...
    def test_redirect_overwrite(self):
        dangerous, key, desc = detect_dangerous_command(
            "echo 'approvals:' > ~/.openagents/config.yaml"
        )
        assert dangerous is True
        assert key is not None
    ...
    def test_append(self):
        dangerous, key, desc = detect_dangerous_command(
            "echo '  mode: off' >> ~/.openagents/config.yaml"
        )
        assert dangerous is True

    def test_tee(self):
        dangerous, key, desc = detect_dangerous_command(
            "echo x | tee ~/.openagents/config.yaml"
        )
        assert dangerous is True

    def test_cp_over_config(self):
        dangerous, key, desc = detect_dangerous_command(
            "cp /tmp/evil.yaml ~/.openagents/config.yaml"
        )
        assert dangerous is True
    ...
    def test_sed_in_place(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "sed -i 's/manual/off/' ~/.openagents/config.yaml"
        )
        assert dangerous is True
    ...
    def test_sed_in_place_long_flag(self):
        dangerous, key, desc = detect_dangerous_command(
            "sed --in-place 's/manual/off/' ~/.openagents/config.yaml"
        )
        assert dangerous is True
    ...
    def test_custom_hermes_home(self):
        dangerous, key, desc = detect_dangerous_command(
            "echo x | tee $OPENAGENTS_HOME/config.yaml"
        )
        assert dangerous is True
