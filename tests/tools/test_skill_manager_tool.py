...
def _skill_dir(tmp_path):
    ...
    only the temp directory — not the real ~/.openagents/skills/."""
...
class TestSecurityScanGate:
    ...
    def test_guard_flag_reads_config_default_false(self):
        ...
        with patch("openagents_cli.config.load_config", return_value={"skills": {}}):
...
class TestExternalSkillMutations:
    ...
    caused agents to create duplicate copies in ~/.openagents/skills/ as a
    workaround.
    """
...
def _curator_pass(tmp_path, *, monkeypatch):
    ...
    Points OPENAGENTS_HOME at ``tmp_path/.hermes`` so skill_usage's archive path
    (``get_openagents_home()``) resolves into the same tree the skill manager
