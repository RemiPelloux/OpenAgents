...
def test_openagents_cli_init_does_not_eagerly_resolve_runtime_provider(monkeypatch):
    ...
    monkeypatch.setattr(
        "openagents_cli.runtime_provider.resolve_runtime_provider",
        _unexpected_runtime_resolve,
    )
    monkeypatch.setattr(
        "openagents_cli.runtime_provider.format_runtime_provider_error",
        lambda exc: str(exc),
    )
...
def test_runtime_resolution_failure_is_not_sticky(monkeypatch):
    ...
    monkeypatch.setattr(
        "openagents_cli.runtime_provider.resolve_runtime_provider", _runtime_resolve
    )
    monkeypatch.setattr(
        "openagents_cli.runtime_provider.format_runtime_provider_error",
        lambda exc: str(exc),
...
def test_model_flow_nous_prints_subscription_guidance_without_mutating_explicit_tts(
    ...
    monkeypatch.setattr(
        "openagents_cli.auth._update_config_for_provider", lambda provider, url: None
    )
...
def test_model_flow_nous_does_not_restore_stale_custom_api_key(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "openagents_cli.models.get_pricing_for_provider", lambda provider: {}
    )
    monkeypatch.setattr(
        "openagents_cli.models.check_nous_free_tier", lambda **kwargs: False
    )
    monkeypatch.setattr(
...
def test_model_flow_openrouter_clears_stale_custom_key(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "openagents_cli.models.get_pricing_for_provider", lambda *a, **k: {}
    )
    monkeypatch.setattr(
        "openagents_cli.auth._prompt_model_selection",
...
def test_model_flow_anthropic_clears_stale_custom_key_and_mode(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "openagents_cli.auth.get_anthropic_key", lambda: "sk-ant-api03-test"
    )
    monkeypatch.setattr(
...
def test_model_flow_nous_offers_tool_gateway_prompt_when_unconfigured(
    ...
    monkeypatch.setattr(
        "openagents_cli.setup.prompt_checklist", _fake_checklist, raising=False
    )
...
def test_model_flow_custom_saves_verified_v1_base_url(monkeypatch, capsys):
    ...
    saved_env = {}
    monkeypatch.setattr(
        "openagents_cli.config.save_env_value",
        lambda key, value: saved_env.__setitem__(key, value),
    )
    monkeypatch.setattr(
        "openagents_cli.auth._save_model_choice",
        lambda model: saved_env.__setitem__("MODEL", model),
    )
    monkeypatch.setattr("openagents_cli.auth.deactivate_provider", lambda: None)
    monkeypatch.setattr(
        "openagents_cli.main._save_custom_provider", lambda *args, **kwargs: None
    )
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        "openagents_cli.secret_prompt.masked_secret_prompt",
...
def test_model_flow_custom_persists_selected_api_mode(monkeypatch):
    ...
    monkeypatch.setattr(
        "openagents_cli.config.save_config", lambda cfg: saved_cfg.update(cfg)
    )
    monkeypatch.setattr(
...
def test_cmd_model_forwards_nous_login_tls_options(monkeypatch):
    ...
    monkeypatch.setattr("openagents_cli.config.save_env_value", lambda key, value: None)
    monkeypatch.setattr(
        "openagents_cli.auth.resolve_provider", lambda requested, **kwargs: "nous"
    )
    monkeypatch.setattr(
        "openagents_cli.auth.get_provider_auth_state", lambda provider_id: None
    )
    monkeypatch.setattr(
