...
def hermes_home(tmp_path, monkeypatch):
    """Isolated OPENAGENTS_HOME so SessionDB.state_meta writes stay hermetic."""
    home = tmp_path / ".hermes"
    home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    monkeypatch.setenv("OPENAGENTS_HOME", str(home))

    # Bust the goal module's DB cache so it re-resolves OPENAGENTS_HOME each test.
...
class TestInterruptAutoPause:
    def test_interrupted_turn_pauses_goal_and_skips_continuation(self, hermes_home):
        ...
        with patch("openagents_cli.goals.judge_goal") as judge_mock:
    ...
    def test_interrupted_turn_is_resumable(self, hermes_home):
        ...
        with patch("openagents_cli.goals.judge_goal"):
            cli._maybe_continue_goal_after_turn()
...
class TestHealthyTurnStillRuns:
    def test_clean_response_enqueues_continuation_when_judge_says_continue(
        ...
        with patch(
            "openagents_cli.goals.judge_goal",
