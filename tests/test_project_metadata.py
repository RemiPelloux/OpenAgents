...
def test_matrix_extra_not_in_all():
    ...
    assert "matrix" in optional_dependencies, (
        "[matrix] extra must still exist for explicit `pip install openagents[matrix]`"
    )
...
def test_lazy_installable_extras_excluded_from_all():
    ...
    for extra in lazy_covered_extras:
        offending = [spec for spec in all_extra_specs if f"openagents[{extra}]" in spec]
...
def test_dev_extra_excluded_from_all():
    ...
    assert not any(spec == "openagents[dev]" for spec in optional_dependencies["all"])
...
def test_dingtalk_extra_includes_qrcode_for_qr_auth():
    """DingTalk's QR-code device-flow auth (openagents_cli/dingtalk_auth.py)
...
def test_nemo_relay_extra_uses_official_0_3_distribution():
    ...
    assert not any(
        spec == "openagents[nemo-relay]" for spec in optional_dependencies["all"]
    )
