...
def test_resolve_credentials_quarantines_dead_tokens_on_terminal_refresh_failure():
    ...
    with (
        patch("openagents_cli.auth.get_provider_auth_state", return_value=stale_state),
        patch(
            "openagents_cli.auth._refresh_minimax_oauth_state",
            side_effect=_terminal_refresh,
        ),
        patch(
            "openagents_cli.auth._minimax_save_auth_state", side_effect=_capture_save
        ),
    ):
        with pytest.raises(AuthError) as exc_info:
            resolve_minimax_oauth_runtime_credentials()
...
def test_token_provider_returns_current_access_token_when_fresh():
    ...
    with (
        patch("openagents_cli.auth.get_provider_auth_state", return_value=state),
