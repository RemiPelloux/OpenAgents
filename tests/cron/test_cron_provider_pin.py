...
resolve_runtime_provider against a temp OPENAGENTS_HOME) and the create_job
...
def _run_with_current_provider(job, current_provider, tmp_path):
    ...
    with patch("cron.scheduler._hermes_home", tmp_path), \
         ...
         patch("openagents_state.SessionDB", return_value=fake_db), \
         patch(
...
class TestCreateJobSnapshot:
    ...
    def test_pinned_job_skips_snapshot(self, monkeypatch):
        ...
        with patch("openagents_cli.runtime_provider.resolve_runtime_provider", resolver):
            job = jobs.create_job(
    ...
    def test_unpinned_model_captures_model_snapshot(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(
            "cron.jobs.get_openagents_home", lambda: tmp_path, raising=True
        )
        with patch(
            "openagents_cli.runtime_provider.resolve_runtime_provider",
            return_value={"provider": "openrouter"},
...
def _run_with_current_provider_and_model(job, current_provider, current_model, tmp_path):
    ...
    fake_db = MagicMock()
    with patch("cron.scheduler._hermes_home", tmp_path), \
         patch("cron.scheduler._get_openagents_home", return_value=tmp_path), \
         patch("cron.scheduler._resolve_origin", return_value=None), \
         patch("dotenv.load_dotenv"), \
         patch("openagents_state.SessionDB", return_value=fake_db), \
         patch(
