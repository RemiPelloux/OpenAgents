...
profile's OPENAGENTS_HOME and EXECUTE under it.

- Storage: a job created under profile ``coder`` writes to
  ``~/.openagents/profiles/coder/cron/jobs.json`` — NOT the shared default root.
- Execution: the profile-scoped gateway's in-process ticker resolves the
  active OPENAGENTS_HOME (profile home) at call time, so jobs run with that
...
one store and runs them under whatever OPENAGENTS_HOME the ticker happens to have —
...
def _set_profile_env(monkeypatch, root: Path, profile_home: Path) -> None:
    ...
    OPENAGENTS_HOME is a profile under it (``<root>/profiles/<name>``)."""
    import openagents_constants

    monkeypatch.setattr(
        openagents_constants, "_get_platform_default_openagents_home", lambda: root
    )
    monkeypatch.setenv("OPENAGENTS_HOME", str(profile_home))


def test_cron_storage_anchors_at_profile_home(tmp_path, monkeypatch):
    """Under a profile OPENAGENTS_HOME (<root>/profiles/<name>), the cron store
...
def test_cron_execution_home_follows_active_profile(tmp_path, monkeypatch):
    ...
    assert scheduler._get_openagents_home().resolve() == profile_home.resolve()
    assert scheduler._get_openagents_home().resolve() != root.resolve()


def test_cron_storage_unaffected_when_no_profile(tmp_path, monkeypatch):
    """With no profile (OPENAGENTS_HOME == root), the store is the root's cron dir
