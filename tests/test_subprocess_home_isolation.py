...
OpenAgents state stays profile-scoped through OPENAGENTS_HOME. Host subprocesses should
...
See: https://github.com/NousResearch/openagents/issues/25114
See: https://github.com/NousResearch/openagents/issues/36144
See: https://github.com/NousResearch/openagents/issues/29015
"""
...
class TestGetSubprocessHome:
    """Unit tests for openagents_constants.get_subprocess_home()."""

    def _host_mode(self, monkeypatch):
        monkeypatch.setattr(openagents_constants, "is_container", lambda: False)
    ...
    def test_real_mode_repairs_parent_home_already_pointing_at_profile(self, tmp_path, monkeypatch):
        ...
        from openagents_constants import get_subprocess_home, get_real_home
    ...
    def test_two_profiles_get_different_homes(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("OPENAGENTS_HOME", str(base / "alpha"))
        home_a = get_subprocess_home()

        monkeypatch.setenv("OPENAGENTS_HOME", str(base / "beta"))
    ...
    def test_context_override_is_thread_local(self, tmp_path, monkeypatch):
        ...
        def read_from_other_thread():
            ...
            seen.append(str(get_openagents_home()))

        thread = threading.Thread(target=read_from_other_thread)
        thread.start()
        assert ready.wait(timeout=5)

        token = set_openagents_home_override(profile)
        try:
            assert get_openagents_home() == profile
            release.set()
        ...
        assert get_openagents_home() == root
...
class TestMakeRunEnvHomeInjection:
    ...
    def test_context_override_bridges_to_subprocess_env(self, tmp_path, monkeypatch):
        ...
        assert result["OPENAGENTS_HOME"] == str(profile)
...
class TestProfileBootstrap:
    ...
    def test_profile_dirs_includes_home(self):
        from openagents_cli.profiles import _PROFILE_DIRS
    ...
    def test_create_profile_bootstraps_home_dir(self, tmp_path, monkeypatch):
        ...
        from openagents_cli.profiles import create_profile
