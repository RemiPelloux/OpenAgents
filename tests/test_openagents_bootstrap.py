"""Tests for openagents_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every OpenAgents entry point
(hermes, openagents, openagents-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every OpenAgents entry point imports openagents_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported openagents_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("openagents_bootstrap", None)
    import openagents_bootstrap  # noqa: WPS433

    return openagents_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the OpenAgents CLI
...
class TestEntryPointsImportBootstrap:
    """Every OpenAgents entry point must import openagents_bootstrap as its
    ...
    # Entry points that invoke OpenAgents as a process.  Each one must
    # import openagents_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "openagents_cli/main.py",  # hermes CLI (console_script)
        "run_agent.py",  # openagents (console_script)
        "acp_adapter/entry.py",  # openagents-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import openagents_bootstrap' and that
        ...
        ``openagents_bootstrap`` as a top-level module) leaves hermes
        ...
        """
        # Resolve relative to the openagents repo root.  Tests live
        # at tests/test_openagents_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # hermes start even when openagents_bootstrap hasn't been
        ...
        assert first_import_name == "openagents_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'openagents_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import openagents_bootstrap' line to be the first import."
        )
