...
def test_normalize_retain_tags_accepts_csv_and_dedupes():
    assert _normalize_retain_tags(
        "agent:fakeassistantname, source_system:openagents, agent:fakeassistantname"
    ) == [
...
class TestConfig:
    ...
    def test_default_values(self, provider):
        ...
        assert (
            provider._retain_context == "conversation between OpenAgents and the User"
        )
...
class TestPostSetup:
    def test_setup_cancel_at_mode_picker_writes_nothing(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "plugins.memory.hindsight.get_openagents_home", lambda: hermes_home
        )
        ...
        run = MagicMock()
        monkeypatch.setattr(
            "openagents_cli.memory_setup._curses_select",
    ...
    def test_local_embedded_setup_materializes_profile_env(self, tmp_path, monkeypatch):
        ...
        saved_configs = []
        monkeypatch.setattr(
            "openagents_cli.config.save_config",
...
class TestSyncTurn:
    ...
    def test_sync_turn_uses_aretain_batch(self, provider):
        ...
        assert (
            ...
            == "conversation between OpenAgents and the User"
        )
...
class TestBankIdTemplate:
    ...
    def test_resolve_empty_template_uses_fallback(self):
        result = _resolve_bank_id_template("", fallback="openagents", profile="coder")
