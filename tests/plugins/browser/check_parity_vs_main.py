...
Run from the PR worktree:
    ...
    cd ~/.openagents/openagents/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.openagents/openagents``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.openagents/openagents
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a openagents checkout"
)
assert (PR_DIR / "tools" / "browser_tool.py").exists(), (
    f"PR_DIR={PR_DIR} doesn't look like a openagents checkout"
)
...
# Isolated OPENAGENTS_HOME for the config write.
