"""Tests for the secret-source tracking in ``openagents_cli.env_loader``.
...
from openagents_cli import env_loader  # noqa: E402
...
def test_apply_external_secret_sources_dedupes_within_process(tmp_path, monkeypatch):
    """``load_openagents_dotenv()`` is called at module-import time from several
    hot modules (cli.py, openagents_cli/main.py, run_agent.py, ...).  The
    ...
    runs exactly once per OPENAGENTS_HOME per process.
    """
    ...
    # cli.py, openagents_cli/main.py, run_agent.py, trajectory_compressor.py,
