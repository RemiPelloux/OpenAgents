"""Shared fixtures for the openagents test suite.
...
2. **Isolated OPENAGENTS_HOME.** OPENAGENTS_HOME points to a per-test tempdir so
   code reading ``~/.openagents/*`` via ``get_openagents_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".hermes"`` instead of the canonical
   ``get_openagents_home()`` is a bug to fix at the callsite.)
...
_HERMES_BEHAVIORAL_VARS = frozenset({
    ...
    "HERMES_EXEC_ASK",
    "OPENAGENTS_HOME_MODE",
    "HERMES_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.openagents/kanban.db instead of the per-test OPENAGENTS_HOME.
    "HERMES_KANBAN_DB",
    "HERMES_KANBAN_BOARD",
    "HERMES_KANBAN_HOME",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and OPENAGENTS_HOME to per-test tempdirs so code that
    reads ``~/.openagents/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 3. Redirect OPENAGENTS_HOME to a per-test tempdir. Code that reads
    #    ``~/.openagents/*`` via ``get_openagents_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.openagents/*`` via
    #    ``Path.home() / ".hermes"`` instead of ``get_openagents_home()``
    ...
    monkeypatch.setenv("OPENAGENTS_HOME", str(fake_hermes_home))
    ...
    #    ~/.openagents/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import openagents_cli.plugins as _plugins_mod
...
# environment and finds the developer's live ``openagents-gateway`` process
...
# correlated with a ``tests/openagents_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> openagents-gateway``
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart openagents-gateway"``,
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in tokens:
            ...
            if head in _PROCESS_KILLERS:
                ...
                # whose cmdline contains "python -m openagents_cli.main".
                if (
                    "openagents" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live openagents-gateway systemd unit. Mock "
        ...
        # equivalent `python -m openagents_cli.main update`).  These commands
        ...
        if "update" in low and (
            ...
            ("openagents" in low and "update" in low.split())
            or
            # python -m openagents_cli.main update --gateway
            ("openagents_cli" in low and "update" in low.split())
            or
