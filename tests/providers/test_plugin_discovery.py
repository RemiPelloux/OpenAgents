...
Verifies that:
 ...
 2. User plugins at $OPENAGENTS_HOME/plugins/model-providers/<name>/ override bundled
...
def test_user_plugin_overrides_bundled(tmp_path, monkeypatch):
    ...
    # Point OPENAGENTS_HOME at a fresh temp dir
    hermes_home = tmp_path / ".hermes"
    hermes_home.mkdir()
    monkeypatch.setenv("OPENAGENTS_HOME", str(hermes_home))
    # get_openagents_home() may be module-cached depending on codebase; ensure the
...
def test_general_plugin_manager_skips_model_provider_kind(tmp_path, monkeypatch):
    ...
    from openagents_cli import plugins as plugin_mod
