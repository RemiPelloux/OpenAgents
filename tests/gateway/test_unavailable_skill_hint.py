...
* ``disabled`` membership is checked by the declared name, because that
  is what :func:`openagents_cli.skills_config.save_disabled_skills` stores.
"""
...
def tmp_skills(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Isolated skills dir + OPENAGENTS_HOME so the real user config is untouched."""
...
def test_optional_skill_uses_frontmatter_slug(
    ...
    monkeypatch.setattr(
        "openagents_constants.get_optional_skills_dir",
