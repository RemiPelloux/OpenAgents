...
def adapter(tmp_path):
    ...
    ~/.openagents/google_chat_thread_counts.json.
    """
    ...
    # Replace the production store (which would write to ~/.openagents/...)
...
class TestExtractMessagePayload:
    ...
    def test_relay_flat_format_synthesizes_chat_api_shape(self):
        ...
        GCP credentials off the OpenAgents host. The relay flattens Chat
...
class TestGoogleChatInteractiveSetup:
    def test_interactive_setup_uses_shared_cli_prompt_helpers(self, monkeypatch):
        ...
        monkeypatch.setattr("openagents_cli.config.get_env_value", fake_get_env_value)
        monkeypatch.setattr("openagents_cli.config.save_env_value", fake_save_env_value)
        monkeypatch.setattr("openagents_cli.cli_output.prompt", fake_prompt)
        monkeypatch.setattr(
            "openagents_cli.cli_output.prompt_yes_no", lambda *_a, **_kw: True
        )
        monkeypatch.setattr(
            "openagents_cli.cli_output.print_info", lambda *_a, **_kw: None
        )
        monkeypatch.setattr(
            "openagents_cli.cli_output.print_success", lambda *_a, **_kw: None
        )
        monkeypatch.setattr(
            "openagents_cli.cli_output.print_warning", lambda *_a, **_kw: None
        )
