...
class TestHandleUpdateCommand:
    ...
    async def test_managed_install_returns_package_manager_guidance(self, monkeypatch):
        ...
        assert "brew upgrade openagents" in result
    ...
    async def test_no_git_directory(self, tmp_path):
        ...
        with patch("gateway.run._hermes_home", tmp_path):
            ...
            # project_root will be the real openagents dir (which HAS .git).
    ...
    async def test_no_hermes_binary(self, tmp_path):
        """Returns error when hermes is not on PATH and openagents_cli is not importable."""
        runner = _make_runner()
        event = _make_event()
    ...
    async def test_fallback_to_sys_executable(self, tmp_path):
        """Falls back to sys.executable -m openagents_cli.main when hermes not on PATH."""
        runner = _make_runner()
        event = _make_event()
        ...
        assert "Starting OpenAgents update" in result
        call_args = mock_popen.call_args[0][0]
        # The update_cmd uses sys.executable -m openagents_cli.main
        joined = " ".join(call_args) if isinstance(call_args, list) else call_args
        assert "openagents_cli.main" in joined or "bash" in call_args[0]
    ...
    async def test_resolve_hermes_bin_fallback(self):
        ...
        assert result == [sys.executable, "-m", "openagents_cli.main"]
    ...
    async def test_writes_pending_marker(self, tmp_path):
        ...
        with patch("gateway.run._hermes_home", hermes_home), \
             patch("gateway.run.__file__", fake_file), \
             patch("shutil.which", side_effect=lambda x: "/usr/bin/hermes" if x == "openagents" else "/usr/bin/setsid"), \
