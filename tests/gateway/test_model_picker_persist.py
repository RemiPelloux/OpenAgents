...
closure the PR changed, against a real temp ``OPENAGENTS_HOME``.
"""
...
def _fake_switch_result():
    ...
    from openagents_cli.model_switch import ModelSwitchResult
...
def _setup_isolated_home(tmp_path, monkeypatch, model_yaml_value):
    ...
    monkeypatch.setattr(
        "openagents_cli.model_switch.resolve_display_context_length",
        lambda *a, **k: 272000,
    )
    # save_config writes to ``get_openagents_home() / config.yaml`` — point it here.
    monkeypatch.setattr("openagents_constants.get_openagents_home", lambda: hermes_home)
    monkeypatch.setattr("openagents_cli.config.get_openagents_home", lambda: hermes_home)
    return cfg_path
