"""Tests for openagents_constants module."""
...
class TestGetDefaultHermesRoot:
    """Tests for get_default_openagents_root() — Docker/custom deployment awareness."""

    def test_no_hermes_home_returns_native(self, tmp_path, monkeypatch):
        """When OPENAGENTS_HOME is not set, returns ~/.openagents."""
    ...
    def test_hermes_home_is_docker(self, tmp_path, monkeypatch):
        """When OPENAGENTS_HOME points outside ~/.openagents (Docker), returns OPENAGENTS_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("OPENAGENTS_HOME", str(docker_home))
        assert get_default_openagents_root() == docker_home

    def test_hermes_home_is_custom_path(self, tmp_path, monkeypatch):
        """Any OPENAGENTS_HOME outside ~/.openagents is treated as the root."""
        custom = tmp_path / "my-hermes-data"
        custom.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("OPENAGENTS_HOME", str(custom))
        assert get_default_openagents_root() == custom

    def test_docker_profile_active(self, tmp_path, monkeypatch):
        """When a Docker profile is active (OPENAGENTS_HOME=<root>/profiles/<name>),
...
class TestGetHermesHome:
    """Tests for get_openagents_home() platform-aware fallback."""

    def test_windows_fallback_uses_localappdata(self, tmp_path, monkeypatch):
        """When OPENAGENTS_HOME is unset on Windows, use %LOCALAPPDATA%\\hermes."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("OPENAGENTS_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")
        monkeypatch.setattr(openagents_constants.sys, "platform", "win32")
        monkeypatch.setattr(openagents_constants, "_profile_fallback_warned", False)

        assert get_openagents_home() == local_appdata / "openagents"
...
class TestNodeToolRunnable:
    ...
    def test_broken_managed_npm_heals_when_node_still_runs(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(openagents_constants, "heal_hermes_managed_node", _heal)
...
class TestIsContainer:
    ...
    def _reset_cache(self, monkeypatch):
        ...
        monkeypatch.setattr(openagents_constants, "_container_detected", None)
    ...
    def test_caches_result(self, monkeypatch):
        ...
        monkeypatch.setattr(openagents_constants, "_container_detected", True)
        assert is_container() is True
...
class TestSecureParentDir:
    ...
    def test_safe_path_calls_chmod(self, tmp_path, monkeypatch):
        ...
        safe_dir = tmp_path / "home" / "user" / ".openagents"
...
class TestAgentBrowserRunnable:
    ...
    def test_version_probe_uses_windows_hide_flags(self, tmp_path, monkeypatch):
        ...
        import openagents_cli._subprocess_compat as subprocess_compat
...
class TestGetHermesDir:
    """Tests for ``get_openagents_dir(new_subpath, old_name)``.
    ...
    def test_neither_exists_returns_new(self, tmp_path, monkeypatch):
        ...
        result = get_openagents_dir("platforms/pairing", "pairing")
    ...
    def test_legacy_populated_returns_legacy(self, tmp_path, monkeypatch):
        ...
        result = get_openagents_dir("cache/images", "image_cache")
    ...
    def test_legacy_populated_with_subdir_returns_legacy(self, tmp_path, monkeypatch):
        ...
        result = get_openagents_dir("platforms/matrix/store", "matrix/store")
        assert result == legacy
    ...
    def test_legacy_empty_and_new_missing_returns_new(self, tmp_path, monkeypatch):
        ...
        legacy.mkdir()
        result = get_openagents_dir("cache/audio", "audio_cache")
