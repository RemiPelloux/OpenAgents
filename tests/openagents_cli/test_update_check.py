...
def test_check_for_updates_invalidates_on_version_change(tmp_path, monkeypatch):
    ...
    with (
        ...
        patch("openagents_cli.banner.check_via_pypi", return_value=0) as mock_pypi,
    ):
...
def test_check_for_updates_expired_cache(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("OPENAGENTS_HOME", str(tmp_path))
    with patch(
        "openagents_cli.banner.subprocess.run", return_value=mock_result
    ) as mock_run:
        result = check_for_updates()
...
def test_check_for_updates_official_ssh_origin_uses_https_probe(tmp_path):
    ...
    def fake_run(cmd, **kwargs):
        ...
        if cmd == ["git", "remote", "get-url", "origin"]:
            return MagicMock(
                returncode=0, stdout="git@github.com:RemiPelloux/OpenAgents.git\n"
            )
...
def test_check_via_local_git_shallow_clone_behind_reports_no_count(tmp_path):
    ...
    def fake_run(cmd, **kwargs):
        ...
        if cmd == ["git", "remote", "get-url", "origin"]:
            return MagicMock(
                returncode=0, stdout="https://github.com/RemiPelloux/OpenAgents.git\n"
            )
...
def test_check_for_updates_docker_returns_none(tmp_path, monkeypatch):
    ...
    with (
        patch("openagents_cli.config.detect_install_method", return_value="docker"),
        patch("openagents_cli.banner.subprocess.run") as mock_run,
...
def test_invalidate_update_cache_clears_all_profiles(tmp_path):
    ...
    from openagents_cli.main import _invalidate_update_cache

    # Build a fake ~/.openagents with default + two named profiles
    ...
    with (
        ...
        patch.dict(os.environ, {"OPENAGENTS_HOME": str(default_home)}),
    ):
