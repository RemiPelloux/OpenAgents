...
def _login(monkeypatch, provider_id):
    ...
    monkeypatch.setattr("openagents_cli.auth._load_auth_store",
                        lambda: {"active_provider": provider_id})
    monkeypatch.setattr("openagents_cli.auth.get_auth_status",
                        lambda p: {"logged_in": p == provider_id})


def _config(monkeypatch, model_cfg):
    monkeypatch.setattr("openagents_cli.config.load_config", lambda: {"model": model_cfg})
