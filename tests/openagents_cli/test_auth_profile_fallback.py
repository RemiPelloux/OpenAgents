...
When ``OPENAGENTS_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    ...
    * Profile     -> tmp_path/.hermes/profiles/coder   (active, OPENAGENTS_HOME points here)
...
def test_load_provider_state_falls_back_to_global(profile_env):
    ...
    from openagents_cli.auth import _load_auth_store, _load_provider_state
...
def test_classic_mode_does_not_double_read_same_file(tmp_path, monkeypatch):
    """In classic mode (OPENAGENTS_HOME == global root), no fallback path runs.
    ...
    # to the OPENAGENTS_HOME we set (tmp_path/classic), so the guard passes.
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    hermes_home = tmp_path / "classic"
    ...
    from openagents_cli.auth import read_credential_pool, _global_auth_file_path

    # Classic mode: OPENAGENTS_HOME is set to a custom path that is NOT under
    # ~/.openagents/profiles/ — get_default_openagents_root() returns OPENAGENTS_HOME
