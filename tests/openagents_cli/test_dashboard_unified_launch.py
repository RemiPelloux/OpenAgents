...
class TestUnifiedDashboardRouting:
    def test_profile_launch_attaches_to_running_dashboard(self, main_mod, monkeypatch):
        monkeypatch.setattr(
            "openagents_cli.profiles.get_active_profile_name", lambda: "worker_x"
        )
    ...
    def test_profile_launch_reexecs_machine_dashboard(self, main_mod, monkeypatch):
        ...
        # OPENAGENTS_HOME.  For a standard install (OPENAGENTS_HOME unset) that root is
        # the platform-native default (~/.openagents), NOT dropped — see the Docker
        # test below for why we resolve explicitly instead of popping.
        from openagents_constants import get_default_openagents_root

        assert env.get("OPENAGENTS_HOME") == str(get_default_openagents_root())

    def test_reexec_pins_docker_machine_root(self, main_mod, monkeypatch):
        """In the Docker layout (OPENAGENTS_HOME=/opt/data, profiles under
        /opt/data/profiles/<name>) the reroute must pin the child to the
        machine root /opt/data — NOT drop OPENAGENTS_HOME.
        ...
        """
        monkeypatch.setenv("OPENAGENTS_HOME", "/opt/data/profiles/oracle")
        monkeypatch.setattr(
            "openagents_cli.profiles.get_active_profile_name", lambda: "oracle"
        )
        monkeypatch.setattr(main_mod, "_dashboard_listening", lambda host, port: False)
        execs = []
        ...
        # get_default_openagents_root() strips the trailing profiles/<name>, so the
        # child binds /opt/data — where the real default/oracle/saga profiles
        # and the .install_method stamp actually live.
        assert env.get("OPENAGENTS_HOME") == "/opt/data"
    ...
    def test_default_profile_launch_skips_routing(self, main_mod, monkeypatch):
        monkeypatch.setattr(
            "openagents_cli.profiles.get_active_profile_name", lambda: "default"
        )
        listening_calls = []
        monkeypatch.setattr(
            main_mod,
    ...
    def test_dashboard_starts_mcp_discovery_for_ws_backend(self, main_mod, monkeypatch):
        ...
        calls = []
        monkeypatch.setattr(
            "openagents_cli.mcp_startup.start_background_mcp_discovery",
            lambda **kwargs: calls.append(kwargs),
        )
        monkeypatch.setitem(
            sys.modules,
            "openagents_cli.web_server",
