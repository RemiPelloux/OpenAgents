...
def _client():
    ...
    # Keep the state DB under the isolated OPENAGENTS_HOME for any handler that
    # touches it.
...
class TestMemoryEndpoints:
    ...
    def _setup(self, _isolate_hermes_home):
        ...
        (get_openagents_home() / "memories").mkdir(parents=True, exist_ok=True)
    ...
    def test_reset_targets(self):
        ...
        mem = get_openagents_home() / "memories"
...
class TestDebugShareEndpoint:
    ...
    def test_returns_structured_urls(self, monkeypatch):
        import openagents_cli.debug as dbg
        ...
        monkeypatch.setattr("openagents_cli.dump.run_dump", lambda a: None)
