"""Tests for openagents_cli.gateway."""
...
import openagents_cli.gateway as gateway
...
def _install_fake_gateway_run(monkeypatch, start_gateway):
    ...
    # /openagents-gateway.service"`` and runs ``systemctl --user daemon-reload``
    # — both target the *real* user environment because the conftest only
    # sandboxes ``OPENAGENTS_HOME``, not ``HOME``. Tests that drive
    ...
    # unit (baking in the test's pytest-tmp ``OPENAGENTS_HOME`` value, which
...
def test_run_gateway_refuses_root_in_official_docker(monkeypatch, tmp_path, capsys):
    project_root = tmp_path / "opt" / "openagents"
    ...
    assert exc_info.value.code == 1
    out = capsys.readouterr().out
    assert "Refusing to run the OpenAgents gateway as root" in out
...
def test_s6_runtime_snapshot_reports_supervised_service(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "openagents_cli.service_manager.get_service_manager", lambda: FakeS6Manager()
    )
...
def test_gateway_install_in_container_with_operational_systemd_uses_systemd(
    ...
    monkeypatch.setattr(
        ...
        **kw: calls.append(("install", force, system, run_as_user, enable_on_startup)),
    )
    monkeypatch.setattr(
...
def test_gateway_restart_on_windows_without_service_uses_detached_backend(monkeypatch):
    ...
    """
    import openagents_cli.gateway_windows as gateway_windows
...
def test_systemd_install_checks_linger_status(monkeypatch, tmp_path, capsys):
    unit_path = tmp_path / "systemd" / "user" / "openagents-gateway.service"

    monkeypatch.setattr(
        gateway, "get_systemd_unit_path", lambda system=False: unit_path
    )
    # Synthetic unit with a non-temp home: the real generator bakes the
    # hermetic test OPENAGENTS_HOME (a tmp dir), which the temp-home write
...
def test_systemd_install_can_skip_enable_on_startup(monkeypatch, tmp_path, capsys):
    ...
    # hermetic test OPENAGENTS_HOME) stays out of the way.
    monkeypatch.setattr(
        gateway,
        "generate_systemd_unit",
        lambda system=False, run_as_user=None: (
...
def test_systemd_install_system_scope_skips_linger_and_uses_systemctl(
    ...
    unit_path = tmp_path / "etc" / "systemd" / "system" / "openagents-gateway.service"
...
def test_conflicting_systemd_units_warning(monkeypatch, tmp_path, capsys):
    user_unit = tmp_path / "user" / "openagents-gateway.service"
    system_unit = tmp_path / "system" / "openagents-gateway.service"
...
def test_install_linux_gateway_from_setup_system_choice_as_root_installs(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        run_as_user=None,
        enable_on_startup=True,
        **kw: calls.append((force, system, run_as_user, enable_on_startup)),
    )
...
def test_gateway_install_systemd_honors_start_now_flag(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        run_as_user=None,
        enable_on_startup=True,
        **kw: calls.append(("install", enable_on_startup)),
    )
    monkeypatch.setattr(
...
def test_gateway_install_noninteractive_skips_legacy_unit_prompt(monkeypatch, tmp_path):
    ...
    fake_path = tmp_path / "openagents-gateway.service"
    monkeypatch.setattr(
...
def test_module_has_logger():
    ...
    assert gateway.logger.name == "openagents_cli.gateway"
