...
def _refresh_bindings_against_live_module():
    """Rebind module-level names to the *current* ``openagents_cli.main``.

    Other tests in the suite (notably ``test_env_loader.py`` and
    ``test_skills_subparser.py``) reload or delete ``openagents_cli.main`` from
    ``sys.modules``.  When that happens on the same xdist worker before we
    run, our top-of-file ``from openagents_cli.main import ...`` bindings end
    up pointing at the *old* module object.  ``patch(\"openagents_cli.main.X\")``
    ...
    live = sys.modules.get("openagents_cli.main")
    if live is None:
        live = importlib.import_module("openagents_cli.main")
    ...
    def test_matches_running_dashboard(self):
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                returncode=0,
                stdout=_ps_line(
                    12345, "python3 -m openagents_cli.main dashboard --port 9119"
                )
                + "\n",
                stderr="",
            )
        ...
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                ...
                stdout="\n".join([
                    ...
                    _ps_line(12347, "python /home/x/openagents_cli/main.py dashboard"),
                ])
                + "\n",
                stderr="",
            )
    ...
    def test_self_pid_excluded(self):
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                returncode=0,
                stdout="\n".join([
                    _ps_line(os.getpid(), "python3 -m openagents_cli.main dashboard"),
        ...
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                ...
                stdout="\n".join([
                    ...
                    _ps_line(
                        22222,
                        "python3 -m openagents_cli.main chat -q 'rewrite my dashboard'",
                    ),
...
class TestKillStaleDashboardPosix:
    ...
    def test_no_stale_processes_is_a_noop(self, capsys):
        with patch("openagents_cli.main._find_stale_dashboard_pids", return_value=[]):
            _kill_stale_dashboard_processes()
    ...
    def test_sigterm_graceful_exit(self, capsys):
        ...
        with (
            patch(
                "openagents_cli.main._find_stale_dashboard_pids",
    ...
    def test_sigkill_fallback_for_survivors(self, capsys):
        ...
        with (
            patch(
                "openagents_cli.main._find_stale_dashboard_pids", return_value=[99999]
            ),
            patch("os.kill", side_effect=fake_kill),
            patch("time.sleep"),
    ...
    def test_permission_error_is_reported_not_raised(self, capsys):
        ...
        with (
            patch(
                "openagents_cli.main._find_stale_dashboard_pids", return_value=[12345]
            ),
            patch("os.kill", side_effect=fake_kill),
            patch("time.sleep"),
        ):
    ...
    def test_wmic_invoked_with_utf8_ignore_errors(self, monkeypatch):
        ...
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                returncode=0,
                stdout=(
                    "CommandLine=python -m openagents_cli.main dashboard\n"
                    "ProcessId=12345\n"
                ),
                stderr="",
            )
