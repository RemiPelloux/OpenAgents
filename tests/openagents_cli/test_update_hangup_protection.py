...
``_finalize_update_output`` in ``openagents_cli/main.py``.  These exist so
...
class TestInstallHangupProtection:
    ...
    def test_installs_sighup_ignore(self, tmp_path, monkeypatch):
        ...
        # Clear cached get_openagents_home if present
        import openagents_cli.config as _cfg

        if hasattr(_cfg, "_OPENAGENTS_HOME_CACHE"):
            _cfg._OPENAGENTS_HOME_CACHE = None  # type: ignore[attr-defined]
    ...
    def test_non_fatal_if_log_setup_fails(self, monkeypatch):
        """If get_openagents_home() raises, stdio must be left untouched but SIGHUP still handled."""
        prev_out, prev_err = sys.stdout, sys.stderr
        ...
        monkeypatch.setattr(
            "openagents_cli.config.get_openagents_home", _boom, raising=True
        )
