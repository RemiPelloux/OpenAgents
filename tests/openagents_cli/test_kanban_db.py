"""Tests for the Kanban DB layer (openagents_cli.kanban_db)."""
...
from openagents_cli import kanban_db as kb


@pytest.fixture
def kanban_home(tmp_path, monkeypatch):
    """Isolated OPENAGENTS_HOME with an empty kanban DB."""
...
def test_stale_claim_reclaimed(kanban_home, monkeypatch):
    import signal
    import openagents_cli.kanban_db as _kb
...
def test_dispatch_skips_nonspawnable_into_separate_bucket(kanban_home, monkeypatch):
    ...
    from openagents_cli import profiles
...
def test_has_spawnable_ready_true_when_real_profile_present(kanban_home, monkeypatch):
    ...
    has an assignee that maps to a real OpenAgents profile — preserves the
...
def test_dispatch_worktree_task_persists_materialized_workspace_and_branch(kanban_home, tmp_path, monkeypatch):
    ...
    import openagents_cli.profiles as profiles
...
# where `kanban_db_path()` resolved to the active profile's OPENAGENTS_HOME.
# ---------------------------------------------------------------------------

class TestSharedBoardPaths:
    """`kanban_home`/`kanban_db_path`/`workspaces_root`/`worker_log_path`
    must anchor at the **shared root**, not the active profile's OPENAGENTS_HOME."""
    ...
    def test_default_install_anchors_at_home_dot_hermes(
        ...
        # Standard install: OPENAGENTS_HOME == ~/.openagents, no profile active.
    ...
    def test_profile_worker_resolves_to_shared_root(
        ...
        # Reproduces the bug: dispatcher uses ~/.openagents/kanban.db,
        # worker spawned with -p <profile> previously resolved to
        # ~/.openagents/profiles/<profile>/kanban.db. After the fix both
        # converge on ~/.openagents/kanban.db.
        default_home = tmp_path / ".hermes"
        default_home.mkdir()
    ...
    def test_dispatcher_and_profile_worker_converge(
        ...
        # OPENAGENTS_HOME and confirm equality. This is the property the
    ...
    def test_docker_custom_hermes_home_uses_env_path_directly(
        ...
        # Docker / custom deployment: OPENAGENTS_HOME points outside ~/.openagents.
        # `get_default_openagents_root()` returns env_home directly when it
        # is not a `<root>/profiles/<name>` shape and not under
        # `Path.home() / ".hermes"`.
        custom_root = tmp_path / "opt" / "openagents"
    ...
    def test_docker_profile_layout_uses_grandparent(
        ...
        # Docker profile shape: OPENAGENTS_HOME=/opt/hermes/profiles/coder;
        # `get_default_openagents_root()` walks up to /opt/hermes because
    ...
    def test_empty_override_falls_through(self, tmp_path, monkeypatch):
        ...
        default_home.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("OPENAGENTS_HOME", str(default_home))
    ...
    def test_dispatcher_and_worker_share_a_real_database(
        ...
        # OPENAGENTS_HOME perspectives via a real SQLite file. Without the
        ...
        # Worker switches to the profile OPENAGENTS_HOME and reads.
        monkeypatch.setenv("OPENAGENTS_HOME", str(profile_home))
        with kb.connect() as conn:
    ...
    def test_hermes_kanban_db_pin_beats_kanban_home(
        ...
        # HERMES_KANBAN_HOME and the `get_default_openagents_root()` path.
    ...
    def test_dispatcher_spawn_injects_kanban_db_and_workspaces_root(
        ...
        # `-p <profile>` flag rewrites OPENAGENTS_HOME.
        default_home = tmp_path / ".hermes"
        default_home.mkdir()
        self._set_home(monkeypatch, tmp_path, default_home)
...
# ---------------------------------------------------------------------------
# NFS / network-filesystem fallback (see openagents_state.apply_wal_with_fallback)
# ---------------------------------------------------------------------------
...
def test_connect_falls_back_to_delete_on_locking_protocol(tmp_path, monkeypatch, caplog):
    ...
    with _patch("openagents_cli.kanban_db.sqlite3.connect", side_effect=wal_blocking_connect):
        with caplog.at_level("WARNING", logger="openagents_state"):
            conn = kb.connect()
...
# $PATH that doesn't include the venv's bin/, so a bare `["openagents", ...]`
...
def test_resolve_hermes_argv_prefers_path_shim(monkeypatch):
    ...
    import shutil
    import openagents_cli.kanban_db as kb
...
def test_resolve_hermes_argv_avoids_implicit_windows_batch_shim(monkeypatch, tmp_path):
    ...
    assert kb._resolve_hermes_argv() == [sys.executable, "-m", "openagents_cli.main"]
...
def test_resolve_hermes_argv_hermes_bin_bare_name_uses_path(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("HERMES_BIN", "openagents")
...
def test_resolve_hermes_argv_falls_back_to_module_form_when_no_path_shim(monkeypatch):
    """When the shim is not on PATH, fall back to `python -m openagents_cli.main`.
    ...
    monkeypatch.setattr(shutil, "which", lambda name: None)
    argv = kb._resolve_hermes_argv()
    assert argv == [sys.executable, "-m", "openagents_cli.main"]
...
def test_resolve_hermes_argv_module_actually_runs():
    ...
    sufficient — if `openagents_cli.main` ever loses `if __name__ == "__main__"`
    handling or its argparse setup, `python -m openagents_cli.main --version`
    ...
    assert "OpenAgents" in r.stdout, f"unexpected output: {r.stdout[:200]!r}"
...
def test_maybe_emit_scratch_tip_fires_once_per_install(kanban_home, caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="openagents_cli.kanban_db"):
        with kb.connect() as conn:
...
def test_write_txn_healthy_commit_no_exception(tmp_path):
    ...
    from openagents_cli.kanban_db import connect, write_txn
    db = tmp_path / "test.db"
...
def test_write_txn_raises_on_truncated_file(tmp_path):
    ...
    with pytest.raises(sqlite3.DatabaseError, match="torn-extend|page count mismatch"):
        with unittest.mock.patch("openagents_cli.kanban_db.os.path.getsize", side_effect=fake_getsize):
            with write_txn(conn) as c:
                c.execute(
                    "INSERT INTO tasks (id, title, assignee, status, priority, created_at) "
...
def test_write_txn_post_commit_check_fires_every_call(tmp_path):
    ...
    from openagents_cli.kanban_db import connect, write_txn
    import openagents_cli.kanban_db as kanban_db_module
    db = tmp_path / "test.db"
    conn = connect(db_path=db)
    call_count = 0
...
def test_connect_sets_wal_autocheckpoint_100(tmp_path):
    ...
    from openagents_cli.kanban_db import connect
    db = tmp_path / "test.db"
    conn = connect(db_path=db)
...
def test_write_txn_check_reads_correct_header_fields(tmp_path):
    ...
    import struct
    from openagents_cli.kanban_db import connect, _check_file_length_invariant
...
def test_reap_worker_zombies_returns_count():
    ...
    with patch("openagents_cli.kanban_db.os.waitpid", side_effect=fake_waitpid):
        with patch("openagents_cli.kanban_db._record_worker_exit"):
...
def test_reap_worker_zombies_noop_on_windows(monkeypatch):
    ...
    monkeypatch.setattr("openagents_cli.kanban_db.os.name", "nt")
    with patch("openagents_cli.kanban_db.os.waitpid") as mock_waitpid:
...
def test_reap_worker_zombies_noop_no_children():
    ...
    with patch("openagents_cli.kanban_db.os.waitpid", side_effect=ChildProcessError):
        result = kb.reap_worker_zombies()
    assert result == []
...
def test_reap_worker_zombies_records_exit_status():
    ...
    with patch("openagents_cli.kanban_db.os.waitpid", side_effect=fake_waitpid):
        with patch(
            "openagents_cli.kanban_db._record_worker_exit",
...
def test_reap_worker_zombies_handles_waitpid_os_error():
    ...
    with patch("openagents_cli.kanban_db.os.waitpid", side_effect=OSError("test error")):
        result = kb.reap_worker_zombies()
    assert result == []
...
def test_zombie_reaper_survives_all_boards_failing():
    ...
    for tick in range(5):
        ...
        with patch(
            "openagents_cli.kanban_db.os.waitpid", side_effect=make_fake_waitpid(pids)
        ):
            with patch("openagents_cli.kanban_db._record_worker_exit"):
                pids = kb.reap_worker_zombies()
...
def test_dispatch_once_still_reaps_via_extracted_fn(kanban_home):
    ...
    with patch("openagents_cli.kanban_db.os.waitpid", side_effect=fake_waitpid):
        with patch("openagents_cli.kanban_db._record_worker_exit"):
            with patch("openagents_cli.kanban_db.os.name", "posix"):
                pids = kb.reap_worker_zombies()
