...
def test_pip_install_detected_when_no_git_dir(tmp_path):
    ...
    with (
        patch("openagents_cli.config.get_managed_system", return_value=None),
...
def test_managed_install_takes_precedence(tmp_path):
    ...
    (tmp_path / ".git").mkdir()
    with (
        patch("openagents_cli.config.get_managed_system", return_value="NixOS"),
        patch("openagents_cli.config.get_openagents_home", return_value=tmp_path),
    ):
        from openagents_cli.config import detect_install_method
...
def test_banner_warns_on_pip_install(tmp_path):
    ...
    with (
        patch("openagents_cli.config.get_openagents_home", return_value=hh),
