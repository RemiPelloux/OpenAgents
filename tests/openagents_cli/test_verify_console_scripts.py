...
class TestVerifyConsoleScriptsInstalled:
    def test_no_action_when_all_shims_present(self, temp_pyproject, fake_scripts_dir):
        for name in ("openagents", "openagents", "openagents-acp"):
            (fake_scripts_dir / f"{name}.exe").write_bytes(b"fake")

        with (
            patch("openagents_cli.main._is_windows", return_value=True),
            patch(
                "openagents_cli.main._venv_scripts_dir", return_value=fake_scripts_dir
            ),
            patch("openagents_cli.main._run_quarantined_install") as mock_install,
        ):
            from openagents_cli.main import _verify_console_scripts_installed
    ...
    def test_triggers_reinstall_when_hermes_exe_missing(
        ...
        (fake_scripts_dir / "openagents.exe").write_bytes(b"fake")
        (fake_scripts_dir / "openagents-acp.exe").write_bytes(b"fake")
    ...
    def test_skips_off_windows(self, temp_pyproject, fake_scripts_dir):
        with (
            patch("openagents_cli.main._is_windows", return_value=False),
            patch("openagents_cli.main._run_quarantined_install") as mock_install,
        ):
            from openagents_cli.main import _verify_console_scripts_installed
    ...
    def test_load_console_script_names_reads_pyproject(self, temp_pyproject):
        from openagents_cli.main import _load_console_script_names

        names = _load_console_script_names()
        assert names == ["openagents", "openagents", "openagents-acp"]
    ...
    def test_primary_install_success_still_verifies_scripts(self):
        ...
        with (
            ...
            patch("openagents_cli.main._run_quarantined_install") as mock_install,
            patch(
                "openagents_cli.main._verify_console_scripts_installed"
            ) as mock_verify,
        ):
    ...
    def test_quarantine_shims_include_declared_console_scripts(
        ...
        assert {"hermes.exe", "openagents.exe", "openagents-acp.exe"} <= names
        assert "openagents-gateway.exe" in names
