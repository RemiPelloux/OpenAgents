...
1. ``openagents_cli.web_server._discover_dashboard_plugins`` opted into
...
2. ``openagents_cli.web_server._mount_plugin_api_routes`` then imported
...
class TestProjectPluginsEnvGate:
    ...
    def project_plugin(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("OPENAGENTS_HOME", str(tmp_path / "home"))
        (tmp_path / "home").mkdir()
