...
(e.g. orphan merge-conflict markers in ``openagents_cli/config.py``), the CLI
...
markers in ``openagents_cli/config.py``; users who ran ``hermes update`` in
...
def test_validate_critical_files_syntax_detects_conflict_markers(tmp_path):
    ...
    _populate_critical_tree(tmp_path, broken_file="openagents_cli/config.py")
...
def test_validate_critical_files_syntax_detects_break_in_main_py(tmp_path):
    _populate_critical_tree(tmp_path, broken_file="openagents_cli/main.py")

    ok, failing_path, _ = hermes_main._validate_critical_files_syntax(tmp_path)

    assert ok is False
    assert failing_path is not None and failing_path.endswith("openagents_cli/main.py")
...
def test_validate_critical_files_syntax_tolerates_missing_files(tmp_path):
    ...
    # Populate everything except openagents_constants.py
    for relpath in hermes_main._UPDATE_CRITICAL_FILES:
        if relpath == "openagents_constants.py":
            continue
