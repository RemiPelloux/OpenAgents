...
class TestVerifyCoreDependencies:
    def test_no_action_when_all_deps_present(self, temp_pyproject, fake_venv_python):
        ...
        with (
            patch(
                "openagents_cli.main._resolve_install_target_python", return_value=py
            ),
    ...
    def test_triggers_reinstall_when_dep_missing(
        ...
        with (
            ...
            patch(
                "openagents_cli.main.subprocess.run", side_effect=fake_subprocess_run
            ),
            patch("openagents_cli.main._run_install_with_heartbeat") as mock_install,
        ):
            from openagents_cli.main import _verify_core_dependencies_installed
    ...
    def test_no_pyproject_is_noop(self, tmp_path, monkeypatch):
        ...
        with (
            patch("openagents_cli.main._resolve_install_target_python") as mock_resolve,
            patch("openagents_cli.main._run_install_with_heartbeat") as mock_install,
        ):
            from openagents_cli.main import _verify_core_dependencies_installed
    ...
    def test_repair_reinstall_quarantines_running_shim_on_windows(
        ...
        with (
            ...
            patch("openagents_cli.main._run_install_with_heartbeat"),
            patch(
                "openagents_cli.main._quarantine_running_hermes_exe", return_value=[]
            ) as mock_quar,
        ):
            from openagents_cli.main import _verify_core_dependencies_installed
