...
Complements tests/openagents_cli/test_kanban_db.py (schema + CAS atomicity)
and tests/openagents_cli/test_kanban_cli.py (end-to-end run_slash).  The
...
def kanban_home(tmp_path, monkeypatch):
    ...
    # tests in tests/openagents_cli/test_kanban_db.py.
    monkeypatch.setenv("HERMES_KANBAN_CRASH_GRACE_SECONDS", "0")
    kb.init_db()
    return home
...
def test_list_profiles_on_disk(tmp_path, monkeypatch):
    ...
    named profiles under ~/.openagents/profiles/ that contain a config.yaml."""
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
...
def test_list_profiles_on_disk_custom_root(tmp_path, monkeypatch):
    """list_profiles_on_disk respects a custom OPENAGENTS_HOME root."""
...
def test_parse_duration_accepts_formats():
    from openagents_cli.kanban import _parse_duration
...
def test_cli_bulk_complete_with_summary_rejects(kanban_home):
    ...
    # Note: openagents_cli.main doesn't propagate sub-command exit codes
...
def test_cli_create_on_fresh_home_auto_inits(tmp_path, monkeypatch):
    """First CLI action on an empty OPENAGENTS_HOME must not error with
    ...
    env = {**os.environ, "OPENAGENTS_HOME": str(home),
           "PYTHONPATH": str(worktree_root)}
    r = _sp.run(
        [_sys.executable, "-m", "openagents_cli.main", "kanban",
...
def test_connect_auto_inits_fresh_db(tmp_path, monkeypatch):
    """Calling connect() on a fresh OPENAGENTS_HOME must create the
...
def test_check_dispatcher_presence_silent_when_gateway_running(monkeypatch):
    from openagents_cli import kanban as kb_cli
...
def test_gateway_dispatcher_watcher_respects_config_flag_off(monkeypatch):
    ...
    import openagents_cli.config as _cfg_mod
