...
where stdin is still in cooked + echo mode. If a previous OpenAgents session
...
from openagents_cli.main import _suppress_mouse_residue_early
...
class TestEarlyMouseDisable:
    def test_writes_disable_sequence_when_tui_flag_in_argv(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["openagents", "--tui", "-c", "abc"])
