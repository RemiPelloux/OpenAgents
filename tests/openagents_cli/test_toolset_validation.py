"""Unit tests for openagents_cli.toolset_validation (see #38798).
...
def test_38798_corruption_warns_and_suggests_correct_name():
    # The exact reported shape: cli holds 'openagents' instead of 'hermes-cli'.
    warnings = validate_platform_toolsets({"cli": ["openagents"]}, _is_valid)
    unknown = [w for w in warnings if "unknown toolset 'openagents'" in w]
...
def test_all_invalid_reports_each_and_the_zero_state():
    cfg = {"cli": ["openagents"], "discord": ["openagents"]}
    warnings = validate_platform_toolsets(cfg, _is_valid)
    assert sum("unknown toolset" in w for w in warnings) == 2
    assert any("zero valid toolsets" in w for w in warnings)


def test_real_validate_toolset_treats_openagents_cli_valid_and_hermes_invalid():
    ...
    assert validate_toolset("openagents") is False
    warnings = validate_platform_toolsets({"cli": ["openagents"]}, validate_toolset)
