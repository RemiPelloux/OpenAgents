...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./hermes` should use `openagents_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "openagents"
    called = []

    fake_main_module = types.ModuleType("openagents_cli.main")

    def fake_main():
        called.append("openagents_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "openagents_cli.main", fake_main_module)
    ...
    assert called == ["openagents_cli.main"]
