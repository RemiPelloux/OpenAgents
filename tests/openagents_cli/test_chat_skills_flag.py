...
def test_top_level_skills_flag_defaults_to_chat(monkeypatch):
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["openagents", "-s", "openagents-dev,github-auth"],
    )

    main_mod.main()

    assert captured == {
        "skills": ["openagents-dev,github-auth"],
        "command": None,
    }
...
def test_chat_subcommand_accepts_skills_flag(monkeypatch):
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["openagents", "chat", "-s", "github-auth", "-q", "hello"],
    )
...
def test_continue_worktree_and_skills_flags_work_together(monkeypatch):
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["openagents", "-c", "-w", "-s", "openagents-dev"],
    )
