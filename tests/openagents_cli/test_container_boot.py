"""Tests for openagents_cli.container_boot — the cont-init.d-time
reconciliation that recreates per-profile gateway s6 service slots
from the persistent profiles directory.

These tests run against a fake $OPENAGENTS_HOME under tmp_path; no real
...
from openagents_cli.container_boot import (
    ReconcileAction,
...
def _hermetic_container_argv(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    """
    monkeypatch.setattr(
        "openagents_cli.container_boot._read_container_argv",
        lambda: (),
    )
...
def _seed_default_root(
    ...
    *,
    ...
    OPENAGENTS_HOME root (the implicit default profile)."""
    if state is not None:
...
def test_missing_profiles_root_still_registers_default_slot(
    ...
    """When $OPENAGENTS_HOME/profiles doesn't exist (fresh install), the
...
def test_default_slot_always_registered_on_empty_home(tmp_path: Path) -> None:
    """Bare OPENAGENTS_HOME with nothing under it still produces a
...
def test_default_slot_run_script_omits_profile_flag(tmp_path: Path) -> None:
    ...
    that would resolve to $OPENAGENTS_HOME/profiles/default/ instead of
...
def test_default_slot_autostarts_when_root_state_running(tmp_path: Path) -> None:
    """gateway_state.json at the OPENAGENTS_HOME root with state=running
...
def test_default_slot_cleans_up_stale_runtime_files_at_root(
    ...
    """gateway.pid and processes.json at the OPENAGENTS_HOME root (left
