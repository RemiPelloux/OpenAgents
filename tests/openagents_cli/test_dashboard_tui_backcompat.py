...
Older OpenAgents desktop app shells (<= 0.15.x) spawn the backend as::
...
def _run_cli(args, timeout=60):
    """Invoke the real openagents_cli.main parser in a subprocess.
    ...
    return subprocess.run(
        [sys.executable, "-m", "openagents_cli.main", *args],
        cwd=REPO_ROOT,
        env=env,
        capture_output=True,
        text=True,
