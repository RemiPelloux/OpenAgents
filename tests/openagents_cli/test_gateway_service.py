...
import openagents_cli.gateway as gateway_cli
...
class TestSystemdServiceRefresh:
    def test_systemd_install_repairs_outdated_unit_without_force(self, tmp_path, monkeypatch):
        unit_path = tmp_path / "openagents-gateway.service"
    ...
    def test_refresh_refuses_to_bake_pytest_tmpdir_into_real_user_unit(
        ...
        ``generate_systemd_unit()`` bakes ``OPENAGENTS_HOME`` into the unit's
        ...
        ``/tmp/pytest-of-.../hermes_test`` OPENAGENTS_HOME — silently breaking
        ...
        # Realistic generated unit referencing a pytest tmpdir OPENAGENTS_HOME
        polluted_unit = (
            "[Service]\n"
            'Environment="OPENAGENTS_HOME=/tmp/pytest-of-alice/pytest-42/'
    ...
    def test_refresh_refuses_to_bake_any_tempdir_home_into_real_user_unit(
        ...
        """Structural guard: a manual E2E OPENAGENTS_HOME like
        ...
        ``hermes gateway restart`` with a /tmp OPENAGENTS_HOME exported; the
        ...
        must refuse ANY temp-dir OPENAGENTS_HOME, not just pytest-shaped ones.
        """
        unit_path = tmp_path / "openagents-gateway.service"
        unit_path.write_text("old unit\n", encoding="utf-8")
        ...
        polluted_unit = (
            "[Service]\n"
            'Environment="OPENAGENTS_HOME=/tmp/hermes-e2e-41264"\n'
...
class TestTempHomeServiceDefinitionGuard:
    ...
    def test_detects_var_tmp_home(self):
        unit = '[Service]\nEnvironment="OPENAGENTS_HOME=/var/tmp/hermes-x"\n'
    ...
    def test_detects_tempdir_env_home(self, monkeypatch, tmp_path):
        ...
        unit = f'[Service]\nEnvironment="OPENAGENTS_HOME={tmp_path}/hermes-home"\n'
        assert gateway_cli._temp_home_in_service_definition(unit) is not None
    ...
    def test_accepts_real_home(self):
        unit = '[Service]\nEnvironment="OPENAGENTS_HOME=/home/alice/.hermes"\n'
    ...
    def test_accepts_unit_without_hermes_home(self):
        unit = "[Service]\nExecStart=/usr/bin/python -m openagents_cli.main gateway run\n"
        assert gateway_cli._temp_home_in_service_definition(unit) is None
    ...
    def test_tmp_prefixed_non_temp_path_is_accepted(self):
        ...
        unit = '[Service]\nEnvironment="OPENAGENTS_HOME=/tmpfs-data/.hermes"\n'
        assert gateway_cli._temp_home_in_service_definition(unit) is None
...
class TestLaunchdServiceRecovery:
    ...
    def test_launchd_install_repairs_outdated_plist_without_force(self, tmp_path, monkeypatch):
        plist_path = tmp_path / "ai.openagents.gateway.plist"
        ...
        # OPENAGENTS_HOME resolves under the (pytest tmp) test OPENAGENTS_HOME.
        monkeypatch.setattr(
            gateway_cli,
    ...
    def test_launchd_install_falls_back_to_detached_on_bootstrap_5(self, tmp_path, monkeypatch, capsys):
        ...
        # (which would trip on the pytest-tmp test OPENAGENTS_HOME) stays out of
        # the way — this test exercises the bootstrap-error fallback.
        monkeypatch.setattr(
            gateway_cli,
            "generate_launchd_plist",
            lambda: (
                "<plist><key>OPENAGENTS_HOME</key>"
                "<string>/Users/alice/.hermes</string></plist>"
            ),
        )
    ...
    def test_launchd_unsupported_marker_write_and_clear(self, tmp_path, monkeypatch):
        monkeypatch.setattr(gateway_cli, "get_openagents_home", lambda: tmp_path)
    ...
    def test_gateway_restart_does_not_fallback_to_foreground_when_launchd_restart_fails(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda: (_ for _ in ()).throw(
                gateway_cli.subprocess.CalledProcessError(5, ["launchctl", "kickstart", "-k", "gui/501/ai.openagents.gateway"])
            ),
        )
...
class TestSystemUnitHermesHome:
    """OPENAGENTS_HOME in system units must reference the target user, not root."""
    ...
    def test_system_unit_uses_target_user_home_not_calling_user(self, monkeypatch):
        ...
        assert 'OPENAGENTS_HOME=/home/alice/.hermes' in unit
        assert '/root/.hermes' not in unit

    def test_system_unit_remaps_profile_to_target_user(self, monkeypatch):
        # Simulate sudo with a profile: OPENAGENTS_HOME was resolved under root
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("OPENAGENTS_HOME", "/root/.hermes/profiles/coder")
        monkeypatch.setattr(
            gateway_cli, "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", "/home/alice"),
        )
        ...
        assert 'OPENAGENTS_HOME=/home/alice/.hermes/profiles/coder' in unit
        assert '/root/' not in unit

    def test_system_unit_preserves_custom_hermes_home(self, monkeypatch):
        # Custom OPENAGENTS_HOME not under any user's home — keep as-is
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("OPENAGENTS_HOME", "/opt/hermes-shared")
        monkeypatch.setattr(
            gateway_cli, "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", "/home/alice"),
        )
        ...
        assert 'OPENAGENTS_HOME=/opt/hermes-shared' in unit

    def test_user_unit_unaffected_by_change(self):
        # User-scope units should still use the calling user's OPENAGENTS_HOME
        unit = gateway_cli.generate_systemd_unit(system=False)

        hermes_home = str(gateway_cli.get_openagents_home().resolve())
        assert f'OPENAGENTS_HOME={hermes_home}' in unit
...
class TestHermesHomeForTargetUser:
    ...
    def test_keeps_custom_path(self, monkeypatch):
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("OPENAGENTS_HOME", "/opt/hermes")
...
class TestProfileArg:
    ...
    def test_default_hermes_home_returns_empty(self, tmp_path, monkeypatch):
        """Default ~/.openagents should not produce a --profile flag."""
    ...
    def test_named_profile_returns_flag(self, tmp_path, monkeypatch):
        """~/.openagents/profiles/mybot should return '--profile mybot'."""
    ...
    def test_hash_path_returns_empty(self, tmp_path, monkeypatch):
        """Arbitrary non-profile OPENAGENTS_HOME should return empty string."""
        custom_home = tmp_path / "custom" / "openagents"
    ...
    def test_nested_profile_path_returns_empty(self, tmp_path, monkeypatch):
        """~/.openagents/profiles/mybot/subdir should NOT match — too deep."""
    ...
    def test_systemd_unit_includes_profile(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(gateway_cli, "get_openagents_home", lambda: profile_dir)
        unit = gateway_cli.generate_systemd_unit(system=False)
    ...
    def test_systemd_unit_for_target_user_includes_named_profile(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("OPENAGENTS_HOME", str(root_profile))
        monkeypatch.setattr(gateway_cli, "get_openagents_home", lambda: root_profile)
        monkeypatch.setattr(
            gateway_cli,
            "_system_service_identity",
        ...
        assert f'OPENAGENTS_HOME={target_home / ".hermes" / "profiles" / "mybot"}' in unit
    ...
    def test_launchd_plist_path_uses_real_user_home_not_profile_home(self, tmp_path, monkeypatch):
        ...
        assert plist_path == machine_home / "Library" / "LaunchAgents" / "ai.openagents.gateway-orcha.plist"
...
class TestRemapPathForUser:
    ...
    def test_remaps_path_under_current_home(self, monkeypatch, tmp_path):
        ...
        result = gateway_cli._remap_path_for_user(
            str(tmp_path / "root" / ".hermes" / "openagents"),
            str(tmp_path / "alice"),
        )
        assert result == str(tmp_path / "alice" / ".hermes" / "openagents")
    ...
    def test_noop_when_same_user(self, monkeypatch, tmp_path):
        ...
        original = str(tmp_path / "alice" / ".hermes" / "openagents")
...
class TestSystemUnitPathRemapping:
    ...
    def test_system_unit_has_no_root_paths(self, monkeypatch, tmp_path):
        root_home = tmp_path / "root"
        root_home.mkdir()
        project = root_home / ".hermes" / "openagents"
        ...
        monkeypatch.setattr(Path, "home", lambda: root_home)
        monkeypatch.setenv("OPENAGENTS_HOME", str(root_home / ".hermes"))
        monkeypatch.setattr(gateway_cli, "get_openagents_home", lambda: root_home / ".hermes")
        ...
        # WorkingDirectory is anchored at the target user's OPENAGENTS_HOME (stable,
        ...
        assert "WorkingDirectory=/home/alice/.hermes/openagents" not in unit
...
class TestDockerAwareGateway:
    ...
    def test_run_systemctl_raises_runtimeerror_when_missing(self, monkeypatch):
        ...
        with pytest.raises(RuntimeError, match="systemctl is not available"):
            gateway_cli._run_systemctl(["start", "openagents-gateway"])
    ...
    def test_run_systemctl_passes_through_on_success(self, monkeypatch):
        ...
        result = gateway_cli._run_systemctl(["status", "openagents-gateway"])
...
class TestLegacyHermesUnitDetection:
    ...
    renamed to ``openagents-gateway.service``. After PR #5646 (signal recovery
    ...
    (``openagents-gateway-<profile>.service``) or unrelated third-party services.
    """

    # Minimal ExecStart that looks like our gateway
    _OUR_UNIT_TEXT = (
        "[Unit]\nDescription=OpenAgents Gateway\n[Service]\n"
        "ExecStart=/usr/bin/python -m openagents_cli.main gateway run --replace\n"
    )
    ...
    def test_ignores_profile_unit_hermes_gateway_coder(self, tmp_path, monkeypatch):
        ...
        Teknium's concern — ``openagents-gateway-coder.service`` is our standard
        ...
        for base in (user_dir, system_dir):
            (base / "openagents-gateway-coder.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
            (base / "openagents-gateway-orcha.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
            (base / "openagents-gateway.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
    ...
    def test_accepts_alternate_execstart_formats(self, tmp_path, monkeypatch):
        ...
        ExecStart variants we've seen in the wild:
          - python -m openagents_cli.main gateway run
          - python path/to/openagents_cli/main.py gateway run
          - hermes gateway run   (direct binary)
          - python path/to/gateway/run.py
        """
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        variants = [
            "ExecStart=/venv/bin/python -m openagents_cli.main gateway run --replace",
            "ExecStart=/venv/bin/python /opt/hermes/openagents_cli/main.py gateway run",
...
class TestRemoveLegacyHermesUnits:
    ...
    def test_does_not_touch_profile_units_during_migration(
        ...
        """Teknium's constraint: profile units (openagents-gateway-coder.service)
        ...
        profile_unit = user_dir / "openagents-gateway-coder.service"
        profile_unit.write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
        default_unit = user_dir / "openagents-gateway.service"
...
class TestMigrateLegacyCommand:
    ...
    def test_migrate_legacy_subparser_accepts_dry_run_and_yes(self):
        ...
        import openagents_cli.main as cli_main
        ...
        if project_root is None:
            import openagents_cli.gateway as gw
            project_root = gw.PROJECT_ROOT

        result = subprocess.run(
            [sys.executable, "-m", "openagents_cli.main", "gateway", "--help"],
...
class TestGatewayStatusParser:
    def test_gateway_status_subparser_accepts_full_flag(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "openagents_cli.main", "gateway", "status", "-l", "--help"],
...
class TestSystemScopeWizardPreCheck:
    ...
    def _setup_units(tmp_path, monkeypatch, system_present: bool, user_present: bool):
        ...
        usr_dir.mkdir()
        if system_present:
            (sys_dir / "openagents-gateway.service").write_text("[Unit]\n")
        if user_present:
            (usr_dir / "openagents-gateway.service").write_text("[Unit]\n")
        monkeypatch.setattr(
            gateway_cli,
            "get_systemd_unit_path",
            lambda system=False: (sys_dir if system else usr_dir) / "openagents-gateway.service",
        )
...
class TestSystemScopeRemediationOutput:
    ...
    def test_start_remediation_mentions_sudo_systemctl_and_uninstall(self, capsys, monkeypatch):
        monkeypatch.setattr(gateway_cli, "get_service_name", lambda: "openagents-gateway")
        ...
        assert "sudo systemctl start openagents-gateway" in out
    ...
    def test_restart_remediation_uses_systemctl_restart(self, capsys, monkeypatch):
        ...
        assert "sudo systemctl restart openagents-gateway" in out
    ...
    def test_stop_remediation_uses_systemctl_stop(self, capsys, monkeypatch):
        ...
        assert "sudo systemctl stop openagents-gateway" in out
...
class TestServiceWorkingDirIsStable:
    ...
    (OPENAGENTS_HOME), never the source checkout / worktree, so a relocated or
    ...
    def test_stable_working_dir_uses_hermes_home(self, tmp_path, monkeypatch):
        ...
        home.mkdir()
        monkeypatch.setattr(gateway_cli, "get_openagents_home", lambda: home)
    ...
    def test_stable_working_dir_falls_back_to_project_root(self, tmp_path, monkeypatch):
        # OPENAGENTS_HOME points somewhere that does not exist -> fall back.
        missing = tmp_path / "does-not-exist" / ".hermes"
        monkeypatch.setattr(gateway_cli, "get_openagents_home", lambda: missing)
