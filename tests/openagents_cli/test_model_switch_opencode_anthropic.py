...
``openagents_cli.runtime_provider.resolve_runtime_provider`` already strips
``/v1`` at fresh agent init (PR #4918), but the ``/model`` mid-session
switch path in ``openagents_cli.model_switch.switch_model`` was missing the
...
class TestStaleConfigDefaultDoesNotWedgeResolver:
    ...
    def test_kimi_switch_keeps_v1_despite_claude_config_default(
        ...
        # Re-import with the new OPENAGENTS_HOME so config cache is fresh.
        import openagents_cli.config as _cfg_mod

        importlib.reload(_cfg_mod)
        import openagents_cli.runtime_provider as _rp_mod

        importlib.reload(_rp_mod)
        import openagents_cli.model_switch as _ms_mod
