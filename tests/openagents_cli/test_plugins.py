"""Tests for the OpenAgents plugin system (openagents_cli.plugins)."""
...
from openagents_cli.middleware import (
    VALID_MIDDLEWARE,
...
def _make_plugin_dir(base: Path, name: str, *, register_body: str = "pass",
    ...
    *base* is expected to be ``<hermes_home>/plugins/``; we derive
    ...
    if auto_enable:
        # Write/merge plugins.enabled in <OPENAGENTS_HOME>/config.yaml.
        # Config is always read from OPENAGENTS_HOME (not from the project
        # dir for project plugins), so that's where we opt in.
        import os
        hermes_home_str = os.environ.get("OPENAGENTS_HOME")
        if hermes_home_str:
...
class TestPluginDiscovery:
    ...
    def test_discover_user_plugins(self, tmp_path, monkeypatch):
        """Plugins in ~/.openagents/plugins/ are discovered."""
        plugins_dir = tmp_path / "hermes_test" / "plugins"
        _make_plugin_dir(plugins_dir, "hello_plugin")
        monkeypatch.setenv("OPENAGENTS_HOME", str(tmp_path / "hermes_test"))
    ...
    def test_execution_middleware_does_not_retry_downstream_failure(self, monkeypatch):
        ...
        monkeypatch.setattr("openagents_cli.plugins.get_plugin_manager", lambda: manager)
...
class TestPluginHooks:
    ...
    def test_invalid_hook_name_warns(self, tmp_path, monkeypatch, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="openagents_cli.plugins"):
            mgr = PluginManager()
            mgr.discover_and_load()
...
class TestPreToolCallBlocking:
    ...
    def test_block_message_returned_for_valid_directive(self, monkeypatch):
        monkeypatch.setattr(
            "openagents_cli.plugins.invoke_hook",
...
class TestPluginToolVisibility:
    ...
    def test_plugin_tools_in_definitions(self, tmp_path, monkeypatch):
        ...
        import openagents_cli.plugins as plugins_mod
...
class TestPluginCommands:
    ...
    def test_get_plugin_command_handler_found(self):
        ...
        with patch("openagents_cli.plugins._plugin_manager", mgr):
    ...
    def test_commands_in_list_plugins_output(self, tmp_path, monkeypatch):
        ...
        plugins_dir = tmp_path / "hermes_test" / "plugins"
        # Set OPENAGENTS_HOME BEFORE _make_plugin_dir so auto-enable targets
...
class TestPluginCommandResultResolution:
    ...
    def test_awaits_async_result_with_running_loop(self, monkeypatch):
        ...
        monkeypatch.setattr("openagents_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
    ...
    def test_running_loop_timeout_does_not_hang_forever(self, monkeypatch):
        ...
        monkeypatch.setattr("openagents_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
        monkeypatch.setattr("openagents_cli.plugins._PLUGIN_COMMAND_AWAIT_TIMEOUT_SECS", 0.1)
...
class TestPluginDispatchTool:
    ...
    def test_dispatch_tool_calls_registry(self):
        ...
        with patch("openagents_cli.plugins.PluginContext.dispatch_tool.__module__", "openagents_cli.plugins"):
...
class TestPluginDebugLogging:
    ...
    def test_debug_handler_not_installed_when_env_var_absent(self, monkeypatch):
        ...
        from openagents_cli import plugins as plugins_mod
...
class TestPluginContextProfileName:
    """ctx.profile_name resolves from OPENAGENTS_HOME in every context."""
    ...
    def test_default_profile(self, tmp_path, monkeypatch):
        """OPENAGENTS_HOME at the root resolves to 'default'."""
    ...
    def test_named_profile(self, tmp_path, monkeypatch):
        """OPENAGENTS_HOME under profiles/<name> resolves to that name."""
        prof = tmp_path / ".hermes" / "profiles" / "coder"
        prof.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("OPENAGENTS_HOME", str(prof))
