...
def test_detect_service_manager_s6_keys_off_s6_running_not_is_container(
    ...
    monkeypatch.setattr(
        "openagents_cli.service_manager._s6_running",
        lambda: True,
    )
...
def test_systemd_manager_lifecycle_delegates(monkeypatch: pytest.MonkeyPatch) -> None:
    called: list[str] = []
    monkeypatch.setattr(
        "openagents_cli.gateway.systemd_start",
        lambda: called.append("start"),
    )
    monkeypatch.setattr(
        "openagents_cli.gateway.systemd_stop",
        lambda: called.append("stop"),
    )
    monkeypatch.setattr(
        "openagents_cli.gateway.systemd_restart",
...
def test_launchd_manager_lifecycle_delegates(monkeypatch: pytest.MonkeyPatch) -> None:
    called: list[str] = []
    monkeypatch.setattr(
        "openagents_cli.gateway.launchd_start",
        lambda: called.append("start"),
    )
    monkeypatch.setattr(
        "openagents_cli.gateway.launchd_stop",
        lambda: called.append("stop"),
    )
    monkeypatch.setattr(
        "openagents_cli.gateway.launchd_restart",
        lambda: called.append("restart"),
    )
    monkeypatch.setattr(
        "openagents_cli.gateway._probe_launchd_service_running",
        lambda: False,
    )
    mgr = LaunchdServiceManager()
    mgr.start("ignored")
...
def test_get_service_manager_returns_correct_backend(
    ...
    monkeypatch.setattr(
        "openagents_cli.service_manager.detect_service_manager",
        lambda: kind,
    )
