"""Tests for openagents_cli.service_manager — the abstract ServiceManager
...
def test_detect_service_manager_s6_keys_off_s6_running_not_is_container(
    ...
    monkeypatch.setattr(
        "openagents_cli.service_manager._s6_running", lambda: True,
    )
...
def test_s6_running_true_when_comm_and_basedir_match(
    ...
    from openagents_cli.service_manager import _s6_running
...
def test_systemd_manager_lifecycle_delegates(monkeypatch: pytest.MonkeyPatch) -> None:
    called: list[str] = []
    monkeypatch.setattr(
        "openagents_cli.gateway.systemd_start", lambda: called.append("start"),
    )
    monkeypatch.setattr(
        "openagents_cli.gateway.systemd_stop", lambda: called.append("stop"),
    )
    monkeypatch.setattr(
        "openagents_cli.gateway.systemd_restart", lambda: called.append("restart"),
    )
    monkeypatch.setattr(
        "openagents_cli.gateway._probe_systemd_service_running",
...
def test_launchd_manager_lifecycle_delegates(monkeypatch: pytest.MonkeyPatch) -> None:
    called: list[str] = []
    monkeypatch.setattr(
        "openagents_cli.gateway.launchd_start", lambda: called.append("start"),
    )
    monkeypatch.setattr(
        "openagents_cli.gateway.launchd_stop", lambda: called.append("stop"),
    )
    monkeypatch.setattr(
        "openagents_cli.gateway.launchd_restart", lambda: called.append("restart"),
    )
    monkeypatch.setattr(
        "openagents_cli.gateway._probe_launchd_service_running", lambda: False,
    )
    mgr = LaunchdServiceManager()
    mgr.start("ignored")
    mgr.stop("ignored")
...
def test_windows_manager_lifecycle_delegates(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # against the `openagents_cli` package succeeds — gateway_windows is
    # imported lazily inside the wrapper and may not yet be loaded.
    import openagents_cli.gateway_windows  # noqa: F401
    ...
    monkeypatch.setattr("openagents_cli.gateway_windows", _FakeWindowsModule)
    monkeypatch.setattr(
        "openagents_cli.gateway.find_gateway_pids",
        lambda **kw: [12345],
    )
    mgr = WindowsServiceManager()
    mgr.start("ignored")
...
def test_get_service_manager_returns_correct_backend(
    ...
    monkeypatch.setattr(
        "openagents_cli.service_manager.detect_service_manager", lambda: kind,
    )
...
def test_get_service_manager_raises_when_unsupported(
    ...
    monkeypatch.setattr(
        "openagents_cli.service_manager.detect_service_manager", lambda: "none",
    )
...
def test_get_service_manager_returns_s6_instance(
    ...
    monkeypatch.setattr(
        "openagents_cli.service_manager.detect_service_manager", lambda: "s6",
    )
...
def test_s6_register_creates_service_dir_and_triggers_scan(
    ...
    # in openagents_cli/gateway.py::_gateway_command_inner.
...
def test_render_run_script_uses_replace_to_take_over_stale_holder() -> None:
    ...
    the per-OPENAGENTS_HOME PID lock; the supervised slot then execs a bare
    ...
    Covers both the default (root OPENAGENTS_HOME, no ``-p``) and named-profile
    render paths.
    """
...
def test_s6_lifecycle_persists_default_profile_desired_state(
    ...
    monkeypatch.setenv("OPENAGENTS_HOME", str(hermes_home / "profiles" / "coder"))
...
def test_lifecycle_raises_s6_command_error_on_subprocess_failure(
    ...
    import subprocess as _sp
    from openagents_cli.service_manager import S6CommandError
...
def test_s6_log_run_chowns_gateways_parent(s6_scandir, fake_subprocess_run) -> None:
    ...
    parent_chown = 'chown hermes:hermes "$OPENAGENTS_HOME/logs/gateways"'
    ...
    assert 'chown -R hermes:hermes "$OPENAGENTS_HOME/logs/gateways"' not in log_text
