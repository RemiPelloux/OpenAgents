"""Tests for openagents_cli.relaunch — unified self-relaunch utility."""

import sys

import pytest

from openagents_cli import relaunch as relaunch_mod
...
class TestResolveHermesBin:
    ...
    def test_resolves_relative_argv0(self, monkeypatch, tmp_path):
        fake = tmp_path / "openagents"
        fake.write_text("#!/bin/sh\n")
        fake.chmod(0o755)
        monkeypatch.setattr(sys, "argv", [str(fake.name)])
        monkeypatch.chdir(tmp_path)
        # Ensure we don't accidentally match a real 'openagents' on PATH
    ...
    def test_falls_back_to_path_which(self, monkeypatch):
        ...
        monkeypatch.setattr(
            relaunch_mod.shutil, "which", lambda name: "/usr/bin/hermes" if name == "openagents" else None
        )
...
class TestBuildRelaunchArgv:
    ...
    def test_falls_back_to_python_module(self, monkeypatch):
        ...
        argv = relaunch_mod.build_relaunch_argv(["--resume", "abc"])
        assert argv == [sys.executable, "-m", "openagents_cli.main", "--resume", "abc"]
...
class TestResolveHermesBinWindowsPyGuard:
    ...
    def test_windows_rejects_py_argv0_falls_through_to_path(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(
            ...
            lambda name: r"C:\venv\Scripts\hermes.exe" if name == "openagents" else None,
        )
