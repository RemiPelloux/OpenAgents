"""Tests for openagents_cli.gui_uninstall — GUI-only uninstall + install discovery.
...
import openagents_cli.gui_uninstall as gu


def _make_agent(hermes_home: Path) -> Path:
    """Create a fake agent install: source package + venv."""
    agent_root = hermes_home / "openagents"
    (agent_root / "openagents_cli").mkdir(parents=True)
    (agent_root / "openagents_cli" / "__init__.py").write_text("")
...
def _make_gui_build(hermes_home: Path) -> None:
    ...
    desktop = hermes_home / "openagents" / "apps" / "desktop"
    ...
    (hermes_home / "openagents" / "node_modules").mkdir(parents=True)
...
def test_agent_is_installed_venv_only(tmp_path):
    ...
    hermes_home = tmp_path / ".hermes"
    (hermes_home / "openagents" / "venv").mkdir(parents=True)
    assert gu.agent_is_installed(hermes_home) is True
...
def test_uninstall_gui_removes_only_gui_artifacts(tmp_path, monkeypatch):
    ...
    assert (agent_root / "openagents_cli" / "__init__.py").exists()
...
def test_run_uninstall_yes_keep_data_is_non_interactive(tmp_path, monkeypatch):
    ...
    OPENAGENTS_HOME with all the destructive externals stubbed out.
    """
    import openagents_cli.uninstall as uninstall
    ...
    monkeypatch.setattr(uninstall, "get_openagents_home", lambda: hermes_home)
    ...
    from openagents_cli import gui_uninstall as gu_mod
...
def test_run_uninstall_yes_full_wipes_home(tmp_path, monkeypatch):
    """``--yes --full`` removes the whole OPENAGENTS_HOME non-interactively."""
    import openagents_cli.uninstall as uninstall

    hermes_home = tmp_path / ".hermes"
    (hermes_home / "openagents" / "openagents_cli").mkdir(parents=True)
    (hermes_home / "config.yaml").write_text("x: 1\n")
    fake_code = tmp_path / "checkout"
    fake_code.mkdir()
...
def test_uninstall_module_main_gui_mode(tmp_path, monkeypatch):
    """`python -m openagents_cli.uninstall --mode gui` runs the GUI-only path.
    ...
    monkeypatch.setattr(gu_mod, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu_mod, "desktop_userdata_dir", lambda: tmp_path / "none")
    monkeypatch.setattr(gu_mod, "get_openagents_home", lambda: hermes_home)
    ...
    assert not (desktop / "release").exists()
    assert not (hermes_home / "desktop-build-stamp.json").exists()
    assert (agent_root / "openagents_cli").exists()
    assert (hermes_home / "config.yaml").exists()
