...
profile switcher can target any profile's OPENAGENTS_HOME. These tests pin:
reads/writes land in the REQUESTED profile, the dashboard's own profile
stays untouched, and the chat PTY env is scoped via OPENAGENTS_HOME.
"""
...
def isolated_profiles(tmp_path, monkeypatch, _isolate_hermes_home):
    ...
    default_home = get_openagents_home()
...
class TestProfileScopedMcp:
    ...
    def test_mcp_probe_runs_inside_profile_scope(
        ...
        import openagents_cli.mcp_config as mcp_config
        from openagents_constants import get_openagents_home
        ...
        def fake_probe(name, config, connect_timeout=30):
            seen["home"] = str(get_openagents_home())
            return [("tool-a", "desc")]
...
class TestProfileScopedPostSetup:
    def test_post_setup_spawns_with_profile_flag(
        ...
        config / write per-profile state see the same OPENAGENTS_HOME the rest
        ...
        monkeypatch.setattr(
            "openagents_cli.tools_config.valid_post_setup_keys",
...
class TestProfileScopedGateway:
    ...
    def test_status_reads_requested_profile_home(
        ...
        def fake_get_running_pid():
            seen_homes.append(str(get_openagents_home()))
            return None
...
class TestProfileScopedChatPty:
    ...
    def test_chat_argv_unscoped_keeps_legacy_env(self, isolated_profiles, monkeypatch):
        ...
        assert env is not None
        assert env.get("OPENAGENTS_HOME") != str(isolated_profiles["worker_beta"])
