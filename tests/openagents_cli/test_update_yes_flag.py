...
class TestUpdateYesConfigMigration:
    ...
    @patch("openagents_cli.config.migrate_config")
    ...
    @patch("openagents_cli.config.get_missing_env_vars", return_value=["NEW_KEY"])
    ...
    def test_no_yes_flag_still_prompts_in_tty(
        ...
        # real ``sys`` module instead of replacing ``openagents_cli.main.sys`` with
        # a MagicMock. The MagicMock approach was flaky under ``pytest-xdist``
        # — a sibling test that imported ``openagents_cli.main`` first could leave
