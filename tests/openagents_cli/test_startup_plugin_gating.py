...
``openagents_cli.main`` skips eager plugin discovery at argparse-setup time
...
def _live_subcommand_names() -> set[str]:
    ...
    built-in-only set.
    """
    from openagents_cli import main as _main

    argv_backup = sys.argv[:]
    sys.argv = ["openagents", "--help"]
    buf = io.StringIO()
    try:
...
@pytest.mark.parametrize(
    ...
    [
        (["openagents"], None),
        (["openagents", "--help"], None),
        (["openagents", "-h"], None),
        (["openagents", "--version"], None),
        (["openagents", "-w"], None),
        ...
        (["openagents", "-w", "--tui"], None),
        (["openagents", "version"], "version"),
        (["openagents", "--tui", "chat"], "chat"),
        (["openagents", "-w", "logs"], "logs"),
        (["openagents", "chat", "hello world"], "chat"),
        (["openagents", "gateway", "run"], "gateway"),
        # Top-level value-taking flags: the value should be skipped.
        (["openagents", "-m", "gpt5", "chat"], "chat"),
        (["openagents", "--model", "gpt5", "chat", "hi"], "chat"),
        ...
        (["openagents", "-m", "gpt5", "some-plugin-cmd"], "some-plugin-cmd"),
    ],
)
...
def test_builtin_set_covers_every_registered_subcommand():
    ...
    assert not missing_from_declaration, (
        ...
        f"openagents_cli/main.py::_BUILTIN_SUBCOMMANDS so plugin discovery "
