...
def test_save_env_value_invalidates_cache(tmp_path, monkeypatch):
    ...
    from openagents_cli import config as config_mod
    from openagents_cli.config import invalidate_env_cache, load_env, save_env_value
...
def test_load_env_handles_missing_file():
    ...
    try:
        with patch("openagents_cli.config.get_env_path", return_value=nonexistent):
            assert load_env() == {}
