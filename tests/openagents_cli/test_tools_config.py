...
def test_reconfigure_lists_enabled_web_without_existing_provider_config(monkeypatch):
    ...
    monkeypatch.setattr(
        "openagents_cli.tools_config._prompt_choice", fake_prompt_choice
    )
    monkeypatch.setattr(
...
def test_first_install_nous_auto_configures_managed_defaults(monkeypatch):
    monkeypatch.setattr(
        "openagents_cli.nous_subscription.managed_nous_tools_enabled", lambda: True
    )
    config = {
        "model": {"provider": "nous"},
        "platform_toolsets": {"cli": []},
...
class TestImagegenModelPicker:
    ...
    def test_picker_with_gpt_image_does_not_prompt_quality(self):
        ...
        config = {}
        with patch(
            "openagents_cli.tools_config._prompt_choice", side_effect=fake_prompt
        ):
            _configure_imagegen_model("fal", config)
...
def test_get_platform_tools_recovers_non_configurable_toolsets_from_composite():
    ...
    with mock_patch(
        "openagents_cli.tools_config.PLATFORMS", {**PLATFORMS, **test_platforms}
    ):
...
def test_reconfigure_provider_runs_post_setup_for_env_var_providers(
    ...
    called = []
    monkeypatch.setattr(
        "openagents_cli.tools_config._run_post_setup", lambda key: called.append(key)
    )
...
def test_vision_picker_writes_provider_and_model(tmp_path, monkeypatch):
    ...
    with (
        patch(
            "openagents_cli.model_switch.list_authenticated_providers",
