...
def _stub_tts(monkeypatch):
    ...
    monkeypatch.setattr(
        "openagents_cli.setup.prompt_choice",
        lambda q, c, d=0: (
...
def test_select_provider_and_model_warns_if_named_custom_provider_disappears(
    ...
    monkeypatch.setattr(
        "openagents_cli.main._prompt_provider_choice", fake_prompt_provider_choice
    )
    monkeypatch.setattr(
...
def test_modal_setup_can_use_nous_subscription_without_modal_creds(
    ...
    monkeypatch.setattr(
        "openagents_cli.setup._prompt_container_resources", lambda config: None
    )
    monkeypatch.setattr(
...
def test_modal_setup_persists_direct_mode_when_user_chooses_their_own_account(
    ...
    monkeypatch.setattr("openagents_cli.setup.prompt_choice", fake_prompt_choice)
    monkeypatch.setattr(
        "openagents_cli.setup.prompt", lambda *args, **kwargs: next(prompt_values)
    )
    monkeypatch.setattr(
        "openagents_cli.setup._prompt_container_resources", lambda config: None
    )
