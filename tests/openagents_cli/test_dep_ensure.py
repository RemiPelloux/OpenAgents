...
def test_ensure_dependency_returns_false_when_missing_noninteractive():
    ...
    with patch("openagents_cli.dep_ensure.shutil") as mock_shutil:
        ...
        with patch(
            "openagents_cli.dep_ensure._find_install_script", return_value=(None, None)
        ):
            result = ensure_dependency("node", interactive=False)
...
def test_find_install_script_from_checkout(tmp_path):
    ...
    with patch("openagents_cli.dep_ensure._IS_WINDOWS", False):
        path, shell = _find_install_script(
            package_dir=tmp_path / "openagents_cli", repo_root=tmp_path
        )
...
def test_find_install_script_falls_back_to_repo_root(tmp_path):
    ...
    with patch("openagents_cli.dep_ensure._IS_WINDOWS", False):
        path, shell = _find_install_script(
            package_dir=tmp_path / "openagents_cli", repo_root=repo_root
        )
...
def test_has_system_browser_checks_windows_names():
    ...
    with (
        patch("openagents_cli.dep_ensure._IS_WINDOWS", True),
...
def test_has_system_browser_checks_posix_names():
    ...
    with (
        patch("openagents_cli.dep_ensure._IS_WINDOWS", False),
        patch("openagents_cli.dep_ensure.shutil") as mock_shutil,
    ):
        mock_shutil.which.return_value = None
...
def test_ensure_dependency_uses_powershell_on_windows(tmp_path):
    ...
    with (
        ...
        patch(
            "openagents_cli.dep_ensure._find_install_script",
            return_value=(scripts_dir / "install.ps1", "powershell"),
        ),
        patch("openagents_cli.dep_ensure.shutil") as mock_shutil,
        patch(
            "openagents_constants.get_openagents_home",
