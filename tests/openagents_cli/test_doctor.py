"""Tests for openagents_cli.doctor."""
...
import openagents_cli.doctor as doctor
import openagents_cli.gateway as gateway_cli
from openagents_cli import doctor as doctor_mod
from openagents_cli.doctor import _has_provider_env_config
...
class TestDoctorEnvFileEncoding:
    ...
    def test_doctor_reads_env_as_utf8_even_when_locale_is_not_utf8(
        ...
        monkeypatch.setattr(doctor_mod, "OPENAGENTS_HOME", hermes_home)
...
class TestDoctorMemoryProviderSection:
    ...
    def _make_hermes_home(self, tmp_path, provider=""):
        """Create a minimal OPENAGENTS_HOME with config.yaml."""
...
def test_has_healthy_oauth_fallback_returns_false_for_unknown_provider():
    from openagents_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
...
class TestHasHealthyOauthFallbackForXai:
    def test_returns_true_when_xai_oauth_healthy(self, monkeypatch):
        ...
        from openagents_cli.doctor import (
    ...
    def test_returns_false_when_xai_import_unavailable(self, monkeypatch):
        ...
        monkeypatch.delattr(
            "openagents_cli.auth.get_xai_oauth_auth_status", raising=False
        )
        # Force doctor module to re-import the function
        monkeypatch.delitem(sys.modules, "openagents_cli.doctor", raising=False)
        from openagents_cli.doctor import (
            _has_healthy_oauth_fallback_for_apikey_provider,
        )
...
class TestDoctorStaleMaxIterationsDrift:
    ...
    def _run_config_section(
        ...
        monkeypatch.setattr(doctor_mod, "OPENAGENTS_HOME", hermes_home)
        monkeypatch.setattr(doctor_mod, "get_openagents_home", lambda: hermes_home)
...
def test_npm_audit_fix_hint_avoids_crashing_workspace_flag(monkeypatch, tmp_path):
    ...
    # left thinking a crashing command means a broken OpenAgents install.
