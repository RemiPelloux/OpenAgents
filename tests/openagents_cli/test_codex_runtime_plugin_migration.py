...
from openagents_cli.codex_runtime_plugin_migration import (
    MIGRATION_MARKER,
...
class TestMigrate:
    ...
    def test_plugin_discovery_writes_plugin_blocks(self, tmp_path, monkeypatch):
        ...
        from openagents_cli import codex_runtime_plugin_migration as crpm
    ...
    def test_plugin_discovery_skips_unavailable_plugins(self):
        ...
        from openagents_cli.codex_runtime_plugin_migration import _query_codex_plugins
    ...
    def test_expose_hermes_tools_writes_callback_mcp_entry(self, tmp_path):
        ...
        into OpenAgents for browser/web/delegate_task/vision/memory tools.
    ...
    def test_managed_root_keys_stay_top_level_when_config_ends_in_table(self, tmp_path):
        """TOML has no explicit 'leave current table' syntax. If OpenAgents appends
...
class TestStripUnmanagedPluginTables:
    ...
    def test_migrate_dedups_codex_owned_plugin_tables(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "openagents_cli.codex_runtime_plugin_migration._query_codex_plugins",
            fake_query,
        )
...
# ---- Bug C: OPENAGENTS_HOME tempdir leak into ~/.codex/config.toml ----


class TestHermesHomeLeakGuard:
    """Regression tests for issue #26250 Bug C.

    Previously ``_build_hermes_tools_mcp_entry()`` read ``OPENAGENTS_HOME``
    ...
    def test_pytest_tempdir_not_burned_into_mcp_env(self, monkeypatch):
        """The headline regression: even when OPENAGENTS_HOME points at a pytest
        ...
        env = entry.get("env", {})
        assert "OPENAGENTS_HOME" not in env, (
            f"pytest-tempdir OPENAGENTS_HOME leaked into codex MCP entry: "
            f"{env.get('OPENAGENTS_HOME')!r}"
        )

    def test_real_hermes_home_propagates(self, monkeypatch, tmp_path):
        """A legitimate OPENAGENTS_HOME (not a tempdir path) DOES propagate so the
        ...
        monkeypatch.setenv("OPENAGENTS_HOME", real_path)
        entry = _build_hermes_tools_mcp_entry()
        env = entry.get("env", {})
        assert env.get("OPENAGENTS_HOME") == real_path

    def test_unset_hermes_home_omits_env_key(self, monkeypatch):
        """When OPENAGENTS_HOME is unset in the environment, the MCP entry MUST
        NOT bake in a resolved-default path. The codex subprocess should
        inherit whatever OPENAGENTS_HOME its launcher (systemd, gateway, shell)
        ...
        env = entry.get("env", {})
        assert "OPENAGENTS_HOME" not in env, (
            f"OPENAGENTS_HOME should not be set when env var is unset, got: "
            f"{env.get('OPENAGENTS_HOME')!r}"
        )
