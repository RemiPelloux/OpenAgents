...
def _enter_existing_install_patches(stack, **extra):
    ...
    for target, kwargs in [
        ("openagents_cli.setup.ensure_hermes_home", {}),
        ("openagents_cli.setup.is_interactive_stdin", {"return_value": True}),
        ("openagents_cli.config.is_managed", {"return_value": False}),
        ("openagents_cli.setup.load_config", {"return_value": {}}),
        ("openagents_cli.setup.save_config", {}),
        ("openagents_cli.setup.get_env_value", {"return_value": None}),
        ("openagents_cli.auth.get_active_provider", {"return_value": "openrouter"}),
        ("openagents_cli.setup._print_setup_summary", {}),
        ("openagents_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
...
def _enter_fresh_install_patches(stack, **extra):
    for target, kwargs in [
        ...
        ("openagents_cli.setup.load_config", {"return_value": {}}),
        ("openagents_cli.setup.save_config", {}),
        ("openagents_cli.auth.get_active_provider", {"return_value": None}),
        ("openagents_cli.setup.get_env_value", {"return_value": None}),
        ("openagents_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
        stack.enter_context(patch(target, **kwargs))
    ...
    def test_bare_setup_runs_full_reconfigure_without_menu(self, existing_install):
        ...
        with ExitStack() as stack:
            m = _enter_existing_install_patches(
                stack,
                prompt_choice="openagents_cli.setup.prompt_choice",
                quick="openagents_cli.setup._run_quick_setup",
                model="openagents_cli.setup.setup_model_provider",
                terminal="openagents_cli.setup.setup_terminal_backend",
                agent="openagents_cli.setup.setup_agent_settings",
                gateway="openagents_cli.setup.setup_gateway",
                tools="openagents_cli.setup.setup_tools",
            )
    ...
    def test_bare_setup_runs_first_time_flow(self, fresh_install):
        ...
        with ExitStack() as stack:
            m = _enter_fresh_install_patches(
                stack,
                prompt=("openagents_cli.setup.prompt_choice", {"return_value": 0}),
                first="openagents_cli.setup._run_first_time_quick_setup",
            )
            from openagents_cli.setup import run_setup_wizard
...
class TestArgparse:
    ...
    def test_reconfigure_flag_reaches_cmd_setup(self, monkeypatch):
        ...
        captured = {}
        monkeypatch.setattr(
            "openagents_cli.setup.run_setup_wizard",
            lambda args: captured.setdefault("args", args),
        )
        monkeypatch.setattr(sys, "argv", ["openagents", "setup", "--reconfigure"])
        try:
            main()
        except SystemExit:
            pass
    ...
    def test_quick_flag_reaches_cmd_setup(self, monkeypatch):
        ...
        monkeypatch.setattr(sys, "argv", ["openagents", "setup", "--quick"])
        try:
            main()
        except SystemExit:
            pass
    ...
    def test_bare_setup_has_both_flags_false(self, monkeypatch):
        ...
        monkeypatch.setattr(sys, "argv", ["openagents", "setup"])
        try:
            main()
        except SystemExit:
            pass
