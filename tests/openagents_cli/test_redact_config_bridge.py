...
`HERMES_REDACT_SECRETS` at module-import time. `openagents_cli/main.py` at
...
env var in `openagents_cli/main.py` BEFORE the `setup_logging()` call.
"""
...
def test_redact_secrets_false_in_config_yaml_is_honored(tmp_path):
    ...
    # Spawn a fresh Python process that imports openagents_cli.main and checks
    ...
    probe = textwrap.dedent(
        ...
        sys.path.insert(0, %r)
        import openagents_cli.main  # triggers the bridge + setup_logging
        import agent.redact
    ...
    env = dict(os.environ)
    env["OPENAGENTS_HOME"] = str(hermes_home)
