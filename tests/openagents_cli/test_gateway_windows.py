...
def test_build_gateway_argv_uses_base_pythonw_for_uv_venv_launcher(
    ...
    monkeypatch.setattr(
        "openagents_cli.config.get_openagents_home", lambda: str(hermes_home)
    )
...
class TestStableWindowsGatewayWorkingDir:
    ...
    def test_stable_gateway_working_dir_falls_back_to_project_root(
        ...
        monkeypatch.setattr(
            "openagents_cli.config.get_openagents_home", lambda: missing
        )
...
def test_stop_writes_planned_stop_marker_before_killing(monkeypatch):
    ...
    monkeypatch.setattr(
        "openagents_cli.gateway._get_restart_drain_timeout", lambda: 5.0
    )
