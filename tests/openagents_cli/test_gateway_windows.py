"""Tests for openagents_cli.gateway_windows."""
...
import openagents_cli.setup as setup
...
def test_build_gateway_argv_uses_base_pythonw_for_uv_venv_launcher(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("openagents_cli.config.get_openagents_home", lambda: str(hermes_home))

    argv, cwd, env_overlay = gateway_windows._build_gateway_argv()

    assert argv[:3] == [str(base_pythonw), "-m", "openagents_cli.main"]
...
class TestStableWindowsGatewayWorkingDir:
    def test_stable_gateway_working_dir_uses_hermes_home(self, tmp_path, monkeypatch):
        ...
        home.mkdir()
        monkeypatch.setattr("openagents_cli.config.get_openagents_home", lambda: home)
    ...
    def test_stable_gateway_working_dir_falls_back_to_project_root(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("openagents_cli.config.get_openagents_home", lambda: missing)
...
def test_gateway_cmd_script_uses_pythonw_without_replace_or_start_churn(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda exe: (exe.replace("python.exe", "pythonw.exe"), r"C:\\Hermes\\openagents\\venv", []),
    )

    content = gateway_windows._build_gateway_cmd_script(
        r"C:\\Hermes\\openagents\\venv\\Scripts\\python.exe",
        r"C:\\Hermes\\openagents",
...
def test_gateway_vbs_script_is_console_less(monkeypatch):
    ...
    assert "pythonw.exe" in content
    assert "openagents_cli.main" in content
    assert "gateway run" in content
    assert ", 0, False" in content  # hidden window, detached/async
    for var in ("OPENAGENTS_HOME", "PYTHONIOENCODING", "HERMES_GATEWAY_DETACHED", "VIRTUAL_ENV", "PYTHONPATH"):
...
def test_install_access_denied_launches_elevated_install_before_startup_fallback(monkeypatch, tmp_path, capsys):
    ...
    assert "Launched elevated OpenAgents gateway install prompt" in out
...
def test_uninstall_access_denied_prompts_before_elevating(monkeypatch, tmp_path, capsys):
    ...
    assert "UAC is Windows' admin approval prompt" in out
    assert "Launched elevated OpenAgents gateway uninstall prompt" in out
...
def test_stop_writes_planned_stop_marker_before_killing(monkeypatch):
    ...
    monkeypatch.setattr("openagents_cli.gateway.kill_gateway_processes", fake_kill)
    monkeypatch.setattr("openagents_cli.gateway._get_restart_drain_timeout", lambda: 5.0)
