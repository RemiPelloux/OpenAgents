...
def test_gui_installs_packages_and_launches_desktop_app(tmp_path, monkeypatch):
    ...
    with (
        patch("openagents_cli.main.shutil.which", return_value="/usr/bin/npm"),
        patch(
            "openagents_cli.main._run_npm_install_deterministic",
        ...
        patch(
            "openagents_cli.main.subprocess.run", side_effect=[pack_ok, launch_ok]
        ) as mock_run,
...
def test_gui_exits_when_npm_missing(tmp_path, monkeypatch, capsys):
    ...
    with (
        patch("openagents_cli.main.shutil.which", return_value=None),
        pytest.raises(SystemExit) as exc,
    ):
        cli_main.cmd_gui(_ns())
...
def test_gui_linux_configures_sandbox_before_launch(tmp_path, monkeypatch):
    ...
    with (
        patch("openagents_cli.main.shutil.which", return_value="/usr/bin/sudo"),
...
def test_gui_linux_falls_back_to_no_sandbox_when_userns_is_restricted(
    ...
    with (
        patch("openagents_cli.main._desktop_linux_sandbox_fixup", return_value=False),
...
def test_gui_linux_exits_when_sandbox_fixup_fails_without_safe_fallback(
    ...
    with (
        patch("openagents_cli.main._desktop_linux_sandbox_fixup", return_value=False),
        patch(
            "openagents_cli.main._desktop_linux_needs_no_sandbox", return_value=False
        ),
        patch("openagents_cli.main.subprocess.run") as mock_run,
        pytest.raises(SystemExit) as exc,
    ):
...
def test_gui_source_mode_uses_renderer_build_and_electron(tmp_path, monkeypatch):
    ...
    with (
        ...
        patch("openagents_cli.main._write_desktop_build_stamp"),
        patch(
            "openagents_cli.main.subprocess.run", side_effect=[build_ok, launch_ok]
        ) as mock_run,
        pytest.raises(SystemExit) as exc,
    ):
...
def test_gui_retries_pack_once_after_purging_build_cache(tmp_path, monkeypatch):
    ...
    packaged_exe = (
        root / "apps" / "desktop" / "release" / "linux-unpacked" / "openagents"
    )
    ...
    with (
        ...
        patch("openagents_cli.main._write_desktop_build_stamp"),
        patch(
            "openagents_cli.main._purge_electron_build_cache",
        ...
        patch(
            "openagents_cli.main.subprocess.run", side_effect=run_side_effect
        ) as mock_run,
        pytest.raises(SystemExit) as exc,
    ):
        cli_main.cmd_gui(_ns())
...
def test_gui_redownloads_electron_via_mirror_then_repacks(
    ...
    with (
        ...
        patch("openagents_cli.main._electron_dist_ok", return_value=False),
        patch(
            "openagents_cli.main._redownload_electron_dist", side_effect=[False, True]
        ) as mock_dl,
        patch(
            "openagents_cli.main.subprocess.run", side_effect=[pack_fail, pack_fail]
        ) as mock_run,
        pytest.raises(SystemExit) as exc,
    ):
        cli_main.cmd_gui(_ns())
...
def test_gui_install_failure_self_heals_electron_and_continues(
    ...
    with (
        ...
        patch("openagents_cli.main._electron_dist_ok", return_value=False),
        patch(
            "openagents_cli.main._try_redownload_electron_dist", return_value=True
        ) as mock_dl,
        patch(
            "openagents_cli.main.subprocess.run", side_effect=[pack_ok, launch_ok]
        ) as mock_run,
...
def test_gui_does_not_retry_after_packaged_executable_exists(
    ...
    with (
        ...
        patch(
            "openagents_cli.main._redownload_electron_dist", return_value=True
        ) as mock_dl,
...
def test_gui_does_not_override_user_electron_mirror(tmp_path, monkeypatch, capsys):
    ...
    with (
        ...
        patch("openagents_cli.main._desktop_macos_relaunchable_fixup"),
        patch(
            "openagents_cli.main._purge_electron_build_cache", return_value=[]
        ) as mock_purge,
        patch(
            "openagents_cli.main.subprocess.run", side_effect=[pack_fail]
        ) as mock_run,
        pytest.raises(SystemExit) as exc,
    ):
        cli_main.cmd_gui(_ns())
...
def test_redownload_electron_dist_missing_installer(tmp_path, monkeypatch):
    ...
    with (
        patch("openagents_cli.main.shutil.which", return_value="/usr/bin/node"),
        patch("openagents_cli.main.subprocess.run") as mock_run,
    ):
...
def test_redownload_electron_dist_returns_false_when_download_fails(
    ...
    with (
        patch("openagents_cli.main.shutil.which", return_value="/usr/bin/node"),
        patch(
            "openagents_cli.main.subprocess.run",
