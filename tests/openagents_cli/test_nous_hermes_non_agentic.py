...
``"openagents"`` anywhere (case-insensitive). That false-positived on unrelated
local Modelfiles such as ``hermes-brain:qwen3-14b-ctx16k`` — a tool-capable
Qwen3 wrapper that happens to live under the "openagents" tag namespace.
...
def test_matches_real_nous_hermes_chat_models(model_name: str) -> None:
    assert is_nous_hermes_non_agentic(model_name), (
        f"expected {model_name!r} to be flagged as Nous OpenAgents 3/4"
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "deepseek-chat",
        # Non-chat OpenAgents models we don't warn about
        "hermes-llm-2",
        "hermes2-pro",
        "nous-hermes-2-mistral",
        # Edge cases
        "",
        "openagents",  # bare "openagents" isn't the 3/4 family
        "hermes-brain",
...
def test_does_not_match_unrelated_models(model_name: str) -> None:
    assert not is_nous_hermes_non_agentic(model_name), (
        f"expected {model_name!r} NOT to be flagged as Nous OpenAgents 3/4"
    )
