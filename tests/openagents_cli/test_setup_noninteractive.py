...
import pytest
from openagents_cli.config import DEFAULT_CONFIG, load_config, save_config
...
class TestNonInteractiveSetup:
    ...
    def test_cmd_setup_allows_noninteractive_flag_without_tty(self):
        ...
        from openagents_cli.main import cmd_setup

        args = _make_setup_args(non_interactive=True)

        with (
            patch("openagents_cli.setup.run_setup_wizard") as mock_run_setup,
    ...
    def test_chat_first_run_headless_skips_setup_prompt(self, capsys):
        ...
        from openagents_cli.main import cmd_chat
    ...
    def test_main_accepts_tts_setup_section(self, monkeypatch):
        ...
        monkeypatch.setattr("sys.argv", ["openagents", "setup", "tts"])
