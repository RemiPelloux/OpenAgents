"""Tests for openagents_cli.uninstall.remove_node_symlinks.

Regression for #34536: the POSIX installer drops node/npm/npx symlinks in
~/.local/bin pointing into $OPENAGENTS_HOME/node and prepends ~/.local/bin to
...
def _make_hermes_node(hermes_home: Path) -> Path:
    """Create a fake $OPENAGENTS_HOME/node/bin/{node,npm,npx} tree."""
...
def test_removes_dangling_symlink_into_hermes_node(fake_home):
    """A link into the OpenAgents node dir is removed even if the target file is
...
def test_only_some_links_present(fake_home):
    """Removes the OpenAgents links that exist; ignores the ones that don't."""
    hermes_home = fake_home / ".hermes"
    node_bin = _make_hermes_node(hermes_home)
    local_bin = fake_home / ".local" / "bin"
