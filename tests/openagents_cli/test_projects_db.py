"""Tests for the per-profile Projects store (openagents_cli/projects_db)."""
...
def test_create_get_list(conn):
    pid = pdb.create_project(conn, name="OpenAgents", folders=["/tmp/hermes"])
    proj = pdb.get_project(conn, pid)

    assert proj is not None
    assert proj.slug == "openagents"
    assert proj.name == "OpenAgents"
    ...
    # Lookup by slug too.
    assert pdb.get_project(conn, "openagents").id == pid
    assert len(pdb.list_projects(conn)) == 1


def test_slug_collision_disambiguates(conn):
    pdb.create_project(conn, name="OpenAgents")
    pdb.create_project(conn, name="OpenAgents")
    slugs = sorted(p.slug for p in pdb.list_projects(conn))

    assert slugs == ["openagents", "openagents-2"]
...
def test_per_profile_isolation(tmp_path):
    # Two distinct DB paths stand in for two profiles' OPENAGENTS_HOME.
...
def test_db_path_under_hermes_home():
    # Resolves under OPENAGENTS_HOME (set by the autouse isolation fixture).
