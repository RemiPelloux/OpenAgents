...
openagents_cli.config._load_config_impl, so the managed-scope merge has to be
...
def _load_cli_config(home):
    ...
    cli.py binds ``_hermes_home = get_openagents_home()`` at import time (module
    singleton), so monkeypatching OPENAGENTS_HOME after import doesn't move it.
    ...
    this only matters for tests that swap OPENAGENTS_HOME.)
    """
    import cli
