...
files. In OpenAgents the equivalent isolation is:

* ``--ignore-user-config`` → skip ``~/.openagents/config.yaml`` in ``load_cli_config()``
...
class TestCmdChatWiring:
    """The wiring inside ``cmd_chat()`` in ``openagents_cli/main.py`` must set
...
class TestArgparseFlagsRegistered:
    ...
    def test_main_py_registers_both_flags(self):
        ...
        import inspect
        import openagents_cli.main as hm
