"""Tests for openagents_cli.web_server and related config utilities."""
...
def _install_example_plugin(_isolate_hermes_home):
    """Drop the example-dashboard fixture into the per-test OPENAGENTS_HOME
    ...
    from openagents_cli import web_server
    ...
    #      and start serving requests against a torn-down OPENAGENTS_HOME.
    app = web_server.app
...
class TestReloadEnv:
    ...
    def test_removes_deleted_known_vars(self, tmp_path):
        """reload_env() removes known OpenAgents vars not present in .env."""
        env_file = tmp_path / ".env"
...
class TestSessionTokenInjection:
    ...
    def test_honors_injected_token(self, monkeypatch):
        import importlib
        import openagents_cli.web_server as ws
...
class TestWebServerEndpoints:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_hermes_home):
        """Create a TestClient and isolate the state DB under the test OPENAGENTS_HOME."""
        try:
    ...
    def test_get_status_hides_update_capability_in_managed_runtime(self, monkeypatch):
        import openagents_cli.web_server as web_server
    ...
    def test_dashboard_update_capability_detects_generic_container(self, monkeypatch):
        ...
        monkeypatch.setattr(openagents_constants, "is_container", lambda: True)
    ...
    def test_get_memory_provider_config_returns_safe_defaults(self):
        ...
        assert fields["bank_id"]["value"] == "openagents"
    ...
    def test_put_memory_provider_config_writes_config_and_secret(self):
        from openagents_constants import get_openagents_home
        from openagents_cli.config import load_config, load_env
        ...
        config_path = get_openagents_home() / "hindsight" / "config.json"
    ...
    def test_get_media_404_for_missing_file(self):
        ...
        missing = get_openagents_home() / "images" / "nope.png"
        resp = self.client.get("/api/media", params={"path": str(missing)})
        assert resp.status_code == 404

    def test_get_media_requires_auth(self):
        from openagents_cli.web_server import _SESSION_HEADER_NAME
    ...
    def test_get_sessions_forwards_min_messages(self, monkeypatch):
        ...
        monkeypatch.setattr("openagents_state.SessionDB", _FakeDB)
    ...
    def test_desktop_audio_routes_registered(self):
        ...
        without failing CI.
        """
        from openagents_cli.web_server import app
    ...
    def test_update_hermes_returns_docker_guidance_without_spawning(self, monkeypatch):
        ...
        assert "docker pull nousresearch/openagents:latest" in data["message"]
        ...
        assert any("docker pull nousresearch/openagents:latest" in line for line in status_data["lines"])
    ...
    def test_get_env_vars_marks_channel_managed_keys(self):
        from openagents_cli.web_server import _channel_managed_env_keys
    ...
    def test_platform_scoped_messaging_env_vars_are_channel_managed(self):
        from openagents_cli.web_server import (
    ...
    def test_model_set_requires_confirmation_for_expensive_model(self, monkeypatch):
        monkeypatch.setattr(
            "openagents_cli.model_cost_guard.expensive_model_warning",
    ...
    def test_model_set_maps_unknown_vendor_to_aggregator(self, monkeypatch):
        ...
        a OpenAgents provider — keep the user's aggregator instead of writing a
    ...
    def test_ops_backup_defaults_to_dashboard_downloadable_archive(self, monkeypatch):
        ...
        assert archive.parent == get_openagents_home() / "backups"
    ...
    def test_ops_backup_uses_hosted_hermes_home(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("OPENAGENTS_HOME", str(hosted_home))
        captured = {}
    ...
    def test_reveal_env_var(self, tmp_path):
        ...
        from openagents_cli.web_server import _SESSION_HEADER_NAME, _SESSION_TOKEN
    ...
    def test_reveal_env_var_legacy_authorization_header_still_works(self, tmp_path):
        ...
        from openagents_cli.config import save_env_value
        from openagents_cli.web_server import _SESSION_TOKEN
    ...
    def test_weixin_messaging_metadata_describes_personal_ilink_setup(self):
        ...
        assert weixin["docs_url"] == (
            "https://openagents.nousresearch.com/docs/user-guide/messaging/weixin/"
        )
    ...
    def test_teams_messaging_metadata_links_setup_guide(self):
        ...
        from openagents_cli.web_server import _build_catalog_entry

        teams = _build_catalog_entry("teams")
        assert teams["docs_url"] == (
            "https://openagents.nousresearch.com/docs/user-guide/messaging/teams"
        )
    ...
    def test_google_chat_messaging_metadata_links_setup_guide(self):
        ...
        assert google_chat["docs_url"] == (
            "https://openagents.nousresearch.com/docs/user-guide/messaging/google_chat"
        )
    ...
    def test_update_messaging_platform_saves_slack_allowed_users(self):
        from openagents_cli.config import load_env
    ...
    def test_telegram_onboarding_worker_request_uses_httpx(self, monkeypatch):
        ...
        assert kwargs["json"] == {"bot_name": "OpenAgents"}
    ...
    def test_path_traversal_dotdot_blocked(self):
        ...
        resp = self.client.get("/%2e%2e/openagents_cli/web_server.py")
        assert resp.status_code in {200, 404}
        if resp.status_code == 200:
    ...
    def test_set_model_main_nous_applies_gateway_defaults(self, monkeypatch):
        ...
        import openagents_cli.nous_subscription as ns
    ...
    def test_apply_main_model_assignment_base_url_and_context_reconcile(self):
        ...
        from openagents_cli.web_server import _apply_main_model_assignment
    ...
    def test_parse_model_ids_handles_openai_and_bare_shapes(self):
        ...
        from openagents_cli.web_server import _parse_model_ids
    ...
    def test_recommended_default_nous_honors_free_tier(self, monkeypatch):
        ...
        import openagents_cli.models as models_mod
...
class TestBuildSchemaFromConfig:
    def test_produces_expected_field_count(self):
        from openagents_cli.web_server import CONFIG_SCHEMA
    ...
    def test_empty_prefix_produces_correct_keys(self):
        from openagents_cli.web_server import _build_schema_from_config
...
class TestConfigRoundTrip:
    ...
    def test_round_trip_preserves_schema_invisible_nested_keys(self):
        ...
        from openagents_cli.config import load_config, read_raw_config, save_config
...
class TestNewEndpoints:
    ...
    def test_profiles_list_includes_default(self):
        from openagents_constants import get_openagents_home
        get_openagents_home().mkdir(parents=True, exist_ok=True)
    ...
    def test_profiles_list_falls_back_when_profile_listing_fails(self, monkeypatch):
        from openagents_constants import get_openagents_home
        import openagents_cli.profiles as profiles_mod
    ...
    def test_profile_setup_command_uses_named_profile_wrapper(self):
        ...
        (get_openagents_home() / "profiles" / "coder").mkdir(parents=True)
    ...
    def test_profiles_create_with_clone_from_copies_source_skills(self, monkeypatch):
        ...
        (get_openagents_home() / "config.yaml").write_text(
            "model:\n  provider: openrouter\n",
            encoding="utf-8",
        )
        default_skill = get_openagents_home() / "skills" / "custom" / "new-skill"
        ...
        assert resp.status_code == 200
        cloned_root = get_openagents_home() / "profiles" / "cloned"
    ...
    def test_profiles_create_with_clone_from_duplicates_source(self, monkeypatch):
        ...
        source_skill = get_openagents_home() / "profiles" / "source-prof" / "skills" / "custom" / "src-skill"
        ...
        assert resp.status_code == 200
        cloned_skill = (
            get_openagents_home() / "profiles" / "source-prof-copy" / "skills" / "custom" / "src-skill" / "SKILL.md"
        )
        assert cloned_skill.exists()
    ...
    def test_profiles_create_clone_all_from_named_source(self, monkeypatch):
        ...
        source_dir = get_openagents_home() / "profiles" / "full-src"
        ...
        assert resp.status_code == 200
        target_dir = get_openagents_home() / "profiles" / "full-copy"
    ...
    def test_profiles_create_without_clone_seeds_bundled_skills(self, monkeypatch):
        ...
        assert resp.status_code == 200
        seeded_skill = get_openagents_home() / "profiles" / "fresh" / "skills" / "software-development" / "plan" / "SKILL.md"
    ...
    def test_profiles_create_builder_fields_model_mcp_and_keep_skills(self, monkeypatch):
        ...
        prof_dir = get_openagents_home() / "profiles" / "builder"
        token = set_openagents_home_override(str(prof_dir))
        try:
            cfg = load_config()
    ...
    def test_profile_model_round_trip(self, monkeypatch):
        ...
        import yaml
        cfg_path = get_openagents_home() / "profiles" / "model-prof" / "config.yaml"
    ...
    def test_skills_list_includes_disabled_skills(self, monkeypatch):
        ...
        import openagents_cli.skills_config as skills_config
        import openagents_cli.web_server as web_server
    ...
    def test_toolsets_list_matches_cli_enabled_state(self, monkeypatch):
        import openagents_cli.tools_config as tools_config
...
class TestModelContextLength:
    ...
    def test_normalize_extracts_context_length_from_dict(self):
        ...
        from openagents_cli.web_server import _normalize_config_for_web
    ...
    def test_denormalize_writes_context_length_into_model_dict(self):
        ...
        from openagents_cli.web_server import _denormalize_config_from_web
...
class TestNormaliseThemeDefinition:
    ...
    def test_rejects_missing_name(self):
        from openagents_cli.web_server import _normalise_theme_definition
...
class TestDiscoverUserThemes:
    """Tests for _discover_user_themes() — scans ~/.openagents/dashboard-themes/."""
...
class TestBulkDeleteSessionsEndpoint:
    ...
    1. Route-ordering: ``/api/sessions/bulk-delete`` must shadow the
       ...
       the block comment in ``openagents_cli/web_server.py``).
    ...
    def test_route_order_not_shadowed_by_session_id(self):
        ...
        assert "deleted" in body, (
            ...
            "registration order in openagents_cli/web_server.py."
        )
...
class TestDeleteEmptySessionsEndpoint:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_hermes_home):
        ...
        # Pin the SessionDB to the isolated OPENAGENTS_HOME so each test
...
class TestPluginAPIAuth:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_hermes_home, _install_example_plugin):
        ...
        installs it into the per-test ``OPENAGENTS_HOME``.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
    ...
    def test_plugin_route_allows_auth(self):
        ...
        test fixture (installed into OPENAGENTS_HOME by the class-level
...
class TestPtyWebSocket:
    ...
    def test_resolve_chat_argv_uses_dashboard_scroll_env(self, monkeypatch):
        ...
        import openagents_cli.main as main_mod
    ...
    def test_unavailable_platform_closes_with_message(self, monkeypatch):
        from openagents_cli.pty_bridge import PtyUnavailableError
        ...
        import openagents_cli.web_server as ws_mod
    ...
    def test_pub_broadcasts_to_events_subscribers(self):
        ...
        """
        import asyncio
        from openagents_cli import web_server as ws_mod
...
class TestDashboardPluginStaticAssetAllowlist:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_hermes_home, _install_example_plugin):
        ...
        the per-test ``OPENAGENTS_HOME`` user-plugins dir.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
