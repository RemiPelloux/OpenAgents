...
The guard fires when a tool tries to write into another OpenAgents profile's
...
accidentally edited skills under both ~/.openagents/profiles/hermes-security/skills/
AND ~/.openagents/skills/ (the default profile's skills), realizing only
...
# ---------------------------------------------------------------------------
# Helpers — set up a fake OpenAgents root with two profiles, monkeypatch the
...
def fake_hermes(tmp_path, monkeypatch):
    """Build a fake OpenAgents layout:
    ...
    monkeypatch.setattr(openagents_constants, "get_default_openagents_root", lambda: root)
...
class TestResolveActiveProfileName:
    ...
    def test_falls_back_to_default_on_resolution_failure(self, fake_hermes, monkeypatch):
        """If OPENAGENTS_HOME resolution raises, return 'default' rather than crashing the tool."""
        import agent.file_safety as fs
...
class TestClassifyCrossProfileTarget:
    ...
    def test_non_hermes_path_returns_none(self, fake_hermes, monkeypatch, tmp_path):
        _set_active_home(monkeypatch, fake_hermes["security_home"])
        from agent.file_safety import classify_cross_profile_target
        # Path outside any OpenAgents root
