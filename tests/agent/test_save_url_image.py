...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate OPENAGENTS_HOME under tmp_path."""
    monkeypatch.setenv("OPENAGENTS_HOME", str(tmp_path / ".hermes"))
    (tmp_path / ".hermes").mkdir()

    # Force the constants/image cache helpers to re-read OPENAGENTS_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("openagents_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_hermes_home_cache(self, http_server):
        ...
        # The cache directory must be under OPENAGENTS_HOME — gateway cleanup
