...
class TestMultiplexActiveFailClosed:
    ...
    def test_global_env_still_reads_environ_under_multiplex(self, monkeypatch):
        monkeypatch.setenv("OPENAGENTS_HOME", "/opt/data")
        ss.set_multiplex_active(True)
        # No scope, multiplex on — but OPENAGENTS_HOME is global, so no raise.
        assert ss.get_secret("OPENAGENTS_HOME") == "/opt/data"
