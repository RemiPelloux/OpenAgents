"""
Tests for timezone support (openagents_time module + integration points).
...
import openagents_time


def _reset_openagents_time_cache():
    """Reset the openagents_time module cache (replacement for removed reset_cache)."""
    openagents_time._cached_tz = None
    openagents_time._cached_tz_name = None
    openagents_time._cache_resolved = False


# =========================================================================
# openagents_time.now() — core helper
# =========================================================================
...
class TestHermesTimeNow:
    ...
    def test_invalid_timezone_falls_back(self, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="openagents_time"):
            result = openagents_time.now()
    ...
    def test_cache_invalidation(self):
        ...
        os.environ["HERMES_TIMEZONE"] = "UTC"
        _reset_openagents_time_cache()
        r1 = openagents_time.now()
        assert r1.utcoffset() == timedelta(0)

        os.environ["HERMES_TIMEZONE"] = "Asia/Kolkata"
        _reset_openagents_time_cache()
        r2 = openagents_time.now()
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo_for_valid(self):
        ...
        tz = openagents_time.get_timezone()
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        absolute time when system-local tz != OpenAgents tz.  The fix interprets
    ...
    def test_ensure_aware_normalizes_aware_to_hermes_tz(self):
        """Already-aware datetimes should be normalized to OpenAgents tz."""
        from cron.jobs import _ensure_aware
        ...
        # Must be in OpenAgents tz (Kolkata) but same absolute instant
    ...
    def test_ensure_aware_due_job_not_skipped_when_system_ahead(
        ...
        """Reproduce the actual bug: system tz ahead of OpenAgents tz caused
        overdue jobs to appear as not-yet-due.

        Scenario: system is Asia/Kolkata (UTC+5:30), OpenAgents is UTC.
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        """Naive past timestamps must be detected as due even when OpenAgents tz
        ...
        # Use a OpenAgents timezone far behind UTC so that the numeric wall time
        ...
        due = get_due_jobs()
        assert len(due) == 1, (
            "Naive past timestamp should be due regardless of OpenAgents timezone"
        )
