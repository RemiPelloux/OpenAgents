...
This validates the IPC + lifecycle story that mocks can't:
  ...
  - the child process resolves openagents_cli.kanban_db on its own
  ...
  - worker logs are captured to OPENAGENTS_HOME/kanban/logs/<task>.log
...
def main():
    ...
    # openagents_cli.main. We do this by putting a shim on PATH.
    shim_dir = os.path.join(home, "bin")
    os.makedirs(shim_dir, exist_ok=True)
    shim_path = os.path.join(shim_dir, "openagents")
    with open(shim_path, "w") as f:
        f.write(f"""#!/bin/sh
exec {PY} -m openagents_cli.main "$@"
""")
