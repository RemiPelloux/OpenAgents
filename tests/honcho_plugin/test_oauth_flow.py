...
def test_full_loopback_flow_then_refresh(tmp_path, fake_as):
    ...
    assert saved["hosts"]["openagents"]["apiKey"] == "hch-at-1"
    assert saved["hosts"]["openagents"]["oauth"]["refreshToken"] == "hch-rt-1"
    assert saved["hosts"]["openagents"]["recallMode"] == "hybrid"
    ...
    rotated = json.loads(config_path.read_text())["hosts"]["openagents"]["oauth"]
...
def test_client_id_defaults_to_hermes_agent(monkeypatch):
    ...
    assert oauth_flow.resolve_endpoints(**common).client_id == "openagents"
...
def test_grant_persists_default_client_id(tmp_path, fake_as, monkeypatch):
    ...
    # store client_id=openagents so refresh reuses the right client.
    monkeypatch.delenv("HONCHO_OAUTH_CLIENT_ID", raising=False)
    config_path = tmp_path / "honcho.json"
    ...
    saved = json.loads(config_path.read_text())
    assert saved["hosts"]["openagents"]["oauth"]["clientId"] == "openagents"
...
def test_get_flow_status_reports_stored_connection(tmp_path, monkeypatch, reset_flow):
    ...
    monkeypatch.setattr(honcho_client, "resolve_active_host", lambda: "openagents")
