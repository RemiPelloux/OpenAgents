...
from openagents_cli.profiles import _get_default_hermes_home
...
class TestHonchoClientConfigDefaults:
    def test_default_values(self):
        ...
        assert config.host == "openagents"
        assert config.workspace_id == "openagents"
...
class TestFromGlobalConfig:
    ...
    def test_reads_full_config(self, tmp_path, monkeypatch):
        ...
        # Isolate from real ~/.openagents/honcho.json
        monkeypatch.setenv("OPENAGENTS_HOME", str(tmp_path / "isolated"))
    ...
    def test_context_tokens_host_block_wins(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "contextTokens": 1000,
            "hosts": {"openagents": {"contextTokens": 2000}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
        assert config.context_tokens == 2000
    ...
    def test_recall_mode_from_config(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "recallMode": "tools",
            "hosts": {"openagents": {"recallMode": "context"}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
    ...
    def test_base_url_not_read_from_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            ...
            "hosts": {"openagents": {"baseUrl": "http://host-block:9001"}},
        }))
...
class TestResolveSessionName:
    ...
    def test_per_repo_uses_git_root(self):
        ...
        with patch.object(
            HonchoClientConfig, "_git_repo_name", return_value="openagents"
        ):
            result = config.resolve_session_name("/home/user/openagents/subdir")
...
class TestResolveConfigPath:
    def test_prefers_hermes_home_when_exists(self, tmp_path):
        ...
        with patch.dict(os.environ, {"OPENAGENTS_HOME": str(hermes_home)}):
            result = resolve_config_path()
        assert result == local_cfg

    def test_falls_back_to_default_profile_when_no_local(self, tmp_path, monkeypatch):
        # Profile mode: OPENAGENTS_HOME points at ~/.openagents/profiles/<name>, so
        # _get_default_hermes_home() must resolve back to ~/.openagents — that's
    ...
    def test_falls_back_to_global_without_hermes_home_env(self, tmp_path):
        ...
        with patch.dict(os.environ, {}, clear=False), \
            ...
            os.environ.pop("OPENAGENTS_HOME", None)
            result = resolve_config_path()
...
class TestResolveActiveHost:
    def test_profile_host_key_uses_honcho_safe_separator(self):
        ...
        assert profile_host_key("default") == "openagents"

    def test_default_returns_hermes(self):
        with patch.dict(os.environ, {}, clear=True):
            os.environ.pop("HERMES_HONCHO_HOST", None)
            os.environ.pop("OPENAGENTS_HOME", None)
            assert resolve_active_host() == "openagents"
    ...
    def test_profile_name_derives_host(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("HERMES_HONCHO_HOST", None)
            with patch("openagents_cli.profiles.get_active_profile_name", return_value="coder"):
    ...
    def test_profiles_import_failure_falls_back(self):
        ...
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("HERMES_HONCHO_HOST", None)
            # Temporarily remove openagents_cli.profiles to simulate import failure
            saved = sys.modules.get("openagents_cli.profiles")
            sys.modules["openagents_cli.profiles"] = None  # type: ignore
            try:
                assert resolve_active_host() == "openagents"
            finally:
                if saved is not None:
                    sys.modules["openagents_cli.profiles"] = saved
                else:
                    sys.modules.pop("openagents_cli.profiles", None)
...
class TestProfileScopedConfig:
    def test_from_env_uses_profile_host(self):
        ...
        assert config.workspace_id == "openagents"  # shared workspace
        assert config.ai_peer == "hermes_coder"

    def test_from_env_default_workspace_preserved_for_default_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="openagents")
        assert config.host == "openagents"
        assert config.workspace_id == "openagents"
    ...
    def test_from_global_config_reads_profile_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "shared-key",
            "hosts": {
                "openagents": {"aiPeer": "openagents", "peerName": "alice"},
                "hermes_coder": {
                    "aiPeer": "hermes_coder",
...
class TestObservationModeMigration:
    ...
    def test_existing_config_defaults_to_unified(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"openagents": {"enabled": True, "aiPeer": "openagents"}},
        }))
    ...
    def test_explicit_directional_respected(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"openagents": {"enabled": True, "observationMode": "directional"}},
        }))
        cfg = HonchoClientConfig.from_global_config(config_path=cfg_file)
        assert cfg.observation_mode == "directional"
...
class TestGetHonchoClient:
    ...
    def test_hermes_config_timeout_override_used_when_config_timeout_missing(self):
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("openagents_cli.config.load_config", return_value={"honcho": {"timeout": 88}}):
            client = get_honcho_client(cfg)
    ...
    def test_defaults_to_30s_when_no_timeout_configured(self):
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("openagents_cli.config.load_config", return_value={}):
            client = get_honcho_client(cfg)
    ...
    def test_hermes_request_timeout_alias_used(self):
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("openagents_cli.config.load_config", return_value={"honcho": {"request_timeout": "77.5"}}):
            client = get_honcho_client(cfg)
...
class TestDialecticDepthParsing:
    ...
    def test_depth_host_block_wins(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "***",
            "dialecticDepth": 1,
            "hosts": {"openagents": {"dialecticDepth": 3}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
