...
Under a gateway (Telegram, Discord, Slack, ...) OpenAgents passes the
...
class TestCrossPlatformMemoryUnification:
    """The same physical user talking to OpenAgents via Telegram AND Discord
    ...
    def test_host_pinUserPeer_wins_over_root_pinPeerName(self, tmp_path):
        ...
        config_file.write_text(
            json.dumps({
                ...
                "peerName": "eri",
                "pinPeerName": False,
                "hosts": {"openagents": {"pinUserPeer": True}},
            })
        )
        config = HonchoClientConfig.from_global_config(config_path=config_file)
        assert config.pin_peer_name is True
    ...
    def test_host_pinUserPeer_false_disables_root_pinPeerName(self, tmp_path):
        ...
        config_file.write_text(
            json.dumps({
                ...
                "peerName": "eri",
                "pinPeerName": True,
                "hosts": {"openagents": {"pinUserPeer": False}},
            })
        )
        config = HonchoClientConfig.from_global_config(config_path=config_file)
        assert config.pin_peer_name is False, (
    ...
    def test_pinPeerName_still_works_unchanged(self, tmp_path):
        ...
        config_file.write_text(
            json.dumps({
                "apiKey": "***",
                "peerName": "eri",
                "hosts": {"openagents": {"pinPeerName": True}},
            })
        )
        config = HonchoClientConfig.from_global_config(config_path=config_file)
        assert config.pin_peer_name is True
...
class TestProfilePeerUniqueness:
    """Each OpenAgents profile can pin to its own unique peerName.
