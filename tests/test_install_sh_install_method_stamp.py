...
($INSTALL_DIR), not into the shared $OPENAGENTS_HOME.

Background (shared-$OPENAGENTS_HOME bug)
------------------------------------
$OPENAGENTS_HOME is a data directory users frequently bind-mount into a Docker
gateway as well (``~/.openagents:/opt/data``). The published image stamps 'docker'
there on boot, so if install.sh had written its 'git' marker into the same
$OPENAGENTS_HOME the two installs would fight over one slot — and the container,
...
``~/.openagents/openagents``), which is unique to this install and immune to the
shared data dir.
"""
...
def test_install_sh_stamps_code_tree_not_home() -> None:
    ...
    assert not re.search(r'>\s*"\$OPENAGENTS_HOME/\.install_method"', text), (
        "install.sh must not stamp $OPENAGENTS_HOME/.install_method — that data "
