...
def test_pyproject_defines_termux_all_without_known_blockers() -> None:
    ...
    assert '"openagents[termux]"' in text
    assert '"openagents[matrix]"' not in text.split("termux-all = [", 1)[1].split("]", 1)[0]
    assert '"openagents[voice]"' not in text.split("termux-all = [", 1)[1].split("]", 1)[0]
