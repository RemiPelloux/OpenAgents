"""
Tests for mcp_serve — OpenAgents MCP server.
...
def _isolate_hermes_home(tmp_path, monkeypatch):
    """Redirect OPENAGENTS_HOME to a temp directory."""
    monkeypatch.setenv("OPENAGENTS_HOME", str(tmp_path))
    try:
        import openagents_constants

        monkeypatch.setattr(
            openagents_constants, "get_openagents_home", lambda: tmp_path
        )
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking openagents_state schema."""
