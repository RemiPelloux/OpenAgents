...
class TestExecuteToolCalls:
    ...
    def test_keyboard_interrupt_emits_cancelled_post_tool_hook(
        ...
        monkeypatch.setattr("openagents_cli.plugins.invoke_hook", _capture_hook)
        monkeypatch.setattr("openagents_cli.plugins.has_hook", lambda name: True)
...
class TestConcurrentToolExecution:
    ...
    def test_sequential_browser_type_callbacks_redact_api_key(self, agent):
        ...
        agent.tool_progress_callback = (
            lambda event, name, preview, args, **kw: progress.append((
                event,
                name,
                preview,
                args,
    ...
    def test_invoke_tool_agent_level_tool_emits_terminal_post_tool_hook(
        ...
        hook_calls = []
        monkeypatch.setattr(
            "openagents_cli.plugins.get_pre_tool_call_block_message",
    ...
    def test_sequential_agent_level_tool_execution_middleware_wraps_inline_dispatch(
        ...
        monkeypatch.setattr(
            "openagents_cli.plugins.get_plugin_manager", lambda: manager
        )
        monkeypatch.setattr(
            "openagents_cli.plugins.invoke_middleware",
...
class TestRunConversation:
    ...
    def test_ollama_small_runtime_context_fails_before_api_call(self, agent, caplog):
        ...
        assert "Ollama runtime context too small for OpenAgents tool use" in caplog.text
    ...
    def test_request_scoped_api_hooks_fire_for_each_api_call(self, agent):
        ...
        with (
            patch("run_agent.handle_function_call", return_value="search result"),
            patch(
                "openagents_cli.plugins.has_hook",
                side_effect=lambda name: name
                in {"pre_api_request", "post_api_request"},
            ),
            patch("openagents_cli.plugins.invoke_hook", side_effect=_record_hook),
            patch.object(agent, "_persist_session"),
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
        ):
    ...
    def test_api_request_error_hook_skips_payload_work_without_listener(
        ...
        monkeypatch.setattr("openagents_cli.plugins.has_hook", lambda name: False)
        monkeypatch.setattr("openagents_cli.plugins.invoke_hook", _invoke_hook)
    ...
    def test_request_scoped_api_hooks_skip_payload_work_without_listeners(
        ...
        monkeypatch.setattr("openagents_cli.plugins.has_hook", _has_hook)
        ...
        with (
            patch("openagents_cli.plugins.invoke_hook", return_value=[]),
            patch.object(agent, "_persist_session"),
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
        ):
    ...
    def test_stub_stall_mid_tool_call_recovers_within_3_retries(self, agent):
        ...
        from openagents_constants import PARTIAL_STREAM_STUB_ID
    ...
    def test_kanban_block_called_on_iteration_exhaustion(self, agent, monkeypatch):
        ...
        with (
            ...
            patch("openagents_cli.kanban_db._record_task_failure", mock_record_failure),
            patch("openagents_cli.kanban_db.connect", mock_connect),
            patch.object(agent, "_persist_session"),
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
        ):
...
class TestNousCredentialRefresh:
    ...
    def test_try_refresh_nous_client_credentials_rebuilds_client(
        ...
        monkeypatch.setattr(
            "openagents_cli.auth.resolve_nous_runtime_credentials", _fake_resolve
        )
...
class TestSystemPromptStability:
    ...
    def test_fresh_build_when_no_history(self, agent):
        ...
        assert "OpenAgents" in agent._cached_system_prompt
