...
Thank you for contributing to OpenAgents! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — macOS, different Linux distros, and WSL2 on Windows. We want OpenAgents to work everywhere.
...
Related: #38284 covers the agent-side analog — OpenAgents itself checking existing issues and PRs before deep self-troubleshooting. This section is the human-contributor complement.
...
Bundled skills (in `skills/`) ship with every OpenAgents install. They should be **broadly useful to most users**:
...
The installer creates the OpenAgents venv, wires the `hermes` command, stamps the
install method for `hermes update`, and clones the full git project into
`$OPENAGENTS_HOME/openagents` (usually `~/.openagents/openagents`). That keeps your
...
Use this only if you intentionally do not want OpenAgents' managed install layout
...
```bash
mkdir -p ~/.openagents/{cron,sessions,logs,memories,skills}
cp cli-config.yaml.example ~/.openagents/config.yaml
touch ~/.openagents/.env

# Add at minimum an LLM provider key:
echo "OPENROUTER_API_KEY=***" >> ~/.openagents/.env
```
...
### User configuration (stored in `~/.openagents/`)

| Path | Purpose |
|------|---------|
| `~/.openagents/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.openagents/.env` | API keys and secrets |
| `~/.openagents/auth.json` | OAuth credentials (Nous Portal) |
...
The user may skip setup and keep loading the skill. OpenAgents only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `hermes setup` or update `~/.openagents/.env` locally.
...
2. **Tools referenced in SKILL.md prose must be native OpenAgents tools or MCP servers the skill explicitly expects.** When the skill needs a capability, point at the proper tool by name in backticks: `` `terminal` ``, `` `web_extract` ``, `` `web_search` ``, `` `read_file` ``, `` `write_file` ``, `` `patch` ``, `` `search_files` ``, `` `vision_analyze` ``, `` `browser_navigate` ``, `` `delegate_task` ``, `` `image_generate` ``, `` `text_to_speech` ``, `` `cronjob` ``, `` `memory` ``, `` `skill_view` ``, `` `todo` ``, `` `execute_code` ``.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing OpenAgents tools (`web_extract`, `terminal`, `read_file`).
...
OpenAgents uses a data-driven skin system — no code changes needed to add a new skin.

**Option A: User skin (YAML file)**

Create `~/.openagents/skins/<name>.yaml`:
...
Add to `_BUILTIN_SKINS` dict in `openagents_cli/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.

**Activating:**
- CLI: `/skin mytheme` or set `display.skin: mytheme` in config.yaml
- Config: `display: { skin: mytheme }`

See `openagents_cli/skin_engine.py` for the full schema and existing skins as examples.

---

## Cross-Platform Compatibility

OpenAgents runs on Linux, macOS, and native Windows (plus WSL2). When writing code
...
16. **Two different quoting schemes in one command line.** `subprocess.run
    ...
    helpers and never cross them. See `openagents_cli/gateway_windows.py::
...
OpenAgents has terminal access. Security matters.
