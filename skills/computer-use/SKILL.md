...
OpenAgents drives [cua-driver](https://github.com/trycua/cua) under the hood
for the platform plumbing. The Hermes-side `computer_use` tool exposed
in this skill is a higher-level OpenAgents vocabulary; the raw cua-driver
...
OpenAgents intentionally keeps THIS skill focused on the Hermes-side
...
When `cua-driver skills install` autodetects OpenAgents (planned follow-up
