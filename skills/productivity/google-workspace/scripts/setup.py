#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for OpenAgents.
...
from _hermes_home import display_openagents_home, get_openagents_home

OPENAGENTS_HOME = get_openagents_home()
TOKEN_PATH = OPENAGENTS_HOME / "google_token.json"
CLIENT_SECRET_PATH = OPENAGENTS_HOME / "google_client_secret.json"
PENDING_AUTH_PATH = OPENAGENTS_HOME / "google_oauth_pending.json"
...
def _format_missing_scopes(missing_scopes: list[str]) -> str:
    ...
    return (
        ...
        f"{bullets}\n"
        "Run the Google Workspace setup again from this same OpenAgents profile to refresh consent."
    )
...
def install_deps():
    ...
    # Fallback: the interpreter has no pip (the OpenAgents Docker image's venv is
    ...
    print(
        "On environments without pip (e.g. Nix, or the OpenAgents Docker image's "
        "uv-managed venv), install the optional extra instead:"
    )
    print("  pip install 'openagents[google]'")
...
def store_client_secret(path: str):
    """Copy and validate client_secret.json to OpenAgents home."""
...
def exchange_auth_code(code: str):
    ...
    print(f"Profile-scoped token location: {display_openagents_home()}/google_token.json")
