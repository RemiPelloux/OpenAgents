...
OPENAGENTS_HOME_DIR="${OPENAGENTS_HOME:-$HOME/.hermes}"
HERMES_CFG="${OPENAGENTS_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure OpenAgents config has twozero_td MCP entry ──
if [[ ! -f "$HERMES_CFG" ]]; then
    echo -e " ${FAIL} OpenAgents config not found at ${HERMES_CFG}"
    manual_steps+=("Create ${HERMES_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$HERMES_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in OpenAgents config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to OpenAgents config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
    ...
    manual_steps+=("Restart OpenAgents session to pick up config change")
fi
