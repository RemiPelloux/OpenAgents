...
## 5. OpenAgents Configuration

### Config location

`$OPENAGENTS_HOME/config.yaml` (defaults to `~/.openagents/config.yaml` when `OPENAGENTS_HOME` is unset)
...
Restart the OpenAgents session for changes to take effect. The MCP connection is
