"""Durable NDJSON outbox for OpenRec — shared path with OpenAgents mesh producers."""
...
def drain_rec_outbox(max_items: int = 20) -> int:
    ...
    root = _outbox_dir()
    sent = 0
    for path in sorted((root / "pending").glob("*.json"))[:max_items]:
        ...
        dest = root / ("failed" if attempts >= MAX_ATTEMPTS else "pending") / path.name
