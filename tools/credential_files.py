...
def register_credential_file(
    ...
    *relative_path* is relative to ``OPENAGENTS_HOME`` (e.g. ``google_token.json``).
    ...
    The resolved host path must remain inside OPENAGENTS_HOME so that a malicious
    skill cannot declare ``required_credential_files: ['../../.ssh/id_rsa']``
    and exfiltrate sensitive host files into a container sandbox.
    """
    hermes_home = _resolve_hermes_home()

    # Reject absolute paths — they bypass the OPENAGENTS_HOME sandbox entirely.
    if os.path.isabs(relative_path):
        logger.warning(
            "credential_files: rejected absolute path %r (must be relative to OPENAGENTS_HOME)",
            relative_path,
        )
        return False
    ...
    # that traversal like ``../. ssh/id_rsa`` cannot escape OPENAGENTS_HOME.
...
# Each tuple is (new_subpath, old_name) matching openagents_constants.get_openagents_dir().
...
def get_cache_directory_mounts(
    ...
    ``get_openagents_dir()`` for backward compatibility with old directory layouts.
    """
    from openagents_constants import get_openagents_dir

    mounts: List[Dict[str, str]] = []
    for new_subpath, old_name in _CACHE_DIRS:
        host_dir = get_openagents_dir(new_subpath, old_name)
        if host_dir.is_dir():
