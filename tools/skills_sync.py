...
Copies bundled skills from the repo's skills/ directory into ~/.openagents/skills/
...
The manifest lives at ~/.openagents/skills/.bundled_manifest.
"""
...
from openagents_constants import get_bundled_skills_dir, get_openagents_home, get_optional_skills_dir
...
SKILLS_DIR = OPENAGENTS_HOME / "skills"
MANIFEST_FILE = SKILLS_DIR / ".bundled_manifest"

# Marker file written by `hermes profile create --no-skills` (named profiles)
# and by the installer's `--no-skills` flag (the default ~/.openagents profile).
# When present in OPENAGENTS_HOME, sync_skills() is a no-op so neither the
# installer, `hermes update`, nor a direct sync re-injects bundled skills.
# Delete the file to opt back in. Mirrors
# openagents_cli.profiles.NO_BUNDLED_SKILLS_MARKER (kept as a literal here to
...
def _read_suppressed_names() -> set:
    ...
    to reading ``~/.openagents/skills/.curator_suppressed`` directly if that import
...
def _compute_relative_dest(skill_dir: Path, bundled_dir: Path) -> Path:
    ...
    e.g., bundled/skills/mlops/axolotl -> ~/.openagents/skills/mlops/axolotl
    """
...
def sync_skills(quiet: bool = False) -> dict:
    """
    Sync bundled skills into ~/.openagents/skills/ using the manifest.

    Returns:
        dict with keys: copied (list), updated (list), skipped (int),
                        user_modified (list), cleaned (list), total_bundled (int)
    """
    # Opt-out: a profile (named or the default ~/.openagents) that wrote the
    ...
    if (OPENAGENTS_HOME / NO_BUNDLED_SKILLS_MARKER).exists():
        if not quiet:
    ...
    for skill_name, skill_src in bundled_skills:
        ...
        # (~/.openagents/skills/.curator_suppressed) is written when the curator
...
def _rmtree_writable(path: Path) -> None:
    ...
    # ``OPENAGENTS_HOME/skills/`` to prevent the catastrophic wipe of
    # ``~/.openagents/`` (``.env``, ``MEMORY.md``, ``kanban.db``, custom
    ...
    # path join, a missing ``OPENAGENTS_HOME`` default, a malicious
    # bundled-manifest entry, or a mid-flight exception that leaves a
    # stale path in scope — this guard turns the resulting
    # ``shutil.rmtree(~/.openagents)`` into a loud, recoverable ``ValueError``
    ...
    # ``OPENAGENTS_HOME``. Require a strict-child relationship so both escape
...
def set_bundled_skills_opt_out(enabled: bool) -> dict:
    ...
    When ``enabled`` is True, writes OPENAGENTS_HOME/.no-bundled-skills so the
    ...
    Returns:
        dict with keys: ok (bool), changed (bool), marker (str path),
                        message (str).
    """
    marker = OPENAGENTS_HOME / NO_BUNDLED_SKILLS_MARKER
    existed = marker.exists()
    try:
        if enabled:
            OPENAGENTS_HOME.mkdir(parents=True, exist_ok=True)
            marker.write_text(
...
def is_bundled_skills_opt_out() -> bool:
    ...
    return (OPENAGENTS_HOME / NO_BUNDLED_SKILLS_MARKER).exists()
...
if __name__ == "__main__":
    print("Syncing bundled skills into ~/.openagents/skills/ ...")
