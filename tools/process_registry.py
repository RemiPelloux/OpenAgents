...
CHECKPOINT_PATH = get_openagents_home() / "processes.json"
...
class ProcessRegistry:
    ...
    def _daemon_term_grace_seconds() -> float:
        ...
        """
        try:
            from openagents_cli.config import read_raw_config, cfg_get, DEFAULT_CONFIG
    ...
    def is_session_waiting(self, session_id: str) -> bool:
        ...
        Used by the goal-loop wait barrier (``openagents_cli.goals``) to support
    ...
    def request_close_terminal(self, session_id: str) -> dict:
        ...
        if sink is None:
            return {
                "status": "error",
                "error": "close_terminal is only available in the OpenAgents desktop app.",
            }
