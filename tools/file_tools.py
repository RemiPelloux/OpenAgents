...
def _expand_tilde(path: str) -> str:
    ...
    mirrors ``openagents_constants.get_subprocess_home()`` so that ``~`` resolves
    ...
    if not path or "~" not in path:
        return path
    try:
        from openagents_constants import get_subprocess_home
...
def _get_hermes_config_resolved() -> str | None:
    """Return the resolved absolute path of the OpenAgents config file (cached)."""
    ...
    try:
        from openagents_cli.config import get_config_path
        _hermes_config_resolved = str(get_config_path().resolve())
    except Exception:
        try:
            _hermes_config_resolved = str(Path(_expand_tilde("~/.openagents/config.yaml")).resolve())
        except Exception:
...
def _check_sensitive_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    if resolved in _SENSITIVE_EXACT_PATHS or normalized in _SENSITIVE_EXACT_PATHS:
        return _err
    # Prevent agents from modifying the OpenAgents config file directly.
    ...
    if hermes_config and (resolved == hermes_config or normalized == hermes_config):
        return (
            f"Refusing to write to OpenAgents config file: {filepath}\n"
            "Agent cannot modify security-sensitive configuration. "
            "Edit ~/.openagents/config.yaml directly or use 'hermes config' instead."
        )
    return None


def _get_container_mirror_prefix_for_task(task_id: str = "default") -> str | None:
    """Return the container-side OpenAgents mirror prefix for Docker file tools."""
    try:
...
def _check_cross_profile_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    state, or the Docker container's sandbox mirror of OpenAgents state.
    ...
    * sandbox-mirror (#32049) — writes that hit the
      ...
      OpenAgents process never reads the mirror and the authoritative file is
      left untouched.
    * container-mirror (#32049 follow-up) — writes from inside a Docker
    ...
    Returns ``None`` when the write is in-scope or outside OpenAgents scope.
    ...
    # in a session that cd'd into ``~/.openagents/profiles/other/`` is
...
def read_file_tool(path: str, offset: int = 1, limit: int = 500, task_id: str = "default") -> str:
    ...
    try:
        ...
        # ── OpenAgents internal path guard ────────────────────────────────
        # Prevent prompt injection via catalog or hub metadata files,
        # and block credential stores under OPENAGENTS_HOME.  Pass the
        # already-resolved path so a relative-path read against
        # TERMINAL_CWD == OPENAGENTS_HOME (e.g. "auth.json") still hits the
...
WRITE_FILE_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "cross_profile": {
                "type": "boolean",
                "description": "Opt out of the cross-profile soft guard. Defaults to false. Set true ONLY after explicit user direction to edit another OpenAgents profile's skills/plugins/cron/memories — by default these writes are blocked with a warning because they affect a different profile than the one this session is running under.",
                "default": False,
            },
        },
...
PATCH_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "cross_profile": {
                "type": "boolean",
                "description": "Opt out of the cross-profile soft guard. Defaults to false. Set true ONLY after explicit user direction to edit another OpenAgents profile's skills/plugins/cron/memories.",
                "default": False,
            },
        },
        "required": ["mode"],
