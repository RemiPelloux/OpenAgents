...
it is automatically downloaded from GitHub releases to $OPENAGENTS_HOME/bin/tirith.
...
def _get_openagents_home() -> str:
    """Return the OpenAgents home directory, respecting OPENAGENTS_HOME env var."""
    return str(get_openagents_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_openagents_home(), ".tirith-install-failed")
...
def _hermes_bin_dir() -> str:
    """Return $OPENAGENTS_HOME/bin, creating it if needed."""
    d = os.path.join(_get_openagents_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $OPENAGENTS_HOME/bin/tirith.
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $OPENAGENTS_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $OPENAGENTS_HOME/bin/tirith
