...
transport, discovers their tools, and registers them into the openagents
tool registry so the agent can call them like any built-in tool.

Configuration is read from ~/.openagents/config.yaml under the ``mcp_servers`` key.
...
# per-profile log file (~/.openagents/logs/mcp-stderr.log), tagged with the
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            hermes_home = os.path.expanduser(
                os.getenv(
                    "OPENAGENTS_HOME", os.path.join(os.path.expanduser("~"), ".hermes")
                )
            )
            candidates = [
                ...
                # image (which the OpenAgents Docker image copies node + npm +
...
# ---------------------------------------------------------------------------
# MCP ImageContent block → OpenAgents MEDIA tag
# ---------------------------------------------------------------------------
...
def _cache_mcp_image_block(block) -> str:
    ...
    return a ``MEDIA:<path>`` tag that OpenAgents gateways know how to render.
...
class MCPServerTask:
    ...
    async def _run_stdio(self, config: dict):
        ...
        # ~/.openagents/logs/mcp-stderr.log.
    ...
    async def run(self, config: dict):
        ...
        while True:
            try:
                ...
                # and the MCP server would stay dead until OpenAgents is fully
...
def _handle_auth_error_and_retry(
    ...
    return json.dumps({
        "error": (
            ...
            f"file under ~/.openagents/mcp-tokens/ and restart). Do NOT retry "
...
def _wrap_with_home_override(coro: "Coroutine") -> "Coroutine":
    """Carry the caller's context-local OPENAGENTS_HOME override into ``coro``.
    ...
    try:
        ...
        home_override = get_openagents_home_override()
    except Exception:
        return coro
    ...
    async def _scoped():
        token = set_openagents_home_override(home_override)
        try:
            return await coro
        finally:
...
def _run_on_mcp_loop(coro_or_factory, timeout: float = 30):
    ...
    # Propagate the context-local OPENAGENTS_HOME override onto the MCP loop.
    ...
    # vanish here: OAuth token stores and any other get_openagents_home()
...
def _load_mcp_config() -> Dict[str, dict]:
    """Read ``mcp_servers`` from the OpenAgents config file.
    ...
    ``os.environ`` (which includes ``~/.openagents/.env`` loaded at startup).
    """
    try:
...
def _make_tool_handler(server_name: str, tool_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        async def _call():
            ...
            # OpenAgents' MEDIA tag + cache_image_from_bytes) was the cleaner of
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the OpenAgents registry schema format.
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Does NOT register tools in the OpenAgents registry.
