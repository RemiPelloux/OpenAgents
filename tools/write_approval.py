...
(see ``openagents_cli`` slash handlers): memory shows full content, skills show
...
Pending records live under ``<OPENAGENTS_HOME>/pending/{memory,skills}/<id>.json``
...
def write_approval_enabled(subsystem: str) -> bool:
    ...
    if subsystem not in _SUBSYSTEMS:
        return False
    try:
        from openagents_cli.config import load_config, cfg_get
...
def _pending_dir(subsystem: str) -> Path:
    return get_openagents_home() / "pending" / subsystem
