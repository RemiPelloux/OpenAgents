#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the OpenAgents Skills Hub.
...
Used by openagents_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            hermes_meta = metadata.get("openagents", {})
...
class OptionalSkillSource(SkillSource):
    ...
    ~/.openagents/skills/ during setup.  They are discoverable via the Skills Hub
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                hermes_meta = meta_block.get("openagents", {})
                if isinstance(hermes_meta, dict):
                    tags = hermes_meta.get("tags", [])
...
# ---------------------------------------------------------------------------
# OpenAgents centralized index source
# ---------------------------------------------------------------------------

HERMES_INDEX_URL = "https://openagents.nousresearch.com/docs/api/skills-index.json"
...
class HermesIndexSource(SkillSource):
    """Skill source backed by the centralized OpenAgents Skills Index.
