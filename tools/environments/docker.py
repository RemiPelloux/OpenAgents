...
def _load_hermes_env_vars() -> dict[str, str]:
    """Load ~/.openagents/.env values without failing Docker command execution."""
    try:
...
def _get_active_profile_name() -> str:
    """Return the active OpenAgents profile name, or ``"default"`` on any error.
...
def reap_orphan_containers(
    ...
    * ``label=openagents=1`` (created by this codebase)
    * ``status=exited`` (running containers are NEVER reaped — they may
      belong to a sibling OpenAgents process whose reuse path will pick them
    ...
    * ``State.FinishedAt`` older than *max_age_seconds* ago (so a sibling
    ...
    subsequent OpenAgents process scheduled to reuse that exact (task, profile)
    pair.
    """
    docker = docker_exe or find_docker() or "docker"
    filters = ["--filter", "label=openagents=1", "--filter", "status=exited"]
    if profile_filter:
...
#   SETUID/SETGID - the image's init drops from root to the 'openagents'
...
def _image_uses_init_entrypoint(docker_exe: str, image: str) -> bool:
    ...
    ``openagents:latest``) already provide their own PID-1 init and execute
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # (TERMINAL_SANDBOX_DIR, default ~/.openagents/sandboxes/). Non-persistent
        ...
        # s6-overlay images (e.g. openagents:latest) already use /init as PID 1
        ...
        #   * the orphan reaper (`openagents=1` for the global sweep filter)
        #   * future cross-process reuse (`hermes-task-id`, `hermes-profile`)
        #   * operators running `docker ps --filter label=openagents=1`
        # Values are limited to the safe character set defined by
        # _sanitize_label_value(); the active OpenAgents profile is captured at
        ...
        # container shared across sessions").  If a prior OpenAgents process
    ...
    def _build_init_env_args(self) -> list[str]:
        ...
        # win over the generic OpenAgents secret blocklist. Only implicit passthrough
        # keys are filtered.
    ...
    def _find_reusable_container(self, task_label: str, profile_label: str) -> Optional[tuple[str, str]]:
        ...
        # container) but can happen if a previous OpenAgents process crashed
    ...
    def cleanup(self, *, force_remove: bool = False):
        ...
        container shared across sessions" and stopping it on every OpenAgents exit
        breaks that promise:
        ...
        * The user-visible difference between "ONE long-lived container" and
        ...
        OpenAgents process touches a labeled container for ``2 × lifetime_seconds``
        it gets ``docker rm -f``'d at the next OpenAgents startup. That covers the
        ...
        # outlives OpenAgents processes, processes inside it stay alive, and
