...
# OpenAgents-internal env vars that should NOT leak into terminal subprocesses.
_HERMES_PROVIDER_ENV_FORCE_PREFIX = "_HERMES_FORCE_"

# OpenAgents-managed AWS *inference* credentials for ``auth_type="aws_sdk"``
# providers (Bedrock).  Scoped DELIBERATELY NARROW: this lists only the
# Bedrock-specific bearer token, which is a OpenAgents inference secret exactly
...
def _build_provider_env_blocklist() -> frozenset:
    ...
    try:
        from openagents_cli.config import OPTIONAL_ENV_VARS
...
# project's dependencies into the OpenAgents venv path instead of the project's own
# ``.venv`` — silently clobbering the OpenAgents environment (e.g. a project pinned
# to a different Python version overwrites it and breaks the gateway). The
# OpenAgents venv stays reachable via PATH (its bin dir is first), so stripping
# these markers is safe and only prevents the cross-project clobber (#23473).
_ACTIVE_VENV_MARKER_VARS = ("VIRTUAL_ENV", "CONDA_PREFIX")


def _inject_context_hermes_home(env: dict) -> None:
    """Bridge the context-local OpenAgents home override into subprocess env."""
    try:
        from openagents_constants import get_openagents_home_override

        value = get_openagents_home_override()
        if value:
            env["OPENAGENTS_HOME"] = value
    except Exception:
        pass
...
# legitimate child OpenAgents spawns needs them, and they are the highest-value
...
def hermes_subprocess_env(*, inherit_credentials: bool = False) -> dict[str, str]:
    ...
    * **Tier 1 (always):** ``_ALWAYS_STRIP_KEYS`` — gateway bot tokens, GitHub
      ...
      ``inherit_credentials``.  No child OpenAgents spawns legitimately needs them.
...
def _find_bash() -> str:
    ...
    _hermes_portable_git = os.path.join(_local_appdata, "openagents", "git") if _local_appdata else ""
    ...
    raise RuntimeError(
        "Git Bash not found. OpenAgents requires Git for Windows on Windows.\n"
...
def _resolve_hermes_bin_dir() -> str | None:
    ...
    Resolution order (cheap, no heavy imports):
      1. ``shutil.which("openagents")`` — normal PATH-installed shim.
    ...
    if candidate is None:
        ...
        if (
            os.path.isabs(argv0)
            and (base == "openagents" or base.startswith("hermes."))
    ...
    if candidate is None:
        ...
        if exe_dir:
            shim = "hermes.exe" if _IS_WINDOWS else "openagents"
...
def _resolve_shell_init_files() -> list[str]:
    ...
    an explicit list — once they have, OpenAgents trusts them.
    """
...
class LocalEnvironment(BaseEnvironment):
    ...
    def get_temp_dir(self) -> str:
        ...
        **Windows:** hardcoded ``/tmp`` is wrong in two ways — native Python
        ...
        ``OPENAGENTS_HOME`` instead — single-word path, guaranteed to exist, same
        string resolves in both Git Bash and native Python.
        """
        if _IS_WINDOWS:
            # Derive a Windows-safe temp dir under OPENAGENTS_HOME.  Using
            ...
            try:
                ...
                cache_dir = get_openagents_home() / "cache" / "terminal"
            except Exception:
