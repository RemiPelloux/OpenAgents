...
PROPOSE_IMPROVEMENT_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            "target": {"type": "string", "enum": ["memory", "prompt", "routing", "skill"]},
        ...
        "required": ["target", "title", "rationale", "patch", "evidence_refs", "confidence"],
    },
}
...
def propose_improvement(args: Dict[str, Any], **_kwargs) -> str:
    ...
    if not title or not rationale or target not in {"memory", "prompt", "routing", "skill"}:
    ...
    proposal = {
        ...
        "evidence_refs": [str(item)[:512] for item in (args.get("evidence_refs") or [])[:16]],
        "confidence": max(0.0, min(1.0, float(args.get("confidence", 0)))),
    }
    return json.dumps({"success": True, "message": "Improvement proposed", "proposal": proposal})
