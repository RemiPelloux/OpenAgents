"""
Cron job management tools for OpenAgents.
...
#      saying "cat ~/.openagents/.env" or "rm -rf /". `_scan_cron_prompt()` runs
...
#      `openagents-dev` skill contains a security postmortem mentioning
#      `cat ~/.openagents/.env`, which tripped `read_secrets` and silently
...
def _resolve_model_override(model_obj: Optional[Dict[str, Any]]) -> tuple:
    ...
    if provider_name == "custom":
        try:
            from openagents_cli.runtime_provider import has_named_custom_provider
...
def _validate_cron_script_path(script: Optional[str]) -> Optional[str]:
    ...
    Scripts must be relative paths that resolve within OPENAGENTS_HOME/scripts/.
    ...
    # Only relative paths within ~/.openagents/scripts/ are allowed.
    if raw.startswith(("/", "~")) or (len(raw) >= 2 and raw[1] == ":"):
        return (
            f"Script path must be relative to ~/.openagents/scripts/. "
            f"Got absolute or home-relative path: {raw!r}. "
            f"Place scripts in ~/.openagents/scripts/ and use just the filename."
        )

    # Validate containment after resolution
    from tools.path_security import validate_within_dir

    scripts_dir = get_openagents_home() / "scripts"
...
Important safety rule: cron-run sessions should not recursively schedule more cron jobs.""",
    "parameters": {
        ...
        "properties": {
            ...
            "script": {
                "type": "string",
                "description": f"Optional path to a script that runs each tick. In the default mode its stdout is injected into the agent's prompt as context (data-collection / change-detection pattern). With no_agent=True, the script IS the job and its stdout is delivered verbatim (classic watchdog pattern). Relative paths resolve under {display_openagents_home()}/scripts/. ``.sh``/``.bash`` extensions run via bash, everything else via Python. On update, pass empty string to clear.",
            },
            "no_agent": {
                "type": "boolean",
                "default": False,
