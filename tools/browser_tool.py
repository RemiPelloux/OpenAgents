...
from openagents_constants import agent_browser_runnable, get_openagents_home
from utils import env_int, is_truthy_value
from openagents_cli.config import DEFAULT_CONFIG, cfg_get
...
# means a compromised transitive dependency could read every OpenAgents secret
...
# plugins drop in under ``~/.openagents/plugins/browser/<vendor>/``.
...
def _get_cloud_provider() -> Optional[CloudBrowserProvider]:
    ...
    browser plugins (``~/.openagents/plugins/browser/<vendor>/``) participate
...
def _lightpanda_fallback_reason(
    ...
    the fallback result so CLI/TUI/gateway users can see when OpenAgents silently
...
def _run_chrome_fallback_command(
    ...
    def _run_tmp(cmd: str, cmd_args: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            #   in the OpenAgents CLI where sys.stdout and sys.stderr both report
    ...
    try:
        ...
        if ensure_dependency("browser"):
            candidates = [
                ...
                shutil.which(
                    "agent-browser",
                    path=str(get_openagents_home() / "node_modules" / ".bin"),
                ),
                shutil.which(
                    "agent-browser", path=str(get_openagents_home() / "node" / "bin")
                ),
                shutil.which("agent-browser", path=str(get_openagents_home() / "node")),
            ]
...
def browser_vision(
    ...
    screenshots_dir = get_openagents_dir("cache/screenshots", "browser_screenshots")
    ...
    if engine == "lightpanda" and _should_inject_engine(engine):
        ...
        if fb_result.get("success"):
            ...
            if fb_path and os.path.exists(fb_path):
                ...
                screenshots_dir = get_openagents_dir(
        ...
        try:
            ...
            if _requires_real_termux_browser_install(browser_cmd):
                ...
                if _running_in_docker():
                    ...
                    print("       docker pull ghcr.io/nousresearch/openagents:latest")
                else:
