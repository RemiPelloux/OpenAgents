...
def has_xai_credentials() -> bool:
    ...
    2. ``~/.openagents/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
    ...
    try:
        ...
        auth_path = get_openagents_home() / "auth.json"
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.openagents/.env`` first, then ``os.environ``.

    Wraps :func:`openagents_cli.config.get_env_value` so tests can patch
    ``tools.xai_http.get_env_value`` to inject dotenv-only secrets into the
    xAI credential resolver.
    """
    try:
        from openagents_cli.config import get_env_value as _hermes_get_env_value
...
def resolve_xai_http_credentials(*, force_refresh: bool = False) -> Dict[str, str]:
    ...
    to ``XAI_API_KEY`` resolved via ``openagents_cli.config.get_env_value`` so keys
    stored in ``~/.openagents/.env`` (the standard OpenAgents location) are honored —
