...
- Users can declare any number of named providers with ``type: command``
  under ``tts.providers.<name>`` in ``~/.openagents/config.yaml``. Hermes
...
Configuration is loaded from ~/.openagents/config.yaml under the 'tts:' key.
...
def get_env_value(name, default=None):
    ...
    Tests may monkeypatch and later restore ``openagents_cli.config.get_env_value``
    before this module is imported. Resolve the helper at call time so TTS does
    not keep a stale imported function for the rest of the test process.
    """
    try:
        from openagents_cli.config import get_env_value as _get_env_value
    except ImportError:
...
def _get_default_output_dir() -> str:
    from openagents_constants import get_openagents_dir
    return str(get_openagents_dir("cache/audio", "audio_cache"))
...
# ===========================================================================
# Config loader -- reads tts: section from ~/.openagents/config.yaml
# ===========================================================================
def _load_tts_config() -> Dict[str, Any]:
    """
    Load TTS configuration from ~/.openagents/config.yaml.
    ...
    try:
        ...
        logger.debug("openagents_cli.config not available, using default TTS config")
        return {}
    except Exception as e:
...
# custom voice-cloning scripts, etc.) into OpenAgents without any Python code
...
#           output_format: wav
#
# OpenAgents writes the input text to a temp UTF-8 file, runs the command with
...
def _generate_xai_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    # output_format when OpenAgents actually needs a non-default format/override.
...
def _get_piper_voices_dir() -> Path:
    """Return the directory where OpenAgents caches Piper voice models.

    Resolves to ``~/.openagents/cache/piper-voices/`` under the active
    OPENAGENTS_HOME so voice downloads follow profile boundaries.
    """
    from openagents_constants import get_openagents_dir
    root = Path(get_openagents_dir("cache/piper-voices", "piper_voices_cache"))
...
def text_to_speech_tool(
    ...
    Reads provider/voice config from ~/.openagents/config.yaml (tts: section).
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Mistral provider selected but 'mistralai' package not installed. "
                             "Run: pip install 'openagents[mistral]'"
                }, ensure_ascii=False)
...
TTS_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "output_path": {
                "type": "string",
                "description": f"Optional custom file path to save the audio. Defaults to {display_openagents_home()}/audio_cache/<timestamp>.mp3"
            }
        },
        "required": ["text"]
    }
