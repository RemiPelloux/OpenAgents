# OpenAgents Security Policy

This document describes OpenAgents's trust model, names the one
security boundary the project treats as load-bearing, and defines the
scope for vulnerability reports.

## 1. Reporting a Vulnerability

Report privately via [GitHub Security Advisories](https://github.com/NousResearch/openagents/security/advisories/new)
or **security@nousresearch.com**. Do not open public issues for
security vulnerabilities. **OpenAgents does not operate a bug
bounty program.**
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted OpenAgents access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in OpenAgents's documentation
...
OpenAgents supports two OS-level isolation postures. They address
...
OpenAgents supports this in two ways:

- **OpenAgents's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, OpenAgents's in-process heuristics
...
OpenAgents filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in OpenAgents
itself. Bugs in OpenAgents's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   OpenAgents does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, OpenAgents's own documentation, or reasonable
  operator expectations would predict — including cases where
  OpenAgents has documented a stance about how its output should
...
- **Community-contributed skills and plugins.** Third-party skills
  ...
  plugins are in the operator's review surface, not OpenAgents's
  trust surface (§2.4, §2.5). A skill or plugin doing something
  malicious is the expected failure mode of one that wasn't
  reviewed, not a vulnerability in OpenAgents. Bugs in Hermes
...
- OpenAgents includes supply-chain guards for MCP server
