...
test('uninstallArgsForMode maps each mode to the module-runner argv', () => {
  assert.deepEqual(uninstallArgsForMode('gui'), ['-m', 'openagents_cli.uninstall', '--mode', 'gui'])
  assert.deepEqual(uninstallArgsForMode('lite'), ['-m', 'openagents_cli.uninstall', '--mode', 'lite'])
  assert.deepEqual(uninstallArgsForMode('full'), ['-m', 'openagents_cli.uninstall', '--mode', 'full'])
})
...
test('buildPosixCleanupScript waits for the PID, runs the uninstall module, removes bundle', () => {
  const script = buildPosixCleanupScript({
    desktopPid: 4321,
    pythonExe: '/home/x/.hermes/openagents/venv/bin/python',
    pythonPath: null,
    agentRoot: '/home/x/.hermes/openagents',
    uninstallArgs: ['-m', 'openagents_cli.uninstall', '--mode', 'gui'],
  ...
  assert.match(script, /'-m' 'openagents_cli\.uninstall' '--mode' 'gui'/)
  assert.match(script, /rm -rf '\/opt\/hermes\/linux-unpacked'/)
  assert.match(script, /export OPENAGENTS_HOME='\/home\/x\/\.hermes'/)
})
...
test('buildPosixCleanupScript exports PYTHONPATH when pythonPath is set (lite/full)', () => {
  const script = buildPosixCleanupScript({
    ...
    pythonPath: '/home/x/.hermes/openagents',
    agentRoot: '/home/x/.hermes/openagents',
    uninstallArgs: ['-m', 'openagents_cli.uninstall', '--mode', 'full'],
    appPath: null,
    hermesHome: '/home/x/.hermes'
  })
  // System python + source on PYTHONPATH so import openagents_cli works while the
  // venv is torn down.
  assert.match(script, /export PYTHONPATH='\/home\/x\/\.hermes\/openagents'/)
  assert.match(script, /'\/usr\/bin\/python3' '-m' 'openagents_cli\.uninstall' '--mode' 'full'/)
})
...
test('buildPosixCleanupScript omits the bundle rm when appPath is null', () => {
  const script = buildPosixCleanupScript({
    ...
    pythonPath: null,
    agentRoot: '/a',
    uninstallArgs: ['-m', 'openagents_cli.uninstall', '--mode', 'lite'],
    appPath: null,
    hermesHome: '/h'
  })
  assert.doesNotMatch(script, /rm -rf '\//)
  // Still runs the uninstall.
  assert.match(script, /'-m' 'openagents_cli\.uninstall' '--mode' 'lite'/)
})
...
test('buildWindowsCleanupScript waits (bounded) for PID, runs uninstall, rmdir bundle', () => {
  ...
  // PYTHONPATH set so a system python can import openagents_cli from source.
  assert.match(script, /set "PYTHONPATH=C:\\hermes;%PYTHONPATH%"/)
  assert.match(script, /"C:\\Python313\\python.exe" "-m" "openagents_cli\.uninstall" "--mode" "full"/)
