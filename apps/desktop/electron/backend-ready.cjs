...
// compile and import the whole `openagents_cli.main` → `web_server` → FastAPI/
...
function waitForDashboardPort(child, timeoutMs = resolvePortAnnounceTimeoutMs()) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      cleanup()
      reject(new Error(`Timed out waiting for OpenAgents backend port announcement (${timeoutMs}ms)`))
    }, timeoutMs)
