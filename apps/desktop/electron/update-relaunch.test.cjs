 ...
 *      internals; OPENAGENTS_HOME / HERMES_DESKTOP_* env + cwd preserved) and is
 *      safely shell-quoted.
...
const ROOT = '/home/u/.hermes/openagents'
...
test('resolveUnpackedRelease returns the dir for a binary UNDER release/<plat>-unpacked', () => {
  const exec = path.join(UNPACKED, 'openagents')
...
test('resolveUnpackedRelease is null for AppImage / .deb / .rpm / dev / unresolved paths', () => {
  ...
  // dev electron
  assert.equal(
    resolveUnpackedRelease('/home/u/.hermes/openagents/node_modules/electron/dist/electron', ROOT, 'linux'),
    null
  )
  // empty / missing
  assert.equal(resolveUnpackedRelease('', ROOT, 'linux'), null)
  assert.equal(resolveUnpackedRelease(path.join(UNPACKED, 'openagents'), '', 'linux'), null)
})
...
test('resolveUnpackedRelease is not fooled by a sibling prefix dir', () => {
  ...
  const sneaky = path.join(ROOT, 'apps', 'desktop', 'release', 'linux-unpacked-evil', 'openagents')
...
test('collectRelaunchEnv preserves OPENAGENTS_HOME + HERMES_DESKTOP_* + sandbox opt-out only', () => {
  const env = {
    OPENAGENTS_HOME: '/home/u/.hermes',
...
test('buildRelaunchScript embeds pid/exec/args/env/cwd and is valid bash', () => {
  const script = buildRelaunchScript({
    pid: 4242,
    execPath: '/home/u/.hermes/openagents/apps/desktop/release/linux-unpacked/Hermes',
    args: ['hermes://open/agent/42', "--note=it's fine"],
    env: { OPENAGENTS_HOME: '/home/u/.hermes', HERMES_DESKTOP_REMOTE_URL: 'http://box:9119' },
    cwd: '/home/u/work dir'
  })
  ...
  assert.match(script, /export OPENAGENTS_HOME='\/home\/u\/\.hermes'/)
