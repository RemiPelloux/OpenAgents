...
//      in openagents_cli/provider_catalog.py) — the SAME provider identity
...
function ConnectedProviderRow({
  ...
  // OpenAgents can clear this provider's creds via the API.
  const canDisconnect = provider.disconnectable ?? provider.flow !== 'external'
  // External (CLI-managed) provider OpenAgents can't clear via the API, but ships a
...
export function ProvidersSettings({ onClose, onViewChange, view }: ProvidersSettingsProps) {
  ...
  // OpenAgents never deletes creds another tool owns behind a silent API call.
