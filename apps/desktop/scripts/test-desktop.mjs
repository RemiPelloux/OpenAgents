...
// no longer bundles the OpenAgents Python payload (that's fetched at first
...
const APP = (() => {
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, 'openagents'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
// Default OPENAGENTS_HOME for non-sandboxed runs -- matches main.cjs's
// resolveHermesHome(). On Windows it's %LOCALAPPDATA%\hermes; elsewhere
// it's ~/.openagents. The fresh-install sandbox launchFresh() sets its own
// OPENAGENTS_HOME and never touches this.
const DEFAULT_OPENAGENTS_HOME = (() => {
  if (PLATFORM === 'win32' && process.env.LOCALAPPDATA) {
    return path.join(process.env.LOCALAPPDATA, 'openagents')
  }
  return path.join(os.homedir(), '.hermes')
})()
const VENV_ROOT = path.join(DEFAULT_OPENAGENTS_HOME, 'openagents', 'venv')
...
function launchFresh() {
  ...
  console.log(`  OPENAGENTS_HOME: ${hermesHome}`)
  console.log(`  cwd: ${cwd}`)

  return { runtimeRoot: path.join(hermesHome, 'openagents', 'venv') }
}

// Validate the packaged bundle matches the thin-installer architecture:
//   - The OpenAgents Python payload is NOT shipped (it's fetched at first
...
function validateBundle() {
  ...
  // present anymore. If a stray ship of openagents_cli sneaks back in we want
  // to fail loudly rather than re-introduce the 400MB delta we just removed.
  const staleFactoryMarker = path.join(APP.resourcesPath, 'openagents', 'openagents_cli', 'main.py')
...
function help() {
  ...
  npm run test:desktop:fresh     # build packaged app, launch with temp userData + OPENAGENTS_HOME
