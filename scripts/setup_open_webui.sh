...
# Bootstrap Open WebUI against OpenAgents's OpenAI-compatible API server.
#
# Idempotent by design:
# - ensures ~/.openagents/.env has API server settings
...
#   HERMES_API_MODEL_NAME='OpenAgents'

OPEN_WEBUI_PORT="${OPEN_WEBUI_PORT:-8080}"
OPEN_WEBUI_HOST="${OPEN_WEBUI_HOST:-127.0.0.1}"
OPEN_WEBUI_NAME="${OPEN_WEBUI_NAME:-OpenAgents WebUI}"
...
HERMES_API_MODEL_NAME="${HERMES_API_MODEL_NAME:-OpenAgents}"
...
install_systemd_user_service() {
  ...
  cat > "$unit" <<EOF
[Unit]
Description=Open WebUI connected to OpenAgents
...
main() {
  ...
  log 'Ensuring OpenAgents API server is configured...'
  ...
  log 'Restarting OpenAgents gateway so API server settings take effect...'
