...
VERSION_FILE = REPO_ROOT / "openagents_cli" / "__init__.py"
...
AUTHOR_MAP = {
    ...
    "274096618+openagents-dhabibi@users.noreply.github.com": "dhabibi",
    ...
    "58116817+TurgutKural@users.noreply.github.com": "TurgutKural",  # salvage of #23356 (OPENAGENTS_HOME inject)
    ...
    "kdkcfp@gmail.com": "slowtokki0409",  # PR #29025 (ignore local OpenAgents runtime files)
    ...
    "2663402852@qq.com": "x1am1",  # PR #35098 (chown root-owned top-level OPENAGENTS_HOME state files)
...
def _update_acp_registry_versions(semver: str) -> None:
    ...
    if ACP_REGISTRY_MANIFEST.exists():
        ...
        if "package" in uvx:
            uvx["package"] = f"openagents[acp]=={semver}"
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/NousResearch/openagents",
    ...
    lines.append(f"# OpenAgents v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for OpenAgents. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="OpenAgents Release Tool")
    ...
    print(f"{'='*60}")
    print(f"  OpenAgents Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"OpenAgents v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"OpenAgents v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'OpenAgents v{new_version} ({calver_date})' "
