...
#   2. ~/.openagents/node/ from a prior Hermes-managed install
#   3. fnm, proto, nvm (in that order) if the user already uses a version manager
#   4. Termux `pkg`, macOS Homebrew
#   5. pinned nodejs.org tarball into ~/.openagents/node/ (always works, zero shell rc edits)
#
# Usage:
...
#   OPENAGENTS_HOME               (default: $HOME/.hermes)
# ============================================================================
...
# (already on PATH) instead of the default $OPENAGENTS_HOME/node/bin, which is off
...
_nb_configure_npm_prefix() {
    ...
    printf 'prefix=%s\n' "$(dirname "$_link_dir")" > "$OPENAGENTS_HOME/node/etc/npmrc"
}
...
_nb_install_bundled_node() {
    ...
    _nb_log "Extracting to $OPENAGENTS_HOME/node/..."
    ...
    mv "$extracted" "$OPENAGENTS_HOME/node"
    rm -rf "$tmp"

    local _link_dir
    _link_dir="$(_nb_get_link_dir)"
    mkdir -p "$_link_dir"
    ln -sf "$OPENAGENTS_HOME/node/bin/node" "$_link_dir/node"
    ln -sf "$OPENAGENTS_HOME/node/bin/npm"  "$_link_dir/npm"
    ln -sf "$OPENAGENTS_HOME/node/bin/npx"  "$_link_dir/npx"
    ...
    _nb_have_modern_node || return 1
    _nb_ok "Node $(node --version) installed to $OPENAGENTS_HOME/node/"
    return 0
}
...
_nb_managed_tool_broken() {
    ...
    local probe
    for probe in \
        "$OPENAGENTS_HOME/node/bin/$tool" \
        "$OPENAGENTS_HOME/node/${tool}.exe" \
        "$OPENAGENTS_HOME/node/$tool"; do
...
# absent. Used by openagents_constants.find_hermes_node_executable() and safe
# to call from install reruns.
heal_managed_node() {
    [ -d "$OPENAGENTS_HOME/node" ] || return 1
    if ! _nb_managed_node_needs_heal; then
        return 0
    fi
    _nb_log "Hermes-managed Node is broken — redownloading to $OPENAGENTS_HOME/node/..."
    _nb_install_bundled_node
}
...
ensure_node() {
    ...
    if [ -x "$OPENAGENTS_HOME/node/bin/node" ]; then
        export PATH="$OPENAGENTS_HOME/node/bin:$PATH"
        if _nb_have_modern_node; then
