...
# Python-driven tool session (e.g. OpenAgents terminal tool). A pre-set PYTHONPATH
...
# Configuration
REPO_URL_SSH="git@github.com:NousResearch/openagents.git"
REPO_URL_HTTPS="https://github.com/NousResearch/openagents.git"
OPENAGENTS_HOME="${OPENAGENTS_HOME:-$HOME/.hermes}"
...
#   code at /usr/local/lib/openagents, command at /usr/local/bin/hermes,
#   data still at /root/.hermes (OPENAGENTS_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   write \$OPENAGENTS_HOME/.no-bundled-skills so future"
            ...
            echo "                   default (non-root):  ~/.openagents/openagents"
            echo "                   default (root, Linux): /usr/local/lib/openagents"
            echo "  --hermes-home PATH  Data directory (default: ~/.openagents, or \$OPENAGENTS_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, OpenAgents installs the code under"
            echo "  /usr/local/lib/openagents and links the command into"
            echo "  /usr/local/bin/hermes (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$OPENAGENTS_HOME"
            echo "  (default /root/.hermes).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$OPENAGENTS_HOME/openagents are preserved in-place."
...
emit_manifest() {
    ...
    printf '%s' '{"protocol_version":1,"stages":[{"name":"prerequisites","title":"System prerequisites","category":"runtime","needs_user_input":false},{"name":"repository","title":"Download OpenAgents","category":"runtime","needs_user_input":false},{"name":"venv","title":"Create Python virtual environment","category":"runtime","needs_user_input":false},{"name":"python-deps","title":"Install Python dependencies","category":"runtime","needs_user_input":false},{"name":"node-deps","title":"Install browser-tool dependencies","category":"runtime","needs_user_input":false},{"name":"path","title":"Install hermes command","category":"runtime","needs_user_input":false},{"name":"config","title":"Prepare config and skills","category":"configuration","needs_user_input":false},{"name":"setup","title":"Configure API keys and settings","category":"configuration","needs_user_input":true},{"name":"gateway","title":"Configure gateway service","category":"configuration","needs_user_input":true},'"$desktop_stage"'{"name":"complete","title":"Finish install","category":"runtime","needs_user_input":false}]}'
    printf '\n'
}
...
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $OPENAGENTS_HOME/openagents
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $OPENAGENTS_HOME/openagents
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/openagents
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $OPENAGENTS_HOME/openagents — then preserve it)
#
...
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in OPENAGENTS_HOME.
    if is_termux; then
        INSTALL_DIR="$OPENAGENTS_HOME/openagents"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$OPENAGENTS_HOME/openagents/.git" ]; then
            INSTALL_DIR="$OPENAGENTS_HOME/openagents"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/openagents)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/openagents"
        ...
        log_info "  Data:    $OPENAGENTS_HOME (unchanged)"
...
# so global package binaries land in $OPENAGENTS_HOME/node/bin — which is NOT on
...
# OpenAgents's own global installs pass an explicit --prefix and are unaffected.
...
configure_managed_node_npm_prefix() {
    [ -x "$OPENAGENTS_HOME/node/bin/npm" ] || return 0
    local link_dir
    link_dir="$(get_command_link_dir)"
    mkdir -p "$OPENAGENTS_HOME/node/etc"
    printf 'prefix=%s\n' "$(dirname "$link_dir")" > "$OPENAGENTS_HOME/node/etc/npmrc"
}
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://openagents.nousresearch.com/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    # OpenAgents owns its own uv at $OPENAGENTS_HOME/bin/uv.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (openagents_cli/managed_uv.py) looks in the same
    # place, so install.sh and `hermes update` stay in sync.
    local _managed_uv="$OPENAGENTS_HOME/bin/uv"
    ...
    log_info "Installing managed uv into $OPENAGENTS_HOME/bin ..."
    mkdir -p "$OPENAGENTS_HOME/bin"
    ...
    # directly into $OPENAGENTS_HOME/bin instead of ~/.local/bin.
    if UV_UNMANAGED_INSTALL="$OPENAGENTS_HOME/bin" sh "$_uv_installer" >>"$_uv_install_log" 2>&1; then
        rm -f "$_uv_installer"
        if [ -x "$_managed_uv" ]; then
            UV_CMD="$_managed_uv"
        else
...
check_node() {
    ...
    if [ -x "$OPENAGENTS_HOME/node/bin/node" ] && node_satisfies_build "$("$OPENAGENTS_HOME/node/bin/node" --version)"; then
        export PATH="$OPENAGENTS_HOME/node/bin:$PATH"
        log_success "Node.js $("$OPENAGENTS_HOME/node/bin/node" --version) found (Hermes-managed)"
        HAS_NODE=true
        return 0
    fi
...
install_node() {
    ...
    log_info "Extracting to ~/.openagents/node/..."
    ...
    # Place into ~/.openagents/node/ and symlink binaries into the same bin dir
    # the hermes command uses (get_command_link_dir): /usr/local/bin for root
    # FHS installs, $PREFIX/bin on Termux, ~/.local/bin otherwise.
    rm -rf "$OPENAGENTS_HOME/node"
    mkdir -p "$OPENAGENTS_HOME"
    mv "$extracted_dir" "$OPENAGENTS_HOME/node"
    rm -rf "$tmp_dir"

    local node_link_dir
    node_link_dir="$(get_command_link_dir)"
    mkdir -p "$node_link_dir"
    ln -sf "$OPENAGENTS_HOME/node/bin/node" "$node_link_dir/node"
    ln -sf "$OPENAGENTS_HOME/node/bin/npm"  "$node_link_dir/npm"
    ln -sf "$OPENAGENTS_HOME/node/bin/npx"  "$node_link_dir/npx"
    ...
    local installed_ver
    installed_ver=$("$OPENAGENTS_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.openagents/node/"
    HAS_NODE=true
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. OpenAgents install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "OpenAgents itself does not require or retain root access."
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if OpenAgents behaves unexpectedly."
                    else
...
EOF
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        PATH_COMMENT='# OpenAgents — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# OpenAgents — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# OpenAgents — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
...
copy_config_templates() {
    ...
    # Create ~/.openagents directory structure (config at top level, code in subdir)
    mkdir -p "$OPENAGENTS_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.openagents/.env (top level, easy to find)
    if [ ! -f "$OPENAGENTS_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$OPENAGENTS_HOME/.env"
            log_success "Created ~/.openagents/.env from template"
        else
            touch "$OPENAGENTS_HOME/.env"
            log_success "Created ~/.openagents/.env"
        fi
    else
        log_info "~/.openagents/.env already exists, keeping it"
    fi
    ...
    chmod 600 "$OPENAGENTS_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.openagents/config.yaml (top level, easy to find)
    if [ ! -f "$OPENAGENTS_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$OPENAGENTS_HOME/config.yaml"
            log_success "Created ~/.openagents/config.yaml from template"
        fi
    else
        log_info "~/.openagents/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file).
    # This MUST match DEFAULT_SOUL_MD in openagents_cli/default_soul.py — the
...
SOUL_EOF
    ...
    log_success "Configuration directory ready: ~/.openagents/"

    # Seed bundled skills into ~/.openagents/skills/ (manifest-based, one-time per skill)
    if [ "$NO_SKILLS" = true ]; then
        ...
        printf '%s\n' \
            ...
            > "$OPENAGENTS_HOME/.no-bundled-skills" 2>/dev/null || true
        log_info "Skipping bundled skills (--no-skills). Wrote $OPENAGENTS_HOME/.no-bundled-skills"
        log_info "  Future 'hermes update' runs will not inject bundled skills. Delete the marker to opt back in."
    else
        log_info "Syncing bundled skills to ~/.openagents/skills/ ..."
        if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
            log_success "Skills synced to ~/.openagents/skills/"
        else
            # Fallback: simple directory copy if Python sync fails
            if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$OPENAGENTS_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
                cp -r "$INSTALL_DIR/skills/"* "$OPENAGENTS_HOME/skills/" 2>/dev/null || true
                log_success "Skills copied to ~/.openagents/skills/"
            fi
        fi
    fi
}
...
strip_snap_browser_override() {
    ...
    local env_file="$OPENAGENTS_HOME/.env"
    ...
    local tmp
    tmp="$(mktemp)" || return 0
    if grep -Ev '^AGENT_BROWSER_EXECUTABLE_PATH=/snap/|^# OpenAgents browser tools' "$env_file" > "$tmp"; then
        mv "$tmp" "$env_file"
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# OpenAgents browser tools — explicit browser override."
...
run_setup_wizard() {
    ...
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m openagents_cli.main setup < /dev/tty
    else
        python -m openagents_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$OPENAGENTS_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    log_info "The gateway needs to be running for OpenAgents to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$OPENAGENTS_HOME/whatsapp/session/creds.json"
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            nohup $HERMES_CMD gateway > "$OPENAGENTS_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.openagents/logs/gateway.log"
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $OPENAGENTS_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $OPENAGENTS_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $OPENAGENTS_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'openagents' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'openagents' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'openagents' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$OPENAGENTS_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$OPENAGENTS_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$OPENAGENTS_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    if ! run_with_timeout "$NODE_DEPS_TIMEOUT" "$npm_bin" install -g --prefix "$OPENAGENTS_HOME/node" --silent --ignore-scripts \
    ...
    local ab_bin="$OPENAGENTS_HOME/node/bin/agent-browser"
    if [ -x "$ab_bin" ]; then
...
postinstall_mode() {
    ...
    log_info "Post-install mode: setting up OpenAgents for pip install"
    ...
    if [ -n "$HERMES_CMD" ]; then
        ...
        log_info "Try: python -m openagents_cli.main setup"
    fi
}
...
# extract a tree MISSING the electron binary, so the `electron`->`openagents` rename
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        desktop)
            ...
            # provisioned during prerequisites/node-deps (at $OPENAGENTS_HOME/node/bin)
        ...
        complete)
            ...
            # $OPENAGENTS_HOME. $OPENAGENTS_HOME is a shared data dir (it can be
...
main() {
    ...
    # Code-scoped stamp: write next to the install tree, not into $OPENAGENTS_HOME.
    # $OPENAGENTS_HOME is a shared data dir (it can be bind-mounted into a Docker
