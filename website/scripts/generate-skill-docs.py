...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/NousResearch/openagents/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    hermes_meta = (fm.get("metadata") or {}).get("openagents") or {}
...
def build_catalog_md_bundled(
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with OpenAgents"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Hermes ships with a large built-in skill library copied into `~/.openagents/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Hermes also syncs bundled skills on `hermes update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.openagents/skills/` tree, it is still shipped with Hermes; restore it with `hermes skills reset <name> --restore`.",
        "",
