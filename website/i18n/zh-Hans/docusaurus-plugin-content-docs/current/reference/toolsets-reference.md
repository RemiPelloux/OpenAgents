...
| `code_execution` | `execute_code` | 运行以编程方式调用 OpenAgents 工具的 Python 脚本。 |
...
| `openagents-acp` | 移除了 `clarify`、`cronjob`、`image_generate`、`send_message`、`text_to_speech` 以及全部四个 Home Assistant 工具。专注于 IDE 环境中的编码任务。 |
...
| `openagents-gateway` | 内部 gateway 编排器工具集——所有 `hermes-<platform>` 工具集的并集；当 gateway 需要接受任意消息来源时使用。 |
