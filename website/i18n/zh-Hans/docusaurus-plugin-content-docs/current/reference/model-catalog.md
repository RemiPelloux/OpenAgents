...
OpenAgents 从托管于文档站点旁的 JSON 清单中获取 **OpenRouter** 和 **Nous Portal** 的精选模型列表。这样维护者无需发布新的 `openagents` 版本即可更新选择器列表。
...
缓存位置：`~/.openagents/cache/model_catalog.json`。
...
```bash
# 从仓库内硬编码列表重新生成（在编辑 openagents_cli/models.py 中的
