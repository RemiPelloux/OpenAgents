...
sidebar_position: 8
title: "Mattermost"
description: "将 OpenAgents 配置为 Mattermost 机器人"
---

# Mattermost 配置

OpenAgents 以机器人身份集成到 Mattermost，让你可以通过私信或团队频道与 AI 助手对话。Mattermost 是一个自托管的开源 Slack 替代品——运行在你自己的基础设施上，完全掌控数据。机器人通过 Mattermost 的 REST API（v4）和 WebSocket 连接以接收实时事件，将消息通过 OpenAgents 管道（包括工具调用、记忆和推理）处理后实时响应。支持文本、文件附件、图片和斜杠命令。

无需额外的 Mattermost 库——适配器使用 `aiohttp`，该库已作为 OpenAgents 的依赖项包含在内。
...
| 场景 | 行为 |
|---------|----------|
| **私信（DM）** | OpenAgents 响应每一条消息，无需 `@提及`。每个私信有独立的会话。 |
| **公开/私有频道** | OpenAgents 仅在被 `@提及` 时响应。未被提及时，Hermes 忽略消息。 |
...
:::tip
如果你希望 OpenAgents 以线程对话方式回复（嵌套在原始消息下方），请设置 `MATTERMOST_REPLY_MODE=thread`。默认值为 `off`，即在频道中发送普通消息。
:::
...
:::tip
你也可以使用**个人访问 token** 代替机器人账户。前往**个人资料** → **安全** → **个人访问 Token** → **创建 Token**。如果你希望 OpenAgents 以你自己的用户身份发帖而非独立的机器人用户，这种方式很有用。
:::
...
OpenAgents 使用你的 Mattermost 用户 ID 来控制谁可以与机器人交互。查找方式：
...
`MATTERMOST_REPLY_MODE` 设置控制 OpenAgents 发布响应的方式：

| 模式 | 行为 |
|------|----------|
| `off`（默认） | OpenAgents 在频道中发送普通消息，与普通用户一样。 |
| `thread` | OpenAgents 在你的原始消息下方以线程形式回复。在大量来回交流时保持频道整洁。 |

在你的 `~/.openagents/.env` 中设置：
...
**解决方法**：将你的用户 ID 添加到 `~/.openagents/.env` 中的 `MATTERMOST_ALLOWED_USERS`，然后重启 gateway。注意：用户 ID 是 26 位字母数字字符串，不是你的 `@用户名`。
...
- **无额外依赖**：适配器使用 `aiohttp` 处理 HTTP 和 WebSocket，该库已包含在 OpenAgents 中。
