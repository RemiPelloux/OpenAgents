...
sidebar_position: 4
title: "Slack"
description: "使用 Socket Mode 将 OpenAgents 设置为 Slack 机器人"
---

# Slack 设置

使用 Socket Mode 将 OpenAgents 作为机器人连接到 Slack。Socket Mode 使用 WebSocket 而非公开 HTTP 端点，因此你的 OpenAgents 实例无需公开访问——它可以在防火墙后、笔记本电脑上或私有服务器上正常运行。
...
最快的方式是粘贴 OpenAgents 为你生成的 manifest（清单文件）。它会一次性声明所有内置斜杠命令（`/btw`、`/stop`、`/model`……）、所有必需的 OAuth 权限范围、所有事件订阅，并启用 Socket Mode。

### 方式 A：使用 OpenAgents 生成的 manifest（推荐）

1. 生成 manifest：
   ```bash
   hermes slack manifest --write
   ```
   此命令会将 `~/.openagents/slack-manifest.json` 写入磁盘并打印粘贴说明。
...
2. 点击 **Create New App**
3. 选择 **From scratch**
4. 输入应用名称（例如 "OpenAgents"）并选择你的工作区
5. 点击 **Create App**
...
即使拥有所有正确的权限范围和事件订阅，除非启用 Messages Tab，否则 Slack 不允许用户向机器人发送私信。这是 Slack 平台的要求，而非 OpenAgents 的配置问题。
:::
...
OpenAgents 使用 Slack **Member ID**（而非用户名或显示名称）作为白名单。
...
每个 OpenAgents 命令（`/btw`、`/stop`、`/new`、`/model`、`/help`……）都是原生 Slack 斜杠命令——与它们在 Telegram 和 Discord 上的工作方式完全相同。在 Slack 中输入 `/`，自动补全选择器会列出每个 OpenAgents 命令及其描述。

底层实现：Hermes 附带一个生成的 Slack 应用 manifest（见第一步，方式 A），它将 [`COMMAND_REGISTRY`](https://github.com/NousResearch/openagents/blob/main/openagents_cli/commands.py) 中的每个命令声明为斜杠命令。在 Socket Mode 下，无论 manifest 的 `url` 字段如何，Slack 都会通过 WebSocket 路由命令事件。

### 更新后刷新斜杠命令

当 OpenAgents 添加新命令时（例如执行 `hermes update` 后），重新生成 manifest 并更新你的 Slack 应用：
...
3. 粘贴 `~/.openagents/slack-manifest.json` 的新内容
...
了解 OpenAgents 在不同场景下的行为：

| 场景 | 行为 |
|---------|----------|
| **私信** | 机器人响应每条消息——无需 @ 提及 |
| **频道** | 机器人**仅在被 @ 提及时响应**（例如 `@OpenAgents what time is it?`）。在频道中，Hermes 在该消息附带的话题中回复。 |
...
除了第八步中的必需环境变量外，你还可以通过 `~/.openagents/config.yaml` 自定义 Slack 机器人行为。
...
为 `true`（默认值）时，共享频道中的每个用户都有自己独立的对话会话。在 `#general` 中与 OpenAgents 对话的两个人将有各自独立的历史记录和上下文。
...
确保机器人已被**邀请到该频道**（`/invite @OpenAgents`）。

---

## 多工作区支持

OpenAgents 可以使用单个 gateway 实例**同时连接多个 Slack 工作区**。每个工作区使用其自己的机器人用户 ID 独立认证。
...
| 机器人在私信中正常但在频道中不响应 | **最常见问题。** 将 `message.channels` 和 `message.groups` 添加到事件订阅，重新安装应用，并用 `/invite @OpenAgents` 邀请机器人加入频道 |
...
| 机器人响应但无法在频道中发帖 | 用 `/invite @OpenAgents` 邀请机器人加入频道 |
...
7. ✅ 已**邀请**机器人加入频道（`/invite @OpenAgents`）
...
- Token 应存储在 `~/.openagents/.env` 中（文件权限 `600`）
- 定期通过 Slack 应用设置轮换 token
- 审计谁有权访问你的 OpenAgents 配置目录
