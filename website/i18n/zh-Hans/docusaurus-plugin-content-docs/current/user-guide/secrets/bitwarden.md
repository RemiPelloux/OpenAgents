...
在进程启动时从 [Bitwarden Secrets Manager](https://bitwarden.com/products/secrets-manager/) 拉取 API 密钥，而不是以明文形式存储在 `~/.openagents/.env` 中。一个引导密钥（机器账户访问令牌）替代了 N 个提供商密钥，轮换凭据只需在 Bitwarden Web 应用中修改一次即可。

## 工作原理

1. 在 Bitwarden Secrets Manager 中创建一个**机器账户**，授予其对某个项目的读取权限，并生成一个**访问令牌**。
2. OpenAgents 将该单一令牌以 `BWS_ACCESS_TOKEN` 的形式存储在 `~/.openagents/.env` 中。
3. 每次 `hermes`（或 gateway，或 cron 任务）启动时，在加载 `~/.openagents/.env` 之后，Hermes 会调用 `bws secret list <project_id>` 并将返回的密钥写入 `os.environ`。
4. 默认情况下，Hermes **覆盖**环境中已有的值，因此 Bitwarden 是唯一可信来源——在 Web 应用中轮换一次密钥，每个 OpenAgents 进程在下次启动时即可获取最新值。如果希望 `.env` 优先，可在配置中将 `override_existing: false`。

`bws` 二进制文件在首次使用时会自动下载到 `~/.openagents/bin/`，无需 `apt`、`brew` 或 `sudo`。
...
4. **Machine accounts → New machine account → My OpenAgents machine** → **Projects** 标签页 → 授予对你的项目的 Read 权限。
...
1. 下载并验证 `bws v2.0.0`，存放至 `~/.openagents/bin/bws`。
2. 提示输入访问令牌（输入内容隐藏）。以 `BWS_ACCESS_TOKEN` 形式存储在 `~/.openagents/.env` 中。
...
| `auto_install` | `true` | 为 true 时，首次使用时自动将 `bws` 下载到 `~/.openagents/bin/`。 |

## 故障模式

Bitwarden 永远不会阻塞 OpenAgents 启动。如果出现任何问题，stderr 会显示一行警告，Hermes 继续使用 `.env` 中已有的凭据：
...
- **单机个人使用**，`~/.openagents/.env` 已经够用。你只是用一个凭据换了另一个，并在启动时增加了网络依赖。
- **无法访问 `api.bitwarden.com` 的隔离环境**。
- **CI/CD** 场景，已有现成的 secret 注入机制（GitHub Actions secrets、Vault 等）——选择一种方式，不要两者并用。

适合使用此功能的场景：多机器集群、共享开发机、gateway VPS，或任何需要跨多个 OpenAgents 安装进行集中轮换和吊销管理的场景。