P---
title: "Windows（原生）指南"
description: "在 Windows 10 / 11 上原生运行 OpenAgents — 安装、功能矩阵、UTF-8 控制台、Git Bash、将 gateway 作为计划任务、编辑器处理、PATH、卸载及常见问题"
...
| `-InstallDir` | `%LOCALAPPDATA%\hermes\openagents` | 覆盖代码存放位置                                |

安装程序会自动重试不稳定的 git 拉取，并剥离下载的 `install.ps1` 内容中的 BOM，因此 HTTP 传输中携带的 UTF-8 BOM 不再会破坏 `[scriptblock]::Create((irm ...))` 形式。

### 桌面安装程序（备选方案）

也提供了一个轻量 GUI 安装程序——如果你更倾向于双击 `.exe` 而非打开 PowerShell，可以使用它。下载 OpenAgents Desktop，运行安装程序，首次启动时 GUI 会在后台调用 `install.ps1` 来配置 Python（通过 `uv`）、Node、PortableGit 以及下文描述的其余依赖引导流程。首次运行后，桌面应用与 PowerShell 安装的 `hermes` CLI 共享同一个 `%LOCALAPPDATA%\hermes\openagents` 安装目录和 `%USERPROFILE%\.hermes` 数据目录——可以在 GUI 和 CLI 之间自由切换。

如果你想要熟悉的 Windows 安装体验，或者要将 OpenAgents 交给非开发者使用，请使用桌面安装程序；如果你已经在终端中，请使用 PowerShell 一行命令。

### 依赖引导（`dep_ensure`）

在首次启动时（以及检测到缺少工具时按需触发），Hermes 会运行一个小型 Python 引导程序——`openagents_cli/dep_ensure.py`——检查并懒加载安装所需的非 Python 依赖。在 Windows 上，相关依赖如下：

| 依赖            | OpenAgents 需要它的原因                                                                             |
| --------------- | ----------------------------------------------------------------------------------------------- |
...
5. **将仓库克隆**到 `%LOCALAPPDATA%\hermes\openagents` 并在其中创建 virtualenv。
...
8. **设置 `HERMES_GIT_BASH_PATH`** 为解析后的 `bash.exe` 路径，使 OpenAgents 在新 shell 中能确定性地找到它。
...
## OpenAgents 在 Windows 上如何运行 shell 命令

OpenAgents 的终端工具通过 **Git Bash** 运行命令，与 Claude Code 采用相同策略。这在不重写每个工具的情况下绕过了 POSIX 与 Windows 的差异。
...
安装程序会显式设置 `HERMES_GIT_BASH_PATH`，使新 PowerShell 会话无需重新发现。如果你想让 OpenAgents 使用特定的 bash——例如系统 Git Bash 或通过符号链接的 WSL bash——可以覆盖此变量。
...
修复逻辑位于 `openagents_cli/stdio.py::configure_windows_stdio()`，在每个入口点（`cli.py::main`、`openagents_cli/main.py::main`、`gateway/run.py::main`）的早期调用。它会：
...
OpenAgents 的 Windows stdio 垫片现在将 `EDITOR=notepad` 设为默认值。Notepad 随每个 Windows 安装附带，可作为阻塞式编辑器使用——`subprocess.call(["notepad", file])` 会阻塞直到窗口关闭。
...
服务需要管理员权限安装，并将 gateway 的生命周期绑定到机器启动，而非用户登录。典型的 OpenAgents 用户希望：登录 → gateway 可用，注销 → gateway 消失。计划任务无需提权即可实现这一点。如果你确实需要服务，可以手动使用 `nssm` 或 `sc create`——但你可能并不需要。

## 数据布局

| 路径                                  | 内容                                                            |
| ------------------------------------- | --------------------------------------------------------------- |
| `%LOCALAPPDATA%\hermes\openagents\` | Git 检出 + venv。可安全执行 `Remove-Item -Recurse` 后重新安装。 |
...
这种分离是有意为之：`%LOCALAPPDATA%\hermes` 是可丢弃的基础设施（可以删除后用一行命令恢复）。`%USERPROFILE%\.hermes` 是你的数据——配置、记忆、技能、会话历史——其结构与 Linux 安装完全相同。在机器间同步它，你的 OpenAgents 就随之迁移。

**覆盖 `OPENAGENTS_HOME`：** 设置该环境变量以指向不同的数据目录。与 Linux 上的用法相同。
...
- `shutil.which("agent-browser", path=...)` 会自动找到 `.cmd` 垫片——`CreateProcessW` 无法执行无扩展名的 shebang 脚本，因此 OpenAgents 始终解析到 `.CMD` 包装器。不要手动调用 shebang 脚本；始终通过 `.cmd` 调用。
- Playwright Chromium 在首次运行时自动安装（`npx playwright install chromium`）。如果安装失败，`hermes doctor` 会给出修复提示。

## 在 Windows 上运行 OpenAgents — 实用说明
...
OpenAgents 同时支持 `$env:X`（进程作用域）和用户环境变量（永久，在系统属性 → 环境变量中设置）。将 API key 放在 `%USERPROFILE%\.hermes\.env` 中是标准做法——与 Linux 相同：
...
这是干净的卸载路径——移除 schtasks 条目、Startup 文件夹快捷方式、`hermes.cmd` 垫片，删除 `%LOCALAPPDATA%\hermes\openagents\`，并从用户 PATH 中移除相关条目。它会保留 `%USERPROFILE%\.hermes\`（你的配置、认证、技能、会话、日志），以防你需要重新安装。
...
安装程序在 `%LOCALAPPDATA%\hermes\node` 配置了 Node 22，但你的 PATH 中可能有更靠前的旧版系统 Node 18。要么将 OpenAgents 的 node 目录移到 PATH 前面，要么如果你不在其他地方使用 Node，删除系统安装。
...
这与 Windows 无关，但有时首先在 Windows 上暴露。通常意味着 JSON 请求体中的文件路径包含未转义的反斜杠。Telegram 应该收到 OpenAgents 规范化后的路径，而非原始 Windows 路径——如果你在自定义插件中看到此问题，请确保传递的是 OpenAgents 提供的路径，而非来自用户输入的 `str(Path(...))`。

**`git pull` 后出现"在我另一台机器上能用"的编码怪象。**
如果你在 Windows 上使用非 UTF-8 编辑器（旧版 Windows 的 Notepad、某些中文输入法）编辑了 OpenAgents 配置或技能文件，该文件可能带 BOM 保存。Hermes 在大多数配置读取中能容忍 `utf-8-sig`，但折叠 YAML 标量（`description: >`）内部的 BOM 会静默破坏 YAML 解析。请将文件重新保存为不带 BOM 的纯 UTF-8。
