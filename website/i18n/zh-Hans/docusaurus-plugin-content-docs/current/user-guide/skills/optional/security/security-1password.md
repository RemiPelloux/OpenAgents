...
- `tmux` 可用，用于在 OpenAgents 终端调用期间保持稳定的已认证会话（仅限桌面应用流程）
...
在 `~/.openagents/.env` 中设置 `OP_SERVICE_ACCOUNT_TOKEN`（skill 首次加载时会提示输入）。
...
## OpenAgents 执行模式（桌面应用流程）

OpenAgents 终端命令默认为非交互式，且在多次调用之间可能丢失认证上下文。
