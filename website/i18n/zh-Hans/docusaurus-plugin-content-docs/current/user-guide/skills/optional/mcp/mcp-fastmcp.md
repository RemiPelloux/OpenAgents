...
- 除工具外还需暴露资源或 prompt
- 在接入 OpenAgents 或其他客户端之前，使用 FastMCP CLI 对服务器进行冒烟测试
...
- 使用 `native-mcp` skill，在 `~/.openagents/config.yaml` 中配置服务器，或
...
### OpenAgents 无法看到已部署的服务器

服务器构建部分可能正确，但 OpenAgents 配置有误。加载 `native-mcp` skill 并在 `~/.openagents/config.yaml` 中配置服务器，然后重启 Hermes。
