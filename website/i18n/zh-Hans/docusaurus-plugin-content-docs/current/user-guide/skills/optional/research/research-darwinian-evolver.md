...
调用来调用它（纯聚合方式）。**不得**将上游类导入 OpenAgents 本身。
...
```bash
mkdir -p ~/.openagents/cache/darwinian-evolver && cd ~/.openagents/cache/darwinian-evolver
...
```bash
cd ~/.openagents/cache/darwinian-evolver/darwinian_evolver \
...
在浏览器中打开 `~/.openagents/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
并加载 JSON 日志，即可查看进化树。
...
SKILL_DIR=~/.openagents/skills/research/darwinian-evolver
DE_DIR=~/.openagents/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL 协议。** 永远不要在 OpenAgents 核心中使用 `from darwinian_evolver import ...`。`~/.openagents/skills/...` 下的自定义驱动脚本属于用户侧，没有问题。
