...
Gmail、Calendar、Drive、Contacts、Sheets 和 Docs —— 通过 OpenAgents 管理的 OAuth（开放授权）和轻量 CLI 封装器实现。若已安装 `gws`，该 skill 将以其作为执行后端以获得更广泛的 Google Workspace 覆盖；否则回退到内置的 Python 客户端实现。
...
- `scripts/google_api.py` —— 兼容性封装 CLI。在可用时优先使用 `gws` 执行操作，同时保留 OpenAgents 现有的 JSON 输出契约。
...
```bash
GSETUP="python ${OPENAGENTS_HOME:-$HOME/.hermes}/skills/productivity/google-workspace/scripts/setup.py"
```
...
>
> OpenAgents CLI 重要提示：若文件路径以 `/` 开头，请勿在 CLI 中单独发送该裸路径，因为它可能被误识别为斜杠命令。请将其放在句子中发送，例如：
...
此命令返回包含 `auth_url` 字段的 JSON，并将该 URL 保存至 `~/.openagents/google_oauth_last_url.txt`。
...
- Token 存储于 `~/.openagents/google_token.json`，自动刷新。
- 待处理的 OAuth 会话状态/验证器临时存储于 `~/.openagents/google_oauth_pending.json`，直至交换完成。
- 若已安装 `gws`，`google_api.py` 会将其指向同一个 `~/.openagents/google_token.json` 凭据文件。用户无需单独运行 `gws auth login` 流程。
- 撤销授权：`$GSETUP --revoke`
...
```bash
GAPI="python ${OPENAGENTS_HOME:-$HOME/.hermes}/skills/productivity/google-workspace/scripts/google_api.py"
```
