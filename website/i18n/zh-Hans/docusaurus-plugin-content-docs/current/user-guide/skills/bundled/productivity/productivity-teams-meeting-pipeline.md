...
description: "通过 OpenAgents CLI 操作 Teams 会议摘要流水线 — 总结会议、检查流水线状态、重放任务、管理 Microsoft Graph 订阅"
---
...
通过 OpenAgents CLI 操作 Teams 会议摘要流水线 — 总结会议、检查流水线状态、重放任务、管理 Microsoft Graph 订阅。
...
使用流水线前，请确认以下变量已在 `~/.openagents/.env` 中设置：
