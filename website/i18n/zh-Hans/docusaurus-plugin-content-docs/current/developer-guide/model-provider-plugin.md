...
sidebar_position: 10
title: "模型提供商插件"
description: "如何为 OpenAgents 构建模型提供商（推理后端）插件"
---

# 构建模型提供商插件

模型提供商插件声明一个推理后端——兼容 OpenAI 的端点、Anthropic Messages 服务器、Codex 风格的 Responses API，或 Bedrock 原生接口——Hermes 可通过这些后端路由 `AIAgent` 调用。每个内置提供商（OpenRouter、Anthropic、GMI、DeepSeek、Nvidia……）都以此类插件形式提供。第三方可通过在 `$OPENAGENTS_HOME/plugins/model-providers/` 下放置一个目录来添加自己的提供商，无需对仓库做任何修改。
...
1. **内置插件** — `<repo>/plugins/model-providers/<name>/` — 随 OpenAgents 一同发布
2. **用户插件** — `$OPENAGENTS_HOME/plugins/model-providers/<name>/` — 放入任意目录；后续会话无需重启即可生效
3. **旧版单文件** — `<repo>/providers/<name>.py` — 为树外可编辑安装提供向后兼容

**同名用户插件会覆盖内置插件**，因为 `register_provider()` 采用后写者优先策略。放入 `$OPENAGENTS_HOME/plugins/model-providers/gmi/` 目录即可替换内置 GMI profile，无需修改仓库。
...
| 集成点 | 位置 | 获得的能力 |
|---|---|---|
| 凭据解析 | `openagents_cli/auth.py` | `PROVIDER_REGISTRY["acme-inference"]` 从 profile 填充 |
| `--provider` CLI 标志 | `openagents_cli/main.py` | 接受 `acme-inference` |
| `hermes model` 选择器 | `openagents_cli/models.py` | 出现在 `CANONICAL_PROVIDERS` 中，从 `{base_url}/models` 获取模型列表 |
| `hermes doctor` | `openagents_cli/doctor.py` | 对 `ACME_API_KEY` 及 `{base_url}/models` 进行健康检查 |
| `hermes setup` | `openagents_cli/config.py` | `ACME_API_KEY` 出现在 `OPTIONAL_ENV_VARS` 和设置向导中 |
| URL 反向映射 | `agent/model_metadata.py` | 主机名 → 提供商名称，用于自动检测 |
| 辅助模型 | `agent/auxiliary_client.py` | 使用 `default_aux_model` 进行压缩/摘要 |
| 运行时解析 | `openagents_cli/runtime_provider.py` | 返回正确的 `base_url`、`api_key`、`api_mode` |
...
假设你想将 `gmi` 指向私有测试端点进行测试。创建 `~/.openagents/plugins/model-providers/gmi/__init__.py`：
...
`auth_type` 控制哪些代码路径将你的提供商视为"简单 api-key 提供商"——若不是 `api_key`，PluginManager 仍会记录 manifest，但 OpenAgents CLI 层面的自动化（doctor 检查、`--provider` 标志、设置向导委托）可能会跳过它。
...
将 `OPENAGENTS_HOME` 指向临时目录，避免污染真实配置：
...
完整的入口点设置请参阅 [构建 OpenAgents 插件](/guides/build-a-hermes-plugin#distribute-via-pip)。
...
- [构建 OpenAgents 插件](/guides/build-a-hermes-plugin) — 通用插件编写指南