...
sidebar_position: 3
title: "创建 Skill"
description: "如何为 OpenAgents 创建 skill——SKILL.md 格式、规范与发布"
---

# 创建 Skill

Skill 是为 OpenAgents 添加新能力的首选方式。与 tool 相比，skill 更易于创建，无需修改 agent 代码，且可与社区共享。
...
:::tip 如何选择
对 API key、token 及其他**密钥**使用 `required_environment_variables`（存储在 `~/.openagents/.env`，不向模型展示）。对**路径、偏好设置及非敏感配置**使用 `config`（存储在 `config.yaml`，在 config show 中可见）。
:::
...
:::tip 如何选择
对简单的 API key 和 token（存储在 `~/.openagents/.env` 中的字符串）使用 `required_environment_variables`。对 OAuth token 文件、客户端密钥、服务账号 JSON、证书或任何以磁盘文件形式存在的凭证使用 `required_credential_files`。
:::
...
优先使用标准库 Python、curl 以及现有 OpenAgents tool（`web_extract`、`terminal`、`read_file`）。若确实需要依赖项，请在 skill 中记录安装步骤。
...
内置 skill（位于 `skills/`）随每次 OpenAgents 安装一起发布，应对**大多数用户广泛有用**：
...
信任级别：
- `builtin`——随 OpenAgents 一起发布（始终受信任）
...
OpenAgents 现在可以通过多种外部发现模型使用第三方 skill：
