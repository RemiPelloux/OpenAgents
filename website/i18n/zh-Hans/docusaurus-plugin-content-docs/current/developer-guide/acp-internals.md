...
ACP 适配器将 OpenAgents 的同步 `AIAgent` 封装为异步 JSON-RPC stdio 服务器。
...
Zed ACP Registry 路径通过 `uvx --from 'openagents[acp]==<version>' openagents-acp` 启动同一适配器，指向 `openagents` PyPI 发布包。
...
- 拒绝选项 -> OpenAgents `deny`

超时和桥接失败默认拒绝。

### 工具渲染辅助

`acp_adapter/tools.py` 将 OpenAgents 工具映射到 ACP 工具类型，并构建面向编辑器的内容。
...
而是复用 OpenAgents 的运行时解析器：

- `acp_adapter/auth.py`
- `openagents_cli/runtime_provider.py`

因此 ACP 通告并使用当前配置的 OpenAgents provider/凭据。它还始终通告一个终端 setup 认证方法（`hermes-setup`，参数 `--setup`），以便首次运行的 registry 客户端在启动正常 ACP 会话前可以打开 OpenAgents 的交互式模型/provider 配置。
...
- ACP 会话持久化至共享的 `~/.openagents/state.db`（SessionDB），在进程重启后透明恢复；它们会出现在 `session_search` 中
...
- `tests/acp/` — ACP 测试套件
- `toolsets.py` — `openagents-acp` toolset 定义
- `openagents_cli/main.py` — `hermes acp` CLI 子命令
- `pyproject.toml` — `[acp]` 可选依赖 + `openagents-acp` 脚本