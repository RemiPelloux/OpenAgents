...
sidebar_position: 3
title: "更新与卸载"
description: "如何将 OpenAgents 更新至最新版本或将其卸载"
---
...
1. **配对数据快照** — 保存一份轻量级的更新前状态快照（涵盖 `~/.openagents/pairing/`、飞书评论规则及其他运行时修改的状态文件）。可通过 [快照与回滚](../user-guide/checkpoints-and-rollback.md) 中描述的快照恢复流程进行恢复，或从 OpenAgents 写入 `~/.openagents/` 目录旁的最新快速快照 zip 文件中提取。
2. **Git pull** — 从 `main` 分支拉取最新代码并更新子模块
3. **依赖安装** — 运行 `uv pip install -e ".[all]"` 以获取新增或变更的依赖项
4. **配置迁移** — 检测自当前版本以来新增的配置选项并提示设置
5. **Gateway 自动重启** — 更新完成后刷新正在运行的 gateway，使新代码立即生效。由服务管理的 gateway（Linux 上的 systemd、macOS 上的 launchd）通过服务管理器重启；手动启动的 gateway 在 OpenAgents 能将运行中的 PID 映射回某个 profile 时会自动重新启动。
...
对于高价值 profile（生产环境 gateway、团队共享安装），可选择在拉取前对 `OPENAGENTS_HOME`（配置、认证、会话、技能、配对数据）进行完整备份：
...
在 Windows 上，如果 `hermes update` 检测到另一个 `hermes.exe` 进程持有 venv 入口点可执行文件的句柄，它将拒绝运行 — 最常见的情况是 OpenAgents Desktop 应用启动的后端进程、另一个终端中打开的 `hermes` REPL，或正在运行的 gateway：
...
- 更新运行期间，所有输出会同步镜像到 `~/.openagents/logs/update.log`。如果终端消失，重新连接后检查日志，确认更新是否完成以及 gateway 重启是否成功：
...
与 [GitHub releases 页面](https://github.com/NousResearch/openagents/releases) 上的最新版本进行比较。
...
卸载程序会提供选项，让你保留配置文件（`~/.openagents/`）以便将来重新安装。
...
rm -rf ~/.openagents            # 可选 — 如计划重新安装则保留
```
