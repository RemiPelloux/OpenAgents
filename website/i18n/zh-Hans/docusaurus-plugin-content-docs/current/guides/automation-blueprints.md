...
3. For each merged PR, check if it modified:
   ...
   - CLI commands (openagents_cli/commands.py, openagents_cli/main.py) — may need docs/reference/cli-commands.md update
   - Config options (openagents_cli/config.py) — may need docs/user-guide/configuration.md update
...
hermes cron create "0 6 * * *" \
  "Run a dependency security audit on the openagents project.

1. cd ~/.openagents/openagents && source .venv/bin/activate
...
```python title="~/.openagents/scripts/check-uptime.py"
...
hermes cron create "every 30m" \
  ...
  --script ~/.openagents/scripts/check-uptime.py \
  --name "Uptime monitor" \
  --deliver telegram
```
...
```bash
hermes cron create "0 8 * * *" \
  "Search arXiv for the 3 most interesting papers on 'language model reasoning' OR 'tool-use agents' from the past day. For each paper, create an Obsidian note with the title, authors, abstract summary, key contribution, and potential relevance to OpenAgents development." \
...
hermes cron create "0 3 * * 0" \
  "Run a comprehensive security audit of the openagents codebase.
