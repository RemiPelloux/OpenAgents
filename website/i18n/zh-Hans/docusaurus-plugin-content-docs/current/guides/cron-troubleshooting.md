...
sidebar_position: 12
title: "Cron 故障排查"
description: "诊断并修复常见的 OpenAgents cron 问题——任务未触发、投递失败、skill 加载错误及性能问题"
---
...
| 目标 | 所需配置 |
|--------|----------|
| `telegram` | `~/.openagents/.env` 中的 `TELEGRAM_BOT_TOKEN` |
| `discord` | `~/.openagents/.env` 中的 `DISCORD_BOT_TOKEN` |
| `slack` | `~/.openagents/.env` 中的 `SLACK_BOT_TOKEN` |
| `whatsapp` | 已配置 WhatsApp gateway |
| `signal` | 已配置 Signal gateway |
| `matrix` | 已配置 Matrix homeserver |
| `email` | `config.yaml` 中已配置 SMTP |
| `sms` | 已配置 SMS 提供商 |
| `local` | 对 `~/.openagents/cron/output/` 有写权限 |
...
1. 任务投递的聊天会话（若投递成功）
2. `~/.openagents/logs/agent.log`（调度器消息）或 `errors.log`（警告信息）
3. 通过 `hermes cron list` 查看任务的 `last_run` 元数据

### 检查 2：常见错误模式

**脚本报 "No such file or directory"**
`script` 路径必须为绝对路径（或相对于 OpenAgents 配置目录的路径）。验证：
```bash
ls ~/.openagents/scripts/your-script.py   # 必须存在
...
任务存储在 `~/.openagents/cron/jobs.json`。若该文件对当前用户不可读写，调度器将静默失败：

```bash
ls -la ~/.openagents/cron/jobs.json
chmod 600 ~/.openagents/cron/jobs.json   # 应由你的用户拥有
```
...
hermes logs                         # 查看近期 OpenAgents 日志
...
2. 查看 `~/.openagents/logs/agent.log` 中的调度器消息和 `~/.openagents/logs/errors.log` 中的警告
3. 在 [github.com/NousResearch/openagents](https://github.com/NousResearch/openagents) 提交 issue，并附上：
   - 任务 ID 和调度表达式
   - 投递目标
   - 预期行为与实际行为
   - 日志中的相关错误信息
