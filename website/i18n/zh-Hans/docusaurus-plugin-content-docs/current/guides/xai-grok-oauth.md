...
description: "使用 SuperGrok 或 X Premium+ 订阅登录，在 OpenAgents 中使用 Grok 模型——无需 API 密钥"
---

# xAI Grok OAuth（SuperGrok / X Premium+）

OpenAgents 通过基于浏览器的 OAuth 登录流程支持 xAI Grok，认证服务器为 [accounts.x.ai](https://accounts.x.ai)，支持 **SuperGrok 订阅**（[grok.com](https://x.ai/grok)）或 **X Premium+ 订阅**（已关联的 X 账号）。无需 `XAI_API_KEY`——登录一次后，Hermes 会在后台自动刷新会话。
...
同一 OAuth bearer token 也会被 OpenAgents 中所有直连 xAI 的功能复用——TTS、图像生成、视频生成和转录——因此单次登录即可覆盖全部四项功能。
...
xAI 的后端对 OAuth API 接口维护自己的白名单，已有记录显示即使应用内订阅处于激活状态，标准 SuperGrok 订阅者也会收到 `HTTP 403`（见 issue [#26847](https://github.com/NousResearch/openagents/issues/26847)）。如果浏览器中 OAuth 登录成功但推理返回 403，请设置 `XAI_API_KEY` 并切换到 API 密钥路径（`provider: xai`）——该接口目前不受相同限制。
:::
...
# → OpenAgents 在浏览器中打开 accounts.x.ai
# → 在浏览器中批准访问
...
首次登录后，凭据存储在 `~/.openagents/auth.json` 中，并在过期前自动刷新。
...
完整操作说明请参阅 [OAuth over SSH / Remote Hosts](./oauth-over-ssh.md#browser-only-remote-cloud-shell--codespaces--ec2-instance-connect)。此为 [#26923](https://github.com/NousResearch/openagents/issues/26923) 的回归修复。

## 登录流程说明

1. OpenAgents 在浏览器中打开 `accounts.x.ai`。
2. 你登录（或确认现有会话）并批准访问。
3. xAI 重定向回 Hermes，token 保存到 `~/.openagents/auth.json`。
...
登录后，`~/.openagents/config.yaml` 将包含：
...
OpenAgents 在每次会话前刷新 token，并在收到 401 时响应式地再次刷新。如果刷新因 `invalid_grant` 失败（刷新 token 被撤销或账号已轮换），Hermes 会显示类型化的重新认证消息，而不是崩溃。
...
OpenAgents 检测到授权服务器返回的 `state` 值与发送的不匹配。
...
这**不是** token 过期问题——重新运行 `hermes model` 不会改变结果。xAI 的后端已被观察到将 OAuth API 访问限制在特定 SuperGrok 套餐，即使应用内订阅处于激活状态（issue [#26847](https://github.com/NousResearch/openagents/issues/26847)）。
...
- [OAuth over SSH / Remote Hosts](./oauth-over-ssh.md) — 如果 OpenAgents 与浏览器不在同一台机器上，必读
