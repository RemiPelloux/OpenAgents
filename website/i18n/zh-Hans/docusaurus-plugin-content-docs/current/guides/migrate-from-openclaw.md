...
sidebar_position: 10
title: "从 OpenClaw 迁移"
description: "将 OpenClaw / Clawdbot 配置迁移到 OpenAgents 的完整指南——包括迁移内容、配置键映射及迁移后的检查事项。"
---
...
| `--overwrite` | 冲突时覆盖已有 OpenAgents 文件（默认：计划存在冲突时拒绝执行）。 |
| `--migrate-secrets` | 包含 API 密钥。即使使用 `--preset full` 也需要显式指定——没有任何预设会静默导入密钥。 |
| `--no-backup` | 跳过迁移前对 `~/.openagents/` 的 zip 快照备份（默认在执行前写入单个还原点归档，位于 `~/.openagents/backups/pre-migration-*.zip`；可通过 `hermes import` 还原）。 |
...
| 内容 | OpenClaw 来源 | OpenAgents 目标 | 备注 |
|------|----------------|-------------------|-------|
| Persona | `workspace/SOUL.md` | `~/.openagents/SOUL.md` | 直接复制 |
| 工作区指令 | `workspace/AGENTS.md` | `--workspace-target` 中的 `AGENTS.md` | 需要 `--workspace-target` 标志 |
| 长期记忆 | `workspace/MEMORY.md` | `~/.openagents/memories/MEMORY.md` | 解析为条目，与现有内容合并并去重，使用 `§` 分隔符 |
| 用户档案 | `workspace/USER.md` | `~/.openagents/memories/USER.md` | 与记忆相同的条目合并逻辑 |
| 每日记忆文件 | `workspace/memory/*.md` | `~/.openagents/memories/MEMORY.md` | 所有每日文件合并至主记忆 |
...
| 来源 | OpenClaw 位置 | OpenAgents 目标 |
|--------|------------------|-------------------|
| 工作区 skills | `workspace/skills/` | `~/.openagents/skills/openclaw-imports/` |
| 托管/共享 skills | `~/.openclaw/skills/` | `~/.openagents/skills/openclaw-imports/` |
| 个人跨项目 skills | `~/.agents/skills/` | `~/.openagents/skills/openclaw-imports/` |
| 项目级共享 skills | `workspace/.agents/skills/` | `~/.openagents/skills/openclaw-imports/` |

Skill 冲突由 `--skill-conflict` 处理：`skip` 保留现有 OpenAgents skill，`overwrite` 替换，`rename` 创建带 `-imported` 后缀的副本。

### 模型与 Provider 配置

| 内容 | OpenClaw 配置路径 | OpenAgents 目标 | 备注 |
|------|---------------------|-------------------|-------|
...
| Provider API 密钥 | `models.providers.*.apiKey` | `~/.openagents/.env` | 需要 `--migrate-secrets`。参见下方 [API 密钥解析](#api-key-resolution) |

### Agent 行为

| 内容 | OpenClaw 配置路径 | OpenAgents 配置路径 | 映射规则 |
|------|---------------------|-------------------|---------|
...
| OpenClaw 配置路径 | OpenAgents 配置路径 | 备注 |
|---------------------|-------------------|-------|
...
| TTS 资源文件 | `~/.openagents/tts/`（文件复制） |

### 消息平台

| 平台 | OpenClaw 配置路径 | OpenAgents `.env` 变量 | 备注 |
|----------|---------------------|----------------------|-------|
...
| 内容 | OpenClaw 路径 | OpenAgents 路径 | 备注 |
|------|-------------|-------------|-------|
...
### 已归档（无对应 OpenAgents 等效项）

以下内容保存至 `~/.openagents/migration/openclaw/<timestamp>/archive/` 供人工审查：

| 内容 | 归档文件 | 在 OpenAgents 中的重建方式 |
|------|-------------|--------------------------|
| `IDENTITY.md` | `archive/workspace/IDENTITY.md` | 合并至 `SOUL.md` |
| `TOOLS.md` | `archive/workspace/TOOLS.md` | OpenAgents 内置工具说明 |
...
| 多 Agent 列表 | `archive/agents-list.json` | 使用 OpenAgents profiles |
...
2. **审查归档文件** — `~/.openagents/migration/openclaw/<timestamp>/archive/` 中的所有内容需要人工处理。

3. **开启新会话** — 导入的 skills 和记忆条目在新会话中生效，当前会话不受影响。

4. **验证 API 密钥** — 运行 `hermes status` 检查 provider 认证状态。

5. **测试消息平台** — 若迁移了平台 token，重启 gateway：`systemctl --user restart openagents-gateway`
...
导入的 skills 位于 `~/.openagents/skills/openclaw-imports/`。开启新会话后生效，或运行 `/skills` 验证是否已加载。
