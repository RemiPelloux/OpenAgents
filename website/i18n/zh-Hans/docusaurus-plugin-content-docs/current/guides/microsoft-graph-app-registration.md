...
完成本指南需要**租户管理员权限**（或由管理员代为授予同意）。请记录收集到的值——最终需要填入 `~/.openagents/.env`。
...
```powershell
# Create a policy scoped to the OpenAgents app
New-CsApplicationAccessPolicy `
  -Identity "Hermes-Meeting-Pipeline-Policy" `
  -AppIds "<MSGRAPH_CLIENT_ID>" `
  -Description "Restrict OpenAgents meeting pipeline to allow-listed users"
...
OpenAgents 内置了 Graph 身份验证冒烟测试。在 OpenAgents 安装目录下执行：
...
2. 用新值更新 `~/.openagents/.env` 中的 `MSGRAPH_CLIENT_SECRET`。
