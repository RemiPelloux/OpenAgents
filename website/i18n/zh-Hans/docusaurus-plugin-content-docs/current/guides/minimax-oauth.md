...
sidebar_position: 15
title: "MiniMax OAuth"
description: "通过浏览器 OAuth 登录 MiniMax，在 OpenAgents 中使用 MiniMax-M2.7 模型——无需 API 密钥"
---

# MiniMax OAuth

OpenAgents 通过基于浏览器的 OAuth 登录流程支持 **MiniMax**，使用与 [MiniMax 门户](https://www.minimax.io) 相同的凭据。无需 API 密钥或信用卡——登录一次，Hermes 即可自动刷新您的会话。
...
# → OpenAgents 在浏览器中打开 MiniMax 授权页面
# → 在浏览器中批准访问
...
首次登录后，凭据将存储在 `~/.openagents/auth.json` 下，并在每次会话前自动刷新。
...
OpenAgents 将打印验证 URL 和用户码——在任意设备上打开该 URL，并在提示时输入用户码。

## OAuth 流程

OpenAgents 针对 MiniMax OAuth 端点实现了 PKCE 设备码流程：

1. OpenAgents 生成 PKCE verifier/challenge 对和一个随机 state 值。
...
4. OpenAgents 轮询 `{base_url}/oauth/token`，直到令牌到达（或超过截止时间）。
5. 令牌（`access_token`、`refresh_token`、过期时间）以 `minimax-oauth` 为键保存到 `~/.openagents/auth.json`。
...
登录后，`~/.openagents/config.yaml` 将包含类似如下的条目：
...
OpenAgents 在每次会话启动时，若 access token 距过期不足 60 秒则刷新令牌。如果 access token 已经过期（例如长时间离线后），刷新将在下一次请求时自动触发。如果刷新失败并返回 `refresh_token_reused` 或 `invalid_grant`，Hermes 会将会话标记为需要重新登录。
...
OpenAgents 检测到授权服务器返回的 `state` 值与其发送的值不匹配。
...
OpenAgents 将打印 URL 和用户码。在任意设备上打开该 URL 并在那里完成流程。
