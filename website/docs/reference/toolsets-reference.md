...
description: "Reference for OpenAgents core, composite, platform, and dynamic toolsets"
---
...
| `code_execution` | `execute_code` | Run Python scripts that call OpenAgents tools programmatically. |
...
| `openagents-acp` | Drops `clarify`, `cronjob`, `image_generate`, `text_to_speech`, and all four Home Assistant tools. Focused on coding tasks in IDE context. |
...
| `openagents-gateway` | Internal gateway orchestrator toolset — union of every `hermes-<platform>` toolset; used when the gateway needs to accept any message source. |
