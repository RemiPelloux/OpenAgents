...
description: "Frequently asked questions and solutions to common issues with OpenAgents"
---
...
OpenAgents works with any OpenAI-compatible API. Supported providers include:
...
Set your provider with `hermes model` or by editing `~/.openagents/.env`. See the [Environment Variables](./environment-variables.md) reference for all provider keys.
...
### I run OpenAgents in WSL2. What's the best way to control my normal Windows Chrome?

Prefer an MCP bridge over `/browser connect`.

Recommended pattern:

- run OpenAgents inside WSL2
...
- let OpenAgents use the resulting MCP browser tools

This is more reliable than trying to force OpenAgents core browser transport to attach directly across the WSL2/Windows boundary.
...
API calls go **only to the LLM provider you configure** (e.g., OpenRouter, your local Ollama instance). OpenAgents does not collect telemetry, usage data, or analytics. Your conversations, memory, and skills are stored locally in `~/.openagents/`.
...
# Context length: 64000   ← OpenAgents minimum; set this to match your server's actual context window
```
...
OpenAgents persists the endpoint, provider, and base URL in `config.yaml` so it survives restarts. If your local server has exactly one model loaded, `/model custom` auto-detects it. You can also set `provider: custom` in config.yaml — it's a first-class provider, not an alias for anything else.
...
:::tip Ollama users
If you set a custom `num_ctx` in Ollama (e.g., `ollama run --num_ctx 64000`), make sure to set the matching context length in OpenAgents — Ollama's `/api/show` reports the model's *maximum* context, not the effective `num_ctx` you configured.
:::

:::tip Timeouts with local models
OpenAgents auto-detects local endpoints and relaxes streaming timeouts (read timeout raised from 120s to 1800s, stale stream detection disabled). If you still hit timeouts on very large contexts, set `HERMES_STREAM_READ_TIMEOUT=1800` in your `.env`. See the [Local LLM guide](../guides/local-llm-on-mac.md#timeouts) for details.
:::

### How much does it cost?

OpenAgents itself is **free and open-source** (MIT license). You pay only for the LLM API usage from your chosen provider. Local models are completely free to run.

### Can multiple people use one instance?

Yes. The [messaging gateway](../user-guide/messaging/index.md) lets multiple users interact with the same OpenAgents instance via Telegram, Discord, Slack, WhatsApp, or Home Assistant. Access is controlled through allowlists (specific user IDs) and DM pairing (first user to message claims access).
...
Yes. Import the `AIAgent` class and use OpenAgents programmatically:
...
**Cause:** OpenAgents requires Python 3.11 or newer.
...
**Cause:** OpenAgents builds a per-session environment snapshot by running `bash -l` once at startup. A bash login shell reads `/etc/profile`, `~/.bash_profile`, and `~/.profile`, but **does not source `~/.bashrc`** — so tools that install themselves there (`nvm`, `asdf`, `pyenv`, `cargo`, custom `PATH` exports) stay invisible to the snapshot. This most commonly happens when OpenAgents runs under systemd or in a minimal shell where nothing has pre-loaded the interactive shell profile.

**Solution:** OpenAgents auto-sources `~/.bashrc` by default. If that's not enough — e.g. you're a zsh user whose PATH lives in `~/.zshrc`, or you init `nvm` from a standalone file — list the extra files to source in `~/.openagents/config.yaml`:
...
```bash
# Exit the OpenAgents chat session first (Ctrl+C or /quit)
...
:::warning
Make sure the key matches the provider. An OpenAI key won't work with OpenRouter and vice versa. Check `~/.openagents/.env` for conflicting entries.
:::
...
**Cause:** The conversation has grown too long for the model's context window, or OpenAgents detected the wrong context length for your model.
...
If this happens on the first long conversation, OpenAgents may have the wrong context length for your model. Check what it detected:
...
**Cause:** OpenAgents detected a potentially destructive command (e.g., `rm -rf`, `DROP TABLE`). This is a safety feature.
...
:::tip
This is working as intended — OpenAgents never silently runs destructive commands. The approval prompt shows you exactly what will execute.
:::
...
- Check gateway logs: `cat ~/.openagents/logs/gateway.log | tail -50`
...
Configure in `~/.openagents/config.yaml` under your gateway's settings. See the [Messaging docs](../user-guide/messaging/index.md).
...
cd ~/.openagents/openagents && uv pip install -e ".[messaging]"  # Telegram, Discord, Slack, and shared gateway deps
...
Verify your `~/.openagents/config.yaml` MCP configuration:
```yaml
mcp_servers:
  filesystem:
    command: "npx"
...
# Restart OpenAgents or reload MCP after config changes
hermes chat
```
...
:::warning
If an MCP server crashes mid-request, OpenAgents will report a timeout. Check the server's own logs (not just OpenAgents logs) to diagnose the root cause.
:::
...
There is no hard limit. Each profile is just a directory under `~/.openagents/profiles/`. The practical limit depends on your disk space and how many concurrent gateways your system can handle (each gateway is a lightweight Python process). Running dozens of profiles is fine; each idle profile uses no resources.
...
**Solution: Delegation config.** OpenAgents can route subagents to a different model automatically. Set this in `~/.openagents/config.yaml`:
...
Now when you tell OpenAgents "write me a Twitter thread about X" and it spawns a `delegate_task` subagent, that subagent runs on Gemini instead of your main model. Your primary conversation stays on GPT-5.4.
...
**Scenario:** In OpenClaw, you had multiple independent agents bound to specific WhatsApp chats — one for a family shopping list group, another for your private chat. Can OpenAgents do this?

**Current limitation:** OpenAgents profiles each require their own WhatsApp number/session. You cannot bind multiple profiles to different chats on the same WhatsApp number — the WhatsApp bridge (Baileys) uses one authenticated session per number.
...
**Scenario:** You see gateway exec logs, OpenAgents reasoning, and tool call details in Telegram instead of just the final output.
...
**Current behavior:** OpenAgents creates sessions keyed by user ID on most platforms, so each person gets their own conversation context. This is by design for privacy and context isolation.
...
### Exporting OpenAgents to another machine
...
1. Install OpenAgents on the new machine:
   ```bash
   curl -fsSL https://openagents.nousresearch.com/install.sh | bash
   ```
...
2. On the **source machine**, create a full backup:
   ...
   hermes backup
   ```
   This creates a zip of your entire `~/.openagents/` directory — config, API keys, memories, skills, sessions, and profiles — saved to your home directory as `~/hermes-backup-<timestamp>.zip`.
...
| **Scope** | Global (entire `~/.openagents` directory) | Local (single profile directory) |
...
:::tip
`hermes backup` produces a consistent snapshot even while OpenAgents is actively running. The restored archive excludes machine-local runtime files like `gateway.pid` and `cron.pid`.
:::

### Permission denied when reloading shell after install

**Scenario:** After running the OpenAgents installer, `source ~/.zshrc` gives a permission denied error.
...
1. **Search existing issues:** [GitHub Issues](https://github.com/NousResearch/openagents/issues)
2. **Ask the community:** [Nous Research Discord](https://discord.gg/nousresearch)
3. **File a bug report:** Include your OS, Python version (`python3 --version`), OpenAgents version (`hermes --version`), and the full error message
