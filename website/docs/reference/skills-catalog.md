...
OpenAgents ships with a large built-in skill library copied into `~/.openagents/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.

OpenAgents also syncs bundled skills on `hermes update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.openagents/skills/` tree, it is still shipped with Hermes; restore it with `hermes skills reset <name> --restore`.
...
| [`openagents`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-openagents) | Configure, extend, or contribute to OpenAgents. | `autonomous-ai-agents/openagents` |
...
| [`teams-meeting-pipeline`](/docs/user-guide/skills/bundled/productivity/productivity-teams-meeting-pipeline) | Operate the Teams meeting summary pipeline via OpenAgents CLI — summarize meetings, inspect pipeline status, replay jobs, manage Microsoft Graph subscriptions. | `productivity/teams-meeting-pipeline` |
...
| Skill | Description | Path |
|-------|-------------|------|
| [`openagents-skill-authoring`](/docs/user-guide/skills/bundled/software-development/software-development-openagents-skill-authoring) | Author in-repo SKILL.md: frontmatter, validator, structure. | `software-development/openagents-skill-authoring` |
