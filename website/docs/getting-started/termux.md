...
description: "Run OpenAgents directly on an Android phone with Termux"
---

# OpenAgents on Android with Termux
...
OpenAgents can run directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop OpenAgents from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.

---

## Option 1: One-line installer

OpenAgents now ships a Termux-aware installer path:
...
Or set keys directly in `~/.openagents/.env`.
