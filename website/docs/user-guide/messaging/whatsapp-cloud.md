...
description: "Set up OpenAgents as a WhatsApp bot via Meta's official Business Cloud API"
---

# WhatsApp Business Cloud API Setup

OpenAgents can connect to WhatsApp through Meta's **official** WhatsApp Business Cloud API. This is the production-grade path: no Node.js bridge subprocess, no QR codes, no account-ban risk.
...
- The OpenAgents gateway needs a **public HTTPS URL** so Meta can deliver inbound messages via webhook.
- Replies more than 24 hours after the user's last message require a pre-approved **template** (this is Meta's "customer service window" rule, not a OpenAgents limit).
...
4. **Optional but recommended**: ffmpeg on `PATH` so outbound voice messages render as native WhatsApp voice-note bubbles (green waveform) instead of MP3 audio attachments. OpenAgents degrades gracefully if absent.
...
6. Copy the token → update `WHATSAPP_CLOUD_ACCESS_TOKEN` in `~/.openagents/.env` → restart the gateway.

System User tokens don't expire unless you explicitly revoke them.

---

## Exposing OpenAgents to the internet

The Cloud API delivers inbound messages by HTTPS POST to your webhook URL — that means the OpenAgents gateway has to be reachable from Meta's servers.  Three common ways:
...
2. Generate a **Verify Token** — the wizard does this for you with `secrets.token_urlsafe(32)`; if you're configuring manually, run:
   ...
   ```
   Save it as `WHATSAPP_CLOUD_VERIFY_TOKEN` in `~/.openagents/.env`.
3. Start the OpenAgents gateway: `hermes gateway`.
...
In addition to Meta's recipient whitelist, OpenAgents has its own per-platform allowlist that controls **which incoming messages the agent processes**.  Add to `~/.openagents/.env`:
...
All settings live in `~/.openagents/.env`.  Required values are in **bold**.
...
When the agent invokes any of these flows, OpenAgents uses WhatsApp's native interactive messages — tap-to-answer buttons instead of "reply with the number" prompts:
...
OpenAgents acknowledges inbound messages immediately:
...
OpenAgents TTS produces MP3. Two paths:
...
OpenAgents warns the agent about this window in its system prompt, so the model knows to mention it when scheduling delayed messages.

Message-template support (the workaround for outside-window sends) is not yet implemented in Hermes. If you need it, please [open an issue](https://github.com/NousResearch/openagents/issues) — it's planned but waiting on a clear demand signal.
...
Meta's default throughput is **80 messages/second per business phone number**, with upgrades available.  OpenAgents doesn't currently enforce this client-side — extremely high-volume sends could hit Meta's limit.
...
- **Verify token mismatch** — the token in `~/.openagents/.env`'s `WHATSAPP_CLOUD_VERIFY_TOKEN` must match exactly what you typed into Meta's dashboard.  Run the curl probe above to confirm the gateway's verify handshake works locally first.
- **Gateway not running** — check `hermes gateway` is up.
- **App Secret not set** — without it, OpenAgents refuses inbound POSTs with 503.  Meta interprets that as "can't validate."
...
If the model emits tool-call-shaped text instead of a structured call, it usually means the toolset was effectively empty.  See `openagents_cli/platforms.py` for the platform → default toolset mapping.
...
- **Treat the App Secret like a password** — anyone with it can forge webhook payloads that OpenAgents will accept as authentic.
...
Most users running OpenAgents for personal projects prefer Baileys. Most users running customer-facing bots prefer Cloud API.
