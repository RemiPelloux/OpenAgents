...
Connect OpenAgents to **iMessage** through [Photon][photon], a managed
...
your first iMessage from OpenAgents — just a phone number we can bind to
your account.
:::
...
both directions. Because the SDK is TypeScript-only, OpenAgents runs it in a
...
2. **Finds or creates** the `OpenAgents` project on your account.
...
Runtime credentials are written to `~/.openagents/.env`
...
(device token, dashboard project id) lives in `~/.openagents/auth.json` under
...
line, OpenAgents replies with a pairing code. Approve it with:
...
**Pre-authorize specific numbers** (in `~/.openagents/.env`):

```bash
PHOTON_ALLOWED_USERS=+15551234567,+15559876543
```

**Open access** (dev only, in `~/.openagents/.env`):
...
By default OpenAgents responds to every authorized DM and group message.
...
they match a wake-word pattern. The defaults match `openagents` and
`@OpenAgents` variants. For a custom agent name, set regex patterns:
...
Send an iMessage to your assigned number and OpenAgents will reply.
...
assigned iMessage line OpenAgents uses. When a Photon token and dashboard project
...
- **Outbound attachments are supported.** OpenAgents sends images, voice
...
| `PHOTON_MENTION_PATTERNS` | OpenAgents wake words  | JSON list / comma / newline regex patterns for group mentions |
