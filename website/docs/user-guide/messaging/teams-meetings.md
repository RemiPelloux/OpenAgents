...
Use the Teams meeting pipeline when you want OpenAgents to ingest Microsoft Graph meeting events, fetch transcripts first, fall back to recordings plus STT when needed, and deliver a structured summary to downstream sinks.
...
Add Graph app-only credentials to `~/.openagents/.env`:
...
Example `~/.openagents/config.yaml`:
...
Use this when you want OpenAgents to post the summary through Microsoft Graph into a Teams chat or channel.
...
Start OpenAgents normally after updating config:

```bash
hermes gateway run
```

Or, if you run OpenAgents in Docker, start the gateway the same way you already do for your deployment.
