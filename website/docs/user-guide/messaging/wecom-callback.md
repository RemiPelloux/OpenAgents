...
Connect OpenAgents to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
OpenAgents supports two WeCom integration modes:
...
3. OpenAgents decrypts the message, queues it for the agent
...
console. A mismatch between the token configured in OpenAgents and the token the
...
in `~/.openagents/.env` values around `=` will also break signature checks. After
