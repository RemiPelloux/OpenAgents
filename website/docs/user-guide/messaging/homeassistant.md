...
description: Control your smart home with OpenAgents via Home Assistant integration.
...
OpenAgents integrates with [Home Assistant](https://www.home-assistant.io/) in two ways:
...
4. Click **Create Token**, give it a name like "OpenAgents"
5. Copy the token
...
OpenAgents registers four tools for smart home control:
...
Configure which events the agent sees in `~/.openagents/config.yaml` under the Home Assistant platform's `extra` section:
...
Outbound messages from the agent are delivered as **Home Assistant persistent notifications** (via `persistent_notification.create`). These appear in the HA notification panel with the title "OpenAgents".
...
The adapter reads credentials from `~/.openagents/.env` (auto-merged at startup) or
from `config.yaml`. Double-check the file lives under the active OpenAgents profile
...
running OpenAgents — `curl -H "Authorization: Bearer <token>" <url>/api/` should
