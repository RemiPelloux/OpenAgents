...
description: "Set up OpenAgents as a Microsoft Teams bot"
---

# Microsoft Teams Setup

Connect OpenAgents to Microsoft Teams as a bot. Unlike Slack's Socket Mode, Teams delivers messages by calling a **public HTTPS webhook**, so your instance needs a publicly reachable endpoint — either a dev tunnel (local dev) or a real domain (production).
...
Teams delivers @mentions as regular messages with `<at>BotName</at>` tags, which OpenAgents strips automatically before processing.
...
Alternatively, configure via `~/.openagents/config.yaml`:
...
| `No inference provider configured` | Check that `ANTHROPIC_API_KEY` (or another provider key) is set in `~/.openagents/.env` |
...
- Store credentials in `~/.openagents/.env` with permissions `600` (`chmod 600 ~/.openagents/.env`)
