...
sidebar_position: 9
title: "Matrix"
description: "Set up OpenAgents as a Matrix bot"
---

# Matrix Setup

OpenAgents integrates with Matrix, the open, federated messaging protocol. Matrix lets you run your own homeserver or use a public one like matrix.org — either way, you keep control of your communications. The bot connects via the `mautrix` Python SDK, processes messages through the OpenAgents pipeline (including tool use, memory, and reasoning), and responds in real time. It supports text, file attachments, images, audio, video, and optional end-to-end encryption (E2EE).

OpenAgents works with any Matrix homeserver — Synapse, Conduit, Dendrite, or matrix.org.

Before setup, here's the part most people want to know: how OpenAgents behaves once it's connected.

## How OpenAgents Behaves

| Context | Behavior |
|---------|----------|
| **DMs** | OpenAgents responds to every message. No `@mention` needed. Each DM has its own session. Set `MATRIX_DM_MENTION_THREADS=true` to start a thread when the bot is `@mentioned` in a DM. |
| **Rooms** | By default, OpenAgents requires an `@mention` to respond. Set `MATRIX_REQUIRE_MENTION=false` or add room IDs to `MATRIX_FREE_RESPONSE_ROOMS` for free-response rooms. Room invites are auto-accepted. |
| **Threads** | OpenAgents supports Matrix threads (MSC3440). If you reply in a thread, OpenAgents keeps the thread context isolated from the main room timeline. Threads where the bot has already participated do not require a mention. |
| **Auto-threading** | By default, OpenAgents auto-creates a thread for each message it responds to in a room. This keeps conversations isolated. Set `MATRIX_AUTO_THREAD=false` to disable. Set `MATRIX_DM_AUTO_THREAD=true` (default false) to also auto-create threads for DM messages — this is distinct from `MATRIX_DM_MENTION_THREADS`, which only starts a thread when the bot is `@mentioned` in a DM. |
| **Commands** | OpenAgents accepts normal `/commands` when your Matrix client sends them. If your client reserves `/` for local commands, use `!commands` instead; OpenAgents normalizes known `!command` aliases to `/command`. |
...
| **Shared rooms with multiple users** | By default, OpenAgents isolates session history per user inside the room. Two people talking in the same room do not share one transcript unless you explicitly disable that. |
...
OpenAgents sends structured Matrix user mentions for explicit Matrix IDs such as `@alice:example.org`. Room-wide `@room` notifications are disabled by default; set `MATRIX_ALLOW_ROOM_MENTIONS=true` only in rooms where the bot is allowed to notify everyone.
:::
...
OpenAgents now includes the current Matrix room name, room ID, topic, message ID,
...
You can also run OpenAgents as your own user. This means the bot posts as you — useful for personal assistants.

## Step 2: Get an Access Token

OpenAgents needs an access token to authenticate with the homeserver. You have two options:
...
Instead of providing an access token, you can give OpenAgents the bot's user ID and password. OpenAgents will log in automatically on startup. This is simpler but means the password is stored in your `.env` file.
...
OpenAgents uses your Matrix User ID to control who can interact with the bot. Matrix User IDs follow the format `@username:server`.
...
Add the following to your `~/.openagents/.env` file:
...
Bridge and appservice deployments need extra loop protection. OpenAgents always
...
Optional behavior settings in `~/.openagents/config.yaml`:
...
OpenAgents supports Matrix end-to-end encryption, so you can chat with your bot in encrypted rooms.
...
- Stores encryption keys in `~/.openagents/platforms/matrix/store/` (legacy installs: `~/.openagents/matrix/store/`)
...
In Matrix conversations, OpenAgents exposes Matrix-specific tools to the agent:
...
Set `MATRIX_APPROVAL_REQUIRE_SENDER=false` if you intentionally want any authorized Matrix user in the room to operate an approval/model picker prompt. The default is requester-bound when OpenAgents knows who requested the action.

### Media Limits

OpenAgents uploads and downloads Matrix images, files, audio, and video through Matrix media APIs. Multiple generated images are sent as one ordered logical batch, preserving captions and thread context across the batch.
...
Inbound media must use Matrix `mxc://` content URIs. OpenAgents rejects arbitrary
...
OpenAgents includes an opt-in Synapse harness for local validation:
...
On each startup, if `MATRIX_RECOVERY_KEY` is set, OpenAgents imports cross-signing keys from the homeserver's secure secret storage and signs the current device. This is idempotent and safe to leave enabled permanently.

If OpenAgents bootstraps a new Matrix recovery key, it never logs the raw key. Set
...
If you delete `~/.openagents/platforms/matrix/store/crypto.db`, the bot loses its encryption identity. Simply restarting with the same device ID will **not** fully recover — the homeserver still holds one-time keys signed with the old identity key, and peers cannot establish new Olm sessions.

OpenAgents detects this condition on startup and refuses to enable E2EE, logging: `device XXXX has stale one-time keys on the server signed with a previous identity key`.
...
Add this to your `~/.openagents/.env`:
...
OpenAgents supports the same gateway commands in Matrix that it supports on other
...
OpenAgents only normalizes `!command` when the command is known to the gateway, a
...
**Cause**: The host's system clock is set ahead of real time. The Matrix adapter applies a 5-second startup-grace filter (`event_ts < startup_ts - 5`) to ignore events replayed from initial sync. When the wall clock is ahead, every incoming event looks "older than startup" and is dropped before reaching the message handler — the bot appears connected but never replies. See [#12614](https://github.com/NousResearch/openagents/issues/12614).
...
If you previously used OpenAgents with `MATRIX_ENCRYPTION=true` and are upgrading to
...
1. **Generate a new access token** to get a fresh device ID. The simplest way:
   ...
   Copy the new `access_token` and update `MATRIX_ACCESS_TOKEN` in `~/.openagents/.env`.
...
3. **Set your recovery key** (if you use cross-signing — most Element users do). Add to `~/.openagents/.env`:
...
a working E2EE setup with a previous version of OpenAgents and are upgrading.
...
| Setting | Description |
|---------|-------------|
| `GATEWAY_PROXY_URL` | URL of the remote OpenAgents API server (e.g., `http://192.168.1.100:8642`) |
...
**Fix**: OpenAgents uses an explicit sync loop that calls `client.handle_sync()` on
...
**Fix**: The sync loop automatically retries every 5 seconds on error. Check the OpenAgents logs for sync-related warnings. If the bot consistently falls behind, ensure your homeserver has adequate resources.

### Bot is offline

**Cause**: The OpenAgents gateway isn't running, or it failed to connect.
...
**Fix**: Add your User ID to `MATRIX_ALLOWED_USERS` in `~/.openagents/.env` and restart the gateway. Use the full `@user:server` format.
...
For more information on securing your OpenAgents deployment, see the [Security Guide](../security.md).
...
- **Media support**: OpenAgents can send and receive images, audio, video, and file attachments. Media is uploaded to your homeserver using the Matrix content repository API.
