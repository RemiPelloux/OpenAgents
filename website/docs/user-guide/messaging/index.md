...
description: "Chat with OpenAgents from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Yuanbao, Microsoft Teams, LINE, Raft, Webhooks, or any OpenAI-compatible frontend via the API server — architecture and setup overview"
---

# Messaging Gateway

Chat with OpenAgents from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Feishu/Lark, WeCom, Weixin, BlueBubbles (iMessage), QQ, Yuanbao, Microsoft Teams, LINE, ntfy, or your browser. The gateway is a single background process that connects to all your configured platforms, handles sessions, runs cron jobs, and delivers voice messages.
...
```mermaid
flowchart TB
    subgraph Gateway["OpenAgents Gateway"]
...
For group chats, hooks, and automation flows, OpenAgents supports explicit silence tokens. If the agent's final response is exactly one supported token, the gateway suppresses outbound delivery and sends nothing to the chat.
...
Silence is a delivery decision only. OpenAgents keeps the assistant silence turn in the session transcript, so the conversation still alternates normally:
...
Failed turns still surface as errors; OpenAgents does not hide failures just because the text resembles a silence token.
...
| `/update` | Update OpenAgents to the latest version |
...
Configure per-platform overrides in `~/.openagents/gateway.json`:
...
The first time you message a busy agent on any platform, OpenAgents appends a one-line reminder to the busy-ack explaining the knob (`"💡 First-time tip — …"`). The reminder fires once per install — a flag under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
...
Control how much tool activity is displayed in `~/.openagents/config.yaml`:
...
Off by default. When enabled, OpenAgents prepends a human-readable timestamp
...
OpenAgents confirms immediately:
...
When the agent running a background session uses `terminal(background=true)` to start long-running processes (servers, builds, etc.), the gateway can push status updates to your chat. Control this with `display.background_process_notifications` in `~/.openagents/config.yaml`:
...
journalctl --user -u openagents-gateway -f  # View logs
...
The unit OpenAgents installs already shuts the gateway down cleanly with `KillMode=mixed` + `KillSignal=SIGTERM`, and uses `Restart=always` with `RestartForceExitStatus` so updates and `/restart` respawn correctly. Do **not** add a systemd drop-in such as `ExecStopPost=/bin/kill -9 $MAINPID` — `ExecStopPost` fires on *every* stop, including clean restarts, so it `SIGKILL`s the freshly spawned instance before it stabilizes and `Restart=always` immediately respawns it. The result is an infinite restart loop (and, on Telegram, a flood of restart messages). If you've added such a drop-in, remove it: `systemctl --user edit openagents-gateway` (or `sudo systemctl edit openagents-gateway` for a system service) and delete the `ExecStopPost` line, then `systemctl --user daemon-reload`.
:::

:::tip Headless VMs: user service + linger avoids root prompts
A system service needs root for every restart — including the automatic gateway restart at the end of `hermes update`. When `hermes update` runs as a non-root user, it tries passwordless `sudo systemctl`; if that's unavailable, it skips the restart and prints the manual `sudo systemctl restart openagents-gateway` command (it never blocks on an interactive password prompt).
...
After that, `hermes update` can restart the gateway without any privileges. If you prefer to keep the system service, either run updates with `sudo hermes update`, or grant the service account passwordless sudo for systemctl, e.g. in `sudo visudo -f /etc/sudoers.d/openagents-gateway`:

```
hermes ALL=(root) NOPASSWD: /usr/bin/systemctl --no-ask-password reset-failed openagents-gateway*, /usr/bin/systemctl --no-ask-password start openagents-gateway*, /usr/bin/systemctl --no-ask-password restart openagents-gateway*
```
:::

Avoid keeping both the user and system gateway units installed at once unless you really mean to. OpenAgents will warn if it detects both because start/stop/status behavior gets ambiguous.

:::info Multiple installations
If you run multiple OpenAgents installations on the same machine (with different `OPENAGENTS_HOME` directories), each gets its own systemd service name. The default `~/.openagents` uses `openagents-gateway`; other installations use `openagents-gateway-<hash>`. The `hermes gateway` commands automatically target the correct service for your current `OPENAGENTS_HOME`.
:::
...
hermes gateway stop                  # Stop the service
hermes gateway status                # Check status
tail -f ~/.openagents/logs/gateway.log   # View logs
```

The generated plist lives at `~/Library/LaunchAgents/ai.openagents.gateway.plist`. It includes three environment variables:
...
- **OPENAGENTS_HOME** — scopes the gateway to your OpenAgents installation.
...
:::info Multiple installations
Like the Linux systemd service, each `OPENAGENTS_HOME` directory gets its own launchd label. The default `~/.openagents` uses `ai.openagents.gateway`; other installations use `ai.openagents.gateway-<suffix>`.
:::
...
1. **Gateway log** (`~/.openagents/logs/gateway.log` or the systemd / launchd unit log). Search for the platform name and `circuit breaker`, `paused`, or `disabled`. The trip event includes the failure count and the last error.
