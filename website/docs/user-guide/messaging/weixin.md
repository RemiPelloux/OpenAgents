...
sidebar_position: 15
title: "Weixin (WeChat)"
description: "Connect OpenAgents to personal WeChat accounts via the iLink Bot API"
---

# Weixin (WeChat)

Connect OpenAgents to [WeChat](https://weixin.qq.com/) (微信), Tencent's personal messaging platform. The adapter uses Tencent's **iLink Bot API** for personal WeChat accounts — this is distinct from WeCom (Enterprise WeChat). Messages are delivered via long-polling, so no public endpoint or webhook is required.
...
QR login connects OpenAgents to an **iLink bot identity** (e.g. `a5ace6fd482e@im.bot`), **not** a fully scriptable ordinary personal WeChat account. Consequences:
...
- The `WEIXIN_GROUP_POLICY` / `WEIXIN_GROUP_ALLOWED_USERS` settings below only take effect when iLink actually returns group events for your account type. If it doesn't, group messages will never reach OpenAgents regardless of policy.
...
- Terminal QR rendering is included when OpenAgents is installed with the `messaging` extra
...
5. Save the account credentials automatically to `~/.openagents/weixin/accounts/`
...
After initial QR login, set at minimum the account ID in `~/.openagents/.env`:
...
OpenAgents QR code with their own accounts; they must message the connected iLink
bot/contact through WeChat, and OpenAgents will process the DM only if the sender's
Weixin user ID is present in `WEIXIN_ALLOWED_USERS`.

A practical setup flow is:

1. Pair OpenAgents once with `hermes gateway setup` and note the connected iLink bot
   account.
...
- Tokens are saved per account+peer to `~/.openagents/weixin/accounts/<account_id>.context-tokens.json`
...
| `Another local OpenAgents gateway is already using this Weixin token` | Stop the other gateway instance first — only one poller per token is allowed |
...
| Terminal QR code doesn't render | Reinstall with the messaging extra: `cd ~/.openagents/openagents && uv pip install -e ".[messaging]"`. Alternatively, open the URL printed above the QR |
