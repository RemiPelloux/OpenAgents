...
sidebar_position: 4
title: "Slack"
description: "Set up OpenAgents as a Slack bot using Socket Mode"
---

# Slack Setup

Connect OpenAgents to Slack as a bot using Socket Mode. Socket Mode uses WebSockets instead of
public HTTP endpoints, so your OpenAgents instance doesn't need to be publicly accessible — it works
behind firewalls, on your laptop, or on a private server.

:::warning Classic Slack Apps Deprecated
Classic Slack apps (using RTM API) were **fully deprecated in March 2025**. OpenAgents uses the modern
...
The fastest path is to paste a manifest OpenAgents generates for you. It
...
1. Generate the manifest:
   ...
   ```
   This writes `~/.openagents/slack-manifest.json` and prints paste-in
   instructions.
...
4. Enter an app name (e.g., "OpenAgents") and select your workspace
5. Click **Create App**
...
it will only work in DMs. Without `files:read`, OpenAgents can chat but **cannot reliably read user-uploaded attachments**.
...
Even with all the correct scopes and event subscriptions, Slack will not allow users to send direct messages to the bot unless the Messages Tab is enabled. This is a Slack platform requirement, not a OpenAgents configuration issue.
:::
...
OpenAgents uses Slack **Member IDs** (not usernames or display names) for the allowlist.
...
Every OpenAgents command (`/btw`, `/stop`, `/new`, `/model`, `/help`, ...)
...
OpenAgents command with its description.

Under the hood: OpenAgents ships with a generated Slack app manifest (see
Step 1, Option A) that declares every command in
[`COMMAND_REGISTRY`](https://github.com/NousResearch/openagents/blob/main/openagents_cli/commands.py)
...
When OpenAgents adds new commands (e.g. after `hermes update`), regenerate
...
3. Paste the new contents of `~/.openagents/slack-manifest.json`
...
`/hermes btw run the tests` — OpenAgents routes it the same way as `/btw
...
As a workaround, OpenAgents recognises a leading `!` as an alternate
command prefix that works in threads (and anywhere else). Type
`!queue`, `!stop`, `!model gpt-5.4`, etc. as a regular thread reply —
OpenAgents treats it identically to the slash form and replies in the same
thread.
...
OpenAgents falls back to a text prompt, the prompt instructs you to reply
...
Understanding how OpenAgents behaves in different contexts:

| Context | Behavior |
|---------|----------|
| **DMs** | Bot responds to every message — no @mention needed |
| **Channels** | Bot **only responds when @mentioned** (e.g., `@OpenAgents what time is it?`). In channels, OpenAgents replies in a thread attached to that message. |
| **Threads** | If you @mention OpenAgents inside an existing thread, it replies in that same thread. Once the bot has an active session in a thread, **subsequent replies in that thread do not require @mention** — the bot follows the conversation naturally. |
...
Beyond the required environment variables from Step 8, you can customize Slack bot behavior through `~/.openagents/config.yaml`.
...
When `true` (the default), each user in a shared channel gets their own isolated conversation session. Two people talking to OpenAgents in `#general` will have separate histories and contexts.
...
slack:
  ...
  # must @mention the bot before OpenAgents will respond.
...
Set `SLACK_HOME_CHANNEL` to a channel ID where OpenAgents will deliver scheduled messages,
...
Make sure the bot has been **invited to the channel** (`/invite @OpenAgents`).

---

## Multi-Workspace Support

OpenAgents can connect to **multiple Slack workspaces** simultaneously using a single gateway instance. Each workspace is authenticated independently with its own bot user ID.
...
In addition to tokens in the environment or config, OpenAgents also loads tokens from an **OAuth token file** at:

```
~/.openagents/slack_tokens.json
```
...
- When a message arrives, OpenAgents uses the correct workspace-specific client to respond.
...
OpenAgents supports voice on Slack:
...
| Bot works in DMs but not in channels | **Most common issue.** Add `message.channels` and `message.groups` to event subscriptions, reinstall the app, and invite the bot to the channel with `/invite @OpenAgents` |
...
| Bot responds but can't post in a channel | Invite the bot to the channel with `/invite @OpenAgents` |
| Bot can chat but can't read uploaded images/files | Add `files:read`, then **reinstall** the app. OpenAgents now surfaces attachment access diagnostics in-chat when Slack returns scope/auth/permission failures. |
...
7. ✅ Bot was **invited** to the channel (`/invite @OpenAgents`)
...
- Tokens should be stored in `~/.openagents/.env` (file permissions `600`)
- Rotate tokens periodically via the Slack app settings
- Audit who has access to your OpenAgents config directory
