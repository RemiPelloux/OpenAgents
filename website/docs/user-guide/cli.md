...
sidebar_position: 1
title: "CLI Interface"
description: "Master the OpenAgents terminal interface — commands, keybindings, personalities, and more"
---

# CLI Interface

OpenAgents's CLI is a full terminal user interface (TUI) — not a web UI. It features multiline editing, slash-command autocomplete, conversation history, interrupt-and-redirect, and streaming tool output. Built for people who live in the terminal.
...
:::tip
OpenAgents also ships a modern TUI with modal overlays, mouse selection, and non-blocking input. Launch it with `hermes --tui` — see the [TUI](tui.md) guide.
:::
...
hermes -s openagents-dev,github-auth
...
<img className="docs-terminal-figure" src="/docs/img/docs/cli-layout.svg" alt="Stylized preview of the OpenAgents CLI layout showing the banner, conversation area, and fixed input prompt." />
<p className="docs-figure-caption">The OpenAgents CLI banner, conversation stream, and fixed input prompt rendered as a stable docs figure instead of fragile text art.</p>
...
| `Ctrl+D` | Exit |
| `Ctrl+Z` | Suspend OpenAgents to background (Unix only). Run `fg` in the shell to resume. |
...
Type `/` to see the autocomplete dropdown. OpenAgents supports a large set of CLI slash commands, dynamic skill commands, and user-defined quick commands.
...
| `/voice tts` | Toggle spoken playback for OpenAgents replies |
...
OpenAgents loads each named skill into the session prompt before the first turn. The same flag works in interactive mode and single-query mode.

## Skill Slash Commands

Every installed skill in `~/.openagents/skills/` is automatically registered as a slash command. The skill name becomes the command:
...
You can also define custom personalities in `~/.openagents/config.yaml`:
...
Where the terminal cannot distinguish them, `Alt+Enter` and `Ctrl+J` continue to work everywhere. **On Windows Terminal specifically, `Alt+Enter` is captured by the terminal (toggles fullscreen) and never reaches OpenAgents — use `Ctrl+Enter` (delivered as `Ctrl+J`) or `Ctrl+J` directly for a newline.**
...
The very first time you press Enter while OpenAgents is working, OpenAgents prints a one-line reminder explaining the `/busy` knob (`"(tip) Your message interrupted the current run…"`). It only fires once per install — a flag in `config.yaml` under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
:::

### Suspending to Background

On Unix systems, press **`Ctrl+Z`** to suspend OpenAgents to the background — just like any terminal process. The shell prints a confirmation:

```
OpenAgents has been suspended. Run `fg` to bring OpenAgents back.
```
...
CLI sessions are stored in Hermes's SQLite state database under `~/.openagents/state.db`. The database keeps:
...
OpenAgents immediately confirms the task and gives you back the prompt:
