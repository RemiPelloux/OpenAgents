...
A **profile distribution** packages a complete OpenAgents — personality, skills, cron jobs, MCP connections, config — as a git repository. Anyone with access to the repo can install the whole agent with one command, update it in place, and keep their own memories, sessions, and API keys untouched.

If a [profile](./profiles.md) is a local agent, a distribution is that agent made shareable.

## What this means

Before distributions, sharing a OpenAgents meant sending someone:
...
The tradeoff: recipients need git installed. On any machine running OpenAgents in 2026, that's already true.
...
# Edit ~/.openagents/profiles/research-bot/SOUL.md
...
Create `~/.openagents/profiles/research-bot/distribution.yaml`:
...
Create `~/.openagents/profiles/research-bot/.gitignore` with at minimum:
...
```bash
cd ~/.openagents/profiles/research-bot
git init
git add .
git commit -m "v1.0.0"
...
5. Copies distribution-owned files into `~/.openagents/profiles/research-bot/` (or wherever the manifest's `name` resolves). The [hard-excluded paths](#whats-not-in-a-distribution-ever) are stripped during this copy, even if the author accidentally left them in the repo.
...
```
# Environment variables required by this OpenAgents distribution.
...
```bash
cp ~/.openagents/profiles/research-bot/.env.EXAMPLE ~/.openagents/profiles/research-bot/.env
...
Path:    ~/.openagents/profiles/research-bot
...
cd ~/.openagents/profiles/pr-reviewer
# ... build and tune ...
git init && git add . && git status   # confirm no secrets staged
...
cd ~/.openagents/profiles/polymarket-trader
...
```yaml
# ~/.openagents/profiles/research-bot/local/my-overrides.yaml
...
# Iterate locally in ~/.openagents/profiles/forked-research-bot/
...
hermes profile install ~/.openagents/profiles/research-bot --name research-bot-test --alias

# Tweak, delete, re-install until it's right
hermes profile delete research-bot-test --yes
hermes profile install ~/.openagents/profiles/research-bot --name research-bot-test
```
