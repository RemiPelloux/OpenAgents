...
sidebar_position: 10
title: "Voice Mode"
description: "Real-time voice conversations with OpenAgents — CLI, Telegram, Discord (DMs, text channels, and voice channels)"
---

# Voice Mode

OpenAgents supports full voice interaction across CLI and messaging platforms. Talk to the agent using your microphone, hear spoken replies, and have live voice conversations in Discord voice channels.
...
1. **OpenAgents installed** — via the install script (see [Installation](/getting-started/installation))
2. **An LLM provider configured** — run `hermes model` or set your preferred provider credentials in `~/.openagents/.env`
3. **A working base setup** — run `hermes` to verify the agent responds to text before enabling voice

:::tip
The `~/.openagents/` directory and default `config.yaml` are created automatically the first time you run `hermes`. You only need to create `~/.openagents/.env` manually for API keys.
:::
...
cd ~/.openagents/openagents && uv pip install -e ".[voice]"

# Discord + Telegram messaging (includes discord.py[voice] for VC support)
cd ~/.openagents/openagents && uv pip install -e ".[messaging]"

# Premium TTS (ElevenLabs)
cd ~/.openagents/openagents && uv pip install -e ".[tts-premium]"

# Local TTS (NeuTTS, optional)
python -m pip install -U neutts[all]

# Everything at once
cd ~/.openagents/openagents && uv pip install -e ".[all]"
```
...
Voice mode is available in both the **classic CLI** (`hermes chat`) and the **TUI** (`hermes --tui`). Behavior is identical across both — same slash commands, same VAD silence detection, same streaming TTS, same hallucination filter. The TUI additionally forwards crash-forensic logs to `~/.openagents/logs/` so push-to-talk failures on exotic audio backends can be reported with a full stack trace rather than disappearing silently.
...
:::tip
The record key is configurable via `voice.record_key` in `~/.openagents/config.yaml` (default: `ctrl+b`).
:::
...
:::tip
To disable the mention requirement in server channels, add to `~/.openagents/.env`:
```bash
...
If you are running OpenAgents inside Docker on a Linux desktop, the container also needs access to your host audio socket. See the [Docker audio bridge](/user-guide/docker#optional-linux-desktop-audio-bridge) notes for a PulseAudio/PipeWire-compatible setup.
...
3. Or set `DISCORD_REQUIRE_MENTION=false` in `~/.openagents/.env`
...
- Review gateway logs: `tail -f ~/.openagents/logs/gateway.log`
