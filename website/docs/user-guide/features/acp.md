...
OpenAgents runs with a curated `openagents-acp` toolset designed for editor workflows. It includes:
...
Any of the following starts OpenAgents in ACP mode:
...
OpenAgents logs to stderr so stdout remains reserved for ACP JSON-RPC traffic.
...
2. Select **OpenAgents** from the built-in agent list.
3. Connect and start chatting.

If you want to define OpenAgents manually, add it through VS Code settings under `acp.agents`:
...
ACP mode uses the same OpenAgents configuration as the CLI:
...
ACP sessions bind the editor's cwd to the OpenAgents task ID so file and terminal tools run relative to the editor workspace, not the server process cwd.
...
| `allow_always` | Allow always | All future sessions | Yes (written to the OpenAgents permanent allowlist) |
