...
OpenAgents can optionally hand `openai/*` and `openai-codex/*` turns to the [Codex CLI app-server](https://github.com/openai/codex) instead of running its own tool loop. When enabled, terminal commands, file edits, sandboxing, and MCP tool calls all execute inside Codex's runtime — OpenAgents becomes the shell around it (sessions DB, slash commands, gateway, memory and skill review).

This is **opt-in only**. Default OpenAgents behavior is unchanged unless you flip the flag. OpenAgents never auto-routes you onto this runtime.
...
- **Native Codex plugins** — Linear, GitHub, Gmail, Calendar, Canva, etc. — installed via `codex plugin` are auto-migrated and active in your OpenAgents session.
- **Hermes' richer tools come along** — web_search, web_extract, browser automation, vision, image generation, skills, and TTS work via an MCP callback. Codex calls back into OpenAgents for tools it doesn't have built in.
...
These ship with `codex app-server` itself — no OpenAgents involvement, no MCP, no plugins. All five are available the moment the runtime starts:
...
- **`web_search`** — codex has its own built-in web search when configured. OpenAgents also exposes `web_search` (Firecrawl-backed) via the callback below; the model picks whichever it prefers.
...
When you enable the runtime, OpenAgents queries codex's `plugin/list` RPC and writes a `[plugins."<name>@openai-curated"]` entry for every plugin you have installed. The plugins themselves are managed by codex and authorized once via codex's own UI.
...
### 3. OpenAgents tool callback (MCP server, registered in `~/.codex/config.toml`)

OpenAgents registers itself as an MCP server so codex can call back for tools codex doesn't ship with. Available via the callback:
...
These four OpenAgents tools require the running AIAgent context (mid-loop state) to dispatch, and a stateless MCP callback can't drive them. Switch back to the default runtime (`/codex-runtime auto`) when you need any of them:
...
- The OpenAgents tool callback for browser_*, vision, image_gen, skills, TTS
...
|  | OpenAgents default runtime | Codex app-server (opt-in) |
|---|---|---|
...
2. **Codex OAuth login.** The codex subprocess reads `~/.codex/auth.json`. Two ways to populate it:
   ...
   ```
   Hermes' own `hermes auth login codex` writes to `~/.openagents/auth.json` — that's a separate session. **Run `codex login` separately** if you haven't.

3. **(Optional) Install the Codex plugins you want.** When you enable the runtime, OpenAgents auto-migrates whichever curated plugins you've already installed via Codex CLI:
   ```bash
   codex plugin marketplace add openai-curated
   # then via codex's TUI, install Linear / GitHub / Gmail / etc.
   ```
   OpenAgents will discover them and write `[plugins."<name>@openai-curated"]` entries to `~/.codex/config.toml` automatically.

## Enabling

In a OpenAgents session:
...
- Migrates user MCP servers from `~/.openagents/config.yaml` to `~/.codex/config.toml`.
...
You can also set it manually in `~/.openagents/config.yaml`:
```yaml
model:
  openai_runtime: codex_app_server   # default is "auto" (= OpenAgents runtime)
```
...
**Both keep working on the codex runtime.** The codex path projects each completed `commandExecution` / `fileChange` / `mcpToolCall` / `dynamicToolCall` item into a synthetic `assistant tool_call` + `tool` result message, so by the time the review runs it sees the same shape it sees on the default OpenAgents runtime.
...
For `apply_patch` (file edit) approvals, OpenAgents shows a summary of what changed (`1 add, 1 update: /tmp/new.py, /tmp/old.py`) when codex provides the data via the corresponding `fileChange` item.
...
(Hermes will preserve your override on re-migration as long as it lives outside the `# managed by openagents` markers.)
...
To route specific aux tasks to a cheaper / different model, set explicit overrides in `~/.openagents/config.yaml`:
...
The self-improvement review fork inherits the main runtime via `_current_main_runtime()` and OpenAgents downgrades it from `codex_app_server` to `codex_responses` automatically (so the fork can actually call `memory` and `skill_manage` — Hermes' own agent-loop tools). That fork still uses your subscription auth unless you've routed aux tasks elsewhere.

## Editing `~/.codex/config.toml` safely

OpenAgents wraps everything it manages between two marker comments:

```toml
# managed by openagents — `hermes codex-runtime migrate` regenerates this section
default_permissions = ":workspace"
[mcp_servers.filesystem]
...
[plugins."github@openai-curated"]
...
# end openagents managed section
```
...
- Add your own MCP servers OpenAgents doesn't know about
...
By default, OpenAgents points the codex subprocess at `~/.codex/` regardless of which OpenAgents profile is active. This means `hermes -p work` and `hermes -p personal` share the same Codex auth, plugins, and config. For most users this is the right behavior — it matches what running `codex` CLI directly would do.

If you want per-profile Codex isolation (separate auth, separate installed plugins, separate config), set `CODEX_HOME` explicitly per profile. The cleanest way is to point at a directory under your `OPENAGENTS_HOME`:

```bash
# Inside the work profile, you might wrap hermes:
CODEX_HOME=~/.openagents/profiles/work/codex hermes chat
```
...
OpenAgents does NOT rewrite `HOME` when spawning the codex app-server subprocess (we use `os.environ.copy()` and only overlay `CODEX_HOME` and `RUST_LOG`). This means:
...
| OpenAgents (`config.yaml`) | Codex (`config.toml`) |
|---|---|
...
Plugins installed via `codex plugin` (Linear, GitHub, Gmail, Calendar, Canva, etc.) are discovered through Codex's `plugin/list` RPC. For each plugin where `installed: true`, OpenAgents writes a `[plugins."<name>@openai-curated"]` block enabling it in your OpenAgents session.

This means: when your friend says "I have Calendar and GitHub set up in my Codex CLI" and they enable Hermes' codex runtime, OpenAgents activates those automatically. No re-configuration needed.
...
- Plugin OAuth — you authorize each plugin once in Codex itself; OpenAgents doesn't touch credentials.

## OpenAgents tool callback (the new MCP server)

Codex's built-in toolset covers shell/file ops/patches but doesn't have web search, browser automation, vision, image generation, etc. To keep those usable in a codex turn, OpenAgents registers itself as an MCP server in `~/.codex/config.toml`:

```toml
[mcp_servers.hermes-tools]
command = "/path/to/python"
args = ["-m", "agent.transports.hermes_tools_mcp_server"]
env = { OPENAGENTS_HOME = "/your/.hermes", PYTHONPATH = "...", OPENAGENTS_QUIET = "1" }
startup_timeout_sec = 30.0
tool_timeout_sec = 600.0
```

When the model calls `web_search` (or another exposed OpenAgents tool), codex spawns the `hermes_tools_mcp_server` subprocess via stdio, the request is dispatched through `model_tools.handle_function_call()`, and the result is projected back to codex like any other MCP response.
...
**Tools NOT available:** `delegate_task`, `memory`, `session_search`, `todo`. These need the running AIAgent context to dispatch (mid-loop state) and a stateless MCP callback can't drive them. Use the default OpenAgents runtime (`/codex-runtime auto`) when you need these.
...
This runtime is **opt-in beta**. Working as of OpenAgents 2026.5 + Codex CLI 0.130.0:

- Multi-turn conversations
- `commandExecution` and `fileChange` (apply_patch) approvals via OpenAgents UI
...
If you find a bug, [open an issue](https://github.com/NousResearch/openagents/issues) with the output of `hermes logs --since 5m`. Mention `codex-runtime` in the title so it's easy to triage.

## Architecture

```
                ┌─── OpenAgents shell (CLI / TUI / gateway) ───┐
...
For implementation details, see [PR #24182](https://github.com/NousResearch/openagents/pull/24182) and the [Codex app-server protocol README](https://github.com/openai/codex/blob/main/codex-rs/app-server/README.md).
