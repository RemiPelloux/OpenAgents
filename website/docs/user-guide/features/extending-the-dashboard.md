...
description: "Build themes and plugins for the OpenAgents web dashboard — palettes, typography, layouts, custom tabs, shell slots, page-scoped slots, and backend API routes"
---

# Extending the Dashboard

The OpenAgents web dashboard (`hermes dashboard`) is built to be reskinned and extended without forking the codebase. Three layers are exposed:

1. **Themes** — YAML files that repaint the dashboard's palette, typography, layout, and per-component chrome. Drop a file in `~/.openagents/dashboard-themes/`; it appears in the theme switcher.
...
Themes are YAML files stored in `~/.openagents/dashboard-themes/`. The file name doesn't matter (the theme's `name:` field is what the system uses), but convention is `<name>.yaml`. Every field is optional — missing keys fall back to the built-in `default` theme, so a theme can be as small as one color.

### Quick start — your first theme

```bash
mkdir -p ~/.openagents/dashboard-themes
```

```yaml
# ~/.openagents/dashboard-themes/neon.yaml
name: neon
label: Neon
...
Themes that reference Google Fonts (all except OpenAgents Teal) load the stylesheet on demand — the first time you switch to them a `<link>` tag is injected into `<head>`.
...
```yaml
# ~/.openagents/dashboard-themes/ocean.yaml
name: ocean
label: Ocean Deep
...
A dashboard plugin is a directory with a `manifest.json`, a pre-built JS bundle, and optionally a CSS file and a Python file with FastAPI routes. Plugins live next to other OpenAgents plugins in `~/.openagents/plugins/<name>/` — the dashboard extension is a `dashboard/` subfolder inside that plugin directory, so one plugin can extend both the CLI/gateway and the dashboard from a single install.
...
```bash
mkdir -p ~/.openagents/plugins/my-plugin/dashboard/dist
```

Write the manifest:

```json
// ~/.openagents/plugins/my-plugin/dashboard/manifest.json
{
  "name": "my-plugin",
  "label": "My Plugin",
  "icon": "Sparkles",
...
```javascript
// ~/.openagents/plugins/my-plugin/dashboard/dist/index.js
(function () {
  "use strict";
...
// OpenAgents API client + raw fetcher
...
#### Calling built-in OpenAgents endpoints
...
| `header-left` | Before the OpenAgents brand in the top bar. |
...
```json
// ~/.openagents/plugins/session-notes/dashboard/manifest.json
{
  "name": "session-notes",
  "label": "Session Notes",
...
```javascript
// ~/.openagents/plugins/session-notes/dashboard/dist/index.js
(function () {
  const SDK = window.__HERMES_PLUGIN_SDK__;
  const { React } = SDK;
...
```python
# ~/.openagents/plugins/my-plugin/dashboard/plugin_api.py
...
#### Accessing OpenAgents internals

Backend routes run inside the dashboard process, so they can import from the openagents codebase directly:
...
|----------|-----------|--------------|
| 1 (wins on conflict) | `~/.openagents/plugins/<name>/dashboard/` | `user` |
...
cp hermes-example-plugins/strike-freedom-cockpit/theme/strike-freedom.yaml \
   ~/.openagents/dashboard-themes/

# Plugin
cp -r hermes-example-plugins/strike-freedom-cockpit ~/.openagents/plugins/
```
...
Check that the file is in `~/.openagents/dashboard-themes/` and ends in `.yaml` or `.yml`. Refresh the page. Run `curl http://127.0.0.1:9119/api/dashboard/themes` — your theme should be in the response. If the YAML has a parse error, the dashboard logs to `errors.log` under `~/.openagents/logs/`.

**My plugin's tab doesn't show up.**
1. Check the manifest is at `~/.openagents/plugins/<name>/dashboard/manifest.json` (note the `dashboard/` subdirectory).
...
4. Tail `~/.openagents/logs/errors.log` for `Failed to load plugin <name> API routes` — import errors are logged there.
...
Dashboard plugins are installed by directory layout, not by pip entry point. The cleanest distribution path today is a git repo the user clones into `~/.openagents/plugins/`. A pip-based installer for dashboard plugins is not currently wired up.
