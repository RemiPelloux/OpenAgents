...
description: "Customize OpenAgents's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

OpenAgents's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `OPENAGENTS_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who OpenAgents is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

OpenAgents now seeds a default `SOUL.md` automatically in:

```text
~/.openagents/SOUL.md
```

More precisely, it uses the current instance's `OPENAGENTS_HOME`, so if you run OpenAgents with a custom home directory, it will use:

```text
$OPENAGENTS_HOME/SOUL.md
```

### Important behavior

- **SOUL.md is the agent's primary identity.** It occupies slot #1 in the system prompt, replacing the hardcoded default identity.
- OpenAgents creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- OpenAgents loads `SOUL.md` only from `OPENAGENTS_HOME`
- OpenAgents does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, OpenAgents falls back to a built-in default identity
...
If OpenAgents loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `OPENAGENTS_HOME`, the personality belongs to the OpenAgents instance itself.

That also makes it easier to teach users:
- "Edit `~/.openagents/SOUL.md` to change Hermes' default personality."
...
- how OpenAgents should handle uncertainty, disagreement, or ambiguity
...
## What OpenAgents injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, OpenAgents falls back to a built-in default identity ("You are OpenAgents, an intelligent AI assistant created by Nous Research..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
OpenAgents ships with built-in personalities you can switch to with `/personality`.
...
These are convenient overlays, but your global `SOUL.md` still gives OpenAgents its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

You can also define named custom personalities in `~/.openagents/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.openagents/SOUL.md`
...
- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how OpenAgents speaks
- `display.skin` and `/skin` affect how OpenAgents looks in the terminal
