...
When OpenAgents runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.openagents/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.openagents/config.yaml` (switchable per session
via `/personality`).
...
Install MCP servers via `~/.openagents/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. OpenAgents's deliverable
