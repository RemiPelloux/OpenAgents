...
All skills live in **`~/.openagents/skills/`** — the primary directory and source of truth. On fresh install, bundled skills are copied from the repo. Hub-installed and agent-created skills also go here. The agent can modify or delete any skill.

You can also point OpenAgents at **external skill directories** — additional folders scanned alongside the local one. See [External Skill Directories](#external-skill-directories) below.
...
**At install time** (applies to the default `~/.openagents` profile):

```bash
curl -fsSL https://openagents.nousresearch.com/install.sh | bash -s -- --no-skills
```
...
The bundled `plan` skill is a good example. Running `/plan [request]` loads the skill's instructions, telling OpenAgents to inspect context if needed, write a markdown implementation plan instead of executing the task, and save the result under `.hermes/plans/` relative to the active workspace/backend working directory.
...
When a missing value is encountered, OpenAgents asks for it securely only when the skill is actually loaded in the local CLI. You can skip setup and keep using the skill. Messaging surfaces never ask for secrets in chat — they tell you to use `hermes setup` or `~/.openagents/.env` locally instead.
...
```text
~/.openagents/skills/                  # Single source of truth
...
If you maintain skills outside of OpenAgents — for example, a shared `~/.agents/skills/` directory used by multiple AI tools — you can tell OpenAgents to scan those directories too.

Add `external_dirs` under the `skills` section in `~/.openagents/config.yaml`:
...
- **Create locally, update in place**: New agent-created skills are written to `~/.openagents/skills/`. Existing skills are modified where they are found, including skills under `external_dirs`, when the agent uses `skill_manage` actions such as `patch`, `edit`, `write_file`, `remove_file`, or `delete`.
- **External dirs are not a write-protection boundary**: If an external skill directory is writable by the OpenAgents process, agent-managed skill updates can change files in that directory. Use filesystem permissions or a separate profile/toolset setup if shared external skills must stay read-only.
- **Local precedence**: If the same skill name exists in both the local dir and an external dir, the local version wins.
- **Full integration**: External skills appear in the system prompt index, `skills_list`, `skill_view`, and as `/skill-name` slash commands — no different from local skills.
- **Non-existent paths are silently skipped**: If a configured directory doesn't exist, OpenAgents ignores it without errors. Useful for optional shared directories that may not be present on every machine.

### Example

```text
~/.openagents/skills/               # Local (primary, read-write)
├── devops/deploy-k8s/
│   └── SKILL.md
└── mlops/axolotl/
    └── SKILL.md
...
Bundles live in **`~/.openagents/skill-bundles/<slug>.yaml`** and look like this:
...
# Re-scan ~/.openagents/skill-bundles/ and report changes
...
- You want to ship a team-wide "task profile" by checking the bundle YAML into a shared dotfiles repo and symlinking it into `~/.openagents/skill-bundles/`.

A bundle is just a YAML alias — it doesn't install skills for you. The skills themselves must already be present (in `~/.openagents/skills/` or an external skill directory). Otherwise the bundle invocation just skips the missing ones.
...
Staged writes survive restarts under `~/.openagents/pending/skills/` and are
...
| `skills-sh` | `skills-sh/vercel-labs/agent-skills/vercel-react-best-practices` | Searchable via `hermes skills search <query> --source skills-sh`. OpenAgents resolves alias-style skills when the skills.sh slug differs from the repo folder. |
...
OpenAgents currently integrates with these skills ecosystems and discovery sources:

#### 1. Official optional skills (`official`)

These are maintained in the OpenAgents repository itself and install with built-in trust.
...
This is Vercel's public skills directory. OpenAgents can search it directly, inspect skill detail pages, resolve alias-style slugs, and install from the underlying source repo.
...
OpenAgents can install directly from GitHub repositories and GitHub-based taps. This is useful when you already know the repo/path or want to add your own custom source repo.
...
[Skills Hub](https://openagents.nousresearch.com/docs) page — instead of a
...
Install a single-file `SKILL.md` directly from any HTTP(S) URL — useful when an author hosts a skill on their own site (no hub listing, no GitHub path to type). OpenAgents fetches the URL, parses the YAML frontmatter, security-scans it, and installs.

- OpenAgents source id: `url`
...
| Level | Source | Policy |
|-------|--------|--------|
| `builtin` | Ships with OpenAgents | Always trusted |
...
If you want to share a curated set of skills — for your team, your org, or publicly — you can publish them as a **tap**: a GitHub repository other OpenAgents users add with `hermes skills tap add <owner/repo>`. No server, no registry sign-up, no release pipeline. Just a directory of `SKILL.md` files.
...
OpenAgents discovers skills by listing every subdirectory of the tap path and probing each for `SKILL.md`.
...
After pushing that to GitHub, any OpenAgents user can subscribe and install:
...
If your skills don't live under `skills/` (common when you're adding a `skills/` subtree to an existing project), edit the tap entry in `~/.openagents/.hub/taps.json`:
...
Taps are stored in `~/.openagents/.hub/taps.json` (created on demand).

## Bundled skill updates (`hermes skills reset`)

OpenAgents ships with a set of bundled skills in `skills/` inside the repo. On install and on every `hermes update`, a sync pass copies those into `~/.openagents/skills/` and records a manifest at `~/.openagents/skills/.bundled_manifest` mapping each skill name to the content hash at the time it was synced (the **origin hash**).

On each sync, OpenAgents recomputes the hash of your local copy and compares it to the origin hash:
...
:::note Profiles
Each profile has its own `.bundled_manifest` under its own `OPENAGENTS_HOME`, so `hermes -p coder skills reset <name>` only affects that profile.
:::
