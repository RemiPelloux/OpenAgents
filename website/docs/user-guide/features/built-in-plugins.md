...
description: "Plugins shipped with OpenAgents that run automatically via lifecycle hooks — disk-cleanup and friends"
---

# Built-in Plugins

OpenAgents ships a small set of plugins bundled with the repository. They live under `<repo>/plugins/<name>/` and load automatically alongside user-installed plugins in `~/.openagents/plugins/`. They use the same plugin surface as third-party plugins — hooks, tools, slash commands — just maintained in-tree.

See the [Plugins](/user-guide/features/plugins) page for the general plugin system, and [Build a OpenAgents Plugin](/guides/build-a-hermes-plugin) to write your own.
...
2. **User** — `~/.openagents/plugins/<name>/`
...
| `hermes-achievements` | dashboard tab | Steam-style collectible badges generated from your real OpenAgents session history |
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `OPENAGENTS_HOME` or `/tmp/hermes-*`, track it silently as `test` / `temp` / `cron-output`. |
...
| empty dirs under OPENAGENTS_HOME | always | Never |
...
**State** — everything lives at `$OPENAGENTS_HOME/disk-cleanup/`:
...
**Safety** — cleanup only ever touches paths under `OPENAGENTS_HOME` or `/tmp/hermes-*`. Windows mounts (`/mnt/c/...`) are rejected. Well-known top-level state dirs (`logs/`, `memories/`, `sessions/`, `cron/`, `cache/`, `skills/`, `plugins/`, `disk-cleanup/` itself) are never removed even when empty — a fresh install does not get gutted on first session end.
...
The wizard collects your keys, `pip install`s the `langfuse` SDK, and adds `observability/langfuse` to `plugins.enabled` for you. Restart OpenAgents and the next turn ships a trace.
...
Then put the credentials in `~/.openagents/.env`:
...
Session grouping keys off the OpenAgents session ID (or task ID for sub-agents) via `langfuse.propagate_attributes`, so everything in a single `hermes chat` session lives under one Langfuse session.
...
- Post-meeting artifacts (transcript, speaker-attributed notes, action items) saved under `~/.openagents/cache/google_meet/<meeting_id>/`
...
**Disabling:** `hermes plugins disable google_meet`. Any cached transcripts and recordings stay in `~/.openagents/cache/google_meet/` until you remove them.

### hermes-achievements

Adds a **Steam-style achievements tab to the dashboard** — 60+ collectible, tiered badges generated from your real OpenAgents session history. Tool-chain feats, debugging patterns, vibe-coding streaks, skill/memory usage, model/provider variety, lifestyle quirks (weekend and night sessions). Originally authored by [@PCinkusz](https://github.com/PCinkusz) as an external plugin; brought in-tree so it stays in lockstep with OpenAgents feature changes.

**How it works:**

- Scans your entire `~/.openagents/state.db` session history on the dashboard backend
...
- Unlock state is persisted to `$OPENAGENTS_HOME/plugins/hermes-achievements/state.json`
...
| Secret | Hidden until OpenAgents detects the first related signal in your history |
...
**State files** — live under `$OPENAGENTS_HOME/plugins/hermes-achievements/`:

| File | Contents |
|---|---|
| `state.json` | Unlock history: which badges you've earned and when. Stable across OpenAgents updates. |
...
**Opting out:** Delete or rename `plugins/hermes-achievements/dashboard/manifest.json`, or override it with a user plugin of the same name in `~/.openagents/plugins/hermes-achievements/` that ships no dashboard. The plugin's state files under `$OPENAGENTS_HOME/plugins/hermes-achievements/` survive — reinstalling preserves your unlock history.

## Adding a bundled plugin

Bundled plugins are written exactly like any other OpenAgents plugin — see [Build a OpenAgents Plugin](/guides/build-a-hermes-plugin). The only differences are:

- Directory lives at `<repo>/plugins/<name>/` instead of `~/.openagents/plugins/<name>/`
