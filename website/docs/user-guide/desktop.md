...
sidebar_position: 3
title: "Desktop App"
description: "The native OpenAgents desktop app — a polished experience for chatting with Hermes, with streaming tool output, side-by-side previews, a file browser, voice, cron, profiles, skills, and settings. macOS, Windows, and Linux."
---

# Desktop App

The OpenAgents desktop app is a native app built around the **same** agent you get from the CLI and the gateway — same config, same API keys, same sessions, same skills, same memory. It is not a separate product or a lightweight clone; it uses the same OpenAgents core and settings, and drives it through a modern & thoughtfully designed UI. If you have used `hermes` in a terminal, everything you set up there is already here, and anything you do here shows up there.

It runs on **macOS, Windows, and Linux**.

:::tip Which interface is which?
OpenAgents has several front ends that all talk to the same agent:
...
Follow the [installation instructions for OpenAgents Desktop](../getting-started/installation.md).

If you already have OpenAgents installed, simply run
...
- **The same conversation history** as every other OpenAgents surface — sessions started here resume in the CLI/TUI and vice versa.
...
Talk to OpenAgents and hear it back, the same [voice mode](./features/voice-mode.md) available elsewhere. On macOS the OS will prompt once for microphone access.
...
The app also surfaces the broader OpenAgents management surface so you don't have to drop to a terminal:
...
- **Profiles** — switch between [OpenAgents profiles](./profiles.md) (isolated config/skills/sessions).
...
The [manual update process](https://openagents.nousresearch.com/docs/getting-started/updating) also works with the GUI.

## Uninstalling

Open **Settings → About → Danger zone** and pick how much to remove:

- **Uninstall Chat GUI only** — removes the desktop app and its data; the OpenAgents, your config, and your chats stay. (Same as `hermes uninstall --gui`.)
...
| `--hermes-root PATH` | Override the OpenAgents source root the app uses (sets `HERMES_DESKTOP_HERMES_ROOT`)          |
...
Set a username and password, then start the backend bound to a reachable address. The credentials live in `~/.openagents/.env` (the secrets file, mode 0600):
...
- **No "Sign in" button — it asks for a session token instead** — the backend's username/password provider isn't active. `/api/status` won't list `"basic"` in `auth_providers`. Make sure both the username and a password (or password hash) are set in `~/.openagents/.env` and that the dashboard process actually loaded them.
...
For the same setup from the web-dashboard angle, see [Web Dashboard → Connecting OpenAgents Desktop to a remote backend](./features/web-dashboard.md#connecting-hermes-desktop-to-a-remote-backend); the env vars are catalogued under [Environment Variables → Web Dashboard & OpenAgents Desktop](../reference/environment-variables.md#web-dashboard--hermes-desktop).

## Troubleshooting

Boot logs land in `OPENAGENTS_HOME/logs/desktop.log` (it includes backend output and recent Python tracebacks) — check it first if the app reports a boot failure. You can also tail it from the CLI:
...
ELECTRON_MIRROR=https://npmmirror.com/mirrors/electron/ \
  bash -c 'cd "$HOME/.hermes/openagents/apps/desktop" && CSC_IDENTITY_AUTO_DISCOVERY=false npm run pack'
```
