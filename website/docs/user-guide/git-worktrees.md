...
title: "Git Worktrees"
description: "Run multiple OpenAgentss safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

OpenAgents is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with OpenAgents so each session has a clean, isolated working directory.

## Why Use Worktrees with Hermes?

OpenAgents treats the **current working directory** as the project root:

- CLI: the directory where you run `hermes` or `hermes chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.openagents/config.yaml`
...
Now you can `cd` into the new worktree and run OpenAgents there:

```bash
cd ../repo-feature

# Start OpenAgents in the worktree
hermes
```
...
- OpenAgents checkpoint data under `~/.openagents/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per OpenAgents experiment**
...
- **Avoid running OpenAgents from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `hermes -w` (Automatic Worktree Mode)

OpenAgents has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Use **git worktrees** to give each OpenAgents session its own clean checkout.
