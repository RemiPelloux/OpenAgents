...
description: "Use when a OpenAgents Kanban worker wants to run Codex CLI as an isolated implementation lane while OpenAgents keeps ownership of task lifecycle, reconciliation, tes..."
---
...
Use when a OpenAgents Kanban worker wants to run Codex CLI as an isolated implementation lane while OpenAgents keeps ownership of task lifecycle, reconciliation, testing, and handoff.
...
| Related skills | [`codex`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-codex), [`openagents`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-openagents) |
...
- A bounded diff can be evaluated by OpenAgents in one run.
- The repo can be copied or checked out in an isolated git worktree/branch.
- OpenAgents can run the relevant tests itself after Codex exits.
...
1. OpenAgents owns the Kanban lifecycle. Codex must never call `kanban_complete`, `kanban_block`, `kanban_create`, gateway messaging, or any OpenAgents board CLI as a substitute for the worker.
2. OpenAgents owns final acceptance. Treat Codex commits/diffs as untrusted patches until reviewed and verified.
3. OpenAgents owns test execution. Codex may run tests, but those runs are advisory; repeat required verification from OpenAgents with the repo's canonical wrapper.
4. OpenAgents owns safety. If Codex changes safety boundaries, risk gates, live trading behavior, or secrets handling, reject the lane even if tests pass.
5. OpenAgents owns cleanup. Kill stuck Codex processes and remove temporary worktrees when they are no longer needed.
...
If the current Kanban workspace is already an isolated git worktree created for this task, you may create a sibling Codex branch inside it only if `git status --short` is clean except for intentional OpenAgents edits. Otherwise create a separate temporary worktree and cherry-pick or copy accepted commits back after reconciliation.
...
Run these before spawning Codex. Missing Codex is a normal reason to skip the lane, not a task blocker if OpenAgents can do the task directly.
...
OpenAgents owns the Kanban lifecycle; do not call OpenAgents kanban tools or messaging.
...
Do not use `--yolo` for prediction-market-bot or safety-sensitive repos. Prefer `--full-auto` inside the isolated worktree, then rely on OpenAgents reconciliation.
...
- Explicit statement: OpenAgents owns Kanban lifecycle; Codex is an input lane only.
...
- Verification commands Codex may run and commands OpenAgents will run afterward.
...
OpenAgents must perform this checklist before accepting any Codex lane result:
...
- [ ] OpenAgents ran the canonical tests itself, using `scripts/run_tests.sh` for OpenAgents or the repo's documented wrapper for other repos.
...
```json
{
  "codex_lane": {
    ...
    "tests_run": [
      {"command": "scripts/run_tests.sh tests/tools/test_x.py", "exit_code": 0, "owner": "openagents"},
...
```json
{
  "codex_lane": {
    ...
    "result": "rejected",
    "accepted_commits": [],
    "rejected_reason": "Direct OpenAgents edit was smaller and safer than spawning Codex.",
    "tests_run": [],
    "artifacts": []
  }
}
...
3. Letting Codex own Kanban. Codex may summarize progress, but OpenAgents writes board state.
...
- [ ] OpenAgents reviewed `git diff` and safety-sensitive files.
- [ ] OpenAgents ran canonical tests independently.
