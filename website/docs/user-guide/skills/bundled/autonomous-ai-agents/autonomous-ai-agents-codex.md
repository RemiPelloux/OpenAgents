...
| Related skills | [`claude-code`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-claude-code), [`openagents`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-openagents) |
...
Delegate coding tasks to [Codex](https://github.com/openai/codex) via the OpenAgents terminal. Codex is OpenAI's autonomous coding agent CLI.
...
For OpenAgents itself, `model.provider: openai-codex` uses Hermes-managed Codex
OAuth from `~/.openagents/auth.json` after `hermes auth add openai-codex`. For the
...
## OpenAgents Gateway Caveat

When invoking the Codex CLI from a OpenAgents gateway/service context (for example,
