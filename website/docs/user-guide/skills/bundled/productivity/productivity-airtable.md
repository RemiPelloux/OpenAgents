...
4. Store the token in `${OPENAGENTS_HOME:-~/.openagents}/.env` (or via `hermes setup`):
   ```
...
- **`AIRTABLE_API_KEY` flows from `${OPENAGENTS_HOME:-~/.openagents}/.env` into the subprocess automatically** when this skill is loaded — no need to re-export it before each `curl` call.
