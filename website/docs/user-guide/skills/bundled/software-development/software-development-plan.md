...
| Version | `2.0.0` |
| Author | OpenAgents (writing-craft adapted from obra/superpowers) |
| License | MIT |
...
Treat that as relative to the active working directory / backend workspace. OpenAgents file tools are backend-aware, so using this relative path keeps the plan with the workspace on local, docker, ssh, modal, and daytona backends.
...
Use OpenAgents tools to understand the project:
