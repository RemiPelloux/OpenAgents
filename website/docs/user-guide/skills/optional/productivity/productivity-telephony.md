---
title: "Telephony — Give OpenAgents phone capabilities without core tool changes"
sidebar_label: "Telephony"
description: "Give OpenAgents phone capabilities without core tool changes"
---
...
Give OpenAgents phone capabilities without core tool changes. Provision and persist a Twilio number, send and receive SMS/MMS, make direct calls, and place AI-driven outbound calls through Bland.ai or Vapi.
...
This optional skill gives OpenAgents practical phone capabilities while keeping telephony out of the core tool list.

It ships with a helper script, `scripts/telephony.py`, that can:
- save provider credentials into `${OPENAGENTS_HOME:-~/.openagents}/.env`
...
It does **not** turn OpenAgents into a real-time inbound phone gateway. Inbound SMS is handled by polling the Twilio REST API. That is enough for many workflows, including notifications and some one-time-code retrieval, without adding core webhook infrastructure.
...
4. Treat third-party phone numbers as sensitive operational data:
   - do not save them to OpenAgents memory
...
### 1) "I want OpenAgents to own a real phone number"
Use **Twilio**.
...
- pairs well with OpenAgents `text_to_speech` plus a public file host or tunnel

## Files and persistent state

The skill persists telephony state in two places:

### `${OPENAGENTS_HOME:-~/.openagents}/.env`
...
### `~/.openagents/telephony_state.json`
...
```bash
SCRIPT="$(find ~/.openagents/skills -path '*/telephony/scripts/telephony.py' -print -quit)"
```
...
3. Buy it and save it into `${OPENAGENTS_HOME:-~/.openagents}/.env` + state:
```bash
python3 "$SCRIPT" twilio-buy "+17025551234" --save-env
```
...
```bash
python3 "$SCRIPT" twilio-call "+15551230000" --message "Hello! This is OpenAgents calling with your status update." --voice Polly.Joanna
```
...
Recommended OpenAgents TTS -> Twilio Play workflow:

1. Generate the audio with OpenAgents `text_to_speech`.
2. Make the resulting MP3 publicly reachable.
3. Place the Twilio call with `--audio-url`.

Example agent flow:
- Ask OpenAgents to create the message audio with `text_to_speech`
...
Important note:
- OpenAgents TTS is great for prerecorded outbound messages
...
- OpenAgents STT/TTS alone is not being used here as a full duplex phone conversation engine; that would require a much heavier streaming/webhook integration than this skill is trying to introduce
...
7. Summarize the outcome without persisting third-party numbers to OpenAgents memory.
...
- Do not store arbitrary third-party phone numbers in OpenAgents memory.
...
3. persist that number to `${OPENAGENTS_HOME:-~/.openagents}/.env`
