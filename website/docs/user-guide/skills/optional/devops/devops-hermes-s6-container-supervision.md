...
description: "Modify, debug, or extend the s6-overlay supervision tree inside the OpenAgents Docker image — adding new services, debugging profile gateways, understandin..."
---

{/* This page is auto-generated from the skill's SKILL.md by website/scripts/generate-skill-docs.py. Edit the source SKILL.md, not this page. */}

# OpenAgents S6 Container Supervision

Modify, debug, or extend the s6-overlay supervision tree inside the OpenAgents Docker image — adding new services, debugging profile gateways, understanding the Architecture B main-program pattern.
...
| Related skills | [`openagents`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-openagents), `openagents-dev` |
...
# OpenAgents s6-overlay Container Supervision

## When to use this skill

Load this skill when you're working on:
- Adding or removing a static service in the OpenAgents Docker image (something that should be supervised at every container start, like the dashboard)
...
If you're just running the OpenAgents and want to use Docker, see `website/docs/user-guide/docker.md` instead.
...
│   │   └── skills_sync.py
│   └── 02-reconcile-profiles          ← openagents_cli.container_boot
│       ├── chown /run/service (hermes-writable for runtime register)
│       └── walk $OPENAGENTS_HOME/profiles/<name>/gateway_state.json
...
│   │   └── log/run     (s6-log → $OPENAGENTS_HOME/logs/gateways/coder/current)
│   └── ...
│
...
| `docker/cont-init.d/02-reconcile-profiles` | Calls `openagents_cli.container_boot` on every boot to restore profile gateway slots from the persistent volume. |
...
| `openagents_cli/service_manager.py` | `S6ServiceManager`: `register_profile_gateway`, `unregister_profile_gateway`, `start/stop/restart/is_running`, `list_profile_gateways`. |
| `openagents_cli/container_boot.py` | `reconcile_profile_gateways()` — walks persistent profiles, regenerates s6 slots, emits `container-boot.log`. |
| `openagents_cli/gateway.py::_dispatch_via_service_manager_if_s6` | Intercepts `hermes gateway start/stop/restart` and routes to s6 when running in a container. |
...
Edit `S6ServiceManager._render_run_script` in `openagents_cli/service_manager.py`. The function is also called by `openagents_cli/container_boot.py::_register_service` during boot reconciliation, so it's the single source of truth. Update the corresponding assertion in `tests/openagents_cli/test_service_manager.py::test_s6_register_creates_service_dir_and_triggers_scan`.

### Run the docker test harness

```sh
docker build -t openagents-harness:latest .
HERMES_TEST_IMAGE=openagents-harness:latest scripts/run_tests.sh tests/docker/ -v
...
The cont-init reconciler runs as hermes (`s6-setuidgid hermes` in `02-reconcile-profiles`). If a profile dir ends up root-owned (e.g. because `docker exec <c> hermes profile create …` ran as root by default), the reconciler can't read SOUL.md and fails with `PermissionError`. Mitigation: `stage2-hook.sh` chowns `$OPENAGENTS_HOME/profiles` to hermes on **every** boot, idempotently. Don't remove that block.

### Files written by `docker exec` are root-owned

`docker exec` defaults to root. Either pass `--user hermes` or rely on the stage2 chown sweep next reboot. Don't write files under `$OPENAGENTS_HOME/profiles/<name>/` as root manually — the next reconcile pass will sweep them but in-flight operations may hit perm errors.
...
- `openagents-dev`: General openagents codebase navigation
