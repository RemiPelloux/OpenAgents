...
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by OpenAgents |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during OpenAgents terminal calls (desktop app flow only)
...
Set `OP_SERVICE_ACCOUNT_TOKEN` in `${OPENAGENTS_HOME:-~/.openagents}/.env` (the skill will prompt for this on first load).
...
## OpenAgents Execution Pattern (desktop app flow)

OpenAgents terminal commands are non-interactive by default and can lose auth context between calls.
