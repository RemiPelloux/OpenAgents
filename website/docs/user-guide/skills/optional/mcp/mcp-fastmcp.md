...
- smoke-test a server with the FastMCP CLI before wiring it into OpenAgents or another client
...
```bash
python ~/.openagents/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.openagents/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is OpenAgents integration, either:

- configure the server in `~/.openagents/config.yaml` using the `native-mcp` skill, or
...
### OpenAgents cannot see the deployed server

The server-building part may be correct while the OpenAgents config is not. Load the `native-mcp` skill and configure the server in `~/.openagents/config.yaml`, then restart Hermes.
