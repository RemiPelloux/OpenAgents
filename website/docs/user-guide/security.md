...
OpenAgents is designed with a defense-in-depth security model. This page covers every security boundary — from command approval to container isolation to user authorization on messaging platforms.
...
Before executing any command, OpenAgents checks it against a curated list of dangerous patterns. If a match is found, the user must explicitly approve it.

### Approval Modes

The approval system supports three modes, configured via `approvals.mode` in `~/.openagents/config.yaml`:
...
| `timeout` | `60` | Seconds OpenAgents waits for an approval reply before timing out. |
...
When YOLO is active, OpenAgents shows two persistent visual reminders so it's hard to forget that approval prompts are bypassed:
...
Some commands are so catastrophic — irreversible filesystem wipes, fork bombs, direct block-device writes — that OpenAgents refuses to run them **regardless** of:
...
Configure the timeout in `~/.openagents/config.yaml`:
...
| `tee` to `/etc/`, `~/.ssh/`, `~/.openagents/.env` | Overwrite sensitive file via tee |
| `>` / `>>` to `/etc/`, `~/.ssh/`, `~/.openagents/.env` | Overwrite sensitive file via redirection |
...
Commands approved with "always" are saved to `~/.openagents/config.yaml`:
...
When running the messaging gateway, OpenAgents controls who can interact with the bot through a layered authorization system.
...
Set allowed user IDs as comma-separated values in `~/.openagents/.env`:
...
For more flexible authorization, OpenAgents includes a code-based pairing system. Instead of requiring user IDs upfront, unknown users receive a one-time pairing code that the bot owner approves via the CLI.
...
Control how unauthorized direct messages are handled in `~/.openagents/config.yaml`:
...
**Storage:** Pairing data is stored in `~/.openagents/pairing/` with per-platform JSON files:
...
When using the `docker` terminal backend, OpenAgents applies strict security hardening to every container.
...
Container resources are configurable in `~/.openagents/config.yaml`:
...
- **Persistent mode** (`container_persistent: true`): Bind-mounts `/workspace` and `/root` from `~/.openagents/sandboxes/docker/<task_id>/`
...
Some skills need **files** (not just env vars) in the sandbox — for example, Google Workspace stores OAuth tokens as `google_token.json` under the active profile's `OPENAGENTS_HOME`. Skills declare these in frontmatter:
...
When loaded, OpenAgents checks if these files exist in the active profile's `OPENAGENTS_HOME` and registers them for mounting:
...
| **terminal** (local) | Blocks explicit OpenAgents infrastructure vars (provider keys, gateway tokens, tool API keys) | ✅ Passthrough vars bypass the blocklist |
...
- OpenAgents infrastructure secrets (provider API keys, gateway tokens) should never be added to `env_passthrough` — they have dedicated mechanisms
...
OpenAgents integrates [tirith](https://github.com/sheeki03/tirith) for content-level command scanning before execution. Tirith detects threats that pattern matching alone misses:
...
Tirith ships prebuilt binaries for Linux (x86_64 / aarch64) and macOS (x86_64 / arm64). On platforms with no prebuilt binary (Windows, etc.), tirith is silently skipped — pattern-matching guards still run, and the CLI does not surface an "unavailable" banner. To use tirith on Windows, run OpenAgents under WSL.
...
4. **Store secrets securely** — keep API keys in `~/.openagents/.env` with proper file permissions
...
9. **Monitor logs** — check `~/.openagents/logs/` for unauthorized access attempts
...
For maximum security, run the gateway on a separate machine or VM. Set `terminal.backend: ssh` in `config.yaml`, then provide host details via environment variables in `~/.openagents/.env`:
...
OpenAgents ships with a built-in advisory scanner that flags Python packages in the active venv that match a curated catalog of known-compromised versions (supply-chain worms like the May 2026 `mistralai 2.4.6` poisoning). Implementation lives in `openagents_cli/security_advisories.py`.
...
Many features (Mistral TTS, ElevenLabs, Honcho memory, Bedrock, Slack, Matrix, …) depend on Python packages that not every user needs. OpenAgents installs these **lazily** on first use rather than eagerly under `openagents[all]`. The implementation lives in `tools/lazy_deps.py`.
