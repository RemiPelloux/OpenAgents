...
description: "How to build a model provider (inference backend) plugin for OpenAgents"
---

# Building a Model Provider Plugin

Model provider plugins declare an inference backend — an OpenAI-compatible endpoint, an Anthropic Messages server, a Codex-style Responses API, or a Bedrock-native surface — that OpenAgents can route `AIAgent` calls through. Every built-in provider (OpenRouter, Anthropic, GMI, DeepSeek, Nvidia, …) ships as one of these plugins. Third parties can add their own by dropping a directory under `$OPENAGENTS_HOME/plugins/model-providers/` with zero changes to the repo.
...
2. **User plugins** — `$OPENAGENTS_HOME/plugins/model-providers/<name>/` — drop in any directory; no restart required for subsequent sessions
3. **Legacy single-file** — `<repo>/providers/<name>.py` — back-compat for out-of-tree editable installs

**User plugins override bundled plugins of the same name** because `register_provider()` is last-writer-wins. Drop a `$OPENAGENTS_HOME/plugins/model-providers/gmi/` directory to replace the built-in GMI profile without touching the repo.
...
|---|---|---|
| Credential resolution | `openagents_cli/auth.py` | `PROVIDER_REGISTRY["acme-inference"]` populated from profile |
| `--provider` CLI flag | `openagents_cli/main.py` | Accepts `acme-inference` |
| `hermes model` picker | `openagents_cli/models.py` | Appears in `CANONICAL_PROVIDERS`, model list fetched from `{base_url}/models` |
| `hermes doctor` | `openagents_cli/doctor.py` | Health check for `ACME_API_KEY` + `{base_url}/models` probe |
| `hermes setup` | `openagents_cli/config.py` | `ACME_API_KEY` appears in `OPTIONAL_ENV_VARS` and the setup wizard |
...
| Runtime resolution | `openagents_cli/runtime_provider.py` | Returns correct `base_url`, `api_key`, `api_mode` |
...
Say you want to point `gmi` at your private staging endpoint for testing. Create `~/.openagents/plugins/model-providers/gmi/__init__.py`:
...
Four values are recognized. OpenAgents picks one based on:
...
Point `OPENAGENTS_HOME` at a temp directory so you don't pollute your real config:

```bash
export OPENAGENTS_HOME=/tmp/hermes-plugin-test
mkdir -p $OPENAGENTS_HOME/plugins/model-providers/my-provider
cat > $OPENAGENTS_HOME/plugins/model-providers/my-provider/__init__.py <<'EOF'
from providers import register_provider
from providers.base import ProviderProfile
register_provider(ProviderProfile(
    name="my-provider",
...
See [Building a OpenAgents Plugin](/guides/build-a-hermes-plugin#distribute-via-pip) for the full entry-points setup.
...
- [Building a OpenAgents Plugin](/guides/build-a-hermes-plugin) — general plugin authoring
