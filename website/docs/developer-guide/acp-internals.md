...
```text
hermes acp / openagents-acp / python -m acp_adapter
...
The Zed ACP Registry path launches the same adapter through `uvx --from 'openagents[acp]==<version>' openagents-acp`, pointed at the `openagents` PyPI release.
...
- `allow_once` -> OpenAgents `once`
- `allow_always` -> OpenAgents `always`
- reject options -> OpenAgents `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps OpenAgents tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["openagents-acp"])
...
- `acp_adapter/auth.py`
- `openagents_cli/runtime_provider.py`

So ACP advertises and uses the currently configured OpenAgents provider/credentials. It also always advertises a terminal setup auth method (`hermes-setup`, args `--setup`) so first-run registry clients can open Hermes' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.openagents/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `openagents-acp` toolset definition
- `openagents_cli/main.py` — `hermes acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `openagents-acp` script
