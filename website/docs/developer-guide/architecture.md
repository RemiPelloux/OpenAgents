...
sidebar_position: 1
title: "Architecture"
description: "OpenAgents internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of OpenAgents internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ openagents_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # OpenAgentsCLI — interactive terminal UI (large file)
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── openagents_state.py           # SQLite session/state database with FTS5
├── openagents_constants.py       # OPENAGENTS_HOME, profile-aware paths
...
│
├── openagents_cli/               # CLI subcommands and setup
...
Exposes OpenAgents as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`hermes -p <name>`) gets its own OPENAGENTS_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
