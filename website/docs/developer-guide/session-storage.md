...
OpenAgents uses a SQLite database (`~/.openagents/state.db`) to persist session
metadata, full message history, and model configuration across CLI and gateway
sessions. This replaces the earlier per-session JSONL file approach.

Source file: `openagents_state.py`
...
db = SessionDB()                           # Default: ~/.openagents/state.db
...
Default path: `~/.openagents/state.db`

This is derived from `openagents_constants.get_openagents_home()` which resolves to
`~/.openagents/` by default, or the value of `OPENAGENTS_HOME` environment variable.
