...
description: "How to build an image-generation backend plugin for OpenAgents"
---
...
A bundled plugin is complete at this point. User plugins at `~/.openagents/plugins/image_gen/<name>/` need to be added to `plugins.enabled` in `config.yaml` (or run `hermes plugins enable <name>`).
