...
* **Host-owned credentials.** OAuth tokens, refresh flows, the
  ...
  concept OpenAgents already has applies. The plugin never sees a
...
openagents itself). For the async surface (`acomplete()` /
...
result = ctx.llm.complete(
    ...
    provider=None,         # optional, gated — OpenAgents provider id (e.g. "openrouter")
...
plugins:
  entries:
    my-plugin:
      llm:
        # Allow this plugin to choose a different OpenAgents provider
        # (must be one OpenAgents already knows about — same names as
        ...
        # plugin sends — OpenAgents does not look anything up.
        allowed_models:
          - openai/gpt-4o-mini
...
* **Auth.** Pulls API keys, OAuth tokens, or refresh tokens from
  `~/.openagents/auth.json` / env, including the credential pool when
...
Existing `ctx.*` methods extend an existing OpenAgents subsystem:
...
* Implementation: [`agent/plugin_llm.py`](https://github.com/NousResearch/openagents/blob/main/agent/plugin_llm.py)
* Tests: [`tests/agent/test_plugin_llm.py`](https://github.com/NousResearch/openagents/blob/main/tests/agent/test_plugin_llm.py)
