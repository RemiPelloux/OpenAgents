...
description: "How OpenAgents builds the system prompt, preserves cache stability, and injects ephemeral layers"
---

# Prompt Assembly

OpenAgents deliberately separates:
...
```
# Layer 1: Agent Identity (from ~/.openagents/SOUL.md)
...
`SOUL.md` lives at `~/.openagents/SOUL.md` and serves as the agent's identity — the very first section of the system prompt. The loading logic in `prompt_builder.py` works as follows:
...
def build_context_files_prompt(cwd=None, skip_soul=False):
    ...
    # SOUL.md from OPENAGENTS_HOME (independent of project context)
    if not skip_soul:
...
`pre_llm_call` plugin context also lands in this API-call-time path: it is appended to the current turn's **user message**, not written into the cached system prompt. When multiple plugins return context, OpenAgents concatenates those context blocks (see [Hooks → `pre_llm_call`](../user-guide/features/hooks.md#pre_llm_call)).
...
Most users should treat `agent/prompt_builder.py` as implementation code, not a configuration surface. The supported customization path is to change the prompt inputs OpenAgents already loads, rather than editing Python templates in place.

### Use these surfaces first

- `~/.openagents/SOUL.md` — replace the built-in default identity block with your own agent persona and standing behavior.
- `~/.openagents/MEMORY.md` and `~/.openagents/USER.md` — provide durable cross-session facts and user profile data that should be snapshotted into new sessions.
...
- if you want to change how OpenAgents assembles prompts for everyone, change Python and treat it as a code contribution
