...
sidebar_position: 2
title: "Adding Tools"
description: "How to add a new tool to OpenAgents — schemas, handlers, registration, and toolsets"
---
...
This page is for adding a **built-in OpenAgents tool** to the repository itself.
If you want a personal, project-local, or otherwise custom tool without
modifying OpenAgents core, use the plugin route instead:

- [Plugins](/user-guide/features/plugins)
- [Build a OpenAgents Plugin](/guides/build-a-hermes-plugin)
...
If your tool requires an API key, add it to `openagents_cli/config.py`:
...
- [ ] Optional: API key added to `OPTIONAL_ENV_VARS` in `openagents_cli/config.py`
