...
description: "How to build a video-generation backend plugin for OpenAgents"
---
...
OpenAgents scans for video-gen backends in three places:

1. **Bundled** — `<repo>/plugins/video_gen/<name>/` (auto-loaded with `kind: backend`)
2. **User** — `~/.openagents/plugins/video_gen/<name>/` (opt-in via `plugins.enabled`)
...
If your backend returns base64, use `save_b64_video()` to write under `$OPENAGENTS_HOME/cache/videos/`. For raw bytes from a follow-up HTTP fetch, use `save_bytes_video()`. Otherwise return the upstream URL directly — the gateway resolves remote URLs on delivery.
