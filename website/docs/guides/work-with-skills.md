...
description: "Find, install, use, and create skills — on-demand knowledge that teaches OpenAgents new workflows"
---

# Working with Skills

Skills are on-demand knowledge documents that teach OpenAgents how to handle specific tasks — from generating ASCII art to managing GitHub PRs. This guide walks you through using them day to day.
...
Every OpenAgents installation ships with bundled skills. See what's available:
...
You can also trigger skills through natural conversation — ask OpenAgents to use a specific skill, and it will load it via the `skill_view` tool.
...
Official optional skills ship with OpenAgents but aren't active by default. Install them explicitly:
...
What happens:
1. The skill directory is copied to `~/.openagents/skills/`
...
For how to ship skills in your own plugin, see [Build a OpenAgents Plugin → Bundle skills](/guides/build-a-hermes-plugin#bundle-skills).
...
When a skill with config is first loaded, OpenAgents prompts you for the values. They're stored in `config.yaml` under `skills.config.*`.
...
```bash
mkdir -p ~/.openagents/skills/my-category/my-skill
```

### 2. Write SKILL.md

```markdown title="~/.openagents/skills/my-category/my-skill/SKILL.md"
---
name: my-skill
...
The skill appears automatically — no registration needed. Drop it in `~/.openagents/skills/` and it's live.

:::info
The agent can also create and update skills itself using `skill_manage`. After solving a complex problem, OpenAgents may offer to save the approach as a skill for next time.
:::
...
**Let the agent create skills.** After a complex multi-step task, OpenAgents will often offer to save the approach as a skill. Say yes — these agent-authored skills capture the exact workflow including pitfalls that were discovered along the way.

**Use categories.** Organize skills into subdirectories (`~/.openagents/skills/devops/`, `~/.openagents/skills/research/`, etc.). This keeps the list manageable and helps the agent find relevant skills faster.

**Update skills when they go stale.** If you use a skill and hit issues not covered by it, tell OpenAgents to update the skill with what you learned. Skills that aren't maintained become liabilities.
