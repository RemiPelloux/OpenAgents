...
#### Option 1: OpenAgents cron (recommended if you already run the OpenAgents gateway)

OpenAgents ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.openagents/scripts/`. First create the script:

```bash
mkdir -p ~/.openagents/scripts
cat > ~/.openagents/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec hermes teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.openagents/scripts/maintain-teams-subscriptions.sh
```
...
```ini
[Unit]
Description=Run OpenAgents Teams pipeline subscription maintenance every 12 hours
...
Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.openagents/.env` at the top of a wrapper script that crontab calls.
...
- [ ] **`maintain-subscriptions` is scheduled** (OpenAgents cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
