...
You need **tenant admin rights** (or an admin to grant consent on your behalf) to finish this. Bookmark the values you collect — they go into `~/.openagents/.env` at the end.
...
Put the three values you collected into `~/.openagents/.env`:
...
OpenAgents ships a Graph auth smoke-test. From your OpenAgents install:
...
2. Update `MSGRAPH_CLIENT_SECRET` in `~/.openagents/.env` with the new value.
