...
title: "Build a OpenAgents Plugin"
description: "Step-by-step guide to building a complete OpenAgents plugin with tools, hooks, data files, and skills"
---

# Build a OpenAgents Plugin

This guide walks through building a complete OpenAgents plugin from scratch. By the end you'll have a working plugin with multiple tools, lifecycle hooks, shipped data files, and a bundled skill — everything the plugin system supports.

:::info Not sure which guide you need?
OpenAgents has several distinct pluggable interfaces — some use Python `register_*` APIs, others are config-driven or drop-in directories. Use this map first:
...
| **Gateway event hooks** (fire on startup, session events, commands) | [Event Hooks](/user-guide/features/hooks#gateway-event-hooks) — drop `HOOK.yaml` + `handler.py` into `~/.openagents/hooks/<name>/` |
...
Plugins that integrate **someone else's product or project** — observability/metrics backends, vendor SaaS connectors, analytics dashboards, paid-service tie-ins — are built and distributed as **standalone plugin repos**, not merged into `NousResearch/openagents`. Users install them into `~/.openagents/plugins/` or via a pip entry point; everything in this guide works the same way from a standalone repo. This is a coupling-and-maintenance decision (the core moves fast and we don't own your backend), not a quality bar — a plugin can be excellent and still belong in its own repo. Promote it in the Nous Research Discord `#plugins-skills-and-skins` channel. See [CONTRIBUTING.md](https://github.com/NousResearch/openagents/blob/main/CONTRIBUTING.md) for the policy.
:::
...
```bash
mkdir -p ~/.openagents/plugins/calculator
cd ~/.openagents/plugins/calculator
```
...
4. **Accept `**kwargs`:** OpenAgents may pass additional context in the future.
...
- If this function crashes, the plugin is disabled but OpenAgents continues fine
...
The same logs are always written to `~/.openagents/logs/agent.log` at WARNING level (failures only) and DEBUG level (everything) when the env var is set. So if you can't run with the env var (e.g. from inside the gateway), tail the log file instead:
...
- **Wrong directory layout** — must be `~/.openagents/plugins/<plugin-name>/plugin.yaml` (flat) or `~/.openagents/plugins/<category>/<plugin-name>/plugin.yaml` (one level of category nesting, max). Anything deeper is ignored.
...
**Key properties:**
- Plugin skills are **read-only** — they don't enter `~/.openagents/skills/` and can't be edited via `skill_manage`.
...
:::tip Legacy pattern
The old `shutil.copy2` pattern (copying a skill into `~/.openagents/skills/`) still works but creates name collision risk with built-in skills. Prefer `ctx.register_skill()` for new plugins.
:::
...
| Rule | Why |
|---|---|
| Your feature key must appear in the in-tree `LAZY_DEPS` allowlist | Prevents a malicious config from coaxing OpenAgents into installing arbitrary packages — only specs OpenAgents itself ships are eligible |
...
auditable in `~/.openagents/logs/agent.log`. Plugins load after built-in
...
This is the only hook whose return value matters. When a `pre_llm_call` callback returns a dict with a `"context"` key (or a plain string), OpenAgents injects that text into the **current turn's user message**. This is the mechanism for memory plugins, RAG integrations, guardrails, and any plugin that needs to provide the model with additional context.

#### Return format

```python
# Dict with context key
return {"context": "Recalled memories:\n- User prefers dark mode\n- Last project: openagents"}
...
- **`ctx.profile_name`** — the active profile name (e.g. `"default"`, or the assignee profile in a kanban worker). Derived from `OPENAGENTS_HOME`, so it works everywhere with no `_cli_ref` dependency.
...
For running a full `hermes <subcommand>` (e.g. `hermes kanban show`), shell out with the `terminal` tool via `ctx.dispatch_tool("terminal", {"command": "hermes kanban show ..."})` — there is no in-process slash-command bridge for headless worker sessions, and tools are the supported way to drive OpenAgents from a hook.
...
OpenAgents has five specialized plugin types beyond the general surface. Each ships as a directory under `plugins/<category>/<name>/` (bundled) or `~/.openagents/plugins/<category>/<name>/` (user). The contract differs by category — pick the one you need, then read its full guide.
...
OpenAgents also accepts extensions that aren't Python plugins at all. These are shown in the [Pluggable interfaces table](/user-guide/features/plugins#pluggable-interfaces--where-to-go-for-each); the sections below sketch each authoring style briefly.

### MCP servers — register external tools

Model Context Protocol (MCP) servers register their own tools into OpenAgents without any Python plugin. Declare them in `~/.openagents/config.yaml`:
...
OpenAgents connects to each server at startup, lists its tools, and registers them alongside built-ins. The LLM sees them exactly like any other tool. **Full guide:** [MCP](/user-guide/features/mcp).

### Gateway event hooks — fire on lifecycle events

Drop a manifest + handler into `~/.openagents/hooks/<name>/`:

```yaml
# ~/.openagents/hooks/long-task-alert/HOOK.yaml
...
```python
# ~/.openagents/hooks/long-task-alert/handler.py
...
```python
# Wrong — will break if OpenAgents passes extra context
def handler(args):
    ...
