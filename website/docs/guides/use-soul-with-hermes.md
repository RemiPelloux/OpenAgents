...
description: "How to use SOUL.md to shape OpenAgents's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with Hermes

`SOUL.md` is the **primary identity** for your OpenAgents instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want OpenAgents to feel like the same assistant every time you talk to it — or if you want to replace the OpenAgents persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm OpenAgents should be
- what OpenAgents should avoid stylistically
- how OpenAgents should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who OpenAgents is and how OpenAgents speaks
...
OpenAgents now uses only the global SOUL file for the current instance:

```text
~/.openagents/SOUL.md
```

If you run OpenAgents with a custom home directory, it becomes:
...
OpenAgents automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, OpenAgents does not overwrite it
- if the file exists but is empty, OpenAgents adds nothing from it to the prompt

## How OpenAgents uses it

When OpenAgents starts a session, it reads `SOUL.md` from `OPENAGENTS_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how OpenAgents feels.
...
OpenAgents already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How OpenAgents should sound.

# Avoid
What OpenAgents should not do.

# Defaults
How OpenAgents should behave when ambiguity appears.
```
...
```bash
nano ~/.openagents/SOUL.md
```

or

```bash
vim ~/.openagents/SOUL.md
```

Then restart OpenAgents or start a new session.
...
4. Talk to OpenAgents for a while
...
### I edited SOUL.md but OpenAgents still sounds the same

Check:
- you edited `~/.openagents/SOUL.md` or `$OPENAGENTS_HOME/SOUL.md`
...
### OpenAgents is ignoring parts of my SOUL.md
