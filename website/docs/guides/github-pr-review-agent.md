...
┌───────────────────────────────────────────────────────────────────┐
│                                                                   │
│   Cron Timer  ──▶  OpenAgents  ──▶  GitHub API  ──▶  Review     │
...
If you have a public endpoint available, check out [Automated GitHub PR Comments with Webhooks](./webhook-github-pr-review.md) — GitHub pushes events to OpenAgents instantly when PRs are opened or updated.
:::
...
Make sure OpenAgents can access GitHub. Start a chat:
...
Still in the chat, ask OpenAgents to review a real PR:
...
A skill gives OpenAgents consistent review guidelines that persist across sessions and cron runs. Without one, review quality varies.

```bash
mkdir -p ~/.openagents/skills/code-review
```

Create `~/.openagents/skills/code-review/SKILL.md`:
...
This is what makes the reviewer actually useful. Start a session and teach OpenAgents your team's standards:
...
2. Teach OpenAgents your conventions via memory (Step 4)
