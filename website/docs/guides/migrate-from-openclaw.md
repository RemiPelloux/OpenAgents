...
description: "Complete guide to migrating your OpenClaw / Clawdbot setup to OpenAgents — what gets migrated, how config maps, and what to check after."
---
...
| `--no-backup` | Skip the pre-migration zip snapshot of `~/.openagents/` (by default a single restore-point archive is written before apply, under `~/.openagents/backups/pre-migration-*.zip`; restorable with `hermes import`). |
...
| What | OpenClaw source | OpenAgents destination | Notes |
|------|----------------|-------------------|-------|
| Persona | `workspace/SOUL.md` | `~/.openagents/SOUL.md` | Direct copy |
| Workspace instructions | `workspace/AGENTS.md` | `AGENTS.md` in `--workspace-target` | Requires `--workspace-target` flag |
| Long-term memory | `workspace/MEMORY.md` | `~/.openagents/memories/MEMORY.md` | Parsed into entries, merged with existing, deduped. Uses `§` delimiter. |
| User profile | `workspace/USER.md` | `~/.openagents/memories/USER.md` | Same entry-merge logic as memory. |
| Daily memory files | `workspace/memory/*.md` | `~/.openagents/memories/MEMORY.md` | All daily files merged into main memory. |
...
| Source | OpenClaw location | OpenAgents destination |
|--------|------------------|-------------------|
| Workspace skills | `workspace/skills/` | `~/.openagents/skills/openclaw-imports/` |
| Managed/shared skills | `~/.openclaw/skills/` | `~/.openagents/skills/openclaw-imports/` |
| Personal cross-project | `~/.agents/skills/` | `~/.openagents/skills/openclaw-imports/` |
| Project-level shared | `workspace/.agents/skills/` | `~/.openagents/skills/openclaw-imports/` |

Skill conflicts are handled by `--skill-conflict`: `skip` leaves the existing OpenAgents skill, `overwrite` replaces it, `rename` creates a `-imported` copy.

### Model and provider configuration

| What | OpenClaw config path | OpenAgents destination | Notes |
|------|---------------------|-------------------|-------|
...
| Provider API keys | `models.providers.*.apiKey` | `~/.openagents/.env` | Requires `--migrate-secrets`. See [API key resolution](#api-key-resolution) below. |

### Agent behavior

| What | OpenClaw config path | OpenAgents config path | Mapping |
|------|---------------------|-------------------|---------|
...
| OpenClaw config path | OpenAgents config path | Notes |
|---------------------|-------------------|-------|
...
| OpenClaw field | OpenAgents field | Notes |
|----------------|-------------|-------|
...
| TTS assets | `~/.openagents/tts/` (file copy) |

### Messaging platforms

| Platform | OpenClaw config path | OpenAgents `.env` variable | Notes |
|----------|---------------------|----------------------|-------|
...
| What | OpenClaw path | OpenAgents path | Notes |
|------|-------------|-------------|-------|
...
### Archived (no direct OpenAgents equivalent)

These are saved to `~/.openagents/migration/openclaw/<timestamp>/archive/` for manual review:

| What | Archive file | How to recreate in OpenAgents |
|------|-------------|--------------------------|
| `IDENTITY.md` | `archive/workspace/IDENTITY.md` | Merge into `SOUL.md` |
| `TOOLS.md` | `archive/workspace/TOOLS.md` | OpenAgents has built-in tool instructions |
...
| Multi-agent list | `archive/agents-list.json` | Use OpenAgents profiles |
...
5. **Test messaging** — if you migrated platform tokens, restart the gateway: `systemctl --user restart openagents-gateway`
...
Imported skills land in `~/.openagents/skills/openclaw-imports/`. Start a new session for them to take effect, or run `/skills` to verify they're loaded.
