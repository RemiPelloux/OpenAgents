...
description: "A practical guide to connecting MCP servers to OpenAgents, filtering their tools, and using them safely in real workflows"
---

# Use MCP with Hermes

This guide shows how to actually use MCP with OpenAgents in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native OpenAgents tool
- you want OpenAgents to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect OpenAgents to internal APIs, databases, or company systems without modifying OpenAgents core

Do not use MCP when:
- a built-in OpenAgents tool already solves the job well
...
- OpenAgents remains the agent
- MCP servers contribute tools
- OpenAgents discovers those tools at startup or reload time
...
If you installed OpenAgents with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- OpenAgents banner/status should show MCP integration when configured
- ask OpenAgents what tools it has available
...
## WSL2: bridge OpenAgents in WSL to Windows Chrome

This is the practical setup when:

- OpenAgents runs inside WSL2
...
In this setup, OpenAgents does **not** connect to Chrome directly. Instead:

- OpenAgents runs in WSL
- OpenAgents starts a local stdio MCP server
...
```text
OpenAgents (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- OpenAgents stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on OpenAgents core browser transport
...
Then start a fresh OpenAgents session or run:
...
Once loaded, OpenAgents can use the MCP-prefixed browser tools directly. For example:
...
If OpenAgents runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start OpenAgents from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start OpenAgents from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
So OpenAgents will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want OpenAgents to reason over a bounded workspace.
...
Use [Open Scaffold](https://github.com/graphanov/open-scaffold) when you want OpenAgents to read a repository's durable AI-work record: mission, plans, evidence notes, handoff packets, and review/gate results. OpenAgents remains the agent; Open Scaffold remains the repo-local record.
...
- Its write tools require the server to be started with `--allow-write`; do not enable that until you explicitly want OpenAgents to mutate `.osc` files.
- Open Scaffold records and gates work; it does not authorize OpenAgents to merge, publish, deploy, or spawn runtimes.
...
Now OpenAgents can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing OpenAgents core.
...
Because OpenAgents now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
