"""Windows UTF-8 bootstrap for OpenAgents entry points.
...
should be imported at the very top of every OpenAgents entry point
(``hermes``, ``openagents``, ``openagents-acp``, ``python -m gateway.run``,
...
def harden_import_path(src_root: str | None = None) -> None:
    """Stop a package in the current directory from shadowing OpenAgents modules.

    OpenAgents ships top-level modules with common names (``utils``, ``proxy``,
    ...
    We drop the relative forms outright, then force the real OpenAgents source root
...
# Apply on import — entry points just need ``import openagents_bootstrap``
# (or ``from openagents_bootstrap import apply_windows_utf8_bootstrap``) at
