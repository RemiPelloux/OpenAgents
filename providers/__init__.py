...
1. Bundled plugins: ``plugins/model-providers/<name>/`` (shipped with openagents)
2. User plugins: ``$OPENAGENTS_HOME/plugins/model-providers/<name>/``
...
def register_provider(profile: ProviderProfile) -> None:
    ...
    plugins under ``$OPENAGENTS_HOME/plugins/model-providers/`` can override
...
def _user_plugins_dir() -> Path | None:
    """Return ``$OPENAGENTS_HOME/plugins/model-providers/`` if it exists."""
    try:
        from openagents_constants import get_openagents_home

        d = get_openagents_home() / "plugins" / "model-providers"
...
def _import_plugin_dir(plugin_dir: Path, source: str) -> None:
    ...
    # multiple OPENAGENTS_HOME profiles don't alias each other.
...
def _discover_providers() -> None:
    ...
    Order:
      ...
      2. User plugins at ``$OPENAGENTS_HOME/plugins/model-providers/<name>/``
    ...
    # 1. Bundled plugins — shipped with openagents.
    ...
    # 2. User plugins — under $OPENAGENTS_HOME/plugins/model-providers/<name>/.
