#!/usr/bin/env python3
"""
OpenAgents CLI - Interactive Terminal Interface

A beautiful command-line interface for the OpenAgents, inspired by Claude Code.
...
os.environ["OPENAGENTS_QUIET"] = "1"  # Our own modules
...
try:
    from openagents_cli.pt_input_extras import (
...
from openagents_cli.banner import _format_context_length, format_banner_version_label
...
from openagents_cli.browser_connect import (
...
def load_cli_config() -> Dict[str, Any]:
    ...
    Config lookup order:
    1. ~/.openagents/config.yaml (user config - preferred)
    ...
    the user config at ``~/.openagents/config.yaml`` is skipped entirely and only the
    built-in defaults plus the project-level ``cli-config.yaml`` (if any) are used.
    Credentials in ``.env`` are still loaded — this flag only suppresses
    behavioral/config settings.
    """
    # Check user config first ({OPENAGENTS_HOME}/config.yaml)
    ...
    defaults = {
        ...
        "agent": {
            ...
            "personalities": {
                ...
                "noir": "The rain hammered against the terminal like regrets on a guilty conscience. They call me OpenAgents - I solve problems, find answers, dig up the truth that hides in the shadows of your codebase. In this city of silicon and secrets, everyone's got something to hide. What's your story, pal?",
        ...
        "display": {
            "compact": False,
            "resume_display": "full",
            # Recap tuning for /resume — see openagents_cli/config.py DEFAULT_CONFIG.
            "resume_exchanges": 10,
    ...
    # openagents_cli.config._load_config_impl (which has its own managed merge), so
    ...
    # Normalize config key: the new config system (openagents_cli/config.py) and all
...
# Initialize centralized logging early — agent.log + errors.log in ~/.openagents/logs/.
# This ensures CLI sessions produce a log trail even before AIAgent is instantiated.
try:
    from openagents_logging import setup_logging
...
from openagents_cli.banner import build_welcome_banner
from openagents_cli.commands import SlashCommandCompleter, SlashCommandAutoSuggest
...
from openagents_cli.callbacks import prompt_for_secret
...
def _resolve_worktree_base(repo_root: str) -> tuple:
    ...
    (the ``~/.openagents/openagents`` clone is updated only by ``hermes update``,
...
def _run_state_db_auto_maintenance(session_db) -> None:
    ...
    :func:`openagents_cli.config.load_config` (the authoritative loader that
    ...
    try:
        ...
        from openagents_constants import get_openagents_home as _get_openagents_home

        _hermes_home_maint = _get_openagents_home()
...
def _run_checkpoint_auto_maintenance() -> None:
    ...
    :func:`openagents_cli.config.load_config`. Honours ``auto_prune`` /
...
#   6. Default: assume dark (matches the legacy OpenAgents assumption)
#
...
def _install_skin_light_mode_hook() -> None:
    ...
    try:
        from openagents_cli.skin_engine import SkinConfig  # type: ignore[import]
    except Exception:
        return
...
def _strip_markdown_syntax(text: str) -> str:
    ...
    # but in OpenAgents output it's common to display cron schedules verbatim.
    #
...
from openagents_constants import is_termux as _is_termux_environment
...
def _build_compact_banner() -> str:
    ...
    if skin_name == "default":
        ...
        agent_name = (
            _skin.get_branding("agent_name", "OpenAgents") if _skin else "OpenAgents"
        )
...
def save_config_value(key_path: str, value: any) -> bool:
    ...
    1. ~/.openagents/config.yaml (user config - preferred, used if it exists)
    2. ./cli-config.yaml (project config - fallback)
    ...
    try:
        # Ensure parent directory exists (for ~/.openagents/config.yaml on first use)
...
class HermesCLI(CLIAgentSetupMixin, CLICommandsMixin):
    """
    Interactive CLI for the OpenAgents.
    ...
    def __init__(
        ...
        if self.model == _DEFAULT_CONFIG_MODEL:
            ...
            if "localhost" in _base_url or "127.0.0.1" in _base_url:
                from openagents_cli.runtime_provider import _auto_detect_local_model
        ...
        # by `hermes chat --ignore-rules` in openagents_cli/main.py. When true we
        ...
        # it's shared across all OpenAgents processes for this OPENAGENTS_HOME.
    ...
    def show_banner(self):
        ...
        if ctx_len and ctx_len < MINIMUM_CONTEXT_LENGTH:
            ...
            self._console_print(
                f"[dim]   OpenAgents needs at least {MINIMUM_CONTEXT_LENGTH:,} tokens. Tool schemas + system prompt use a large fixed prefix.[/]"
            )
        ...
        # Warn if the configured model is a Nous OpenAgents LLM (not agentic)
        from openagents_cli.model_switch import is_nous_hermes_non_agentic

        model_name = getattr(self, "model", "") or ""
        if is_nous_hermes_non_agentic(model_name):
            self._console_print()
            self._console_print(
                "[bold yellow]⚠  Nous Research OpenAgents 3 & 4 models are NOT agentic and are not "
                "designed for use with OpenAgents.[/]"
            )
            self._console_print(
    ...
    def _try_attach_clipboard_image(self) -> bool:
        ...
        Saves the image to ~/.openagents/images/ and appends the path to
        ``_attached_images``.  Returns True if an image was attached.
        """
        from openagents_cli.clipboard import save_clipboard_image

        img_dir = get_openagents_home() / "images"
    ...
    def _fast_command_available(self) -> bool:
        try:
            from openagents_cli.models import model_supports_fast_mode
        except Exception:
            return False
        agent = getattr(self, "agent", None)
    ...
    def show_help(self):
        ...
        from openagents_cli.commands import COMMANDS_BY_CATEGORY

        try:
            from openagents_cli.skin_engine import get_active_help_header
    ...
    def _show_recent_sessions(
        ...
        if reason == "history":
            ...
            print(
                "(◕‿◕) Recent sessions — resume with /resume <number> or /resume <title>:"
            )
        else:
    ...
    def _confirm_expensive_model_switch(self, result) -> bool:
        ...
        choices = [
            (
                "once",
                "Switch anyway",
                "Use this model for the current OpenAgents session.",
            ),
    ...
    def _show_gateway_status(self):
        ...
        try:
            ...
            print()
            print(f"  Configuration file: {display_openagents_home()}/config.yaml")
            print()
            ...
            print(
                f"    2. Or configure settings in {display_openagents_home()}/config.yaml"
            )
            print()
    ...
    def process_command(self, command: str) -> bool:
        ...
        # change in openagents_cli/commands.py instead of touching every dispatch site.
        ...
        if canonical in {"quit", "exit"}:
            ...
            try:
                ...
                if not user_entries:
                    ...
                    print(
                        f"  Or drop a plugin directory into {display_openagents_home()}/plugins/"
                    )
                    if bundled_count:
                        print(
    ...
    def _toggle_verbose(self):
        ...
        from openagents_cli.colors import Colors as _Colors
    ...
    def _manual_compress(self, cmd_original: str = ""):
        ...
        * ``/compress here [N]`` — boundary-aware compression. Summarize
        ...
        from openagents_cli.partial_compress import (
    ...
    def _show_usage(self):
        ...
        if self.verbose:
            ...
            # Console quietness is enforced by openagents_logging not
    ...
    def _billing_render_charge_error(self, state, exc):
        ...
        from openagents_cli.nous_billing import BillingRateLimited
    ...
    def _reload_skills(self) -> None:
        """Reload skills: rescan ~/.openagents/skills/ and queue a note for the
        next user turn.
    ...
    def _print_exit_summary(self):
        ...
        if msg_count > 0:
            ...
            # profile names use the standard OPENAGENTS_HOME, so no -p needed.
            try:
            ...
            try:
                from openagents_cli.skin_engine import get_active_goodbye
    ...
    def _get_tui_prompt_symbols(self) -> tuple[str, str]:
        ...
        """
        try:
            from openagents_cli.skin_engine import get_active_prompt_symbol
    ...
    def _build_tui_style_dict(self) -> dict[str, str]:
        ...
        try:
            from openagents_cli.skin_engine import get_prompt_toolkit_style_overrides
    ...
    def run(self):
        ...
        try:
            ...
            _welcome_text = _welcome_skin.get_branding(
                "welcome",
                "Welcome to OpenAgents! Type your message or /help for commands.",
            )
...
def _run_kanban_goal_loop_q(cli: "HermesCLI", first_response: str) -> None:
    ...
    from openagents_cli import kanban_db as _kb
                    ...
                    if cli._init_agent(
                        ...
                        if isinstance(result, dict) and result.get("failed"):
                            ...
                            if os.environ.get("HERMES_KANBAN_TASK") and result.get(
                                ...
                                try:
                                    from openagents_cli.kanban_db import (
